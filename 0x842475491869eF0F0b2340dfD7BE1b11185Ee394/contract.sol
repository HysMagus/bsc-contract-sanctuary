...
contract Staking is Ownable {
    ...
    modifier onlyToken {
        require(msg.sender == address(token), "Can only be called by YDF contract.");
        _;
    }
    ...
    function unstake(uint amount) external whenStakingActive {
        require(stakeValue[msg.sender] >= amount, "Cannot unstake more YDF than you have staked.");
    ...
    function distribute(uint amount) public {
        require(token.balanceOf(msg.sender) >= amount, "Cannot distribute more YDF than you hold unstaked.");
    ...
    function handleTaxDistribution(uint amount) external onlyToken {
        totalDistributions = totalDistributions.add(amount);
        _increaseProfitPerShare(amount);
        emit OnDistribute(msg.sender, amount);
    }
