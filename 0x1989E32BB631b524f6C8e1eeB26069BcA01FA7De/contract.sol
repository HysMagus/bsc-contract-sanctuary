...
// File: contracts/BUSDBUNNYDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DividendToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //Change dividend mainnet 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
...
contract BUSDBUNNY is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public DividendToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //change dividend token mainnet 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56

    bool private swapping;

    BUSDBUNNYDividendTracker public dividendTracker;
    ...
    address _marketingWalletAddress = 0xC797e91d64cE79A796fb7BEB8856D06868A69db5; //change this
    ...
    constructor() ERC20("BUSDbunny", "BDBUNNY") {
        uint256 _TokenRewardsFee = 9;
    	...
    	dividendTracker = new BUSDBUNNYDividendTracker();
    ...
    function changeSellFee(uint256 amount) external onlyOwner {
	    require(amount >= 100 && amount <= 150, "BUSDBUNNY: SellFee must be updated to between 100 and 150.");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BUSDBUNNY: The dividend tracker already has that address");

        BUSDBUNNYDividendTracker newDividendTracker = BUSDBUNNYDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BUSDBUNNY: The new dividend tracker must be owned by the BUSDBUNNY token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BUSDBUNNY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BUSDBUNNY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BUSDBUNNY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BUSDBUNNY: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BUSDBUNNY: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BUSDBUNNY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BUSDBUNNY: Cannot update gasForProcessing to same value");
    ...
    function updateRewardsFee(uint256 RewardFeePercent) external onlyOwner{
        require(RewardFeePercent <= 20, "BUSDBUNNY: updateRewardsFee must be between 0 and 20.");
        TokenRewardsFee = RewardFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateLiquidityFee(uint256 liquidityFeePercent) external onlyOwner{
        require(liquidityFeePercent <= 5, "BUSDBUNNY: updateRewardsFee must be between 0 and 5.");
        liquidityFee = liquidityFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateMarketingFee(uint256 marketingFeePercent) external onlyOwner{
        require(marketingFeePercent <= 10, "BUSDBUNNY: updateRewardsFee must be between 0 and 10.");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BUSDBUNNY: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialSMMATICBalance = IERC20(DividendToken).balanceOf(address(this));
 
        swapTokensForBusd(tokens, address(this));
        uint256 newBalance = (IERC20(DividendToken).balanceOf(address(this))).sub(initialSMMATICBalance);
...
contract BUSDBUNNYDividendTracker is Ownable, DividendPayingToken{
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BUSDBUNNY_Dividend_Tracker", "BUSDBUNNY_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "BUSDBUNNY: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BUSDBUNNY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BUSDBUNNY_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }

    function updateMinimunForRewards(uint256 newValueMinimunForRewards) external onlyOwner {
        require(newValueMinimunForRewards >= 10000 && newValueMinimunForRewards <= 500000, "BUSDBUNNY_Dividend_Tracker: newValueMinimunForRewards must be updated to between 10000 and 500000");
        require(newValueMinimunForRewards != minimunForRewards, "BUSDBUNNY_Dividend_Tracker: Cannot update claimWait to same value");
