...
contract CommunityFund {
    ...
    function setShareRewardPoolIdAndLpPairAddress(
        address _tokenB,
        uint256 _pid,
        address _lpAdd
            ...
            if (_dollarPrice >= dollarPriceToSell) {
                ...
                if (_dollarPercent > expansionPercent[0]) {
                    ...
                    if (_busdPercent >= expansionPercent[1]) {
                        // enough BUSD
                        if (_usdtPercent < expansionPercent[2]) {
                            // short of USDT: buy WBNB
                            _swapToken(dollar, usdt, _sellingMdo);
                        } else {
                            if (_busdPercent.sub(expansionPercent[1]) <= _usdtPercent.sub(expansionPercent[2])) {
                                // has more USDT than BUSD: buy BUSD
                                _swapToken(dollar, busd, _sellingMdo);
                            } else {
                                // has more BUSD than USDT: buy USDT
                                _swapToken(dollar, usdt, _sellingMdo);
                            }
                        }
                    } else {
                        // short of BUSD
                        if (_usdtPercent >= expansionPercent[2]) {
                            // enough USDT: buy BUSD
                            _swapToken(dollar, busd, _sellingMdo);
                        } else {
                            // short of USDT
                            uint256 _sellingMdoToBusd = _sellingMdo.mul(3).div(4);
                            _swapToken(dollar, busd, _sellingMdoToBusd);
                            _swapToken(dollar, usdt, _sellingMdo.sub(_sellingMdoToBusd));
                        }
                    }
                }
                ...
                if (_busdPercent >= contractionPercent[1]) {
                    // enough BUSD
                    if (_usdtPercent <= contractionPercent[2]) {
                        // short of USDT: sell BUSD
                        uint256 _sellingBUSD = _busdBal.mul(_busdPercent.sub(contractionPercent[1])).div(10000);
                        _swapToken(busd, dollar, _sellingBUSD);
                    } else {
                        if (_busdPercent.sub(contractionPercent[1]) > _usdtPercent.sub(contractionPercent[2])) {
                            // has more BUSD than USDT: sell BUSD
                            uint256 _sellingBUSD = _busdBal.mul(_busdPercent.sub(contractionPercent[1])).div(10000);
                            _swapToken(busd, dollar, _sellingBUSD);
                        } else {
                            // has more USDT than BUSD: sell USDT
                            uint256 _sellingWBNB = _usdtBal.mul(_usdtPercent.sub(contractionPercent[2])).div(10000);
                            _swapToken(usdt, dollar, _sellingWBNB);
                        }
                    }
                } else {
                    // short of BUSD
                    if (_usdtPercent > contractionPercent[2]) {
                        // enough USDT: sell USDT
                        uint256 _sellingWBNB = _usdtBal.mul(_usdtPercent.sub(contractionPercent[2])).div(10000);
                        _swapToken(usdt, dollar, _sellingWBNB);
                    }
                }
