...
contract CfiCreamer {
    ...
    struct PoolInfo {
        ...
        uint256 accCreamPerCream; // Accumulated CREAMs per cream, times 1e18. See below.
    ...
    uint256 public creamPerBlock = 1 ether; // 200K new Cream per week.
    ...
    constructor( ) public {
        cream = IERC20(0xdF6585D955fD55687c0b8A3000D0573faB341bee);
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "CreamCreamer: caller is not the operator");
        _;
    }

    modifier checkhalve() {
        if (block.number >= periodFinish) {
            creamPerBlock = creamPerBlock.mul(80).div(100);
    ...
    function checkPoolDuplicate(IERC20 _lpToken) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].lpToken != _lpToken, "CreamCreamer: existing pool?");
        }
    }
    ...
    function getGeneratedReward(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        if (_to >= endBlock) {
            ...
            if (_from <= startBlock) return endBlock.sub(startBlock).mul(creamPerBlock);
            return endBlock.sub(_from).mul(creamPerBlock);
        } else {
            if (_to <= startBlock) return 0;
            if (_from <= startBlock) return _to.sub(startBlock).mul(creamPerBlock);
            return _to.sub(_from).mul(creamPerBlock);
        }
    }
    ...
    function pendingCream(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 _creamReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accCreamPerCream = accCreamPerCream.add(_creamReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardBlock, block.number);
            uint256 _creamReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accCreamPerCream = pool.accCreamPerCream.add(_creamReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeCreamTransfer(address _to, uint256 _amount) internal {
        uint256 _creamBal = cream.balanceOf(address(this));
        if (_creamBal > 0) {
            if (_amount > _creamBal) {
                cream.safeTransfer(_to, _creamBal);
                totalCirculating = totalCirculating.add(_creamBal);
            } else {
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.number < startBlock + BLOCKS_PER_WEEK * 68) {
            // do not allow to drain core token (CREAM or lps) if less than 6 months after pool ends
