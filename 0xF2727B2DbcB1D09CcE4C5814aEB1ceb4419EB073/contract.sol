...
function _bestTrade(address[] calldata routers,address fromTokenAddress,address toTokenAddress, uint256 amount, bool exactOut)internal view returns(uint256[] memory minMax,address bestRouter){
    ...
    function _swapTokensToBUSD(address routerAddress,uint256 amountOut,uint256 amountInMax, address tokenAddress) internal returns(uint[] memory returnData){
...
function _swapBUSDToTokens(address routerAddress,uint256 amountIn, address tokenAddress) internal returns(uint[] memory returnData){
        Router01 router = Router01(routerAddress);
        address[] memory path = new address[](3);
        path[0] = BUSD;
        path[1] = WBNB;
    ...
    function swapBNBToBUSD(address[] calldata routers,uint256 amountOut) external payable returns(uint[] memory returnData){
        (uint256[] memory minMax,address bestRouter) = bestTrade(routers,address(0), BUSD,amountOut,true);
     ...
     function swapBUSDToBNB(address[] calldata routers,uint256 amountIn) external payable returns(uint[] memory returnData){
        (uint256[] memory minMax,address bestRouter) = bestTrade(routers,BUSD,address(0), amountIn,false);
        Router01 router = Router01(bestRouter);
        address[] memory path = new address[](2);
        path[0] = BUSD;
        path[1] = WBNB;
    ...
    function swapTokensToBUSD(address[] calldata routers,address tokenAddress, uint256 amountInMax,uint256 amountOut) public returns(uint[] memory returnData){
        (uint256[] memory minMax,address bestRouter) = bestTrade(routers,tokenAddress, BUSD,amountOut,true);
        returnData =  _swapTokensToBUSD(bestRouter,amountOut,amountInMax, tokenAddress);
    }

    function swapBUSDToTokens(address[] calldata routers,address tokenAddress, uint256 amountIn) public returns(uint[] memory returnData){
        (uint256[] memory minMax,address bestRouter) = bestTrade(routers,BUSD,tokenAddress, amountIn,false);
        returnData =  _swapBUSDToTokens(bestRouter,amountIn, tokenAddress);
    }
}