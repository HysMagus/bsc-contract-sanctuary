...
contract DfMaxiFarm is Ownable, ReentrancyGuard, Pausable {
    ...
    returns (uint256)
    {
        IERC20(firstAddress).safeTransferFrom(msg.sender, address(this), _amount);
        
        uint256 collateralBUSD = _amount * 70 / 100; 
        uint256 poolBUSD = _amount.sub(collateralBUSD); 
        IVToken(vFirstAddress).mint(collateralBUSD);
        ...
        uint256 secNeeded = poolBUSD.mul(reserve0).div(reserve1);
        
        // IPriceOracle oracle = IVenusDistribution(venusDistributionAddress).oracle();
        // uint256 _borrowSec = oracle.getUnderlyingPrice(vFirstAddress).mul(poolBUSD).div(oracle.getUnderlyingPrice(vSecondAddress));
        IVToken(vSecondAddress).borrow(secNeeded);
        IWBNB(wbnbAddress).deposit{value: secNeeded}(); // BNB -> WBNB
      
        uint256 amountA;
        uint256 amountB;
        uint256 liquidity;
        (amountA, amountB, liquidity) = IPancakeRouter02(uniRouterAddress).addLiquidity(secondAddress, firstAddress, secNeeded, poolBUSD,  0, 0, address(this), now + 600);
        
        if (IERC20(address(pair)).allowance(address(this), address(PANCAKE_MASTER)) != uint256(-1)) { // OPTIMIZE
            IERC20(address(pair)).approve(address(PANCAKE_MASTER), uint256(-1));
        }
        
        IPancakeMaster(PANCAKE_MASTER).deposit(1, liquidity);
    ...
    function earn() public onlyOwner {
        ...
        IPancakeMaster(PANCAKE_MASTER).withdraw(1, 0);
        ...
        IVToken(vSecondAddress).repayBorrow(_unwrapBNB());
    }
    
    function withdraw(uint256 _amount) public
    nonReentrant
    whenNotPaused returns (uint256 userTotalRecv)
    {
        ...
        IPancakeMaster(PANCAKE_MASTER).withdraw(1, userLiquidity);
        
        (uint256 amountBNB, uint256 amountBUSD) = IPancakeRouter02(uniRouterAddress).removeLiquidity(secondAddress, firstAddress, userLiquidity, 0, 0, address(this), now + 600);

        uint256 totalBnbToRepay = IVToken(vSecondAddress).borrowBalanceCurrent(address(this));
        if (totalBnbToRepay > amountBNB) {
            // BNB to BUSD
            IPancakeRouter02(uniRouterAddress).swapTokensForExactTokens(totalBnbToRepay - amountBNB, amountBUSD, bnbTobusdPath, address(this), now.add(600));
        } else {
            // BUSD to BNB
            IPancakeRouter02(uniRouterAddress).swapExactTokensForTokens(amountBUSD, amountBNB - totalBnbToRepay, busdToBnbPath, address(this), now.add(600));
        }
        _unwrapBNB();
        IVToken(vSecondAddress).repayBorrow(totalBnbToRepay);
        
        IVToken(vFirstAddress).redeemUnderlying(_amount * 70 / 100);
        
        userTotalRecv = IERC20(firstAddress).balanceOf(address(this));
