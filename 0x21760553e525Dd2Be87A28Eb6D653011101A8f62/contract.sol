...
contract PeachToken is BEP20('PeachDEX', 'Peach') {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Peach (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Peach
    ...
    event UpdateEmissionRate(address indexed user, uint256 PeachPerBlock);
    ...
    // Update the given pool's Peach allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingPeach(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accPeachPerShare = accPeachPerShare.add(PeachReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 PeachReward = multiplier.mul(PeachPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Peach.mint(devaddr, PeachReward.div(10));
        Peach.mint(address(this), PeachReward);
        pool.accPeachPerShare = pool.accPeachPerShare.add(PeachReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safePeachTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > PeachBal) {
            transferSuccess = Peach.transfer(_to, PeachBal);
        } else {
            transferSuccess = Peach.transfer(_to, _amount);
        }
        require(transferSuccess, "safePeachTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _PeachPerBlock) external onlyOwner {
        massUpdatePools();
        PeachPerBlock = _PeachPerBlock;
        emit UpdateEmissionRate(msg.sender, _PeachPerBlock);
    }
