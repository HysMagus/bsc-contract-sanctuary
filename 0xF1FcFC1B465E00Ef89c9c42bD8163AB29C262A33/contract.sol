...
// File: contracts/FUEL/interfaces/common/IWBNB.sol
pragma solidity ^0.6.0;
...
// File: contracts/FUEL/interfaces/thugs/IThugswapRouter.sol
pragma solidity ^0.6.0;
...
// File: contracts/FUEL/interfaces/thugs/IThugswapPair.sol
pragma solidity ^0.6.0;
...
// File: contracts/FUEL/interfaces/thugs/IOriginalGangster.sol
...
// File: contracts/FUEL/strategies/Thugs/StrategyThugsLP.sol
pragma solidity ^0.6.0;
...
contract StrategyThugsLP is Ownable, Pausable {
     ...
     * {fuel} - JetFuel Finance token, used to send funds to the treasury.
    ...
    address constant public gangster = address(0xb752f0cB591Ecfb1c5058a93e1332F066bf38473);
    uint8 public poolId;

    /**
     * @dev JetFuel Contracts:
     * {rewards} - Reward pool where the strategy fee earnings will go. JETS contract address
     * {treasury} - Address of the JetFuel Finance treasury
     * {vault} - Address of the vault that controls the strategy's funds.
     */
    address constant public rewards  = address(0xf6488205957f0b4497053d6422F49e27944eE3Dd);
    address constant public treasury = address(0x305b0eCf72634825f7231058444c65D885E1f327);
     ...
     *
     * {REWARDS_FEE} - 4% goes to FUEL holders through the {rewards} pool.
    ...
    function deposit() public whenNotPaused {
        ...
        if (pairBal > 0) {
            ...
            IERC20(lpPair).safeApprove(gangster, pairBal);
            IOriginalGangster(gangster).deposit(poolId, pairBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            IOriginalGangster(gangster).withdraw(poolId, _amount.sub(pairBal));
    ...
    function harvest() external whenNotPaused {
        ...
        IOriginalGangster(gangster).deposit(poolId, 0);
        chargeFees();
        addLiquidity();
        deposit();
    }
    ...
    function chargeFees() internal {
        ...
        IThugswapRouter(unirouter).swapExactTokensForTokens(rewardsFee, 0, wbnbToFuelRoute, rewards, now.add(600));
    }
    ...
    function addLiquidity() internal {   
        uint256 toLpToken1 = IERC20(drugs).balanceOf(address(this)).div(2);
        IThugswapRouter(unirouter).swapExactTokensForTokens(toLpToken1, 0, drugsToLp1Route, address(this), now.add(600));

        if (lpToken0 != drugs) {
            uint256 toLpToken0 = IERC20(drugs).balanceOf(address(this));
            IThugswapRouter(unirouter).swapExactTokensForTokens(toLpToken0, 0, drugsToLp0Route, address(this), now.add(600));
        }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IOriginalGangster(gangster).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function panic() external onlyOwner {
        _pause();
        IOriginalGangster(gangster).emergencyWithdraw(poolId);
    }
