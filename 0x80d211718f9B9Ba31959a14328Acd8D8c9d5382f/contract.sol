...
contract DrawStake is Owned {
    ...
    address public PLM = 0x80d211718f9B9Ba31959a14328Acd8D8c9d5382f;
    ...
    function stake(uint256 tokens) external {
        require(IERC20(PLM).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function drawToken(uint256 tokens) external {
        require(IERC20(PLM).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function getReward() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(PLM).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
    ...
    function withdraw(uint256 tokens) external {
        ...
        require(IERC20(PLM).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedPLM(address staker) external view returns(uint256 stakedPLM){
        return stakers[staker].stakedTokens;
    }
    
    // ------------------------------------------------------------------------
    // Get the PLM balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourPLMBalance(address user) external view returns(uint256 PLMBalance){
        return IERC20(PLM).balanceOf(user);
    }
}