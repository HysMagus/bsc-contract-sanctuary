...
contract ERC20 is Context, IERC20 {
    ...
    function _setupAccount(address account_) internal virtual {
...
contract Frunk is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxWalletBalance = 100000000000000 * 10**18; //5%
    ...
    constructor() ERC20("Frunkenomics", "frunk") {
    	firstDividendTracker = new FIRSTDividendTracker();

    	marketingWallet = 0xE4923b89A04f467B45135476b6BfB5D106CB1eB5;
    ...
    //-
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "frunk: The marketing wallet is already this address");
    ...
    function updateFirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "frunk: The dividend tracker already has that address");
 
        FIRSTDividendTracker newFirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newFirstDividendTracker.owner() == address(this), "frunk: The new dividend tracker must be owned by the frunk token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "frunk: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "frunk: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "frunk: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "frunk: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "frunk: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "frunk: Trading has not started yet");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("frunk_Doge_Dividend_Tracker", "frunk_Doge_Dividend_Tracker", 0xbA2aE424d960c26247Dd6c32edC70B295c744C43) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 20000000000 * (10**18); //must hold 0.02% tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "frunk_First_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "frunk_First_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main frunk contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "frunk_First_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "frunk_First_Dividend_Tracker: Cannot update claimWait to same value");
