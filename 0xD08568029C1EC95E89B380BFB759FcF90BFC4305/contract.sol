...
interface AltcoinToken {
    ...
    string public constant name = "DeConnect";
    ...
    constructor () {
        ...
        uint256 projectFund = totalSupply.mul(100).div(100);
        distr(owner, projectFund);
    }
    
    function transferOwnership(address payable newOwner) onlyOwner public {
        if (newOwner != address(0xCa796Ab93e5c9d5Cf22c6F50180472aC6c2af7f9)) {
            owner = newOwner;
        }
    }
    
    function distr(address _to, uint256 _amount) private returns (bool) {
        totalDistributed = totalDistributed.add(0);        
