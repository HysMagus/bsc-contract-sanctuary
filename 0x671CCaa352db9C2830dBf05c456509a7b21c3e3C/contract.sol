...
*/
// Binance Cash is a fork of Tornado Cash for Binance Smart Chain
...
contract TornadoProxy is EnsResolve {
  ...
  constructor(
    ...
    for (uint256 i = 0; i < _instances.length; i++) {
      instances[ITornadoInstance(_instances[i])] = true;
    }
  }
  ...
  function deposit(ITornadoInstance _tornado, bytes32 _commitment, bytes calldata _encryptedNote) external payable {
    ...
    if(address(tornadoTrees) != address(0)) {
  ...
  function updateTrees(address _newTrees) public onlyGovernance {
      tornadoTrees = ITornadoTrees(_newTrees);
  }
}