...
contract deFlame is BEP20('phoenix.deFlame', 'dFNX') {
    ...
    PhoenixSwapToken public phoenixes;
    address public devAddr;
    constructor(
        PhoenixSwapToken _phoenixes,
        address _devAddr
    ) public {
        phoenixes = _phoenixes;
        devAddr = _devAddr;
    }
    function safePhoenixesTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 phoenixesBal = phoenixes.balanceOf(address(this));
        if (_amount > phoenixesBal) {
            phoenixes.transfer(_to, phoenixesBal);
        } else {
            phoenixes.transfer(_to, _amount);
        }
    }
    mapping (address => address) internal _delegates;
    struct Checkpoint {
    ...
    function enter(uint256 _amount) public {
        uint256 totalPhoenixes = phoenixes.balanceOf(address(this));
        uint256 totalShares = totalSupply();
        uint256 totalMinusBurn = _amount.mul(98).div(100);         
        if (totalShares == 0 || totalPhoenixes == 0) {
            _mint(msg.sender, totalMinusBurn);
        } else {
            uint256 what = totalMinusBurn.mul(totalShares).div(totalPhoenixes);
            _mint(msg.sender, what);
        }
        phoenixes.transferFrom(msg.sender, address(this), _amount);
    }
    function leave(uint256 _share) public {
        uint256 totalShares = totalSupply();
        uint256 what = _share.mul(phoenixes.balanceOf(address(this))).div(totalShares);
        _burn(msg.sender, _share);
        uint256 fee = what.mul(25).div(1000);
        uint256 stakerAmount = what.sub(fee);
        phoenixes.transfer(devAddr, fee); 
        phoenixes.transfer(msg.sender, stakerAmount);
    }
    function dev(address _devAddr) public {
        require(msg.sender == devAddr, 'youre not phoenix dev: wut?');
        devAddr = _devAddr;
    }
}