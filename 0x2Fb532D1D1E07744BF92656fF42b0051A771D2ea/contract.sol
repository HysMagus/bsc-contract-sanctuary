...
contract WhaleVaultV1 is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    // Update the given pool's whales allocation point. Can only be called by the owner.
    ...
    function pendingwhale(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
    ...
    // ----
    // Function that adds pending rewards, called by the whale token.
    // ----
    uint256 private whaleBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = whale.balanceOf(address(this)).sub(whaleBalance);

        if(newRewards > 0) {
            whaleBalance = whale.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 whaleRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        whaleRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 whaleRewardFee = whaleRewardWhole.mul(DEV_FEE).div(10000);
        uint256 whaleRewardToDistribute = whaleRewardWhole.sub(whaleRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(whaleRewardFee);

        pool.accwhalePerShare = pool.accwhalePerShare.add(
            whaleRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit  tokens to whaleVault for whale allocation.
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accwhalePerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from whaleVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming whale farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accwhalePerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safewhaleTransfer(from, pending);
        }
    ...
    function safewhaleTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > whaleBal) {
            ...
            whaleBalance = whale.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 whaleBal = whale.balanceOf(address(this));
        if (pending_DEV_rewards > whaleBal) {
            whale.transfer(devaddr, whaleBal);
            whaleBalance = whale.balanceOf(address(this));
        } else {
            whale.transfer(devaddr, pending_DEV_rewards);
            whaleBalance = whale.balanceOf(address(this));
        }
