...
// File: contracts/interfaces/uniswapV2/IBSCswapPair.sol
...
// File: contracts/handlers/BSCswapHandler.sol
...
contract BSCswapHandler is IHandler {
    using SafeMath for uint256;
    ...
    receive() external override payable {
        require(msg.sender != tx.origin, "BSCswapHandler#receive: NO_SEND_ETH_PLEASE");
    }
    ...
    function handle(
        ...
        require(successRelayer, "BSCswapHandler#handle: TRANSFER_ETH_TO_RELAYER_FAILED");
    }
    ...
    function _estimate(address _inputToken, address _outputToken, uint256 _inputAmount) internal view returns (uint256 bought) {
        ...
        IBSCswapPair pair = IBSCswapPair(UniswapUtils.pairForSorted(FACTORY, token0, token1, FACTORY_CODE_HASH));
    ...
    function _swap(address _inputToken, address _outputToken, uint256 _inputAmount, address _recipient) internal returns (uint256 bought) {
        ...
        require(SafeERC20.transfer(IERC20(_inputToken), address(pair), inputAmount), "BSCswapHandler#_swap: ERROR_SENDING_TOKENS");
