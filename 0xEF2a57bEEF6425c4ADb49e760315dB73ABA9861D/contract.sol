...
// File: contracts\BSCXMaker.sol

pragma solidity 0.6.12;


// This contract handles "serving up" rewards for xBSCX holders by trading tokens collected from fees for BSCX.
contract BSCXMaker  is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;
    IUniswapV2Router02 public routerv2;
    IUniswapV2Factory public factory;
    address public bscxSafe;
    address public bscx;
    address public weth;
    ...
    function removeLiqidity(address token0, address token1, uint amount) public {
        require(operators[msg.sender], 'BSCXMaker: no permistion');
    ...
    function marketBuyBSCXWithETH(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'BSCXMaker: no permistion');
      require(path[path.length - 1] == bscx, 'BSCXMaker: wrong path');
      for (uint i = 1; i < path.length - 1; i++) {
        require(acceptTokens[path[i]], 'BSCXMaker: wrong path');
      }
    ...
    function marketBuyBSCXWithToken(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'BSCXMaker: no permistion');
      address token = path[0];
      if (token == bscx) {
          IERC20(token).safeTransfer(bscxSafe, amount);
      } else  {
        require(path[path.length - 1] == bscx, 'BSCXMaker: wrong path');
        for (uint i = 1; i < path.length - 1; i++) {
          require(acceptTokens[path[i]], 'BSCXMaker: wrong path');
    ...
    function convert(address token0, address token1) public {
        ...
        if (IERC20(weth).balanceOf(address(this)) > 0) {
          ...
          _swap(weth, bscx, IERC20(weth).balanceOf(address(this)));
        }

        if (IERC20(bscx).balanceOf(address(this)) > 0) {
          IERC20(bscx).safeTransfer(bscxSafe, IERC20(bscx).balanceOf(address(this)));
        }
    }
    ...
    function _toBaseToken(address token, uint amountIn) internal {
      if (token == bscx) return;

      if (_canSwap(token, bscx)) {
        _swap(token, bscx, amountIn);
      }
