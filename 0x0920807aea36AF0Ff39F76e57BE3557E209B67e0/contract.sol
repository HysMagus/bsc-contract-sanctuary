...
contract ButyToken is BEP20('ButySwap Token', 'BUTY') {
...
contract MasterChef is Ownable {
    ...
    // The BUTY TOKEN!
    ButyToken public buty;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public butyPerBlock;
    // Bonus muliplier for early buty makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _butyPerBlock,
        uint256 _startBlock
    ) public {
        buty = _buty;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        butyPerBlock = _butyPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 butyReward = multiplier.mul(butyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(butyReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 butyReward = multiplier.mul(butyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        buty.mint(devaddr, butyReward.div(10));
        buty.mint(address(this), butyReward);
        pool.accEggPerShare = pool.accEggPerShare.add(butyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe buty transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 butyBal = buty.balanceOf(address(this));
        if (_amount > butyBal) {
            buty.transfer(_to, butyBal);
        } else {
            buty.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _butyPerBlock) public onlyOwner {
        massUpdatePools();
        butyPerBlock = _butyPerBlock;
    }
}