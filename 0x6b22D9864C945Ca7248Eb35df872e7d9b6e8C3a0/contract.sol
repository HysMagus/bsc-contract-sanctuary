...
// Hitman Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract HitmanVaultV1 is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of HITMANs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accHitmanPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accHitmanPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. HITMANs to distribute per block.
        uint256 accHitmanPerShare; // Accumulated HITMANs per share, times 1e12. See below.
    ...
    // Update the given pool's HITMANs allocation point. Can only be called by the owner.
    ...
    // Sets the burn fee (defined as DEV_FEE) for this contract*
    uint16 DEV_FEE;
    function setDevFee(uint16 _DEV_FEE) public onlyOwner {
        require(_DEV_FEE <= 10000, 'Burn fee clamped at 10%');
        DEV_FEE = _DEV_FEE;
    }
    uint256 pending_DEV_rewards;


    // View function to see pending HITMANs on frontend.
    function pendingHitman(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accHitmanPerShare = pool.accHitmanPerShare;

        return user.amount.mul(accHitmanPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the HITMAN token.
    // ----
    uint256 private hitmanBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = hitman.balanceOf(address(this)).sub(hitmanBalance);

        if(newRewards > 0) {
            hitmanBalance = hitman.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 hitmanRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        hitmanRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 hitmanRewardFee = hitmanRewardWhole.mul(DEV_FEE).div(10000);
        uint256 hitmanRewardToDistribute = hitmanRewardWhole.sub(hitmanRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(hitmanRewardFee);

        pool.accHitmanPerShare = pool.accHitmanPerShare.add(
            hitmanRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit  tokens to HitmanVault for HITMAN allocation.
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accHitmanPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw tokens from HitmanVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming HITMAN farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accHitmanPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeHitmanTransfer(from, pending);
        }
    ...
    // Safe hitman transfer function, just in case if rounding error causes pool to not have enough HITMANs.
    function safeHitmanTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 hitmanBal = hitman.balanceOf(address(this));
        if (_amount > hitmanBal) {
            hitman.transfer(_to, hitmanBal);
            hitmanBalance = hitman.balanceOf(address(this));

        } else {
            hitman.transfer(_to, _amount);
            hitmanBalance = hitman.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 hitmanBal = hitman.balanceOf(address(this));
        if (pending_DEV_rewards > hitmanBal) {
            hitman.transfer(devaddr, hitmanBal);
            hitmanBalance = hitman.balanceOf(address(this));
        } else {
            hitman.transfer(devaddr, pending_DEV_rewards);
            hitmanBalance = hitman.balanceOf(address(this));
        }
