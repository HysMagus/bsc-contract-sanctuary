...
contract BvaultsBank is ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        mapping(uint8 => uint256) rewardDebt; // Reward debt. See explanation below.
    ...
    struct PoolInfo {
        ...
        mapping(uint8 => uint256) accRewardPerShare; // Accumulated rewardPool per share, times 1e18.
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({want: _want, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, strat: _strat}));
    }
    ...
    function updateRewardPerBlock(uint8 _rewardPid, uint256 _rewardPerBlock) external nonReentrant onlyOperator {
        massUpdatePools();
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint8 _rewardPoolLength = uint8(rewardPoolInfo.length);
        for (uint8 _rewardPid = 0; _rewardPid < _rewardPoolLength; ++_rewardPid) {
            uint256 _rewardPerBlock = rewardPoolInfo[_rewardPid].rewardPerBlock;
    ...
    function _getReward(uint256 _pid) internal {
        ...
        for (uint8 _rewardPid = 0; _rewardPid < _rewardPoolLength; ++_rewardPid) {
            ...
            if (_pending > 0) {
                address _rewardToken = rewardPoolInfo[_rewardPid].rewardToken;
    ...
    function inCaseTokensGetStuck(
        ...
        require(msg.sender == timelock, "BvaultsBank: !timelock");
