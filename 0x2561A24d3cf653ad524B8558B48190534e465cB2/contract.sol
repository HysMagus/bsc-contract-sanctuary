...
contract GameLobby is Ownable {
    ...
    event Claim(uint256 indexed epoch, string indexed gameName, address indexed target, address claimer);
    ...
    function closeLobby(uint256 epoch) public returns(bool) {
        ...
        address winner = lobby[epoch].scoreCreator > lobby[epoch].scoreJoiner ? lobby[epoch].creator : lobby[epoch].joiner;
    ...
    function claim(uint256 epoch) public returns(bool) {
        require(epoch < lobby.length, "GameLobby: lobby does not exist");
        require(lobby[epoch].open == false, "GameLobby: the lobby must be closed");
        require(lobby[epoch].claimed == false, "GameLobby: the prize is already claimed");

        Lobby storage targetLobby = lobby[epoch];
        targetLobby.claimed = true;
        uint256 prize = ((targetLobby.bet.mul(2)).mul(rewardRate).div(100));
        IERC20(token).safeTransfer(targetLobby.winner, prize);

        emit Claim(epoch, lobby[epoch].gameName, targetLobby.winner, msg.sender);
