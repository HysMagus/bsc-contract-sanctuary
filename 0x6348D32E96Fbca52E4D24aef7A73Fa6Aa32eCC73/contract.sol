...
contract SmartChef is Ownable {
    ...
    constructor(
        ...
        poolInfo.push(PoolInfo({
            ...
            accManyPerShare: 0,
            limitation: 1000000000000000000000000,
            fees: 4
        }));
    ...
    function deposit(uint256 _amount) public {
        ...
        uint256 userAmount;
         ...
         require (pool.lpToken.balanceOf(address(this)).add(_amount) <= pool.limitation, 'exceed max limit');
        ...
        if(_amount > 0) {
            uint256 depositFee = _amount.mul(pool.fees).div(100);
            userAmount = _amount.sub(depositFee);
            
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), userAmount);
            pool.lpToken.safeTransferFrom(address(msg.sender), owner(), depositFee);
