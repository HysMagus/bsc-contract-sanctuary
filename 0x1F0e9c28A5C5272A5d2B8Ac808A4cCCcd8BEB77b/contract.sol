...
// File: contracts/NarwhalPower.sol
...
contract NarwhalPower is IPowerStrategy, Governance {
    using SafeMath for uint256;
    ////
    struct narwhalsSegment {
        uint256 min;
        uint256 max;
    }
    struct countSegment {
    ...
    mapping(uint8 => narwhalsSegment) public _narwhalsSegment;
    ...
    uint256 constant public  _initMaxValue = 500 * (10**18); 
    ...
    function lpIn(address sender, uint256 amount) isNormalPool() override
    external {
    ...
    function lpOut(address sender, uint256 amount) override
    isNormalPool()
    external{
        uint32 playerId = _addressXId[sender];
        if ( playerId > 0 ) {
    ...
    function getRule(address sender) 
    override view external
    returns (uint8) {
    ...
    function updateRuler( uint256 maxCount ) internal{
        ...
        for (uint8 i = 1; i <= _ruler.length; i++) {
            ...
            _narwhalsSegment[i].min = lastBegin + 1;
            _narwhalsSegment[i].max = lastEnd;
            lastBegin = lastEnd;
        }
    }
    ...
    function findSegmentMinPlayer(uint8 segIndex) internal view returns (uint32,uint256){
        uint256 firstMinAmount = _narwhalsSegment[segIndex].max;
        uint256 secondMinAmount = _narwhalsSegment[segIndex].max;
    ...
    function tailSwap( uint8 segIndex) internal returns (uint32){
        ...
        _narwhalsSegment[segIndex].min = secondMinAmount;
    ...
    function joinMid(uint32 playerId) internal {
        ...
        _narwhalsSegment[segIndex].max = _narwhalsSegment[segIndex + 1].min;
    }
    ...
    function settlePowerData(uint32 playerId) internal {
        ...
        for (uint8 i = 1; i <= _high; i++) {
            if (amount < _narwhalsSegment[i].max) {
                segIndex = i;
                break;
            }
        }
        if (segIndex == 0) {
            _narwhalsSegment[_high].max = amount;
            segIndex = _high;
        }
