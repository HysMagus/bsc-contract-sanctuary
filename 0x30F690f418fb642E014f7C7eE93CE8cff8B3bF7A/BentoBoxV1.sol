...
contract MasterContractManager is BoringOwnable, BoringFactory {
    ...
    function _calculateDomainSeparator(uint256 chainId) private view returns (bytes32) {
        return keccak256(abi.encode(DOMAIN_SEPARATOR_SIGNATURE_HASH, keccak256("SwivelBox V1"), chainId, address(this)));
    }
    ...
    /// @notice Other contracts need to register with this master contract so that users can approve them for the SwivelBox.
                    ...
                    abi.encodePacked(
                        ...
                        keccak256(
                            abi.encode(
                                ...
                                approved
                                    ? keccak256("Give FULL access to funds in (and approved to) SwivelBox?")
                                    : keccak256("Revoke access to SwivelBox?"),
                                user,
                                masterContract,
                                approved,
                                nonces[user]++
...
// File contracts/SwivelBox.sol
// License-Identifier: UNLICENSED

/// @title SwivelBox
/// @author BoringCrypto, Keno
/// @notice The SwivelBox is a vault for tokens. The stored tokens can be flash loaned and used in strategies.
/// Yield from this will go to the token depositors.
/// Rebasing tokens ARE NOT supported and WILL cause loss of funds.
/// Any funds transfered directly onto the SwivelBox will be lost, use the deposit function instead.
contract SwivelBoxV1 is MasterContractManager, BoringBatchable {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    struct StrategyData {
        ...
        uint128 balance; // the balance of the strategy that SwivelBox thinks is in there
    }
    ...
    /// If 'from' is the SwivelBox itself, it's allowed. Any ETH, token balances (above the known balances) or SwivelBox balances
    ...
    modifier allowed(address from) {
        if (from != msg.sender && from != address(this)) {
            ...
            require(masterContract != address(0), "SwivelBox: no masterContract");
            require(masterContractApproved[masterContract][from], "SwivelBox: Transfer not approved");
        }
        _;
    }
    ...
    function deposit(
        ...
        // Checks
        require(to != address(0), "SwivelBox: to not set"); // To avoid a bad UI from burning funds
        ...
        require(total.elastic != 0 || token.totalSupply() > 0, "SwivelBox: No tokens");
        if (share == 0) {
    ...
    function withdraw(
        ...
        require(total.base >= MINIMUM_SHARE_BALANCE || total.base == 0, "SwivelBox: cannot empty");
        totals[token] = total;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            require(success, "SwivelBox: ETH transfer failed");
        } else {
    ...
    function transferMultiple(
        ...
        // Checks
        require(tos[0] != address(0), "SwivelBox: to[0] not set"); // To avoid a bad UI from burning funds
    ...
    function flashLoan(
        ...
        require(_tokenBalanceOf(token) >= totals[token].addElastic(fee.to128()), "SwivelBox: Wrong amount");
    ...
    function batchFlashLoan(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            require(_tokenBalanceOf(token) >= totals[token].addElastic(fees[i].to128()), "SwivelBox: Wrong amount");
    ...
    // F5 - Checks-Effects-Interactions pattern followed? (SWC-107)
    // F5: Total amount is updated AFTER interaction. But strategy is under our control.
    // F5: Not followed to prevent reentrancy issues with flashloans and SwivelBox skims?
    function harvest(
        IERC20 token,
        bool balance,
        ...
        if (balanceChange > 0) {
            ...
            // But tokens with balances that large are not supported by the SwivelBox.
            uint256 sub = uint256(-balanceChange);
