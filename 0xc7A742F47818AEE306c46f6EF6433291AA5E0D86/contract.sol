...
/**
 *Keep3rbOracle implementation on Pancakeswap for test
*/
...
contract Keep3rbOraclePancake {
    ...
    IPancakeRouter public constant UNI = IPancakeRouter(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    ...
    function pairForWETH(address tokenA) external pure returns (address) {
        return PancakeLibrary.pairFor(factory, tokenA, address(WETH));
    }
    ...
    function add(address tokenA, address tokenB) external {
        require(msg.sender == governance, "PancakeOracle::add: !gov");
        address pair = PancakeLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function work() public upkeep {
        ...
        require(worked, "PancakeOracle: !work");
    }
    ...
    function current(address tokenIn, uint amountIn, address tokenOut) external view returns (uint amountOut) {
        ...
        require(_valid(pair, periodSize.mul(2)), "PancakeOracle::quote: stale prices");
    ...
    function quote(address tokenIn, uint amountIn, address tokenOut, uint granularity) external view returns (uint amountOut) {
        address pair = PancakeLibrary.pairFor(factory, tokenIn, tokenOut);
        require(_valid(pair, periodSize.mul(granularity)), "PancakeOracle::quote: stale prices");
        (address token0,) = PancakeLibrary.sortTokens(tokenIn, tokenOut);
