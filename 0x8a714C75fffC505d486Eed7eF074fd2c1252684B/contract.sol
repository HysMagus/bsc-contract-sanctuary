...
contract BFI is ERC20 {
    ...
    uint256 public cap = 210000 ether;
    ...
    uint256 public publicFundPercent = 5750; // over 95
    uint256 public communityFundPercent = 2000; // over 95
    uint256 public teamFundPercent = 1750; // over 95

    uint256 public gameFundAmount = 10500 ether; // 210k * 5%

    uint256 public lastMinted;
    uint256 public constant mintingCooldownTime = 72 hours; // cant mint again less than 72 hours to avoid high aggressive emission

    event MintToFund(address indexed fund, uint256 amount);
    ...
    function setSplitPercents(
        uint256 _publicFundPercent,
        uint256 _communityFundPercent,
    ...
    function mintFunds(uint256 _amount) external onlyGovernance checkMintedTime {
        if (publicFundPercent > 0 && publicFund != address(0)) {
            uint256 _publicFundAmt = _amount.mul(publicFundPercent).div(9500);
            mint(publicFund, _publicFundAmt);
            emit MintToFund(publicFund, _publicFundAmt);
        }
        if (communityFundPercent > 0 && communityFund != address(0)) {
            uint256 _communityFundAmt = _amount.mul(communityFundPercent).div(9500);
        ...
        if (teamFundPercent > 0 && teamFund != address(0)) {
            uint256 _teamFundAmt = _amount.mul(teamFundPercent).div(9500);
...
contract BearnEthereumGateway {
    using SafeMath for uint256;

    BFI public bfiToken = BFI(0x81859801b01764D4f0Fa5E64729f5a6C3b91435b);
    uint256 public mininumBurnAmount = 1 ether;

    address public governance;
    mapping(address => bool) public minters;

    uint256 public mintedAmount = 0 ether;

    event WrapToBFIE(address account, uint256 amount);
    event UnwrapFromBFIE(address account, uint256 amount);

    constructor(address _bfi) public {
        if (_bfi != address(0)) bfiToken = BFI(_bfi);
        governance = msg.sender;
    }
    ...
    function setMinimumBurnAmount(uint256 _mininumBurnAmount) external onlyGovernance {
        mininumBurnAmount = _mininumBurnAmount;
    }

    function wrapToBFIE(address _receiver, uint256 _amount) public {
        require(_amount >= mininumBurnAmount, "_amount less than mininumBurnAmount");
        bfiToken.burnFrom(msg.sender, _amount);
        mintedAmount = mintedAmount.add(_amount);
        emit WrapToBFIE(_receiver, _amount);
    }

    function unwrapFromBFIE(address _receiver, uint256 _amount) public onlyMinter {
        mintedAmount = mintedAmount.sub(_amount);
        bfiToken.mint(_receiver, _amount);
        emit UnwrapFromBFIE(_receiver, _amount);
    }
}