 ...
 * the owner.
 */
contract DangoMoneyAirdropRandom is Ownable {
	using SafeMath for uint256;
	...
	uint256 internal _canClaimAt;
	uint256 internal _claimTokenFeeInWei = 2 * 1e16;
	 ...
	 */
	function claimAirdropToken() external payable returns (bool) {
		// Check fee
		require(msg.value >= _claimTokenFeeInWei, "Invalid transaction fee.");

		// Check time
		require(
			block.timestamp >= _canClaimAt,
			"Tokens are not unlocked at this time."
		);
		...
		require(
			_remainingTokens > 0,
			"500 DANGO tokens had been distributed to airdrop users."
		);

		// Check wallet
		require(
			_claimList[msg.sender] == 0,
			"You have claimed your airdrop tokens."
		);

		// Get random amount
		uint256 claimAmount = getRandomAmount();

		require(claimAmount > 0, "Invalid amount to claim.");

		if (claimAmount > _remainingTokens) {
			claimAmount = _remainingTokens;
		}
		...
		// Send tokens
		_airDropToken.safeTransfer(msg.sender, claimAmount);
		_claimList[msg.sender] = claimAmount;
		_paidTokens = _paidTokens.add(claimAmount);
		_remainingTokens = _remainingTokens.sub(claimAmount);
	...
	/**
	 * @notice A method update random amount list
	 */
	...
	/**
	 * @notice A method to clarify user can/can not claim airdrop token
	 */
	function canClaimAirdropToken() external view returns (bool) {
		return _claimList[msg.sender] == 0;
	}

	/**
	 * @notice A method to get user airdrop token information
	 */
	function myAirdropInfo()
		external
		view
		returns (
			bool,
	...
	{
		bool canClaimTokens = _claimList[msg.sender] == 0;
		return (
			canClaimTokens,
			_paidTokens,
			_remainingTokens,
			_claimList[msg.sender],
			_canClaimAt
		);
	}

	/**
	 * @notice A method to withdraw unclaim token to another pool if issues occur
	 */
