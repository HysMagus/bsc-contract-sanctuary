...
/**
 * @title Strafi Gold Treasury contract
 * @notice Monetary policy logic to adjust supplies of Strafi gold assets
...
contract Treasury is ContractGuard, Operator {
    ...
    // core components
    address private gold;
    address private bond;
    address private share;
    address private boardroom;
    address private goldOracle;

    // price
    uint256 public goldPriceOne;
    uint256 public goldPriceCeiling;
    ...
    constructor(
        ...
        goldPriceOne = 10**18;
        goldPriceCeiling = uint256(105).mul(goldPriceOne).div(10**2);

        bondDepletionFloor = uint256(1000).mul(goldPriceOne);
    }
    ...
    // oracle
    function getGoldPrice() public view returns (uint256 goldPrice) {
        try IOracle(goldOracle).consult(gold, 1e18) returns (uint256 price) {
            return price;
        } catch {
    ...
    function initialize() public notInitialized checkOperator {
        ...
        // mint only 1001 gold to itself
        IBasisAsset(gold).mint(address(this), 1001 ether);

        // set seigniorageSaved to it's balance
        seigniorageSaved = IERC20(gold).balanceOf(address(this));
    ...
    function _updateGoldPrice() internal {
        try IOracle(goldOracle).update()  {} catch {}
    }
        ...
        checkOperator
    {
        ...
        require(
            goldPrice < goldPriceOne, // price < $1
            'Treasury: goldPrice not eligible for bond purchase'
        );

        uint256 bondPrice = goldPrice;
        ...
        checkOperator
    {
        ...
        require(goldPrice == targetPrice, 'Treasury: gold price moved');
        require(
            goldPrice > goldPriceCeiling, // price > $1.05
            'Treasury: goldPrice not eligible for bond purchase'
        );
        require(
        ...
        checkOperator
    {
        ...
        uint256 goldSupply = IERC20(gold).totalSupply().sub(seigniorageSaved);
        uint256 percentage = goldPrice.sub(goldPriceOne);
        ...
        if (seigniorageSaved > bondDepletionFloor) {
            ...
            IERC20(gold).safeApprove(boardroom, seigniorage);
