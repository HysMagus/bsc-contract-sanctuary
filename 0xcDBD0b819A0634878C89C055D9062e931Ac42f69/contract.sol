...
contract CyberTradeFighter is TokenBase {
    ...
    function createToken(address _owner, string memory _name, string memory _class, string memory _syndicate, bool _isBoss,
                        string memory _genAttr, string memory _genElem, uint16 _potentialMax
        ...
        _fighter.potentialMax = _potentialMax;
    ...
    function deathFight(uint256 _fighter1, uint256 _fighter2) public ifNotPaused {
        ...
        require(msg.sender == tokenToOwner[_fighter2]);
        require(_fighter1 != _fighter2);
        
        Fighter storage fighter1 = fighters[_fighter1];
        Fighter storage fighter2 = fighters[_fighter2];
        
        require(fighter1.level == 1);
        require(fighter2.level == 1);
    ...
    function levelUp(uint256 _fighter, uint8 _h, uint8 _a, uint8 _d, uint8 _s, uint8 _decPotentialMax) public onlyContractOwnerOrAuthorizedContract ifNotPaused{
        Fighter storage fighter = fighters[_fighter];
        fighter.level++;
        fighter.xp = 0;
        fighter.hp = _h;
        ...
        fighter.defense = _d;
        fighter.agility = _s;
        fighter.potentialMax -= _decPotentialMax;
    }
