...
// Bsc Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract BscVaultV2 is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BSCs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBscPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accBscPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BSCs to distribute per block.
        uint256 accBscPerShare; // Accumulated BSCs per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing BSC governance consensus
    function add(
        uint256 _allocPoint,
        IBEP20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's BSCs allocation point. Can only be called by the owner.
    // Note contract owner is meant to be a governance contract allowing BSC governance consensus
    ...
    // View function to see pending BSCs on frontend.
    function pendingBsc(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accBscPerShare = pool.accBscPerShare;

        return user.amount.mul(accBscPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the BSC token.
    // ----
    uint256 private bscBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = bsc.balanceOf(address(this)).sub(bscBalance);

        if(newRewards > 0) {
            bscBalance = bsc.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 bscRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        bscRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
            .mul(pool.allocPoint)        // getting the pBEPent of total pending rewards this pool should get
            .div(totalAllocPoint);       // we can do this because pools are only mass updated
        uint256 bscRewardFee = bscRewardWhole.mul(DEV_FEE).div(10000);
        uint256 bscRewardToDistribute = bscRewardWhole.sub(bscRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(bscRewardFee);

        pool.accBscPerShare = pool.accBscPerShare.add(
            bscRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit  tokens to BscVault for BSC allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accBscPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accBscPerShare).div(1e12); /// This is deposited for address
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming BSC farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accBscPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeBscTransfer(from, pending);
        }
    ...
    // This means all future UNI CAKE BAKE  like airdrops are covered
    ...
    // Safe bsc transfer function, just in case if rounding error causes pool to not have enough BSCs.
    function safeBscTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 bscBal = bsc.balanceOf(address(this));
        if (_amount > bscBal) {
            bsc.transfer(_to, bscBal);
            bscBalance = bsc.balanceOf(address(this));

        } else {
            bsc.transfer(_to, _amount);
            bscBalance = bsc.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 bscBal = bsc.balanceOf(address(this));
        if (pending_DEV_rewards > bscBal) {
            bsc.transfer(devaddr, bscBal);
            bscBalance = bsc.balanceOf(address(this));
        } else {
            bsc.transfer(devaddr, pending_DEV_rewards);
            bscBalance = bsc.balanceOf(address(this));
        }
    ...
    // allowing BSC governance token holders to do this functions.
