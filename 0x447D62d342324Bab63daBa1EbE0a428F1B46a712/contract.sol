...
library PancakeSlidingOracleLibrary {
...
contract PancakeSlidingOracle {
    ...
    function add(address tokenA, address tokenB) external {
        require(msg.sender == governance, "PancakeSlidingOracle::add: !gov");
        address pair = PancakeLibrary.pairFor(factory, tokenA, tokenB);
        require(!_known[pair], "known");
        _known[pair] = true;
        _pairs.push(pair);

        (uint price0Cumulative, uint price1Cumulative,) = PancakeSlidingOracleLibrary.currentCumulativePrices(pair);
    ...
    function work() public upkeep {
        ...
        require(worked, "PancakeSlidingOracle: !work");
    }
    ...
    function current(address tokenIn, uint amountIn, address tokenOut) external view returns (uint amountOut) {
        ...
        require(_valid(pair, periodSize.mul(2)), "PancakeSlidingOracle::quote: stale prices");
    ...
    function quote(address tokenIn, uint amountIn, address tokenOut, uint granularity) external view returns (uint amountOut) {
        address pair = PancakeLibrary.pairFor(factory, tokenIn, tokenOut);
        require(_valid(pair, periodSize.mul(granularity)), "PancakeSlidingOracle::quote: stale prices");
        (address token0,) = PancakeLibrary.sortTokens(tokenIn, tokenOut);
