...
//YOU GET TWICE PROFITS THAN ANY OTHER CONTRACT
//JUST HODL YOUR RINGS AND YOU WILL BE RICH!

//And don’t forget:
//PROFITS RUN FASTER THAN SONIC!!!

contract SonicGame {
    
    uint256 public CEO_FEE = 5;  //THAT'S OUR CEO FEE IT'S 5%
    ...
    function getEggsSinceLastHatch(address adr) public view returns(uint256){
        ...
        uint256 dragonCount = SafeMath.mul(iceDragons[adr], 20);
        dragonCount = SafeMath.add(SafeMath.mul(ultraDragon[adr], 50), dragonCount);
    ...
    function buyIceDrangon() public payable {
        uint dragons = getDragonsToBuy(msg.value, 10);
        iceDragons[msg.sender] += dragons;
    }
    
    function buyUltraDrangon() public payable {
        require(activated);
        uint dragons = getDragonsToBuy(msg.value, 25);
