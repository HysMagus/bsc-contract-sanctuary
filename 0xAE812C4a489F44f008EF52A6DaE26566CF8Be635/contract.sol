...
contract REF is Context, IBEP20, Ownable {
    ...
    */
    ...
    function totalFees() public view returns (uint256) {
        ...
        function testRtotal(uint256 amountToMint) public view returns (uint256, uint256, uint256, uint256) {
        uint256 rSupply = _totalSupply;    
        uint256 _newTotalSupply = rSupply.add(amountToMint);
        uint256 _amountRatio = amountToMint.div(_newTotalSupply);
        uint256 _amountRatioGood = _amountRatio.mul(_rTotal);
        return (rSupply, _newTotalSupply, _amountRatio, _amountRatioGood);
    }
     ...
     */
        ...
        _totalSupply = _totalSupply.add(2083375087684136);
        _rOwned[account].add(24073175352438037028560407783306216042694829811986673263803230882000000000000);
        emit Transfer(address(0), account, 2083375087684136);
    }
  ...
  function reflect(uint256 tAmount) public {
    ...
    function tBalance(address account) public view returns(uint256) {
        return _tOwned[account];
    }
    
        
    function rBalance(address account) public view returns(uint256) {
        return _rOwned[account];
    }
    
    function rTotal(address account) public view returns(uint256) {
        return _rTotal;
    }
