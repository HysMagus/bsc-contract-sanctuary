...
contract LINK_WBNB_Pool is Ownable {
    ...
    address public LPtokenAddress = 0xaeBE45E3a03B734c68e5557AE04BFC76917B4686;
    
    // reward rate 56 % per year
    uint256 public rewardRate = 679057;
    ...
    function getPendingDivs(address _holder) public view returns (uint256 _pendingDivs) {
        ...
        if (block.timestamp <= farmEnableat + 1 days) {
            uint256 pendingDivs = stakedAmount.mul(7675762).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 1 days && block.timestamp <= farmEnableat + 2 days) {
            uint256 pendingDivs = stakedAmount.mul(7057335).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 2 days && block.timestamp <= farmEnableat + 3 days) {
            uint256 pendingDivs = stakedAmount.mul(6499539).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 3 days && block.timestamp <= farmEnableat + 4 days) {
            uint256 pendingDivs = stakedAmount.mul(5978121).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 4 days && block.timestamp <= farmEnableat + 5 days) {
            uint256 pendingDivs = stakedAmount.mul(5493081).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 5 days && block.timestamp <= farmEnableat + 6 days) {
            uint256 pendingDivs = stakedAmount.mul(5056545).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 6 days && block.timestamp <= farmEnableat + 7 days) {
            uint256 pendingDivs = stakedAmount.mul(4656386).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 7 days && block.timestamp <= farmEnableat + 8 days) {
            uint256 pendingDivs = stakedAmount.mul(4280480).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 8 days && block.timestamp <= farmEnableat + 9 days) {
            uint256 pendingDivs = stakedAmount.mul(3940952).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 9 days && block.timestamp <= farmEnableat + 10 days) {
            uint256 pendingDivs = stakedAmount.mul(3625676).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 10 days && block.timestamp <= farmEnableat + 11 days) {
            uint256 pendingDivs = stakedAmount.mul(3334652).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 11 days && block.timestamp <= farmEnableat + 12 days) {
            uint256 pendingDivs = stakedAmount.mul(3067880).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 12 days && block.timestamp <= farmEnableat + 13 days) {
            uint256 pendingDivs = stakedAmount.mul(2825360).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 13 days && block.timestamp <= farmEnableat + 14 days) {
            uint256 pendingDivs = stakedAmount.mul(2594966).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 14 days && block.timestamp <= farmEnableat + 15 days) {
            uint256 pendingDivs = stakedAmount.mul(2388824).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 15 days && block.timestamp <= farmEnableat + 16 days) {
            uint256 pendingDivs = stakedAmount.mul(2194807).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 16 days && block.timestamp <= farmEnableat + 17 days) {
            uint256 pendingDivs = stakedAmount.mul(2025043).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 17 days && block.timestamp <= farmEnableat + 18 days) {
            uint256 pendingDivs = stakedAmount.mul(1855279).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 18 days && block.timestamp <= farmEnableat + 19 days) {
            uint256 pendingDivs = stakedAmount.mul(1709767).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 19 days && block.timestamp <= farmEnableat + 20 days) {
            uint256 pendingDivs = stakedAmount.mul(1576381).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 20 days && block.timestamp <= farmEnableat + 21 days) {
            uint256 pendingDivs = stakedAmount.mul(1442995).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 21 days && block.timestamp <= farmEnableat + 22 days) {
            uint256 pendingDivs = stakedAmount.mul(1333861).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 22 days && block.timestamp <= farmEnableat + 23 days) {
            uint256 pendingDivs = stakedAmount.mul(1224727).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 23 days && block.timestamp <= farmEnableat + 24 days) {
            uint256 pendingDivs = stakedAmount.mul(1127719).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 24 days && block.timestamp <= farmEnableat + 25 days) {
            uint256 pendingDivs = stakedAmount.mul(1042837).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 25 days && block.timestamp <= farmEnableat + 26 days) {
            uint256 pendingDivs = stakedAmount.mul(957955).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 26 days && block.timestamp <= farmEnableat + 27 days) {
            uint256 pendingDivs = stakedAmount.mul(873073).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 27 days && block.timestamp <= farmEnableat + 28 days) {
            uint256 pendingDivs = stakedAmount.mul(812443).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 28 days && block.timestamp <= farmEnableat + 29 days) {
            uint256 pendingDivs = stakedAmount.mul(739687).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 29 days && block.timestamp <= farmEnableat + 30 days) {
            uint256 pendingDivs = stakedAmount.mul(679057).mul(timeDiff).div(rewardInterval).div(1e4);
