...
contract VTokenMock is ERC20Upgradeable {
    ...
    function mint(uint256 amount) external returns (uint256) {
        ...
        if (totalSupply() == 0) {
            ...
            newVTokens = FixedPoint.multiplyUintByMantissa(
                totalSupply(),
                fractionOfCredit
            );
        }
        _mint(msg.sender, newVTokens);
        require(
    ...
    function redeemUnderlying(uint256 requestedAmount)
        ...
        returns (uint256)
    {
        uint256 vTokens = vTokenValueOf(requestedAmount);
    ...
    function vTokenValueOf(uint256 tokens) public view returns (uint256) {
