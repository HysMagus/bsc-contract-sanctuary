...
contract GFCEPresale is Ownable, ReentrancyGuard, ContractGuard {
    ...
    uint256 public maxPurchasableTokenPerWallet = 5250 * 1e9; // 5000 GFCE considering 5% JETS bonus

    address public GFCEToken;
    uint256 public salePrice = 1E18; // 1 BUSD
    uint256 public totalPurchasedBUSD;
    uint256 public unclaimedPurchasedToken;
    bool public finishPresaleByAdmin;
    ...
    modifier isNotOver() {
        require(!finishPresaleByAdmin, "GFCE_PRESALE:presale is over");
        require(block.timestamp >= startTime, "GFCE_PRESALE:presale is not started yet");
        require(block.timestamp <= endTime, "GFCE_PRESALE:presale is ended");
        require(IBEP20(BusdToken).balanceOf(address(this)) < presaleCapInBUSD, "GFCE_PRESALE: sold out already");
        _;
    }
    modifier isOver() {
        require(finishPresaleByAdmin || block.timestamp < startTime || block.timestamp > endTime || IBEP20(BusdToken).balanceOf(address(this)) >= presaleCapInBUSD, "GFCE_PRESALE:presale not over yet");
        _;
    }

    constructor(address _GFCEToken, uint256 _startTime, uint256 _endTime) public {
        require(
            _GFCEToken != address(0x0),
            "GFCEPresale: GFCE address is 0x0."
        );
        GFCEToken = _GFCEToken;
        finishPresaleByAdmin = false;
        startTime = _startTime;
        endTime = _endTime;
    }

    function presaleIsOn() public view returns (bool) {
        if(!finishPresaleByAdmin && block.timestamp >= startTime && block.timestamp <= endTime && IBEP20(BusdToken).balanceOf(address(this)) < presaleCapInBUSD)
            return true;
        return false;
    }
    
    function presaleIsOver() public view returns (bool) {
        if(finishPresaleByAdmin || block.timestamp < startTime || block.timestamp > endTime || IBEP20(BusdToken).balanceOf(address(this)) >= presaleCapInBUSD)
            return true;
        return false;
    }
        ...
        returns (uint256)
    {
        ...
        uint256 purchaseGFCEAmount;
        if (puchaserJetsValueInUSD > 0) {
            if (puchaserJetsValueInUSD >= purchaseValueInUSD) {
                purchaseGFCEAmount = purchaseValueInUSD
                    .mul(uint256(100).add(bonusFee))
                    .div(100)
                    .div(1e19);
            } else {
                purchaseGFCEAmount = puchaserJetsValueInUSD
                    .mul(uint256(100).add(bonusFee))
                    .div(100)
                    .div(1e19);
                purchaseGFCEAmount = purchaseGFCEAmount.add(
                    purchaseValueInUSD.sub(puchaserJetsValueInUSD).div(1e19)
                );
            }
        } else {
            purchaseGFCEAmount = purchaseValueInUSD.div(1e19);
        }
        
        // calculate the token amount to return considering the salePrice
        return purchaseGFCEAmount.mul(1e18).div(salePrice);
    }
        ...
        isNotOver
    {
        ...
        // calculate the token amount to return considering the salePrice
        purchaseGFCEAmount = purchaseGFCEAmount.mul(1e18).div(salePrice);
        ...
        unclaimedPurchasedToken = unclaimedPurchasedToken.add(purchaseGFCEAmount);
        if( purchasedToken[_msgSender()] ==0){
        ...
        require(purchasedToken[_msgSender()] <= maxPurchasableTokenPerWallet, "GFCE_PRESALE: exceeded max purchasable GFCE amount per wallet.");
        
        //IBEP20(GFCEToken).transfer(_msgSender(), purchaseGFCEAmount);
    }

    function sendPurchased(uint256 startingIndex, uint256 endignIndex)
        external
        nonReentrant
        noContract(msg.sender)
        onlyOwner
        isOver
    {
        for(uint i=startingIndex;i<endignIndex;i++){
            address account=purchasers[i];
            if(purchasedToken[account] != 0){
                uint256 amount = purchasedToken[account];
                purchasedToken[account] = 0;
                unclaimedPurchasedToken = unclaimedPurchasedToken.sub(amount);
                IBEP20(GFCEToken).transfer(account, amount);
            }
        }
    ...
    function withdrawGFCEAfterPresaleOver() public onlyOwner isOver {
        uint256 amount =
            IBEP20(GFCEToken).balanceOf(address(this)).sub(
                unclaimedPurchasedToken
            );
        require(amount > 0, "GFCE_PRESALE: zero amount");
        IBEP20(GFCEToken).transfer(_msgSender(), amount);
    }
    
    function totalPurchasers() public view returns(uint256){
