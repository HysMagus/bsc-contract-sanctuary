...
// ----------------------------------------------------------------------------
// BEP20 Token Standard #20 Interface
// ----------------------------------------------------------------------------
interface IBEP20 {
...
contract Stake is Owned {
    ...
    address public BSC = 0xf5128928f85F16bD67C1E02DdD1b47A71d17aDF5;
    ...
    function STAKE(uint256 tokens) external {
        require(IBEP20(BSC).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function ADDFUNDS(uint256 tokens) external {
        require(IBEP20(BSC).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function CLAIMREWARD() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IBEP20(BSC).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        require(IBEP20(BSC).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedBSC(address staker) external view returns(uint256 stakedBSC){
        return stakers[staker].stakedTokens;
    }
    
    // ------------------------------------------------------------------------
    // Get the BSC balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourBSCBalance(address user) external view returns(uint256 BSCBalance){
