...
contract pokerGame is Ownable {
    ...
    uint public constant timeLocked = 2 weeks;
    uint public constant amountToLock = 3000000000000000000;
    ...
    function addParticipant(address _holder, address _player, string memory _name) public returns (bool){
        ...
        require(Token(tokenAddress).transferFrom(_holder, address(this) , amountToLock ) , "Error locking 3 tokens");
        
        holders.add(_holder);
        player[_holder] = _player;   
        name[_holder] = _name;
        timeLock[_holder] = block.timestamp;
        locked[_holder] = true;
    ...
    function unlock( address _dir) public returns (bool){
        require(msg.sender == _dir);
        require(locked[_dir]);
        require(block.timestamp.sub(timeLock[_dir]) >= timeLocked );
        
        locked[msg.sender] = false;
        require (Token(tokenAddress).transfer(_dir, amountToLock ) );
        
        return true;
        
    }
    
    function tmeToFinishUnlock() public view returns (uint){
        uint returnTime;
        
        if(timeLock[msg.sender] == 0 ){
            returnTime = 0;
        }else{
            returnTime = timeLock[msg.sender].add(timeLocked);
        }
