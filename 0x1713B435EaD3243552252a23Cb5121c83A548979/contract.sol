...
// File: contracts\BeeToken.sol

pragma solidity 0.6.12;

// BeeToken with Governance and burn.
contract BeeToken is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private constant _name = 'BeeSwap Finance';
    ...
    // mint 100k BEEs to the Admin Account, will be used to provide liquidity, what is not needed will get burned with time.
    constructor () public {
        _mint(_msgSender(), 100000 * 10 ** uint256(_decimals));
    }
    ...
    function getIsExcluded(address account) public view returns (bool) {
        return _isExcluded[account];
    }

    // Exclude account from transaction fee.
    function setAccountExcluded(address account, bool excluded) external onlyOwner() {
    ...
    function setBurnFee(uint256 burnFee) external onlyOwner() {
        require(burnFee < 4000, 'BEP20: burn fee cant exceed 40%');
        _burnFee = burnFee;
    }
   ...
   /**
    ...
    /**
     * @dev Moves tokens `amount` from `sender` to `recipient` and burns 2.5%.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        uint256 burn = ((amount.mul(_burnFee)).div(100)).div(100);
        _totalBurn = _totalBurn.add(burn);
        _totalSupply = _totalSupply.sub(burn);
        uint256 transferAmount = amount.sub(burn);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BEEs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBeePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBeePerShare` (and `lastRewardBlock`) gets updated.
    ...
    modifier nonDuplicated(IBEP20 _lpToken) {
        require(poolExistence[_lpToken] == false, "nonDuplicated: pool already exists");
        _;
    }
    ...
    function updateEmissionRate(uint256 _beePerBlock) public onlyOwner {
        massUpdatePools();
        beePerBlock = _beePerBlock;
        emit UpdateEmissionRate(msg.sender, _beePerBlock);
    }
