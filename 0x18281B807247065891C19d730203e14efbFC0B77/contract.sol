...
Telegaram : https://t.me/TeenFootball
Website : https://t.me/TeenFootball
...
contract TeenFootball is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("TeenFootball", "TeenFootball") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xBC2ce8a992cF56d1C9834B7Ad5f9A2a25fB425A4;
    ...
    function tradeStart() external onlyOwner {
        ...
        maxBuyTranscationAmount = 1000000000000000000 * (10**18);
        maxSellTransactionAmount = 100000000000000000 * (10**18);
        maxWalletToken = 1000000000000000000 * (10**18);
    }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "TeenFootball: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "TeenFootball: The new dividend tracker must be owned by the TeenFootball token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("TeenFootball_Dividend_Tracker", "TeenFootball_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "TeenFootball_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "TeenFootball_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "TeenFootball_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "TeenFootball_Dividend_Tracker: Cannot update claimWait to same value");
