...
// File: contracts/NarwhalReward.sol
...
contract NarwhalReward is LPTokenWrapper{
    using SafeERC20 for IERC20;
    ...
    uint256 public _initReward = 840000 * 1e18;
    ...
    function updateBuffRate(address account) public {
        ...
        if(daysCalculate > 0 && userInfo[account].currentBuffrate < maxBuffRate){
            ...
            for(uint8 i=0; i < daysCalculate; i++){
                ...
                buffrate = buffrate.add(_growthBuffRate.mul(buffrate).div(100));
            }

            if(maxBuffRate < buffrate ){
                userInfo[account].currentBuffrate = maxBuffRate;
                userInfo[account].currentPower = maxBuffRate.mul(balanceOf(account));
                userInfo[account].timeStamp =  userInfo[account].timeStamp.mul(daysCalculate);
            }
            else{
        ...
        checkStart
    {
        ...
        if(oldBalance == 0){
            uint256 power =  amount.mul(1e3);
            userInfo[msg.sender].currentBuffrate = 1e3;
            userInfo[msg.sender].timeStamp = now;
            userInfo[msg.sender].currentPower = power;
            _totalPower = _totalPower.add(power);
        }
        else{
            _totalPower = _totalPower.sub(userInfo[msg.sender].currentPower);
            uint256 averageBuffRate = (userInfo[msg.sender].currentBuffrate.mul(oldBalance)).add((amount).mul(1e3));
            uint256 newBuffRate = averageBuffRate.div(balanceOf(msg.sender));
            userInfo[msg.sender].currentBuffrate = newBuffRate;
            userInfo[msg.sender].timeStamp = now;
            userInfo[msg.sender].currentPower = newBuffRate.mul(balanceOf(msg.sender));
        ...
        checkStart
    {
        ...
        userInfo[msg.sender].timeStamp = now;
        userInfo[msg.sender].currentBuffrate = 1e3;
        userInfo[msg.sender].currentPower = balanceOf(msg.sender).mul(1e3);
