...
contract pepedegensDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("pepedegens_Dividend_Tracker", "pepedegens_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "pepedegens_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "pepedegens_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main pepedegens contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "pepedegens_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "pepedegens_Dividend_Tracker: Cannot update claimWait to same value");
...
contract pepedegens is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    pepedegensDividendTracker public dividendTracker;
    ...
    constructor() ERC20("pepedegens", "pepedegens") {
        BNBRewardsFee = 10;
        liquidityFee = 4;
        marketingFee = 6;
        marketingWallet = payable(0x268a2FB15442181B70ADCff0A364dB9F3e0745F4);
        maxWalletToken = 300000000 * (10**18);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new pepedegensDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "pepedegens: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "pepedegens: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "pepedegens: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "pepedegens: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "pepedegens: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "pepedegens: Cannot update gasForProcessing to same value");
