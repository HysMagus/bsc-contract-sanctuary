...
/*
 * CheeseTart one of 3 Elite Jobs from Keep3r BSC Network
...
contract CheeseTart {
  ...
  function acceptGovernance() external {
    ...
    function add(address tokenA, address tokenB) external {
        ...
        (uint price0Cumulative, uint price1Cumulative,) = CheeseTartLibrary.currentCumulativePrices(pair);
    ...
    function _swap(uint _amount) internal returns (uint) {
        ...
        uint[] memory amounts = UNI.swapExactTokensForTokens(_amount, uint256(0), path, address(this), block.timestamp.add(600));
