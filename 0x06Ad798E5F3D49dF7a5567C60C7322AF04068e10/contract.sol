...
interface IyToken {
    function tokenIn(address, uint) external;
    function tokenOut(address, uint) external;
...
contract fToken_proxy is ERC20, Iy_3dToken {
    address constant factory = address(0x32CE36F6eA8d97f9fC19Aab83b9c6D2F52D74470);
    address constant fortube_bank = address(0x0cEA0832e9cdBb5D476040D58Ea07ecfbeBB7672);
    address constant fortube_bank_controller = address(0xc78248D676DeBB4597e88071D3d889eCA70E5469);
    ...
    constructor (address u_token) ERC20(_proxy(IERC20(u_token).name()), _proxy(IERC20(u_token).symbol()), IERC20(u_token).decimals()) public {
        _u = u_token;
        IERC20(_u).approve(fortube_bank_controller, uint(-1));
    }
    ...
    function depositAll() public {
        ERC20 u = ERC20(_u);   
        IyToken y = IyToken(fortube_bank);
        y.tokenIn(_u, u.balanceOf(address(this)));
    }
    
    function withdrawAll() public {
        IyToken y = IyToken(fortube_bank);
        y.tokenOut(_u, uint(-1));
    }
