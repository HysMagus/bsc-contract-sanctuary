 ...
 * the owner.
 */
contract DangoMoneyPool is Ownable {
	using SafeMath for uint256;
	...
	uint256 public _transactionFeeInWei = 100000000000000000; // 0.1 BNB
	...
	constructor(
		...
		_rewardToken = IBEP20(rewardTokenAddress);

		_stakeTokenAddress = stakeTokenAddress;
		_stakeToken = IBEP20(stakeTokenAddress);
	 ...
	 */
	modifier pathUpdateReward(uint256 timestamp) {
		uint256 currentValue = timestamp > _poolEndedAt
			? _poolEndedAt
			: timestamp;

		for (uint256 i = 0; i < _stakeholders.length; i++) {
			(uint256 currentReward, , ) = rewardOf(_stakeholders[i]);
			_rewards[_stakeholders[i]] = currentReward;
			_userStakes[_stakeholders[i]].lastRewardUpdatedAt = currentValue;
		}
		_;
	}
	...
	function addOrUpdateStakeholder(
		...
		if (!exists_) {
			...
			Stake memory newStake = Stake({
				...
				stakeAmount: _amount,
				lastClaimAt: 0,
				lastRewardUpdatedAt: _timestamp,
		...
		_userStakes[_stakeholder].lastRewardUpdatedAt = _timestamp;
	...
	function stake(uint256 _amount)
		...
		poolAvailable
		ensurePoolStart
		pathUpdateReward(block.timestamp)
		...
		)
	{
		...
		uint256 poolSpentTimeInSeconds = lastTimePeriod.sub(
			_userStakes[_stakeholder].lastRewardUpdatedAt
		);

		if (poolSpentTimeInSeconds == 0) {
			return (
				_rewards[_stakeholder],
				_userStakes[_stakeholder].stakeAmount,
				_paidRewards[_stakeholder]
			);
		}

		uint256 newReward = _userStakes[_stakeholder]
			.stakeAmount
			.mul(_poolRewardPerSecondRate.mul(poolSpentTimeInSeconds))
			.div(_poolTotalStake);

		uint256 availableReward = _rewards[_stakeholder].add(newReward);
		return (
			availableReward,
			_userStakes[_stakeholder].stakeAmount,
			_paidRewards[_stakeholder]
