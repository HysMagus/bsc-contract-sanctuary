...
contract EarthToken is Context, IBEP20, Ownable {
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accEarthPerShare;   // Accumulated PLTS per share, times 1e12. See below.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 earthReward = multiplier.mul(earthPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        earth.mint(devaddr, earthReward.div(10));
    ...
    // Safe earth transfer function, just in case if rounding error causes pool to not have enough PLTs.
    function safeEarthTransfer(address _to, uint256 _amount) internal {
        uint256 earthBal = earth.balanceOf(address(this));
        if (_amount > earthBal) {
            earth.transfer(_to, earthBal);
        } else {
            earth.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _earthPerBlock) public onlyOwner {
        massUpdatePools();
        earthPerBlock = _earthPerBlock;
    }
}