...
interface IGnftBscsStartPools {
    function addGenesisNft(uint256 _nftId, uint256 _price, address _nftOwner,address _tokenAddress, bool stakeAble) external;
    ...
    Pausable 
{
    ...
    // nft id to whitelist or not
    mapping(uint256 => bool) public InoNftList;
    // INO nft id sold by bscs or not
    mapping(uint256 => bool) public nftInoSold;
    
    bool public nftStakeAble; // NFT batch stake able or not
    ...
    constructor(
        ...
        // setting stakeable to true by default
        nftStakeAble = true;
    }
    
    function adConfig(address _quoteErcAddress, address _feeAddr, uint256 _fee) external onlyOwner {
    ...
    // set stakeable for nfts
    function setNftStakeAble(bool _stakeAble) external onlyOwner {
        nftStakeAble = _stakeAble;
    }
    ...
    function buyTokenTo(uint256 _tokenId, address _to) public override whenNotPaused {
        ...
        // INO NFT item
        if(InoNftList[_tokenId] && nftInoSold[_tokenId] != true){
            // very first time selling nft INO item (selling by bscs)
            require(inoWhitelisted[msg.sender],'You are not in whitelist');
            // flag as true so later on dont need to check whitelist
            nftInoSold[_tokenId] = true;
        }
    ...
    function readyToSellTokenTo(
        ...
        if (nftPoolAddr != address(0) && address(_msgSender()) == genesisNftCreator) {
            IGnftBscsStartPools(nftPoolAddr).addGenesisNft(
                _tokenId, 
                _price, 
                address(this), 
                address(nft),
        ...
        // bscs admin add genesis INO nft data to nft pool so later on user will able to stake it
