...
contract BBWtest {
	...
	uint256 constant public PROJECT_FEE = 10; // 5% during invest + 5% during withdraw;
    ...
    uint256[7] public defaultPackages = [0.01 ether,0.2 ether,0.4 ether ,0.6 ether,1 ether,4 ether, 10 ether];
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function invest(address referrer) public payable {
		...
		require(msg.value >= INVEST_MIN_AMOUNT,'Min invesment 0.01 BNB');
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){
      ...
      uint256 TotalDeposit = users[_user].amount;
      if(users[_user].refs[0] >=4 && (TotalDeposit >=0.01 ether && TotalDeposit < 4 ether)){
          reivest = 50;
          withdrwal = 50;
