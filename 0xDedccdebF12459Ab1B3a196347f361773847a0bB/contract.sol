...
contract EggTokenN is BEP20('Goose Golden EggN', 'EGGN') {
...
contract MasterChefN is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 eggReward = multiplier.mul(eggPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        eggN.mint(devaddr, eggReward.div(10));
        eggN.mint(address(this), eggReward);
    ...
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 eggBal = eggN.balanceOf(address(this));
        if (_amount > eggBal) {
            eggN.transfer(_to, eggBal);
        } else {
            eggN.transfer(_to, _amount);
        }
    }
