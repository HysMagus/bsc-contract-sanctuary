...
/**
 * @title InfinityAndBeyond price Oracle
 *      This Oracle calculates the average USD price of InfinityAndBeyond based on the BNB-INFB and BUSD-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private INFBBnbPrice0CumulativeLast;
    uint private INFBBnbPrice1CumulativeLast;
    uint32 private INFBBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _INFB_bnb;
    ...
    constructor(
        address __INFB_bnb,   // Address of the INFB-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _INFB_bnb = IUniswapV2Pair(__INFB_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        INFBBnbPrice0CumulativeLast = _INFB_bnb.price0CumulativeLast();
        INFBBnbPrice1CumulativeLast = _INFB_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, INFBBnbBlockTimestampLast) = _INFB_bnb.getReserves();
    ...
    // Get the average price of 1 IFNB in the smallest BNB unit (18 decimals)
    function getINFBBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_INFB_bnb));

        require(_blockTimestamp != INFBBnbBlockTimestampLast, "InfinityAndBeyond Last and current are equal");

        FixedPoint.uq112x112 memory INFBBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - INFBBnbPrice0CumulativeLast) / (_blockTimestamp - INFBBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, INFBBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        (INFBBnbPrice0CumulativeLast, INFBBnbPrice1CumulativeLast, INFBBnbBlockTimestampLast, INFBBnbAverage) = getINFBBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, INFBBnbAverage) = getINFBBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = INFBBnbAverage*(1e6) / busdBnbAverage;
