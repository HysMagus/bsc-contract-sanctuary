...
contract BurningMoonDividentNFT is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    ...
    // Token name
    string private _name="NanoDoge x BurningMoon";
    // Token symbol
    string private _symbol="INDCxBM";
    ...
    uint256 public currentValue=3*10**17; //0.3 BNB for first NFT
    ...
    address constant TokenAddress=0xD3Af8E09894D69E937e37eeF46Df1cBF82b35C81;
    //TestNet
    ...
    constructor() {
        ...
        NFTPrototypes.push(NFTData("Common Nanodoge", "0", Rarity.Common));
        NFTPrototypes.push(NFTData("Rare Nanodoge", "1", Rarity.Rare));
        NFTPrototypes.push(NFTData("Ultra Rare Nanodoge", "2", Rarity.UltraRare));
        NFTPrototypes.push(NFTData("Legendary Nanodoge", "3", Rarity.Legendary));
        NFTPrototypes.push(NFTData("Legendary Shiny Nanodoge", "4", Rarity.LegendaryShiny));
    ...
    function AccountClaimDividents() external{
        ...
        Payout(msg.sender);
        Airdrop(airdropsPerClaim);
    }
    function Payout(address account) public isInFunction{
    ...
    function _distributeBNB() private{
        ...
        if(amount<=0) return;
            ...
            ContractBNB+=amount-(BMAmount+TokenAmount);
    ...
    function _buyAndSacrificeBM(uint256 amount)private{
        ...
        BM.transfer(msg.sender,TokenForSender);
        BM.transfer(address(0xdead),SwapBM-TokenForSender);
    }
    ...
    uint256 public MaxNFTCount=1000;
    ...
    event Mint(address to, uint ID, uint8 rarity);
    function _mint(address to, uint seed) private{
        uint value=_prng(100, seed);
        Rarity rarity;
        if(value==0) rarity=Rarity.LegendaryShiny;
        else if(value<=3) rarity=Rarity.Legendary;
        else if(value<15) rarity=Rarity.UltraRare;
        else if(value<40) rarity=Rarity.Rare;
        else rarity=Rarity.Common; 

        uint256 ID=NFTHolders.length;
        NFTs.push(uint8(rarity));
        ownerID.push(0);
        NFTHolders.push(to);
        _AddNFT(to,ID);
        NFTValue.push(0);
