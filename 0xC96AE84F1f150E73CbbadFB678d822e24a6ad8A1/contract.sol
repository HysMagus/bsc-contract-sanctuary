 ...
 */
contract StrategySpongeLP is Ownable, Pausable {
     ...
     * {soak} - Token generated by staking our funds. In this case it's the CAKEs token.
     ...
     * {soakToWbnbRoute} - Route we take to get from {soak} into {wbnb}.
     * {wbnbToBifiRoute} - Route we take to get from {wbnb} into {bifi}.
     * {soakToLp0Route} - Route we take to get from {soak} into {lpToken0}.
     * {soakToLp1Route} - Route we take to get from {soak} into {lpToken1}.
     */
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(soak).balanceOf(address(this)).mul(45).div(1000);
        IPancakeRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWbnb, 0, soakToWbnbRoute, address(this), now.add(600));
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != soak) {
            IPancakeRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(soakHalf, 0, soakToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != soak) {
            IPancakeRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(soakHalf, 0, soakToLp1Route, address(this), now.add(600));
        }
