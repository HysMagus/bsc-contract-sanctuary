...
contract Presale is Ownable{
    ...
    constructor(
        IBEP20 _token,
        uint256 _normalRate,
        uint256 _loyaltyRate,
        uint256 _startingBlock,
        uint256 _privateBlocks
        ){
            token = _token;
            normalRate = _normalRate;
            loyaltyRate = _loyaltyRate;
            startingBlock=_startingBlock;
            privateBlocks =_privateBlocks;
            MIN =0.1 ether;
            MAX =10 ether;
            raisingAmount = 1300 ether;
            endingBlock = startingBlock + 115200;
    }
    modifier isAvailable(){
        require(endingBlock>block.number,"Presale ended");
    ...
    modifier isEligible(){
        ...
        require(msg.value>=MIN,"Lower than the minimum value : 0.1 BNB");
    ...
    receive() external payable isAvailable isEligible{
        ...
        (uint256[2] memory out,uint256 totalval,bool loyal) = loyalty(_msgSender());
        
        if(loyal){
            rate = loyaltyRate.div(100);
        }
        else{
            rate = normalRate.div(100);
        }
    ...
    function setRate(uint256 _normalRate,uint256 _loyaltyRate) external onlyOwner{
        normalRate = _normalRate;
        loyaltyRate = _loyaltyRate;
    }
    ...
    function setStartingBlock(uint256 _blockCount) external onlyOwner{
     ...
     function setEndingBlock(uint256 _block) external onlyOwner{
        endingBlock = _block;
    }
    ...
    function loyalty(address sender)public view returns(uint256[2] memory out,uint256 totalval,bool loyal){
        uint256 oilCount=IBEP20(OIL).balanceOf(sender)+IBEP20(OIL_STAKE).balanceOf(sender);
        uint256 oil_wbnbCount=IBEP20(OIL_WBNB).balanceOf(sender)+IBEP20(OIL_WBNB_STAKE).balanceOf(sender);
        uint256 oil_busdCount=IBEP20(OIL_BUSD).balanceOf(sender)+IBEP20(OIL_BUSD_STAKE).balanceOf(sender);
