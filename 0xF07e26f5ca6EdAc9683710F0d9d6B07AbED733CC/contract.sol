...
contract BStake is ERC20("BSwap Stake Token", "BSTAKE"), Ownable {
    using SafeMath for uint256;
    IERC20 public erc20;
    ...
    // next period that BStake will remain opened
    ...
    function setBSwapBNBpid(uint _newPid) public onlyOwner {
        pid = _newPid;
    }
    ...
    function pendingERC20() external view returns (uint256) {
      ...
      uint256 bstakeShare = balanceOf(msg.sender);
      uint256 what = bstakeShare.mul(erc20.balanceOf(address(this))).div(totalShares);
