...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event HarvestAndRestake(address indexed user, uint256 indexed harvestPool, uint256 indexed stakePool, uint256 amount);
    ...
    function pendingMaisons(uint256 _pid, address _user) public view validatePoolByPid(_pid) returns (uint256 maisonReward) {
        ...
        maisonReward = user.amount.mul(accMaisonPerShare).div(ACC_MAISON_PRECISION).sub(user.rewardDebt).mul(1000);
    }
    ...
    function harvestAndRestake(uint256 _harvestPool, uint256 _stakePool) public validatePoolByPid(_stakePool) validatePoolByPid(_harvestPool) returns (bool success) {
        require(poolInfo[_stakePool].lpToken == maison, "You can only restake to MSN pool");
        PoolInfo storage poolHarvest = poolInfo[_harvestPool];
        UserInfo storage userHarvest = userInfo[_harvestPool][_msgSender()];
        updatePool(_harvestPool);
        uint256 pending = userHarvest.amount.mul(poolHarvest.accMaisonPerShare).div(ACC_MAISON_PRECISION).sub(userHarvest.rewardDebt);
        if (pending > ZERO) {
            success = true;
            // safeTransferTokenFromThis(maison, _msgSender(), pending);
            poolHarvest.rewarded = poolHarvest.rewarded.add(pending);
            userHarvest.rewardDebt = userHarvest.amount.mul(poolHarvest.accMaisonPerShare).div(ACC_MAISON_PRECISION);
        } else {
            success = false;
        }

        PoolInfo storage poolStake = poolInfo[_stakePool];
        require(block.number <= poolStake.endBlock, "this pool has ended!");
        require(block.number >= poolStake.startBlock, "this pool has not started!");
        if (poolStake.lockEnabled) {
            require(block.number < poolStake.lastDepositBlock);
        }
        UserInfo storage userStake = userInfo[_stakePool][msg.sender];
        poolStake.amount = poolStake.amount.add(pending);
        userStake.amount = userStake.amount.add(pending);
        userStake.rewardDebt = userStake.amount.mul(poolStake.accMaisonPerShare).div(ACC_MAISON_PRECISION);
        if (poolStake.lockEnabled) {
            userStake.unlockTime = block.timestamp + poolStake.vestingTime;
            userStake.withdrawTX = 0;
            userStake.withdrawablePerTime = userStake.amount.div(50);
            userStake.underclaimed = 0;
        }
        emit HarvestAndRestake(_msgSender(), _harvestPool, _stakePool, pending);
    }
    ...
    function getClaimCycle(uint256 _pid, address _address) public view returns (uint256) {
        ...
        uint256 _timeShare = (block.timestamp.sub(user.unlockTime)).div(pool.unlockPeriod);
        if (_timeShare >= 50) {
            _timeShare = 50;
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) payable {
        ...
        if (pool.lockEnabled) {
            ...
            uint256 _realClaimCycle = _amount.div(user.withdrawablePerTime);
            uint256 _CycleMax = _realClaimCycle.mul(user.withdrawablePerTime);
            if (_amount != _CycleMax) {
            ...
            } else if (user.amount == _amount) {
                user.underclaimed = ZERO;
            }
            user.withdrawTX += _realClaimCycle;
        }
