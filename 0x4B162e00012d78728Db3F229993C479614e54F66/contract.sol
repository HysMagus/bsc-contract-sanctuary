...
contract StinkDAO is ERC20 {
    ...
    uint256 blockWaitTime = 28400;
    uint256 holderReward = 2 * _decimals;
    uint256 initiatorReward = 1 * _decimals;
    
    uint256 public genesisRoundBlock;
    
    uint256 public maxSupply = 1_000_000 * _decimals;
    
    bool public allocationStarted;
    
    bool public allAllocated;
    
    struct Round {
        uint256 roundNumber;
        uint256 roundBlock;
        address roundInitiator;
        uint256 roundReward;
    }
    ...
    mapping (address => uint256) unclaimedAmount;
    ...
    modifier onlyHolder() {
        require((LittleShartCollectible(_lshrt).balanceOf(msg.sender) > 0) || (LittleShartMarketplace(_mktplc).getTokensListed(msg.sender).length > 0), "Must hold at least 1 Little Shart!");
        _;
    }
    ...
    function getUnclaimedAmount() public view returns (uint256) {
        return unclaimedAmount[msg.sender];
    }
    
    function updateMaxSupply(uint256 amount) public onlyOwner {
        require(allAllocated, "All tokens must be allocated first!");
        maxSupply = amount * _decimals;
        allAllocated = false;
    }
    
    // To be called by a holder once all tokens have been allocated
    function finishAllocation() public onlyOwner {
        require(allocationStarted, "Allocation has not started yet!");
        require((block.number - blockWaitTime) >= currentRound.roundBlock, "Must wait 28400 blocks in between rounds!");
        allAllocated = true;
    }
    
    // Mint tokens from unclaimedAmount to the holder. Takes a certain amount in wei
    function claimTokens(uint256 amount) public {
        require(unclaimedAmount[msg.sender] >= amount, "Attempting to claim too much!");
        _mint(msg.sender, amount);
        totalUnclaimed -= amount;
        unclaimedAmount[msg.sender] -= amount;
    }
    
    // Mint tokens from unclaimedAmount to the holder. Withdraws all tokens.
    function claimTokens() public {
        require(unclaimedAmount[msg.sender] > 0, "No unclaimed balance!");
        _mint(msg.sender, unclaimedAmount[msg.sender]);
        totalUnclaimed -= unclaimedAmount[msg.sender];
        unclaimedAmount[msg.sender] = 0;
    }
    
    // To be called by any holder, this will allocate and initiate the new round. Rewards the initiator with 1 extra token.
    function initiateRound() onlyHolder public {
        require(allocationStarted, "Allocation has not started yet!");
        require(!allAllocated, "All tokens have been allocated!");
        require((block.number - blockWaitTime) >= currentRound.roundBlock, "Must wait 28400 blocks in between rounds!");
        
        uint256 nftSupply = LittleShartCollectible(_lshrt).totalSupply();
        require(((nftSupply * holderReward) + initiatorReward) <= maxSupply, "Cannot initiate round, remaining supply needs to be burned.");
        
        uint256 rewarded = initiatorReward;
        
        // Reward the initiator of the genesis round
        unclaimedAmount[msg.sender] += initiatorReward;
        
        // Reward all holders of Little Sharts
        for (uint256 i=1;i<=nftSupply;i++) {
            address ownerOfToken = LittleShartCollectible(_lshrt).ownerOf(i);
            if (ownerOfToken == _mktplc) { // Token is listed on the marketplace, make sure people can get funded for this.
                ownerOfToken = LittleShartMarketplace(_mktplc).getListing(i).seller;
            }
            if (ownerOfToken == owner) {
                unclaimedAmount[ownerOfToken] += holderReward / 2;
                rewarded += holderReward / 2;
            } else {
                unclaimedAmount[ownerOfToken] += holderReward;
                rewarded += holderReward;
            }
        }
        uint256 newRoundNumber = currentRound.roundNumber + 1;
        currentRound = Round(newRoundNumber, block.number, msg.sender, rewarded);
        rounds[newRoundNumber] = Round(newRoundNumber, block.number, msg.sender, rewarded);
        totalUnclaimed += rewarded;
    }
    
    // To be called by any holder, this will start the genesis round and begin allocation.
    function startGenesisRound() onlyHolder public {
        require(!allocationStarted, "Genesis Round has already begun.");
        uint256 nftSupply = LittleShartCollectible(_lshrt).totalSupply();
        uint256 rewarded = initiatorReward;
        ...
        currentRound = Round(1, block.number, msg.sender, rewarded);
