...
contract DAI_BUSD_Pool is Ownable {
    ...
    address public LPtokenAddress = 0x3aB77e40340AB084c3e23Be8e5A6f7afed9D41DC;
    
    // reward rate 23 % per year
    uint256 public rewardRate = 2300;
    ...
    function getPendingDivs(address _holder) public view returns (uint256 _pendingDivs) {
        ...
        if (block.timestamp <= farmEnableat + 1 days) {
            uint256 pendingDivs = stakedAmount.mul(25500).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 1 days && block.timestamp <= farmEnableat + 2 days) {
            uint256 pendingDivs = stakedAmount.mul(23500).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 2 days && block.timestamp <= farmEnableat + 3 days) {
            uint256 pendingDivs = stakedAmount.mul(21600).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 3 days && block.timestamp <= farmEnableat + 4 days) {
            uint256 pendingDivs = stakedAmount.mul(21900).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 4 days && block.timestamp <= farmEnableat + 5 days) {
            uint256 pendingDivs = stakedAmount.mul(18300).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 5 days && block.timestamp <= farmEnableat + 6 days) {
            uint256 pendingDivs = stakedAmount.mul(16800).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 6 days && block.timestamp <= farmEnableat + 7 days) {
            uint256 pendingDivs = stakedAmount.mul(13500).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 7 days && block.timestamp <= farmEnableat + 8 days) {
            uint256 pendingDivs = stakedAmount.mul(14200).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 8 days && block.timestamp <= farmEnableat + 9 days) {
            uint256 pendingDivs = stakedAmount.mul(13100).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 9 days && block.timestamp <= farmEnableat + 10 days) {
            uint256 pendingDivs = stakedAmount.mul(12000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 10 days && block.timestamp <= farmEnableat + 11 days) {
            uint256 pendingDivs = stakedAmount.mul(11100).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 11 days && block.timestamp <= farmEnableat + 12 days) {
            uint256 pendingDivs = stakedAmount.mul(10200).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 12 days && block.timestamp <= farmEnableat + 13 days) {
            uint256 pendingDivs = stakedAmount.mul(9400).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 13 days && block.timestamp <= farmEnableat + 14 days) {
            uint256 pendingDivs = stakedAmount.mul(8600).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 14 days && block.timestamp <= farmEnableat + 15 days) {
            uint256 pendingDivs = stakedAmount.mul(7900).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 15 days && block.timestamp <= farmEnableat + 16 days) {
            uint256 pendingDivs = stakedAmount.mul(7300).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 16 days && block.timestamp <= farmEnableat + 17 days) {
            uint256 pendingDivs = stakedAmount.mul(6700).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 17 days && block.timestamp <= farmEnableat + 18 days) {
            uint256 pendingDivs = stakedAmount.mul(6200).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 18 days && block.timestamp <= farmEnableat + 19 days) {
            uint256 pendingDivs = stakedAmount.mul(5700).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 19 days && block.timestamp <= farmEnableat + 20 days) {
            uint256 pendingDivs = stakedAmount.mul(5200).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 20 days && block.timestamp <= farmEnableat + 21 days) {
            uint256 pendingDivs = stakedAmount.mul(4800).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 21 days && block.timestamp <= farmEnableat + 22 days) {
            uint256 pendingDivs = stakedAmount.mul(4400).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 22 days && block.timestamp <= farmEnableat + 23 days) {
            uint256 pendingDivs = stakedAmount.mul(4100).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 23 days && block.timestamp <= farmEnableat + 24 days) {
            uint256 pendingDivs = stakedAmount.mul(3700).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 24 days && block.timestamp <= farmEnableat + 25 days) {
            uint256 pendingDivs = stakedAmount.mul(3400).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 25 days && block.timestamp <= farmEnableat + 26 days) {
            uint256 pendingDivs = stakedAmount.mul(3200).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 26 days && block.timestamp <= farmEnableat + 27 days) {
            uint256 pendingDivs = stakedAmount.mul(2900).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 27 days && block.timestamp <= farmEnableat + 28 days) {
            uint256 pendingDivs = stakedAmount.mul(2700).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 28 days && block.timestamp <= farmEnableat + 29 days) {
            uint256 pendingDivs = stakedAmount.mul(2500).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 29 days && block.timestamp <= farmEnableat + 30 days) {
            uint256 pendingDivs = stakedAmount.mul(2300).mul(timeDiff).div(rewardInterval).div(1e4);
