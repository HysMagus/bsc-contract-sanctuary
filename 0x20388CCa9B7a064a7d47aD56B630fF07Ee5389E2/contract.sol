...
contract CouchPotatoes is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x490B26C54E474aBe8768c8F4cf8D56967F7762FB);

    uint256 public CPOT_MAX = 10000;
    uint256 public PURCHASE_LIMIT = 3;
    uint256 public PRICE = 330_000_000_000_000_000; // 0.33 BNB

    uint256 private _activeDateTime = 1633824000; // Sunday, October 10, 2021 12:00:00 AM GMT
    ...
    Counters.Counter private _publicCPOT;

    constructor() ERC721("Couch Potatoes", "CPOT") {}
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicCPOT.current();

            if (_publicCPOT.current() < CPOT_MAX) {
                _publicCPOT.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicCPOT.current() < CPOT_MAX,
            "Purchase would exceed CPOT_MAX"
        );
