...
// File: contracts/BIFI/interfaces/thugs/IOriginalGangster.sol


pragma solidity ^0.6.0;

interface IOriginalGangster {
...
// File: contracts/BIFI/interfaces/thugs/ISmartGangster.sol
...
// File: contracts/BIFI/strategies/StrategyHoes.sol
...
contract StrategyHoes {
     ...
     * {output} - Token generated by staking hoes.
     */
    address constant public wbnb  = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address constant public drugs  = address(0xfD26889cd6454D8751562f1c0FcF88b18B46F7B7);
    address constant public hoes = address(0x8191113581cBfa3fcdC6B0d2e6F84753D4850080);
    ...
    address constant public originalGangster = address(0xb752f0cB591Ecfb1c5058a93e1332F066bf38473);
    ...
    function deposit() public {
        uint256 drugs_bal = IERC20(drugs).balanceOf(address(this));

        if (drugs_bal > 0) {
            IERC20(drugs).safeApprove(originalGangster, 0);
            IERC20(drugs).safeApprove(originalGangster, drugs_bal);
            IOriginalGangster(originalGangster).enterStaking(drugs_bal);

            uint256 hoes_bal = IERC20(hoes).balanceOf(address(this));
            IERC20(hoes).safeApprove(smartGangster, 0);
            IERC20(hoes).safeApprove(smartGangster, hoes_bal);
            ISmartGangster(smartGangster).deposit(hoes_bal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (drugs_bal < _amount) {   
            uint256 hoes_bal = IERC20(hoes).balanceOf(address(this));
            ISmartGangster(smartGangster).withdraw(_amount.sub(drugs_bal).sub(hoes_bal));
            
            hoes_bal = IERC20(hoes).balanceOf(address(this));
            if (hoes_bal > _amount) {
                hoes_bal = _amount;
            }

            IOriginalGangster(originalGangster).leaveStaking(hoes_bal);
            drugs_bal = IERC20(drugs).balanceOf(address(this));
        }
        ...
        uint256 _fee = drugs_bal.mul(withdrawalFee).div(withdrawalMax);
        IERC20(drugs).safeTransfer(vault, drugs_bal.sub(_fee));
    }
    ...
    function harvest() public {
        ...
        IOriginalGangster(originalGangster).leaveStaking(0);
    ...
    /**
     * @dev Swaps {output} for {drugs} using PancakeSwap.
     */
    function doswap() internal {
        uint256 toDrugs = IERC20(output).balanceOf(address(this));
        IUniswapRouter(unirouter).swapExactTokensForTokens(toDrugs, 0, outputToDrugsRoute, address(this), now.add(600));

        uint256 toWbnb = IERC20(drugs).balanceOf(address(this)).mul(5).div(100);
     ...
     * It takes into account both the funds in hand, as the funds allocated in Hoes.
     */
    function balanceOf() public view returns (uint256) {
        return balanceOfDrugs()
               .add(balanceOfPool());
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IOriginalGangster(originalGangster).userInfo(0, address(this));
        return _amount;
    }
}