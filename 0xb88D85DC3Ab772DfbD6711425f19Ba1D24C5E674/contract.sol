...
contract REF is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint256 private _totalSupply = 75 * 10**6 * 10**9;
...
// File: browser/NotEgg.sol

pragma solidity ^0.6.0;


// EggToken with Governance.
contract NOASToken is REF() {
...
// MasterChef is the master of NOAS. He can make NOAS and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once NOAS is sufficiently
...
contract MasterChef is Ownable {
    ...
    // The NOAS TOKEN!
    NOASToken public NOAS;
    // Dev address.
    address public devaddr;
    // NOAS tokens created per block.
    uint256 public eggPerBlock;
    // Bonus muliplier for early NOAS makers.
    ...
    // Amount of Tao sent to staking contract from depositors
    uint256 public trackDepositedTao = 0;
    // The block number when NOAS mining starts.
    ...
    // Update the given pool's NOAS allocation point and deposit fee. Can only be called by the owner.
    ...
    // Deposit LP tokens to MasterChef for NOAS allocation.
    function deposit(uint256 _pid, uint256 _amount, address referral) public {
        ...
        if(_amount > 0) {
            ...
            if (pool.lpToken == NOAS) {
                trackDepositedTao.add(_amount).sub(depositFee);
            }
        }
    ...
    // Safe NOAS transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function TaoTransfer(address _to, uint256 _amount) internal {
            NOAS.transfer(_to, _amount);
    }
