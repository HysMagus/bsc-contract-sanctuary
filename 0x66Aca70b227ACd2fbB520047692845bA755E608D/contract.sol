...
contract ReeseICO is Owned{
    using SafeMath for uint256;
    uint256 decimals = 18;
    address reeseCoin;
    uint256 public fundsRaised;
    uint256 tokensPerBNB = 10; // 1 BNB = 10 Tokens
    ...
    function invest() internal {
        require(block.timestamp >= 1615680000 && block.timestamp <= 1618790399, "Sale is from March 14,2021 to April 18, 2021");
        require(IBEP20(reeseCoin).transfer(msg.sender, getTokenAmount(msg.value)));
        fundsRaised = fundsRaised.add(msg.value);
        payable(owner).transfer(msg.value);
    }

    function getTokenAmount(uint256 amount) internal view returns(uint256){
        return amount * tokensPerBNB;
    }
}