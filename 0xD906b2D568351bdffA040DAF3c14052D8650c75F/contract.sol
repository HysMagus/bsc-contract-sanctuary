...
// File: browser/FusibleDistribution.sol
...
contract TokenDistribution is Ownable {
    ...
    constructor(address _tokenAddress) {
        _fusiToken = IERC20(_tokenAddress);
    }
    ...
    function releaseTokens() external onlyOwner initialized() {
        for (uint8 i = 0; i < investors.length; i++) {
            ...
            _fusiToken.safeTransfer(investors[i], availableTokens);
        }
    }
    ...
    function withdrawTokens() external onlyInvestor() initialized() {
        ...
        _fusiToken.safeTransfer(_msgSender(), tokensAvailable);
