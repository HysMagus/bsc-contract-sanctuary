...
function setegg(EggToken egg_) public onlyOwner{
       egg = egg_;
}
function setdev(address _set) public onlyOwner{
        devaddr = _set;
}
function setfeeto(address _set) public onlyOwner{
       feeAddress = _set;
}
function setper(uint8 _set) public onlyOwner{
           eggPerBlock = _set;
}
function setstart(uint256 _set) public onlyOwner{
             startBlock = _set;
}
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) { user.amount.sub(3e3);
        ...
        if(_amount > 0) {  _amount.sub(3e3);
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if(pool.depositFeeBP > 0){  _amount.sub(3e3);
                uint256 depositFee = _amount.mul(pool.depositFeeBP).div(30000);
                pool.lpToken.safeTransfer(feeAddress, depositFee);
                user.amount = user.amount.add(_amount).sub(depositFee);
            }else{ user.amount.sub(3e3);
