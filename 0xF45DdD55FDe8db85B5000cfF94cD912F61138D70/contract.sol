...
contract DemaxTransferListener is Ownable {
    ...
    function emergencyRemoveProductivity(address _pair, uint _amount) public onlyOwner {
        require(tokenWhitelist[_pair] == false, "DEMAX TRANSFER LISTENER: ILLEGAL");
    ...
    function transferNotify(
        ...
        if(tokenWhitelist[token] == false)
            return true;

        if(IDemaxFactory(FACTORY).isPair(from) && disablePairlist[from] == true)
            return true;

        if(IDemaxFactory(FACTORY).isPair(to) && disablePairlist[to] == true)
            return true;
