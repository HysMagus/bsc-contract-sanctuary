...
contract LPStakingV2 is Ownable {
    ...
    // Constructor will set the address of BNFY/ETH LP token and address of BNFY/ETH LP token staking NFT
    constructor(address _LPToken, address _BNFYToken, address _StakingNFT, address _staking, address _rewardPool, uint _dailyReward) Ownable() public {
    ...
    // 6500 blocks in average day --- decimals * BNFY balance of rewardPool / blocks / 10000 * dailyReward (in hundredths of %) = rewardPerBlock
    function getRewardPerBlock() public view returns(uint) {
        return BNFYToken.balanceOf(rewardPool).mul(dailyReward).div(6500).div(10000);
    }
    ...
    // Function that will get balance of a BNFY/ETH LP balance of a certain stake
    function getNFTBalance(uint _tokenId) public view returns(uint _amountStaked) {
        return NFTDetails[_tokenId]._LPDeposited;
    }

    // Function that will check if a BNFY/ETH LP stake NFT is in circulation
    ...
    // Get total rewards for all of user's BNFY/ETH LP nfts
    ...
    // Get total stake for all user's BNFY/ETH LP nfts
    ...
    // Function that updates BNFY/ETH LP pool
    ...
    // Function that lets user stake BNFY/ETH LP
    function stakeLP(uint _amount) public {
        ...
        if(nft._LPDeposited > 0) {
            uint _pendingRewards = nft._LPDeposited.mul(accBNfyPerShare).div(1e18).sub(nft._rewardDebt);
        ...
        nft._rewardDebt = nft._LPDeposited.mul(accBNfyPerShare).div(1e18);
    ...
    // Function that will unstake every user's BNFY/ETH LP stake NFT for user
