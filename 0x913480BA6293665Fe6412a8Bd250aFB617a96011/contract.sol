...
contract TKR {
   ...
   // player locked funds into game escrow
    ...
    constructor(
        ...
        address teamaddress7,
        address teamaddress8) {
        owner = msg.sender;
        minDeposit = 100000;
        totalEscrow = 0;
        isContractRunning = true;
        canStartGame = true;
        rakes.push(RakePayout(teamaddress1, 240));
        rakes.push(RakePayout(teamaddress2, 240));
        rakes.push(RakePayout(teamaddress3, 221));
        rakes.push(RakePayout(teamaddress4, 96));
        rakes.push(RakePayout(teamaddress5, 96));
        rakes.push(RakePayout(teamaddress6, 29));
        rakes.push(RakePayout(teamaddress7, 19));
        rakes.push(RakePayout(teamaddress8, 19));
        rakes.push(RakePayout(msg.sender, 40));
    }
    ...
    function rakeCredits(uint256 _amount) private {
        for (uint8 i = 0; i < rakes.length; i++) {
            uint256 _individualRakeAmount = (_amount * rakes[i].percentRake) / 1000;
