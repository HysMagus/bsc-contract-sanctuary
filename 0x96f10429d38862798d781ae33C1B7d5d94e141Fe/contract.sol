...
contract tk720s is ERC721URIStorage, Ownable{
 
    event Mint720s (address indexed minter, uint256 startWith, uint256 times);
    
    
    uint256 public total720s;
    ...
    function mint720(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(total720s + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit Mint720s(_msgSender(), total720s+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + total720s++);
        }
    }
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(total720s + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit Mint720s(_addr, total720s+1, 1);
        _mint(_addr, 1 + total720s++);
    }
