...
contract Locker is Context {
    ...
    function unlock(address tokenAddress) external returns (bool) {
        ...
        uint256 unlockableTimestamp = lockedMap[_msgSender()][tokenAddress].startTimestamp.add(lockedMap[_msgSender()][tokenAddress].lockedHours.mul(uint256(60)));
    ...
    function unlockableTimestamp(address tokenAddress) external view returns (uint256) {
        if(lockedMap[_msgSender()][tokenAddress].startTimestamp > 0)
            return lockedMap[_msgSender()][tokenAddress].startTimestamp.add(lockedMap[_msgSender()][tokenAddress].lockedHours.mul(uint256(60)));
        return 0;
    }
