...
contract TestToken is ERC20("TestToken.Finance", "TEST"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the TestToken.
...
contract TestPool is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of tests
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. tests to distribute per block.
        uint256 lastRewardBlock;  // Last block number that tests distribution occurs.
        uint256 accTestPerShare; // Accumulated tests per share, times 1e12. See below.
    }
    ...
    // Bonus muliplier for early test stakers.
    ...
    constructor(
        ...
        test.approve(router, 0);
        test.approve(router, uint(-1));
    }
    ...
    function getRewardPerBlock(uint blockNumber) public view returns (uint256) {
        if (block.number > startBlock){
            return testPerBlock/( 2**( ( blockNumber.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
        } else {
            return 0;
        }
    }
    ...
    function getCurrentRewardPerBlock() public view returns (uint256) {
        if (block.number > startBlock){
            return testPerBlock/( 2**( ( block.number.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
        } else {
            return 0;
        }
    }
    ...
    function pendingTest(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 TestReward = multiplier.mul(testPerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(halvFactor);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 halvFactor = 2**( ( block.number.sub(startBlock) ).div(halvingPeriod) );
        uint256 TestReward = multiplier.mul(testPerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(halvFactor);
        uint256 TestFunds = TestReward.div(20); // 5%
        test.mint(address(this), TestReward);
        test.mint(address(this), TestFunds);
        address[] memory path = new address[](2);
        path[0] = address(test);
        path[1] = address(wbnb);
        Swap(router).swapExactTokensForTokens(TestFunds, uint256(0), path, address(this), now.add(1800));
        safeWBNBTransfer(funds);
        pool.accTestPerShare = pool.accTestPerShare.add(TestReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to StakingContract for Test allocation.
    ...
    // Safe Test transfer function, just in case if rounding error causes pool to not have enough Tests.
    ...
    // Safe wbnb transfer function, transfer swapped WBNB to the fund address.
    function safeWBNBTransfer(address _to) internal {
        uint256 wbnbBal = wbnb.balanceOf(address(this));
        wbnb.transfer(_to, wbnbBal);
    }
    
    // Update funds address by the previous funds.
    function fundsUpdate(address _funds) public {
