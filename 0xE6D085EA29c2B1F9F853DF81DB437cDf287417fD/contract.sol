...
contract CardToken is BEP20('CardSwap Token', 'CARD') {
...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    CardToken public card;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public cardPerBlock;
    // Bonus muliplier for early card makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _cardPerBlock,
        uint256 _startBlock
    ) public {
        card = _card;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        cardPerBlock = _cardPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cardReward = multiplier.mul(cardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(cardReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cardReward = multiplier.mul(cardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        card.mint(devaddr, cardReward.div(10));
        card.mint(address(this), cardReward);
        pool.accEggPerShare = pool.accEggPerShare.add(cardReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe card transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 cardBal = card.balanceOf(address(this));
        if (_amount > cardBal) {
            card.transfer(_to, cardBal);
        } else {
            card.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _cardPerBlock) public onlyOwner {
        massUpdatePools();
        cardPerBlock = _cardPerBlock;
    }
}