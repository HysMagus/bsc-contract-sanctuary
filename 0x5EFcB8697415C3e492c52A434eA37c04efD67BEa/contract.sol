...
contract Cheemstopus is Context, IERC20, Ownable {
    ...
    address payable private _MarketingWallet = payable(0xF9cA7479dAc1E634105a0a35d90EAAA2d05391e9);
    ...
    string private _name = "Cheemstopus";
    ...
    function _reflectFee(uint256 rFee, uint256 rMarketing, uint256 tFee, uint256 tMarketing) private {
        ...
        _tTotal = _tTotal.sub(tMarketing);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMarketing, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketing).sub(rLiquidity);
