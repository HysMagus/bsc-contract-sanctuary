...
library SafeBEP20 {
    ...
    function safeDecreaseAllowance(IBEP20 token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).sub(value, "SafeBEP20: < 0");
        callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }
    ...
    function callOptionalReturn(IBEP20 token, bytes memory data) private {
        ...
        // solhint-disable-next-line max-line-length
        require(address(token).isContract(), "SafeBEP20: !contract");
        ...
        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "SafeBEP20: !succeed");
        }
    }
}

library Keep3rVLibrary {
    function getReserve(address pair, address reserve) external view returns (uint) {
        (uint _r0, uint _r1,) = IPancakePair(pair).getReserves();
        if (IPancakePair(pair).token0() == reserve) {
            return _r0;
        } else if (IPancakePair(pair).token1() == reserve) {
            return _r1;
        } else {
            return 0;
        }
...
interface IKeep3rVHelper {
    function getQuoteLimit(uint gasUsed) external view returns (uint);
}

contract Keep3rV is ReentrancyGuard {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;

    /// @notice Keep3r Helper to set max prices for the ecosystem
    IKeep3rVHelper public KPRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "Keep3rV";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "KP3RV";
    ...
    /// @notice address used for BNB transfers
    address constant public BNB = address(0xE);
     ...
     */
    function addCreditBNB(address job) external payable {
        require(jobs[job], "addCreditBNB: !job");
        uint _fee = msg.value.mul(FEE).div(BASE);
        credits[job][BNB] = credits[job][BNB].add(msg.value.sub(_fee));
        payable(governance).transfer(_fee);

        emit AddCredit(BNB, job, msg.sender, block.number, msg.value);
    }
    ...
    function addCredit(address credit, address job, uint amount) external nonReentrant {
        require(jobs[job], "addCreditBNB: !job");
        uint _before = IBEP20(credit).balanceOf(address(this));
        IBEP20(credit).safeTransferFrom(msg.sender, address(this), amount);
        uint _received = IBEP20(credit).balanceOf(address(this)).sub(_before);
        uint _fee = _received.mul(FEE).div(BASE);
        credits[job][credit] = credits[job][credit].add(_received.sub(_fee));
        IBEP20(credit).safeTransfer(governance, _fee);
    ...
    function addLiquidityToJob(address liquidity, address job, uint amount) external nonReentrant {
        ...
        IBEP20(liquidity).safeTransferFrom(msg.sender, address(this), amount);
    ...
    function applyCreditToJob(address provider, address liquidity, address job) external {
        ...
        uint _liquidity = Keep3rVLibrary.getReserve(liquidity, address(this));
        uint _credit = _liquidity.mul(liquidityAmount[provider][liquidity][job]).div(IBEP20(liquidity).totalSupply());
    ...
    function unbondLiquidityFromJob(address liquidity, address job, uint amount) external {
        ...
        uint _liquidity = Keep3rVLibrary.getReserve(liquidity, address(this));
        uint _credit = _liquidity.mul(amount).div(IBEP20(liquidity).totalSupply());
    ...
    function removeLiquidityFromJob(address liquidity, address job) external {
        ...
        IBEP20(liquidity).safeTransfer(msg.sender, _amount);
    ...
    function receipt(address credit, address keeper, uint amount) external {
        ...
        lastJob[keeper] = now;
        IBEP20(credit).safeTransfer(keeper, amount);
    ...
    function receiptETH(address keeper, uint amount) external {
        require(jobs[msg.sender], "receipt: !job");
        credits[msg.sender][BNB] = credits[msg.sender][BNB].sub(amount, "workReceipt: insuffient funds");
        lastJob[keeper] = now;
        payable(keeper).transfer(amount);
        emit KeeperWorked(BNB, msg.sender, keeper, block.number, amount);
    }
    ...
    function bond(address bonding, uint amount) external nonReentrant {
        ...
        if (bonding == address(this)) {
            ...
            uint _before = IBEP20(bonding).balanceOf(address(this));
            IBEP20(bonding).safeTransferFrom(msg.sender, address(this), amount);
            amount = IBEP20(bonding).balanceOf(address(this)).sub(_before);
        }
    ...
    function withdraw(address bonding) external nonReentrant {
        ...
        if (bonding == address(this)) {
            ...
            IBEP20(bonding).safeTransfer(msg.sender, partialUnbonding[msg.sender][bonding]);
        }
    ...
    function slash(address bonded, address keeper, uint amount) public nonReentrant {
        ...
        if (bonded == address(this)) {
            ...
            IBEP20(bonded).safeTransfer(governance, amount);
        }
