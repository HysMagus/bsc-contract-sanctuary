...
website: https://gigachad.webflow.io/
telegram: https://t.me/GigaChadGroup
*
*/
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable CAKE = address(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47); //ADA
...
contract GigaChad is ERC20, Ownable {
    ...
    GigaChadDividendTracker public dividendTracker;
    ...
    uint256 public CAKERewardsSplitValue = 5;
    ...
    uint256 public buyingFeesWithDecimals = (CAKERewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    uint256 public sellingFeesWithDecimals = (CAKERewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    address public _marketingWalletAddress = 0xA144Bc59072dB67fC848b03e8ECe0e384351Bb79;
    bool public hasLaunched;
    address public liquidityFeeReceiver = 0xF81A5be70bbd6ED26F4E0BD9a76ADFF44FbCBC52;

    // use by default 300,000 gas to process auto-claiming dividends
    uint256 public gasForProcessing = 300000;
    uint256 public maxBuyTransactionAmount =  3000000000 * (10**18);
    ...
    constructor() public ERC20("Giga Chad (ADA Rewards)", "GIGACHAD") {

    	dividendTracker = new GigaChadDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GigaChad: The dividend tracker already has that address");

        GigaChadDividendTracker newDividendTracker = GigaChadDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GigaChad: The new dividend tracker must be owned by the GigaChad token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GigaChad: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GigaChad: Account is already the value of 'excluded'");
    ...
    function setCAKERewardsSplitValue(uint256 value) external onlyOwner{
        require(value >= 1 && value <= 1000);
        CAKERewardsSplitValue = value;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GigaChad: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GigaChad: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GigaChad: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GigaChad: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 marketingTokens = contractTokenBalance.mul(marketingSplitValue).div(CAKERewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
            swapAndSendToFee(marketingTokens);

            uint256 swapTokens = contractTokenBalance.mul(liquiditySplitValue).div(CAKERewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
            swapAndLiquify(swapTokens);

            totalBananas += contractTokenBalance.mul(bananaRewardSplitValue).div(CAKERewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);

            uint256 sellTokens = contractTokenBalance.mul(CAKERewardsSplitValue).div(CAKERewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
    ...
    function swapAndSendDividends(uint256 tokens) private{
        uint256 initialPancakeBalance = IERC20(CAKE).balanceOf(address(this));
        swapTokensForCAKE(tokens);
        uint256 dividends = (IERC20(CAKE).balanceOf(address(this))).sub(initialPancakeBalance);
...
contract GigaChadDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("GigaChad_Dividend_Tracker", "GigaChad_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "GigaChad_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "GigaChad_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GigaChad contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GigaChad_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GigaChad_Dividend_Tracker: Cannot update claimWait to same value");
