...
contract testTokenN is BEP20('testing testN', 'testN') {
...
// MasterChef is the master of test. He can make test and he is a fair guy.
//
...
contract MasterChefN is Ownable {
    using SafeMath for uint256;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 testReward = multiplier.mul(testPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        testN.mint(devaddr, testReward.div(10));
        testN.mint(address(this), testReward);
    ...
    function safetestTransfer(address _to, uint256 _amount) internal {
        uint256 testBal = testN.balanceOf(address(this));
        if (_amount > testBal) {
            testN.transfer(_to, testBal);
        } else {
            testN.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _testPerBlock) public onlyOwner {
        massUpdatePools();
        testPerBlock = _testPerBlock;
    }
}