...
// File: contracts/interfaces/IStakedbMXX.sol

pragma solidity 0.6.12;

interface IStakedbMXX {
...
contract ERC20WithSnapshot is ERC20 {
    ...
    /// @dev reference to the bMXX governance contract to call (if initialized) on _beforeTokenTransfer
    /// !!! IMPORTANT The bMXX governance is considered a trustable contract, being its responsibility
    /// to control all potential reentrancies by calling back the this contract
    ITransferHook public _bMXXGovernance;

    event SnapshotDone(address owner, uint128 oldValue, uint128 newValue);

    constructor(string memory name, string memory symbol, uint8 decimals) public ERC20(name, symbol, decimals) {}

    function _setbMXXGovernance(ITransferHook bMXXGovernance) internal virtual {
        _bMXXGovernance = bMXXGovernance;
    }
    ...
    */
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        // caching the bMXX governance address to avoid multiple state loads
        ITransferHook bMXXGovernance = _bMXXGovernance;
        if (bMXXGovernance != ITransferHook(0)) {
            bMXXGovernance.onTransfer(from, to, amount);
        }
    }
}
 ...
 *
 * @author bMXX, inspired by the OpenZeppelin Initializable contract
 */
...
// File: contracts/interfaces/IbMXXDistributionManager.sol
...
// File: contracts/stake/bMXXDistributionManager.sol

pragma solidity 0.6.12;

/**
 * @title bMXXDistributionManager
 * @notice Accounting contract to manage multiple staking distributions
 * @author bMXX
 **/
contract bMXXDistributionManager is IbMXXDistributionManager {
    using SafeMath for uint256;
    ...
    function configureAssets(
        ...
        for (uint256 i = 0; i < assetsConfigInput.length; i++) {
            AssetData storage assetConfig = assets[assetsConfigInput[i]
                .underlyingAsset];
     ...
     * @param underlyingAsset The address used as key in the distribution, for example stBMXX or the bTokens addresses on bMXX
...
/**
 * @title StakedToken
 * @notice Contract to stake bMXX token, tokenize the position and get rewards, inheriting from a distribution manager contract
 * @author bMXX
 **/
    ...
    bMXXDistributionManager
{
    ...
    constructor(
        ...
        bMXXDistributionManager(emissionManager, distributionDuration)
    {
    ...
    function stake(address onBehalfOf, uint256 amount) external override {
        ...
        POOL.updateGovernanceStakingRewards(onBehalfOf);
    ...
    function redeem(address to, uint256 amount) external override {
        ...
        POOL.updateGovernanceStakingRewards(msg.sender);
    ...
    function _transfer(
        ...
        // Handle the rewards from RewardManager ( Depositor, Governance )
        POOL.updateGovernanceStakingRewards(from);
        POOL.updateGovernanceStakingRewards(to);
...
// File: contracts/stake/StakedbMXX.sol
