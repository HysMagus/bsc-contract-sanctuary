...
// UBEToken with Governance.
contract UbeToken is ERC20("UbeToken", "UBE"), Ownable, Authorizable {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (UbeFarmer).
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "UBE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "UBE::delegateBySig: invalid nonce");
        require(now <= expiry, "UBE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "UBE::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "UBE::_writeCheckpoint: block number exceeds 32 bits");
...
interface IMigratorToUbeSwap {
    // Perform LP token migration from legacy PancakeSwap to UbeSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to PancakeSwap LP tokens.
    // UbeSwap must mint EXACTLY the same amount of UbeSwap LP tokens or
    // else something bad will happen. Traditional PancakeSwap does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// UbeFarmer is the master of Ube. He can make Ube and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Ube is sufficiently
// distributed and the community can show to govern itself.
//
contract UbeFarmer is Ownable, Authorizable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Ubes
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accUbePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accUbePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Ubes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Ubes distribution occurs.
        uint256 accUbePerShare; // Accumulated Ubes per share, times 1e12. See below.
    }
    ...
    // Ube tokens created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early Ube makers.
    ...
    // The block number when Ube mining starts.
    ...
    IMigratorToUbeSwap public migrator;
    ...
    event SendUbeReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "UbeFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's Ube allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToUbeSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 UbeForDev;
        uint256 UbeForFarmer;
		uint256 UbeForLP;
		uint256 UbeForCom;
		uint256 UbeForFounders;
        (UbeForDev, UbeForFarmer, UbeForLP, UbeForCom, UbeForFounders) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        Ube.mint(address(this), UbeForFarmer);
        pool.accUbePerShare = pool.accUbePerShare.add(UbeForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
        if (UbeForDev > 0) {
            Ube.mint(address(devaddr), UbeForDev);
            ...
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Ube.lock(address(devaddr), UbeForDev.mul(75).div(100));
            }
        }
		if (UbeForLP > 0) {
            Ube.mint(liquidityaddr, UbeForLP);
			...
			if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Ube.lock(address(liquidityaddr), UbeForLP.mul(45).div(100));
            }
        }
		if (UbeForCom > 0) {
            Ube.mint(comfundaddr, UbeForCom);
			//Community Fund has xx% locked during bonus period and then drips out linearly over 3 years.
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Ube.lock(address(comfundaddr), UbeForCom.mul(85).div(100));
            }
        }
		if (UbeForFounders > 0) {
            Ube.mint(founderaddr, UbeForFounders);
			...
			if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Ube.lock(address(founderaddr), UbeForFounders.mul(95).div(100));
            }
        }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer, uint256 forLP, uint256 forCom, uint256 forFounders) {
        ...
        uint256 UbeCanMint = Ube.cap().sub(Ube.totalSupply());

        if (UbeCanMint < amount) {
            forDev = 0;
			forFarmer = UbeCanMint;
			forLP = 0;
			forCom = 0;
			forFounders = 0;
        }
    ...
    // View function to see pending Ubes on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accUbePerShare = pool.accUbePerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 UbeForFarmer;
            (, UbeForFarmer, , ,) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accUbePerShare = accUbePerShare.add(UbeForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accUbePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accUbePerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = Ube.balanceOf(address(this));
            ...
            if(pending > 0) {
                Ube.transfer(msg.sender, pending);
                uint256 lockAmount = 0;
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    lockAmount = pending.mul(PERCENT_LOCK_BONUS_REWARD).div(100);
                    Ube.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendUbeReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accUbePerShare).div(1e12);
        }
    }
    ...
    // Deposit LP tokens to UbeFarmer for $UBE allocation.
    function deposit(uint256 _pid, uint256 _amount, address _ref) public {
        require(_amount > 0, "UbeFarmer::deposit: amount must be greater than 0");
        ...
        devr.rewardDebt = devr.amount.mul(pool.accUbePerShare).div(1e12);
  ...
  // Withdraw LP tokens from UbeFarmer.
    function withdraw(uint256 _pid, uint256 _amount, address _ref) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        UserGlobalInfo storage refer = userGlobalInfo[_ref];
        UserGlobalInfo storage current = userGlobalInfo[msg.sender];
        require(user.amount >= _amount, "UbeFarmer::withdraw: not good");
        if(_ref != address(0)){
                refer.referrals[msg.sender] = refer.referrals[msg.sender] - _amount;
                refer.globalRefAmount = refer.globalRefAmount - _amount;
            }
    ...
    // Safe Ube transfer function, just in case if rounding error causes pool to not have enough Ubes.
    function safeUbeTransfer(address _to, uint256 _amount) internal {
        uint256 UbeBal = Ube.balanceOf(address(this));
        if (_amount > UbeBal) {
            Ube.transfer(_to, UbeBal);
        } else {
            Ube.transfer(_to, _amount);
        }
    }
