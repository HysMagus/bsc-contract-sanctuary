...
contract AutoFarmV2 is Ownable, ReentrancyGuard {
    ...
    bool allowMinting;
    address public AUTO; // 0xa184088a740c695E156F91f5cC086a06bb78b827;
    ...
    constructor(
        bool _allowMinting,
        address _AUTO, // 0xa184088a740c695E156F91f5cC086a06bb78b827;
        address _burnAddress, // 0x000000000000000000000000000000000000dEaD;
        uint256 _ownerAUTOReward, // 138; // 12%
        uint256 _AUTOMaxSupply, // = 80000e18;
        uint256 _AUTOPerBlock, // = 8000000000000000; // AUTO tokens created per block
        uint256 _startBlock // = 3888888; //https://bscscan.com/block/countdown/3888888
    ) public {
        allowMinting = _allowMinting;
        AUTO = _AUTO;
        burnAddress = _burnAddress;
        ownerAUTOReward = _ownerAUTOReward;
        AUTOMaxSupply = _AUTOMaxSupply;
        AUTOPerBlock = _AUTOPerBlock;
    ...
    function getMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {
        if (IERC20(AUTO).totalSupply() >= AUTOMaxSupply) {
            return 0;
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (allowMinting) {
            ...
            AUTOToken(AUTO).mint(address(this), AUTOReward);
        }
    ...
    function safeAUTOTransfer(address _to, uint256 _AUTOAmt) internal {
        ...
        if (_AUTOAmt > AUTOBal) {
            IERC20(AUTO).transfer(_to, AUTOBal);
        } else {
            IERC20(AUTO).transfer(_to, _AUTOAmt);
        }
    }
