...
// Root file: contracts/vaults/legacy/StrategyHelperV1.sol
...
contract StrategyHelperV1 is OwnableUpgradeable {
    ...
    address private constant USDC   = address(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d);
    ...
    address private constant BABY   = address(0x53E562b9B7E5E94b81f10e96Ee70Ad06df3D2657);
    address private constant HUNNY  = address(0x565b72163f17849832A692A3c5928cc502f46D69);
    address private constant XVS    = address(0xcF6BB5389c92Bdda8a3747Ddb454cB7a64626C63);
    address private constant ETH    = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
    address private constant BTC    = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
    address private constant FIL    = address(0x0D8Ce2A99Bb6e3B7Db580eD848240e4a0F9aE153);
    address private constant TUSD   = address(0x14016E85a25aeb13065688cAFB43044C2ef86784);
    address private constant ALPACA = address(0x8F0528cE5eF7B51152A59745bEfDD91D97091d2F);
    ...
    function initialize() external initializer {
        ...
        setTokenFeed(XVS, address(0xBF63F430A79D4036A5900C19818aFf1fa710f206));
    }
    ...
    function tokenPriceInBNB(address _token) public view returns(uint) {
        ...
        } else if (tokenFeeds[_token] != address(0)) {
            return getTokenPriceUSDFromFeed(_token).mul(1e18).div(bnbPriceInUSD());
        } else if (_token == XVS) {
            return xvsPriceInBNB();
        } else {
    ...
    function xvsPriceInUSD() view public returns(uint) {
        (, int price, , ,) = AggregatorV3Interface(tokenFeeds[XVS]).latestRoundData();
        return uint(price).mul(1e10);
    }
    ...
    function xvsPriceInBNB() view public returns(uint) {
        return xvsPriceInUSD().mul(1e18).div(bnbPriceInUSD());
    }
    ...
    function btcPriceInUSD() view public returns(uint) {
        (, int price, , ,) = AggregatorV3Interface(tokenFeeds[BTC]).latestRoundData();
        return uint(price).mul(1e10);
    }

    function getTokenPriceUSDFromFeed(address token) public view returns (uint) {
        (, int price, , ,) = AggregatorV3Interface(tokenFeeds[token]).latestRoundData();
        return uint(price).mul(1e10);
    }

    function ethPriceInBNB() view public returns(uint) {
        return ethPriceInUSD().mul(1e18).div(bnbPriceInUSD());
    }

    function btcPriceInBNB() view public returns(uint) {
        return btcPriceInUSD().mul(1e18).div(bnbPriceInUSD());
    }
    ...
    function tvl(address _flip, uint amount) public view returns (uint) {
        if (_flip == WBNB) {
            return bnbPriceInUSD().mul(amount).div(1e18);
        }
        ...
        if (_flip == BABY) {
            return tokenPriceInBNB(BABY).mul(bnbPriceInUSD()).mul(amount).div(1e36);
        }

        if (_flip == XVS) {
            return xvsPriceInUSD().mul(amount).div(1e18);
        }

        if (_flip == ETH) {
            return ethPriceInUSD().mul(amount).div(1e18);
        }

        if (_flip == BTC) {
            return btcPriceInUSD().mul(amount).div(1e18);
        }

        if (_flip == FIL || _flip == ALPACA) {
            return getTokenPriceUSDFromFeed(_flip).mul(amount).div(1e18);
        }

        // stable coin always have 1USD value
        if (_flip == BUSD || _flip == USDC || _flip == USDT || _flip == TUSD) {
            return amount;
        }
    ...
    function tvlInBNB(address _flip, uint amount) public view returns (uint) {
        ...
        if (_flip == BABY) {
            return tokenPriceInBNB(BABY).mul(amount).div(1e18);
        }

        if (_flip == XVS) {
            return xvsPriceInBNB().mul(amount).div(1e18);
        }

        if (_flip == ETH) {
            return ethPriceInBNB().mul(amount).div(1e18);
        }

        if (_flip == BTC) {
            return btcPriceInBNB().mul(amount).div(1e18);
        }

        if (_flip == FIL || _flip == ALPACA) {
            return tokenPriceInBNB(_flip).mul(amount).div(1e18);
        }

        // stable coin always have 1USD value
        if (_flip == BUSD || _flip == USDC || _flip == USDT || _flip == TUSD) {
            return amount.mul(1e18).div(bnbPriceInUSD());
        }
