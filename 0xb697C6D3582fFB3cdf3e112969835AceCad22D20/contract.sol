...
contract TokenMultiTimelock {
    ...
    function getReleaseAmount() public view virtual returns (uint256) {
        ...
        for (uint256 i = 0; i < _releases.length; i++) {
            Release memory release = _releases[i];
            
            if(!release.released && block.timestamp >= release.timestamp)
                releaseAmount = releaseAmount.add(release.amount);
        }
    ...
    function release() public virtual {
        ...
        for (uint256 i = 0; i < _releases.length; i++) {
            Release storage release = _releases[i];
