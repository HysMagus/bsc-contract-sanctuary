...
https://supr.finance/
https://t.me/supremefinance

Staking pool contract (3-Hours pool)
(Note: Supreme Finance offers different LP- staking pools, the longer you have to lock the tokens the higher the APY % - Please use at your own risk! )
...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0x2ca1a56EEEfeCe776C17D1FDcd3a4c7bc05CDa5B; //pool
...
contract Staker is Poolable{
    ...
    address public Supr3Address;
    
    address constant public UNIROUTER         = 0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F;
    ...
    function stake() public payable{
        require(creationTime + 3 hours <= now, "It has not been 3 hours since contract creation yet");

        address staker = msg.sender;
        
        address poolAddress = Uniswap(FACTORY).getPair(Supr3Address, WETHAddress);
        
        if(price() >= (1.05 * 10**18) && priceCapped){
           
            uint t = IERC20(Supr3Address).balanceOf(poolAddress); //token in pancakeswap
            uint a = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in pancakeswap
            uint x = (sqrt(9*t*t + 3988000*a*t) - 1997*t)/1994;
            
            IERC20(Supr3Address).mint(address(this), x);
            
            address[] memory path = new address[](2);
            path[0] = Supr3Address;
            path[1] = WETHAddress;
            IERC20(Supr3Address).approve(UNIROUTER, x);
            Uniswap(UNIROUTER).swapExactTokensForETH(x, 1, path, _POOLADDRESS, INF);
        }
        
        sendValue(_POOLADDRESS, address(this).balance/2);
        
        uint ethAmount = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in pancakeswap
        uint tokenAmount = IERC20(Supr3Address).balanceOf(poolAddress); //token in pancakeswap
      
        uint toMint = (address(this).balance.mul(tokenAmount)).div(ethAmount);
        IERC20(Supr3Address).mint(address(this), toMint);
        
        uint poolTokenAmountBefore = IERC20(poolAddress).balanceOf(address(this));
        
        uint amountTokenDesired = IERC20(Supr3Address).balanceOf(address(this));
        IERC20(Supr3Address).approve(UNIROUTER, amountTokenDesired ); //allow pool to get tokens
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance }(Supr3Address, amountTokenDesired, 1, 1, address(this), INF);
    ...
    function withdrawRewardTokens(uint amount) public {
        ...
        IERC20(Supr3Address).mint(msg.sender, amount);
    }
    ...
    function viewPooledEthAmount(address who) public view returns (uint){
        ...
        address poolAddress = Uniswap(FACTORY).getPair(Supr3Address, WETHAddress);
        uint ethAmount = IERC20(WETHAddress).balanceOf(poolAddress); //BNB pancakeswap
    ...
    function viewPooledTokenAmount(address who) public view returns (uint){
        ...
        address poolAddress = Uniswap(FACTORY).getPair(Supr3Address, WETHAddress);
        uint tokenAmount = IERC20(Supr3Address).balanceOf(poolAddress); //token pancakeswap
    ...
    function ethEarnCalc(uint eth, uint time) public view returns(uint){
        ...
        address poolAddress = Uniswap(FACTORY).getPair(Supr3Address, WETHAddress);
        uint totalEth = IERC20(WETHAddress).balanceOf(poolAddress); //BNB pancakeswap
    ...
    function ethtimeCalc(uint Supr3) internal view returns(uint){
        return ( Supr3.mul(31557600 * DECIMAL) ).div( rewardValue() );
    }
}