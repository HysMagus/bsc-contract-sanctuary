...
contract Strategy is BaseStrategy {
    ...
    address internal ETH            = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;

    address token0;
    address token1;
    address reward                  = 0x45b07DF4FE00202dd31F2D95f6C06f3A029625D5;
    IERC20 public NYANToken         = IERC20(reward);
    IPancakeFarm public farmer      = IPancakeFarm(0xC885b97220ad6a8dfE4AC7aF7C7A4776c7A21f06);
    IERC20 public LPToken           = IERC20(farmer.poolInfo(pid).lpToken);
    IERC20 internal iETH            = IERC20(ETH);
   ...
   function reward_to_want(uint256 _earned) internal view returns (uint256) {
        ...
        )
    {
        ...
        if(pendingReward() > 0) {
            ...
            _earned = NYANToken.balanceOf(address(this));
            if(_earned > 0)
                compoundRewardsToLP(_earned);
        }
    ...
    function prepareMigration(address _newStrategy) internal override {
        ...
        if(pendingReward() > 0) {
            farmer.deposit(pid,0);
            compoundRewardsToLP(NYANToken.balanceOf(address(this)));
        }
    ...
    function protectedTokens() internal override view returns (address[] memory) {
        ...
        protected[0] = address(NYANToken); // rewards
        return protected;
    }
}