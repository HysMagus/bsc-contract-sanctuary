...
// File: contracts/JETFUEL/interfaces/pancake/ISmartChef.sol
...
// File: contracts/JETFUEL/strategies/Cake/StrategyCakeBest.sol
...
contract StrategyCakeBest is Ownable, Pausable {
    ...
    function withdraw(uint256 _amount) external {
        ...
        IERC20(cake).safeTransfer(treasury, _fee);
    ...
    function chargeFees() internal {
        uint256 toWbnb =
            IERC20(output).balanceOf(address(this)).mul(6).div(100);
    ...
    function approvePool(uint8 poolId) external onlyOwner {
        ...
        require(
            upcomingPool.proposedTime.add(approvalDelay) < now,
            "Delay hasn't fully ocurred"
        );
    ...
    function _approvePool(address _smartchef) internal {
        ...
        IERC20(_output).safeApprove(unirouter, uint256(-1));

        pools.push(
            Pool({smartchef: _smartchef, output: _output, enabled: true})
        );
