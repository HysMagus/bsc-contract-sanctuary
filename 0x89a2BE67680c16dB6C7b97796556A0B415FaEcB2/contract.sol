...
contract WhaleToken is ERC20("SEA WORLD FINANCE", "WHALE"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the WhalePool.
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
    }
}

contract WHALEPool is Ownable {
    using SafeMath for uint256;
    ...
    // Bonus muliplier for early whale stakers.
    ...
    constructor(
        ...
        router = _router;
        funds = msg.sender;
        whalePerBlock = _whalePerBlock;
        halvingPeriod = _halvingPeriod;
        startBlock = _startBlock;

        whale.approve(router, 0);
        whale.approve(router, uint(-1));
    }
    ...
    function getRewardPerBlock(uint blockNumber) public view returns (uint256) {
        if (block.number > startBlock){
            return whalePerBlock/( 2**( ( blockNumber.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
        } else {
            return 0;
        }
    }
    ...
    function getCurrentRewardPerBlock() public view returns (uint256) {
        if (block.number > startBlock){
            return whalePerBlock/( 2**( ( block.number.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
        } else {
            return 0;
        }
    }
    ...
    function pendingWhale(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 WhaleReward = multiplier.mul(whalePerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(halvFactor);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 halvFactor = 2**( ( block.number.sub(startBlock) ).div(halvingPeriod) );
        uint256 WhaleReward = multiplier.mul(whalePerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(halvFactor);
        uint256 WhaleFunds = WhaleReward.div(20); // 5%
        whale.mint(address(this), WhaleReward);
        whale.mint(address(this), WhaleFunds);
        address[] memory path = new address[](2);
        path[0] = address(whale);
        path[1] = wbnb;
        Swap(router).swapExactTokensForETH(WhaleFunds, uint256(0), path, funds, now.add(1800));
        pool.accWhalePerShare = pool.accWhalePerShare.add(WhaleReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to StakingContract for WHALE allocation.
