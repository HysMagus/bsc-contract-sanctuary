...
contract MappedToken is ContextUpgradeSafe, AuthQuota {
    using SafeERC20 for IERC20;
    
    bytes32 public constant REDEEM_TYPEHASH = keccak256("Redeem(address authorizer,address to,uint256 volume,uint256 nonce,uint256 chainId,uint256 txHash)");
	...
	function __MappedToken_init(address governor_, address token_) external initializer {
        __Context_init_unchained();
		__Governable_init_unchained(governor_);
		__MappedToken_init_unchained(token_);
	}
	
	function __MappedToken_init_unchained(address token_) public governance {
        token = token_;
    ...
    function redeem(address authorizer, address to, uint256 volume, uint256 chainId, uint256 txHash, uint8 v, bytes32 r, bytes32 s) external virtual {
        bytes32 structHash = keccak256(abi.encode(REDEEM_TYPEHASH, authorizer, to, volume, nonces[to]++, chainId, txHash));
...
contract MappedMATTER is MappedToken {
}


contract MappingToken is ERC20CappedUpgradeSafe, AuthQuota {
    bytes32 public constant MINT_TYPEHASH = keccak256("Mint(address authorizer,address to,uint256 volume,uint256 nonce,uint256 chainId,uint256 txHash)");
    bytes32 public constant DOMAIN_TYPEHASH = keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)");
    bytes32 public DOMAIN_SEPARATOR;
    mapping (address => uint) public nonces;
    ...
    function mint(address authorizer, address to, uint256 volume, uint256 chainId, uint256 txHash, uint8 v, bytes32 r, bytes32 s) external virtual {
        bytes32 structHash = keccak256(abi.encode(MINT_TYPEHASH, authorizer, to, volume, nonces[to]++, chainId, txHash));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "invalid signature");
        require(signatory == authorizer, "unauthorized");
