...
interface IMigrator {
    // Perform LP token migration from legacy BullFarm to BullFarm.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to BullFarm LP tokens.
    // BullFarm must mint EXACTLY the same amount of BullFarm LP tokens or
    // else something bad will happen. Traditional BullFarm does not
...
contract BullFarm is Ownable {
    ...
    function addList(address[] memory _lpToken, uint256 _allocPoint) public onlyOwner {
        uint256 i;
        for (i = 0; i < _lpToken.length; ++i) {
			add(_allocPoint, IBEP20(_lpToken[i]), false);
		}
    }

    // setToken, can only be called by the owner.
    function setToken(uint _pid, IBEP20 _newToken) public onlyOwner {
