...
contract LockingBB21 is Pausable {
    ...
    constructor(address B21, address payable _owner) public Owned(_owner) {
        ...
        b21Contract = B21;
        feesAddress = _owner;
        feesInEth = 150000000000000000;
        feesInToken = 500000000000000000000;
    ...
    function lockB21TokensFees(uint256 amount) external whenNotPaused returns (bool) {
        ...
        require(IBEP20(b21Contract).balanceOf(msg.sender) >= addTokenFees, 'balance of a user is less then value');
        uint256 checkAllowance = IBEP20(b21Contract).allowance(msg.sender, address(this));
        require(checkAllowance >= addTokenFees, 'allowance is wrong');
        require(IBEP20(b21Contract).transferFrom(msg.sender, address(this), addTokenFees), 'transfer From failed');
    ...
    function lockB21EthFees(uint256 amount) external payable whenNotPaused returns (bool) {
        ...
        require(IBEP20(b21Contract).balanceOf(msg.sender) >= amount, 'balance of a user is less then value');
        uint256 checkAllowance = IBEP20(b21Contract).allowance(msg.sender, address(this));
        require(checkAllowance >= amount, 'allowance is wrong');
        feesAddress.transfer(msg.value);
        require(IBEP20(b21Contract).transferFrom(msg.sender, address(this), amount), 'transfer From failed');
        emit LockTokens(msg.sender, address(this), amount);
        return true;
    }
    ...
    function transferAnyERC20Token(address tokenAddress, uint256 tokens, address transferTo) external whenNotPaused returns (bool success) {
        ...
        return IBEP20(tokenAddress).transfer(transferTo, tokens);
