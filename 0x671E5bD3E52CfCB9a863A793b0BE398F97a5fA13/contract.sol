...
interface IPresale {
    ...
    function contributions_1(address _user) external view returns(uint256);
    function contributions_2(address _user) external view returns(uint256);
...
contract Presale is Basic {
    ...
    uint256 public TOKENS = 230000 ether;                       // Tokens to sell
    ...
    uint256 public ROUND_1_END_TIME = now - 6 days;            // End date of stage 1
    uint256 public ROUND_2_START_TIME = now - 6 days;          // Start date of stage 2
    uint256 public ROUND_2_END_TIME = now - 5 days;            // End date of stage 2
    ...
    mapping(address => uint256) public contributions;         // Contributions Round 0
    ...
    IPresale public sale;
    

    // --- Config ---

    constructor() public {
        token = IERC20(0x887f6F5B0B03B2aE2C03897321CcaE9cb85a0C01);
        member = IMembers(0xef448e1Bdce74b36b3e0700056fAA952BC472923);
        masterchef = IMasterChef(0x3aCFa85F46d743252Ed8EA56Ecb9d1DBc5Be9f93);
        dev = msg.sender;
        sale = IPresale(0x9e4Ea22F736d8189d02A37A544e234F952A5dB57);
    }
    ...
    function claim() external {
        ...
        require(contributions[msg.sender] == 0, "!claim");
        contributions[msg.sender] = contributions[msg.sender].add(sale.contributions_1(msg.sender)).add(sale.contributions_2(msg.sender));
        uint balance = contributions[msg.sender];
        require(balance > 0, "You have no balance to claim");
        if(balance <= address(this).balance){
            token.transferFrom(address(msg.sender), address(this), token.balanceOf(msg.sender));
