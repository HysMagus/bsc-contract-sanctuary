...
contract ForeverBNB {
    ...
    function updateDividendPrice(uint _price) external onlyOwner returns(bool) {
          divPlanInvest = _price;
          return true;
    }  
    
    
    function updateStakingMinDeposit(uint _price) external onlyOwner returns(bool) {
          stakingMinInvest = _price;
          return true;
    } 
    
    function updateIncomeSharePercentage(uint _percentage) external onlyOwner returns(bool) { // (in 18 decimal)
        levelIncomeShare = _percentage;
        return true;
    }
    
    function updateDivPercentage(uint _percentage) external onlyOwner returns(bool) { // (in 18 decimal)
          roiPercentage[0] = _percentage;
          return true;
    } 
    
    function updateStakingPercentage(uint _percentage) external onlyOwner returns(bool) { // (in 18 decimal)
