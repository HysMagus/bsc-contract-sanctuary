...
// File: contracts/SBVAlpacaCompound.sol
...
contract SBVAlpacaCompound is IStakingRewardCompound, ReentrancyGuard, Initializable {
    using SafeMath for uint256;
    ...
    IVaultAlpaca public VAULT_ALPACA;

    /* ========== STATE VARIABLES ========== */

    IERC20 public stakingToken;
    address public userDepositToken;
    ...
    address[] public earnedToDepositPath;
    ...
    function initialize(
        ...
        (address _stakingToken,,,,) = IFairLaunch(_farmAddress).poolInfo(_pid);

        pid = _pid;
        snowBank = _snowBank;
        GALE = _gale;
        userDepositToken = _userDepositToken;
        VAULT_ALPACA = IVaultAlpaca(_stakingToken);
        earnedAddress = _earnedAddress;
        FAIRLAUNCH = IFairLaunch(_farmAddress);
        ...
        if(userDepositToken != earnedAddress){
          earnedToDepositPath = [_earnedAddress, WBNB, _userDepositToken];
        }

        IERC20(_userDepositToken).safeApprove(address(_stakingToken), uint256(-1));
    ...
    function stake(uint256 amount) external override nonReentrant updateReward(msg.sender)
     ...
     isNotEmergencyStop
     {
        ...
        IERC20(userDepositToken).safeTransferFrom(msg.sender, address(this), amount);
        /* stakingToken.safeTransferFrom(msg.sender, address(this), amount); */

        // fee 0.1% go to blizzardPool
       uint256 depositFee = amount.div(1000);
       IERC20(userDepositToken).safeTransfer(blizzardPool, depositFee);
       uint256 amounAfterFee =  amount.sub(depositFee);

       // deposit to vault ibToken
       uint256 ibTokenBefore = stakingToken.balanceOf(address(this));
       VAULT_ALPACA.deposit(amounAfterFee);
       uint256 ibTokenAfter = (stakingToken.balanceOf(address(this))).sub(ibTokenBefore);

       // calculate user share
       uint256 sharesAdded = ibTokenAfter;
        if (wantLockedTotal > 0) {
            sharesAdded = ibTokenAfter
                .mul(sharesTotal)
                .div(wantLockedTotal);
        }

        sharesTotal = sharesTotal.add(sharesAdded);
        wantLockedTotal = wantLockedTotal.add(ibTokenAfter);
        balances[msg.sender] = balances[msg.sender].add(sharesAdded);

        // deposit ibToken to Fairlaunch
        FAIRLAUNCH.deposit(address(this), pid, ibTokenAfter);
    ...
    function withdraw(uint256 amount) public override nonReentrant updateReward(msg.sender) {
        ...
        if(emergencyStop != true){
          FAIRLAUNCH.withdraw(address(this), pid, amountLP);
        }
        ...
        // withdraw from vault ibToken
        uint256 tokenBefore = IERC20(userDepositToken).balanceOf(address(this));
        VAULT_ALPACA.withdraw(amountLP);
        uint256 tokenAfter = (IERC20(userDepositToken).balanceOf(address(this))).sub(tokenBefore);

        IERC20(userDepositToken).safeTransfer(msg.sender, tokenAfter);
    ...
    function _harvest() private {
        ...
        uint256 tokenComp = IERC20(userDepositToken).balanceOf(address(this));

        // deposit to vault ibToken
        uint256 ibTokenBefore = stakingToken.balanceOf(address(this));
        VAULT_ALPACA.deposit(tokenComp);
        uint256 ibTokenAfter = (stakingToken.balanceOf(address(this))).sub(ibTokenBefore);
