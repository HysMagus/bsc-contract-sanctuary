...
contract Treasury is ContractGuard {
    ...
    modifier checkEpoch {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getDollarPrice() > dollarPriceOne) ? 0 : IERC20(dollar).totalSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_dollarPrice <= dollarPriceOne) {
            ...
            uint256 _currentRate = dollarPriceOne.add(_bondAmount.sub(dollarPriceOne).div(2));
            _rate = Math.min(_currentRate, maxDiscountRate);
        }
    }
    ...
    function getBondPremiumRate() public view returns (uint256 _rate) {
        ...
        if (_dollarPrice > dollarPriceCeiling) {
            uint256 _currentRate = dollarPriceOne.add(_dollarPrice.sub(dollarPriceOne).div(2));
            _rate = Math.min(_currentRate, maxPremiumRate);
        }
    }
