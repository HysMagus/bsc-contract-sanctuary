...
contract Harderdaddy is Context, IBEP20, Ownable {
    ...
    string private _name = "Harderdaddy";
    string private _symbol = "HARDR";
    ...
    function _getAntiDumpMultiplier() private view returns (uint256) {
        ...
        uint256 hour = 60 * 60;
        if (time_since_start < 1000 * hour) {
            return (5);
        } else if (time_since_start < 1200 * hour) {
            return (4);
        } else if (time_since_start < 1400 * hour) {
            return (3);
        } else if (time_since_start < 1800 * hour) {
            return (2);
        } else {
            return (1);
        }
        ...
        )
    {
        ...
        uint256 tFee = tAmount.div(20**2).mul(_taxFee).mul(multiplier);
        uint256 tLiquidity =
            tAmount.div(20**2).mul(_liquidityFee).mul(multiplier);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // If you copy this line of AHMET you're a shit dev who doesn't check over code
