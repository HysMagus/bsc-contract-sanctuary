...
contract ProteinToken is ERC20("Protein", "PROT"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (ProteinMaster).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }

    // Copied and modified from YAM:
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "PROT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PROT::delegateBySig: invalid nonce");
        require(now <= expiry, "PROT::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PROT::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PROT::_writeCheckpoint: block number exceeds 32 bits");
