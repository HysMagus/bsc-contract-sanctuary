...
contract YSLOptPancake is YSLOpt {
    using SafeERC20 for IERC20;

    ///@notice ApeSwap router
    address public apeSwap;

    ///@notice PancakeSwap master & router
    address public pancakeMaster;
    ...
    function initialize(
        ...
        pancakeMaster = 0x73feaa1eE314F8c655E354234017bE2193C9E24E;
    ...
    function compound() external override {
        ...
        if (wantBalance > DUST) {
            want.approve(pancakeSwap, wantBalance);
            IPancakeRouter02(pancakeSwap).swapExactTokensForTokens(
                wantBalance,
                1,
                path,
                address(this),
            ...
            collectControllerFee(usdBefore, pancakeSwap);
            ...
            IStrategySwap(stratSwap).swapBusdToLP(
        ...
        if (lpBalance > DUST) {
            lpToken.approve(pancakeMaster, lpToken.balanceOf(address(this)));
            IPancakeMaster(pancakeMaster).deposit(id, lpToken.balanceOf(address(this)));
        }
    }
    ...
    function earn(address _user, bool _isAmplified) external override onlyRole(STRAT_ROLE) {
        ...
        // Get total compounds
        uint256 totalCompounds = IPancakeMaster(pancakeMaster).userInfo(id, address(this)).amount;
        ...
        if (curBalance < userCompounds) {
            IPancakeMaster(pancakeMaster).withdraw(id, userCompounds - curBalance);
        }

        // Sell user's compounded amount into BUSD
        lpToken.approve(stratSwap, userCompounds);
        uint256 usersCompUSD = IStrategySwap(stratSwap).swapLPToBusd(userCompounds, pancakeSwap, path0, path1, lpToken);
    ...
    function transferOut(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        IStrategySwap(stratSwap).swapLPToBusd(fee, pancakeSwap, path0, path1, lpToken);

        // Lock fee
        uint256 amountUSDAfter = IERC20(BUSD).balanceOf(address(this));
        uint256 emission = getPrice(path[0], path[1], amountUSDAfter - amountUSDBefore, apeSwap);
        IsYSL(path[1]).mintFor(address(this), emission);
        addLiquidity(path[0], path[1], amountUSDAfter - amountUSDBefore, emission, apeSwap, adapter);

        // Turn underlying into BUSD
        amountUSDBefore = IERC20(BUSD).balanceOf(address(this));
        lpToken.approve(stratSwap, _amount);
        IStrategySwap(stratSwap).swapLPToBusd(_amount, pancakeSwap, path0, path1, lpToken);
    ...
    function transferIn(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        IStrategySwap(stratSwap).swapBusdToLP(_amount, pancakeSwap, path0, path1, lpToken);
    ...
    function getUserDepositedUSD(address _user) public view override returns (uint256 amount) {
        ...
        if (path0.length == 1) {
            return 2 * IStrategySwap(stratSwap).getBusdAmount(userDeposited[_user], path1, pancakeSwap);
        }
        if (path1.length == 1) {
            return 2 * IStrategySwap(stratSwap).getBusdAmount(userDeposited[_user], path0, pancakeSwap);
        }
        return IStrategySwap(stratSwap).getBusdAmount(userDeposited[_user], lpToken, pancakeSwap, path0, path1);
    }
    ...
    function getTotalDepositedUSD() public view returns (uint256 amount) {
        ...
        if (path0.length == 1) {
            return 2 * IStrategySwap(stratSwap).getBusdAmount(totalDeposited, path1, pancakeSwap);
        }
        if (path1.length == 1) {
            return 2 * IStrategySwap(stratSwap).getBusdAmount(totalDeposited, path0, pancakeSwap);
        }
        return IStrategySwap(stratSwap).getBusdAmount(totalDeposited, lpToken, pancakeSwap, path0, path1);
    }
    ...
    function mintBonus(
        ...
        uint256 syslemission = getPrice(BUSD, _sysl, usersCompUSD, pancakeSwap);
    ...
    function withdrawAcc(uint256 _amount) internal override {
        uint256 totalCompounds = IPancakeMaster(pancakeMaster).userInfo(id, address(this)).amount;
        if (totalCompounds > 0) {
            IPancakeMaster(pancakeMaster).withdraw(id, _amount);
        }
    }

    function restorePool() public onlyRole(STRAT_ROLE) {
        uint256 totalCompounds = IPancakeMaster(pancakeMaster).userInfo(id, address(this)).amount;
        IPancakeMaster(pancakeMaster).withdraw(id, totalCompounds);
    }
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        uint256 usersUSD = getPrice(address(want), BUSD, userCompounds, pancakeSwap);
        // Optimisation
    ...
    function currentReward(address _user) public view override returns (uint256) {
        ...
        return getPrice(BUSD, _sysl, rewardUSD, pancakeSwap);
    }
}