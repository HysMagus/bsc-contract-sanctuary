...
contract DragonFarmer {
    ...
    function buyUltraDrangon() public payable {
        ...
        require(userHatchRate[msg.sender] != 0);
        freeUser[msg.sender] = false;
        uint dragonPrice = getDragonPrice(userHatchRate[msg.sender], address(this).balance) * 15;
        uint dragonAmount = SafeMath.div(msg.value, dragonPrice);
        require(dragonAmount > 0);
        
        ceoEtherBalance += calculatePercentage(msg.value, 20);
        ultraDragon[msg.sender] += dragonAmount;
    }
    ...
    function getDragonPriceNo(uint256 eth) public view returns (uint) {
