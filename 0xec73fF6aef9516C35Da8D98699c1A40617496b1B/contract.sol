...
contract RewardPool {
    ...
    uint256 public rewardFactor = 10600; // 106%
    ...
    function initialize(address _vbswapToken, uint256 _startBlock) public notInitialized {
        ...
        startBlock = _startBlock; // supposed to be 5,128,600 (Tue Feb 23 2021 12:00:00 UTC)
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 _pendingReward) {
        ...
        if (_pid == lockPoolId) {
            ...
            lpSupply = pool.lpToken.balanceOf(address(this)).sub(lockPoolSupply);
        } else {
    ...
    function setLockPeriod(uint256 _pid, uint256 _seconds) external onlyOperator {
