...
contract GoGo {
  ...
  address public buyFactory;
  IUniswapV2Router02 public sellRouter;

  constructor(address _buyFactory, address _sellRouter) public {
    buyFactory = _buyFactory;  
  ...
  function start(
    ...
    address pairAddress = IUniswapV2Factory(buyFactory).getPair(token0, token1);
...
//calle from the buy market 
  function jetswapCall(
   ...
   require(
      msg.sender == UniswapV2Library.pairFor(buyFactory, token0, token1), 
      'Unauthorized!!!!!'); 
    ...
    // about 0.3%
    uint fee = ((amountToken * 3) / 997) + 1;
    uint amountToRepay = amountToken + fee;

    // do stuff here
    emit Log("amount", amountToken);
