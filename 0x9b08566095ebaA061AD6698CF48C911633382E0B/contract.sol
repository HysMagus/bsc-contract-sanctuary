...
// Deltacore Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract DeltaVaultV1 is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DELTAs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDeltaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accDeltaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Deltas to distribute per block.
        uint256 accDeltaPerShare; // Accumulated Deltas per share, times 1e12. See below.
    ...
    // Update the given pool's DELTA allocation point. Can only be called by the owner.
    ...
    // View function to see pending DELTAs on frontend.
    function pendingDelta(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accDeltaPerShare = pool.accDeltaPerShare;

        return user.amount.mul(accDeltaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the DELTACORE token.
    // ----
    uint256 private deltaBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = delta.balanceOf(address(this)).sub(deltaBalance);

        if(newRewards > 0) {
            deltaBalance = delta.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 deltaRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        deltaRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 deltaRewardFee = deltaRewardWhole.mul(DEV_FEE).div(10000);
        uint256 deltaRewardToDistribute = deltaRewardWhole.sub(deltaRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(deltaRewardFee);

        pool.accDeltaPerShare = pool.accDeltaPerShare.add(
            deltaRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit  tokens to DeltaVault for DELTA allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accDeltaPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accDeltaPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from DeltaVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming DELTA farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accDeltaPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeDeltaTransfer(from, pending);
        }
 ...
 // function that lets owner/governance contract
    ...
    // Safe delta transfer function, just in case if rounding error causes pool to not have enough DELTAs.
    function safeDeltaTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 deltaBal = delta.balanceOf(address(this));
        if (_amount > deltaBal) {
            delta.transfer(_to, deltaBal);
            deltaBalance = delta.balanceOf(address(this));

        } else {
            delta.transfer(_to, _amount);
            deltaBalance = delta.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 deltaBal = delta.balanceOf(address(this));
        if (pending_DEV_rewards > deltaBal) {
            delta.transfer(devaddr, deltaBal);
            deltaBalance = delta.balanceOf(address(this));
        } else {
            delta.transfer(devaddr, pending_DEV_rewards);
            deltaBalance = delta.balanceOf(address(this));
        }
