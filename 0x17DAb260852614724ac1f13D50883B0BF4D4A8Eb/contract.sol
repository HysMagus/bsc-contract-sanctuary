    ...
    CryptoAlpacaEnergyListener
{
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt;
        // token reward debt
        mapping(address => uint256) tokenRewardDebt;
        // share
        uint256 share;
    ...
    struct RewardInfo {
        ...
        // Accumulated ALPAs per share.
        uint256 accAlpaPerShare;
        // Accumulated Share
        uint256 accShare;
    }

    struct PartnerRewardTokenInfo {
        // reward token
        IERC20 rewardToken;
        ...
        // Last block number that partner token distribution occurs.
        uint256 lastPartnerRewardBlock;
    }
    ...
    uint256 public maxAlpacaSquadCount = 10;

    // Info of each user.
    mapping(address => UserInfo) public userInfo;

    // stores an array of partner token infos
    PartnerRewardTokenInfo[] public partnerTokenInfo;

    // user address -> partner token address ->
    mapping(address => mapping(address => UserInfo))
        public partnerTokenUserInfo;
    ...
    /**
     * @dev View `_user` pending Reward Token
     */
    function pendingRewardToken(address _user, uint256 _rewardTokenIndex)
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_user];
            ...
            PartnerRewardTokenInfo storage rewardTokenInfo
         = partnerTokenInfo[_rewardTokenIndex];

        uint256 accTokenPerShare = rewardTokenInfo.accTokenPerShare;

        if (
            block.number > rewardTokenInfo.lastPartnerRewardBlock &&
            rewardInfo.accShare != 0
        ) {
            uint256 tokenReward = rewardTokenInfo.tokenPerBlock.mul(
                block.number.sub(rewardTokenInfo.lastPartnerRewardBlock)
            );

            accTokenPerShare = accTokenPerShare.add(
                tokenReward.mul(SAFE_MULTIPLIER).div(rewardInfo.accShare)
            );
        }

        return
            user.share.mul(accTokenPerShare).div(SAFE_MULTIPLIER).sub(
                user.tokenRewardDebt[address(rewardTokenInfo.rewardToken)]
            );
    }

    function partnerTokenLength() external view returns (uint256) {
        return partnerTokenInfo.length;
    }
    ...
    function updatePool() public {
        ...
        for (uint256 i = 0; i < partnerTokenInfo.length; i++) {

                PartnerRewardTokenInfo storage rewardTokenInfo
             = partnerTokenInfo[i];

            if (block.number <= rewardTokenInfo.lastPartnerRewardBlock) {
                continue;
            }
            ...
            rewardTokenInfo.accTokenPerShare = rewardTokenInfo
                .accTokenPerShare
                .add(tokenReward.mul(SAFE_MULTIPLIER).div(rewardInfo.accShare));

            rewardTokenInfo.lastPartnerRewardBlock = block.number;
        }
    ...
    function claim() public nonReentrant {
        ...
        _transferReward(sender, user, user.share);
    }
    ...
    function onERC1155Received(
        ...
        (
            ...
            uint256 numAlpacas,
            uint256 sumEnergy
        ) = _calculateAddition(_from, user, ids);

        updatePool();
        _transferReward(_from, user, share);
        ...
        if (_shouldGrantPermissionToBreed(_data)) {
    ...
    function onERC1155BatchReceived(
        ...
        (
            ...
            uint256 numAlpacas,
            uint256 sumEnergy
        ) = _calculateAddition(_from, user, _ids);
    ...
    */
    function onCryptoAlpacaEnergyChanged(
        ...
        uint256 sumEnergy = uint256(user.sumEnergy).add(_newEnergy).sub(
            _oldEnergy
        );
        uint256 share = sumEnergy.mul(sumEnergy).div(user.numAlpacas);

        updatePool();
        _transferReward(from, user, share);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            (, , , , , , , , , , , uint256 energy, uint256 state) = cryptoAlpaca
                .getAlpaca(id);
            require(state == 1, "AlpacaSquad: invalid alpaca state");
            require(energy > 0, "AlpacaSquad: invalid alpaca energy");
            sumEnergy = sumEnergy.add(energy);

            alpacaOriginalOwner.set(id, _from);
        }
    ...
    {
        numAlpacas = uint256(_user.numAlpacas).sub(_ids.length);
        sumEnergy = _user.sumEnergy;
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            require(
                ...
                "AlpacaSquad: original owner not found"
            );
    ...
    function _safeRewardTokenTransfer(
        IERC20 rewardToken,
        address _to,
        uint256 _amount
    ) private {
    ...
    function _shouldGrantPermissionToBreed(bytes memory _data)
        private
        pure
        returns (bool)
    {
        return _data.length > 0 && uint8(_data[0]) == 1;
    }
    ...
    function _transferReward(
        ...
        if (_user.share > 0) {
            ...
            if (pending > 0) {
                _safeAlpaTransfer(_sender, pending);
            }
        }

        for (uint256 i = 0; i < partnerTokenInfo.length; i++) {
            PartnerRewardTokenInfo storage info = partnerTokenInfo[i];

            if (_user.share > 0) {
                uint256 tokenPending = _user
                    .share
                    .mul(info.accTokenPerShare)
                    .div(SAFE_MULTIPLIER)
                    .sub(_user.tokenRewardDebt[address(info.rewardToken)]);
                if (tokenPending > 0) {
            ...
            _user.tokenRewardDebt[address(info.rewardToken)] = _newShare
                .mul(info.accTokenPerShare)
                .div(SAFE_MULTIPLIER);
        }

        // Update user reward debt with new share
        _user.rewardDebt = _newShare.mul(rewardInfo.accAlpaPerShare).div(
            SAFE_MULTIPLIER
        );
    }
    ...
    function addPartnerToken(
        ...
        partnerTokenInfo.push(
            PartnerRewardTokenInfo({
                rewardToken: _rewardToken,
                tokenPerBlock: _tokenPerBlock,
                accTokenPerShare: 0,
                lastPartnerRewardBlock: _startBlock
            })
        );
    }

    function removePartnerToken(uint256 _index) public onlyOwner {
        require(_index >= 0 && _index < partnerTokenInfo.length);

        uint256 lastIndex = partnerTokenInfo.length - 1;

        partnerTokenInfo[_index] = partnerTokenInfo[lastIndex];

        partnerTokenInfo.pop();
    }
    ...
    modifier fromCryptoAlpaca() {
        require(
            ...
            "AlpacaSquad: received alpaca from unauthenticated contract"
        );
        _;
    }
