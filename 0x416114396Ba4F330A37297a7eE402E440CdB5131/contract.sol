...
contract TESTER is ERC20, Ownable {
    ...
    uint256 public collectedForMarketing;
    uint256 public collectedForBusd;
    uint256 public collectedForCake;
    ...
    constructor() ERC20("TESTER", "TESTER") Ownable() {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (isExcludedFromFee[from] || isExcludedFromFee[to]) {
            ...
            uint256 marketingFeeAmount = amount * marketingFee / 100;
            uint256 busdFeeAmount = amount * busdFee / 100;
            uint256 cakeFeeAmount = amount * cakeFee / 100;
            uint256 liquidityFeeAmount = amount * liquidityFee / 100;

            collectedForMarketing += marketingFeeAmount;
            collectedForBusd += busdFeeAmount;
            collectedForCake += cakeFeeAmount;
            collectedForLiquidity += liquidityFeeAmount;

            uint256 totalFeeAmount = marketingFeeAmount + busdFeeAmount + cakeFeeAmount + liquidityFeeAmount;
    ...
    function _swapAndDistribute() internal lockTheSwap {
        uint256 leaveForLiquidity = collectedForLiquidity / 2;
        ...
        uint256 marketingAmount = totalBalance * collectedForMarketing / totalToSwap;
        uint256 busdAmount = totalBalance * collectedForBusd / totalToSwap;
        uint256 cakeAmount = totalBalance * collectedForCake / totalToSwap;
        uint256 liquidityAmount = totalBalance - marketingAmount - busdAmount - cakeAmount;

        payable(marketingWallet).transfer(marketingAmount);

        _swapETHForTokens(address(busd), busdAmount);
        uint256 busdBalance = busd.balanceOf(address(this));
        busd.safeTransfer(address(tracker), busdBalance);
        tracker.distributeDividends(address(busd), busdBalance);

        _swapETHForTokens(address(cake), cakeAmount);
        ...
        _addLiquidity(leaveForLiquidity, liquidityAmount);

        collectedForMarketing = 0;
        collectedForBusd = 0;
        collectedForCake = 0;
        collectedForLiquidity = balanceOf(address(this));
    }
