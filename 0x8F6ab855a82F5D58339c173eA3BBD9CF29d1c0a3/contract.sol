...
// File: contracts/JulProtocolV3_JULb_SWGb.sol

pragma solidity 0.6.12;

contract JulProtocolV3_JULb_SWGb is Ownable {
    using SafeMath for uint256;
    ...
    uint256 constant MINIMUM_DEPOSIT_AMOUNT = 5 * 10 ** 17 ;    //0.5 JULb for real production      //
    ...
    uint256 constant DEPOSIT_FEE = 0; // 0.4 %                                                      //
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    constructor(address julb_token, address swgb_token) public payable {
        SWGB_TOKEN = swgb_token;
        SwgbToken = IERC20(swgb_token);

        JULB_TOKEN = julb_token;
        JulbToken = IERC20(julb_token);
        ...
        require(pairAddress != address(0), 'JulProtocolV3_JULb_SWGb: pair is not created yet');
    ...
    {
        require(protocolusers[_user].lastDepositedDate > 0, "JulProtocolV3_JULb_SWGb: this is first deposite.");
        require(now > protocolusers[_user].lastDepositedDate, "JulProtocolV3_JULb_SWGb: now should be greater than last deposited date");
    ...
    {
        require(input_amount >= MINIMUM_DEPOSIT_AMOUNT, "JulProtocolV3_JULb_SWGb: Insufficient Liquidity amount");  

        _safeTransferFrom(JULB_TOKEN, msg.sender, address(this), input_amount);
        ...
        uint256 balance = SwgbToken.balanceOf(address(this));
        require(balance >= token1_amount, "JulProtocolV3_JULb_SWGb: Insufficient SWGb token amount");

        _safeTransfer(SWGB_TOKEN, address(pair), token1_amount);
        _safeTransfer(JULB_TOKEN, address(pair), token0_amount);
    ...
    function _toSWGb(uint256 amountIn, address to) internal returns (uint256 amountOut) {
        
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
        (uint reserveIn, uint reserveOut) = token0 == JULB_TOKEN ? (reserve0, reserve1) : (reserve1, reserve0);
        uint amountInWithFee = amountIn.mul(997);
        uint numerator = amountInWithFee.mul(reserveOut);
        uint denominator = reserveIn.mul(1000).add(amountInWithFee);
        amountOut = numerator / denominator;
        (uint amount0Out, uint amount1Out) = token0 == JULB_TOKEN ? (uint(0), amountOut) : (amountOut, uint(0));
        pair.swap(amount0Out, amount1Out, to, new bytes(0));
        return amountOut;
    }

    function removeLiquidity(uint256 withdrawAmount, uint deadline)
    external ensure(deadline) returns(uint256 amountjulb, uint256 amountswgb)
    {
        (, , uint trwi) = readUsersDetails(msg.sender);

        require(trwi >= withdrawAmount, "JulProtocolV3_JULb_SWGb: Insufficient Removable Balance");
        (uint reserveA, ) = BSCswapLibrary.getReserves(
            BSCSWAP_FACTORY,
            JULB_TOKEN,
            SWGB_TOKEN
        );
        require(reserveA > 0 , "Pool have no JULB");
        uint totalSupply = pair.totalSupply();
        uint liqAmt = BSCswapLibrary.quote(withdrawAmount, reserveA, totalSupply);
        uint balance = pair.balanceOf(address(this));
        require(liqAmt <= balance, "JulProtocolV3_JULb_SWGb: Insufficient Liquidity in BSCswap");
        pair.transfer(address(pair), liqAmt);
        (uint amount0, uint amount1) = pair.burn(address(this));
        (address token0,) = BSCswapLibrary.sortTokens(JULB_TOKEN, SWGB_TOKEN);
        (uint amountOut, ) = JULB_TOKEN == token0 ? (amount0, amount1) : (amount1, amount0);
        uint interest_julb;
        ...
        {
            interest_julb = amountOut;
            amountjulb = 0;
            protocolusers[msg.sender].pendingAmount = protocolusers[msg.sender].pendingAmount - amountOut;

        }
        else{
            interest_julb = protocolusers[msg.sender].pendingAmount;
            amountjulb = amountOut - protocolusers[msg.sender].pendingAmount;
            ...
            _safeTransfer(JULB_TOKEN,  msg.sender, amountjulb);
        }
        _safeTransfer(JULB_TOKEN,  address(pair), interest_julb);
        amountswgb = _toSWGb(interest_julb, msg.sender);
        if(FARM_TOKEN != address(0))
        {
            IERC20Mintable(FARM_TOKEN).mint(msg.sender, interest_julb);
        }
        //end
    }
 
    function getLiquidityBalance() public view returns (uint256 liquidity) {
        address pairAddress = BSCswapLibrary.pairFor(BSCSWAP_FACTORY, JULB_TOKEN, SWGB_TOKEN);
    ...
    function withdrawFee(address payable ca, uint256 amount) public onlyOwner{
        require(totalFee >= amount , "JulProtocolV3_JULb_SWGb: Insufficient BUSD amount!");
        uint256 balance = JulbToken.balanceOf(address(this));
        require(balance >= amount , "JulProtocolV3_JULb_SWGb: Insufficient BUSD balance in contract!" );
        _safeTransfer(JULB_TOKEN, ca, amount);
    ...
    {
        require(_interestRadio > 0, "JulProtocolV3_JULb_SWGb: intrest should be greater than zero");
        require(_interestRadio < 50, "JulProtocolV3_JULb_SWGb: intrest should be lesee than 0.5%");
