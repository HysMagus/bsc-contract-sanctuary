...
contract OneEightyDay is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block number when PARA mining ends.
    uint256 public bonusEndBlock;

    // The block number when CPARA mining starts.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 PARAReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(PARAReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 PARAReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(PARAReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
    ...
    function _getMultiplier(uint256 _from, uint256 _to) internal view returns (uint256) {
      ...
      address public devAddy = 0x9F9A54e5F0C07db318bB27091aE398c15608603F;//collects pool use fee
    uint256 public devtxfee = 1; //Fee for pool use, sent to Parafarming pool
