...
contract VaporLPStaker is Ownable {
    using SafeMath for uint256;

    IPancakeERC20 public immutable _VaporBnbPairContract;    // Vapor-BNB Pair contract
    ...
    uint256 private _rewardAmountPerBlock = 127;            // Vapor-BNB LP 0.0000127 (127 / 10^7) per block
    ...
    uint256 public _blockSpeed = 30;                       // block generation speed 3s
    ...
    constructor (IPancakeERC20 VaporBnbPairContract, address devWallet, uint256 devRewardPct, uint256 duration, uint256 lockTime) public {
        _VaporBnbPairContract = VaporBnbPairContract;
    ...
    function stake(uint256 amount) public {
        ...
        require(
            _startTime > 0 &&
            now >= _startTime,
            'VaporLPStaker: staking not started yet.'
        );
        
        require(
            _VaporBnbPairContract.transferFrom(
                _msgSender(),
                address(this),
                amount
            ),
            "VaporLPStaker: stake failed."
        );
    ...
    function unstake(uint256 amount) public {
        updateUnstakeAllow();
        require(_unstakeAllow, "VaporLPStaker: unstake not allowed");
        require(
            _stakeMap[_msgSender()].stakedAmount >= amount,
            "VaporLPStaker: unstake amount exceededs the staked amount."
        );
        ...
        require(
            _VaporBnbPairContract.transfer(
                _msgSender(),
                amount
            ),
            "VaporLPStaker: unstake failed."
        );
    ...
    function calcReward(address staker, uint256 currentBlockNumber) private view returns (uint256) {
        ...
        for(uint i = _stakeMap[staker].lastSnapShotIndex; i < snapShotHistory.length; i++) {
            ...
            rewardAmount = rewardAmount.add(_rewardAmountPerBlock.mul(passedBlockCount).mul(10**(_rewardTokenDecimal - 7)).mul(rewardPctForStakers).div(100).mul(stakedAmount).div(prevTotalStakedAmount));
        }
        return rewardAmount;
    }
    ...
    /**
     * Get reward pool balance (Vapor-BNB LP)
     */
    ...
    function setRewardContract(RewardContract rewardContract) external onlyOwner {
        require(address(rewardContract) != address(0), 'VaporLPStaker: reward contract address should not be zero address.');
    ...
    function setRewardAmountPerBlock(uint256 rewardAmountPerBlock) external onlyOwner {
        require(rewardAmountPerBlock >= 1, 'VaporLPStaker: reward amount per block should be greater than 1.');
    ...
    function setDevWallet(address devWallet) public onlyOwner {
        require(devWallet != address(0), 'VaporLPStaker: devWallet is zero address.');
    ...
    function setDevRewardPct(uint256 devRewardPct) public onlyOwner {
        require(devRewardPct < 4, 'VaporLPStaker: devRewardPct should be less than 4.');
    ...
    function setStartTime() external onlyOwner {
        require (
            ...
            'VaporLPStaker: should set start time later.'
        );
