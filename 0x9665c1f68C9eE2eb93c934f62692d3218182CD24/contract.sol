...
contract LEMONToken is BEP20('LemonSwap', 'LEMON') {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying LEMON (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of LEMON
    ...
    event UpdateEmissionRate(address indexed user, uint256 LEMONPerBlock);
    ...
    function pendingLEMON(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accLEMONPerShare = accLEMONPerShare.add(LEMONReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accLEMONPerShare = pool.accLEMONPerShare.add(LEMONReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeLEMONTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > LEMONBal) {
            transferSuccess = LEMON.transfer(_to, LEMONBal);
        } else {
            transferSuccess = LEMON.transfer(_to, _amount);
        }
        require(transferSuccess, "safeLEMONTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _LEMONPerBlock) external onlyOwner {
        massUpdatePools();
        LEMONPerBlock = _LEMONPerBlock;
        emit UpdateEmissionRate(msg.sender, _LEMONPerBlock);
    }
