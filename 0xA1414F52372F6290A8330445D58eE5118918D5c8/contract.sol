...
// File: contracts/LemonPresale.sol
...
contract LemonPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale Lemon, 5 BUSD
	uint256 BUSDPerToken = 1^10;
	...
	function getLemonTokensLeft() external view returns (uint256) {
		return Lemon.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some Lemon tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed Lemon tokens");
		require(Lemon.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough Lemon tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		Lemon.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldLemon() external onlyOwner {
		uint256 amount = Lemon.balanceOf(address(this)) - totalTokensSold;
		Lemon.safeTransfer(msg.sender, amount);
	}

	function withdrawAllLemon() external onlyOwner {
		Lemon.safeTransfer(msg.sender, Lemon.balanceOf(address(this)));
	}
}