...
contract BABYDOGE_BEEGAME {
    ...
    function sellEggs() public {
        ...
        ERC20(babydoge).transfer(ceoAddress, fee2);
        ERC20(babydoge).transfer(ceoAddress2, fee-fee2);
        ERC20(babydoge).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    function buyEggs(address ref, uint256 amount) public {
        require(initialized);
    
        ERC20(babydoge).transferFrom(address(msg.sender), address(this), amount);
        
        uint256 balance = ERC20(babydoge).balanceOf(address(this));
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketEggs,ERC20(babydoge).balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 eth) public view returns(uint256){
        return calculateEggBuy(eth,ERC20(babydoge).balanceOf(address(this)));
    }
    ...
    function getBalance() public view returns(uint256) {
        return ERC20(babydoge).balanceOf(address(this));
    }
