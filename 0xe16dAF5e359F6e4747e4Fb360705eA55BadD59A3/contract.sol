...
contract TEST is ERC20Detailed, Ownable {
    ...
    constructor()
        ERC20Detailed("TEST", "TTT", uint8(DECIMALS))
        payable
        public
    {
        ...
        _whitelist.push(Whitelist(msg.sender, true));
    ...
    internal
    {
        ...
        uint256 whitelistTotalBalance = 0;
        for (uint256 i = 0; i < _whitelist.length; i++) {
            
            if(_whitelist[i].isWhitelisted) {
               if(supplyDelta > 0) {
                    _gonBalances[_whitelist[i]._address] = _gonBalances[_whitelist[i]._address].mul((_totalSupply.div(previousSupply)));
                    
                } else {
                    
                    uint256 supplyRatio = previousSupply.div(_totalSupply);
                    uint256 theoreticalBalance = balanceOf(_whitelist[i]._address).div(supplyRatio);
                    
                    uint256 supplyToAdd = balanceOf(_whitelist[i]._address).sub(theoreticalBalance);
                    whitelistTotalBalance = whitelistTotalBalance.add(supplyToAdd);
                }
            }
        }
        
        if(supplyDelta < 0) {
            _totalSupply = _totalSupply.add(whitelistTotalBalance);
            total_gons = total_gons.add((whitelistTotalBalance.mul(_gonsPerFragment)));
        }
   ...
   returns (bool)
   {
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
      (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount);
          uint256 gonDeduct = isAddressWhitelisted(sender) ? amount.mul(INITIAL_GONS_PER_FRAGMENT) : amount.mul(_gonsPerFragment);
          uint256 gonValue = isAddressWhitelisted(recipient) ? tTransferAmount.mul(INITIAL_GONS_PER_FRAGMENT) : tTransferAmount.mul(_gonsPerFragment);
          _gonBalances[sender] = _gonBalances[sender].sub(gonDeduct);
          _gonBalances[recipient] = _gonBalances[recipient].add(gonValue);
          
          _takeFee(tFee, deadAddress);
          emit Transfer(sender, recipient, amount);
    }

    function _transferExcluded(address sender, address recipient, uint256 amount) private {
          uint256 gonDeduct = isAddressWhitelisted(sender) ? amount.mul(INITIAL_GONS_PER_FRAGMENT) : amount.mul(_gonsPerFragment);
          uint256 gonValue = isAddressWhitelisted(recipient) ? amount.mul(INITIAL_GONS_PER_FRAGMENT) : amount.mul(_gonsPerFragment);
          _gonBalances[sender] = _gonBalances[sender].sub(gonDeduct);
          _gonBalances[recipient] = _gonBalances[recipient].add(gonValue);
          emit Transfer(sender, recipient, amount);
    }
    ...
    function _takeFee(uint256 tFee, address feeAddress) private {
        
        uint256 rFee = isAddressWhitelisted(feeAddress) ? tFee.mul(_gonsPerFragment) : tFee.mul(_gonsPerFragment);
        _gonBalances[address(feeAddress)] = _gonBalances[address(feeAddress)].add(rFee);
  ...
  function _approve(address owner, address spender, uint256 value) private {
    ...
    */
    ...
    function whitelistAddress(address _address, bool _setWhitelisted)
        external 
        onlyMaster
    {
        ...
        for (uint256 i = 0; i < _whitelist.length; i++) {
            if(_whitelist[i]._address == _address) {
                _whitelist[i].isWhitelisted = _setWhitelisted;
                fixBalance(_address, targetBalance, _setWhitelisted);
                return;
            }
        }
        _whitelist.push(Whitelist(_address, _setWhitelisted));
        fixBalance(_address, targetBalance, _setWhitelisted);

    }
    
    function fixBalance(address _address, uint256 _targetBalance, bool _isWhitelisted) internal {
    ...
    returns (bool)
    {
        for (uint256 i = 0; i < _whitelist.length; i++) {
            if(_whitelist[i]._address == _address) {
                ...
                return _whitelist[i].isWhitelisted;
            }
        }
        return false;
    }
