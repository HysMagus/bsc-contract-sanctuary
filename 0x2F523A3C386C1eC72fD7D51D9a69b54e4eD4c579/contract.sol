...
// File: contracts/BIFI/strategies/Cake/StrategyCrowLP.sol
 ...
 * CROW rewards from providing liquidity are farmed every few minutes, sold and split 50/50. 
...
contract StrategyCrowLP is Ownable, Pausable {
     ...
     * {crow} - Token generated by staking our funds. In this case it's the CROWs token.
    ...
    address constant public crow = address(0xcc2E12a9b5b75360c6FBf23B584c275D52cDdb0E);
    ...
    address constant public masterchef = address(0xB5ff2e278A1093850c4C0892b3b0FBF757BDbe67);
    uint8 public poolId;
    ...
    constructor(address _lpPair, uint8 _poolId, address _vault, address _strategist) public {
        ...
        IERC20(crow).safeApprove(unirouter, uint(-1));
     ...
     * It deposits {lpPair} in the MasterChef to farm {crow}
     */
    ...
    function chargeFees() internal {
        ...
        IPancakeRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWbnb, 0, crowToWbnbRoute, address(this), now.add(600));
    ...
    /**
     * @dev Swaps {crow} for {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {
        uint256 crowHalf = IERC20(crow).balanceOf(address(this)).div(2);

        if (lpToken0 != crow) {
            IPancakeRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(crowHalf, 0, crowToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != crow) {
            IPancakeRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(crowHalf, 0, crowToLp1Route, address(this), now.add(600));
        }
