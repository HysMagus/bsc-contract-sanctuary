...
contract ContentCoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("ContentCoin_Dividend_Tracker", "ContentCoin_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ContentCoin_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "ContentCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ContentCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ContentCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "ContentCoin_Dividend_Tracker: Cannot update claimWait to same value");
...
2% auto distributed in BNB back to $GME holders 
2% auto added back to PancakeSwap Pair 
5% transferred to marketing wallet (denominated in BNB) 

1. Content Coin
2.CNT
3. 9 decimals
4. 1,000,000,000 1 Billion
*/ 
contract ContentCoin is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    ContentCoinDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Content Coin", "CNT") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        MarketingFee = 5;
        MarketingWallet = payable(0x895c160D5c1988d0e92556B1E34aEC818Dc5fB86);

        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee); // total fee transfer and buy

    	dividendTracker = new ContentCoinDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ContentCoin: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ContentCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ContentCoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ContentCoin: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ContentCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ContentCoin: Cannot update gasForProcessing to same value");
