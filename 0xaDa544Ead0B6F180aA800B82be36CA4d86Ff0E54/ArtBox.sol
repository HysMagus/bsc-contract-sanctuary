...
contract ArtBox is ERC721Enumerable, Ownable, Pausable {
  ...
  event BoxLocked(
    ...
    uint8 indexed boxXId,
    uint8 indexed boxYId
  );
  event BoxUpdated(
    address indexed owner,
    uint8 indexed boxXId,
   ...
   */
  function getBoxByCoordinates(uint8 _boxXId, uint8 _boxYId)
    public
    view
   ...
   * @return ArtBoxTypes.Box an artbox with all the attributes
   */
  function getBoxById(uint8 _boxId)
    public
    view
    returns (ArtBoxTypes.Box memory)
  {
   ...
   */
  function lockBox(uint8 boxXId, uint8 boxYId)
    external
    payable
    whenNotPaused
    returns (bool)
   ...
   * */
  function migrateBox(uint8 _boxXId, uint8 _boxYId)
    public
    onlyOwner
    whenNotPaused
  {
