...
contract XVS_WBNB_Pool is Ownable {
    ...
    function getPendingDivs(address _holder) public view returns (uint256 _pendingDivs) {
        ...
        if (block.timestamp <= farmEnableat + 1 days) {
            uint256 pendingDivs = stakedAmount.mul(16127617).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 1 days && block.timestamp <= farmEnableat + 2 days) {
            uint256 pendingDivs = stakedAmount.mul(14862706).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 2 days && block.timestamp <= farmEnableat + 3 days) {
            uint256 pendingDivs = stakedAmount.mul(13661040).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 3 days && block.timestamp <= farmEnableat + 4 days) {
            uint256 pendingDivs = stakedAmount.mul(12585866).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 4 days && block.timestamp <= farmEnableat + 5 days) {
            uint256 pendingDivs = stakedAmount.mul(11573937).mul(timeDiff).div(rewardInterval).div(1e4);
            ...
            uint256 pendingDivs = stakedAmount.mul(9803061).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 7 days && block.timestamp <= farmEnableat + 8 days) {
            uint256 pendingDivs = stakedAmount.mul(8980869).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 8 days && block.timestamp <= farmEnableat + 9 days) {
            uint256 pendingDivs = stakedAmount.mul(8285168).mul(timeDiff).div(rewardInterval).div(1e4);
            ...
            uint256 pendingDivs = stakedAmount.mul(7020257).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 11 days && block.timestamp <= farmEnableat + 12 days) {
            uint256 pendingDivs = stakedAmount.mul(6451047).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 12 days && block.timestamp <= farmEnableat + 13 days) {
            uint256 pendingDivs = stakedAmount.mul(5945083).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 13 days && block.timestamp <= farmEnableat + 14 days) {
            uint256 pendingDivs = stakedAmount.mul(5439118).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 14 days && block.timestamp <= farmEnableat + 15 days) {
            uint256 pendingDivs = stakedAmount.mul(4996399).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 15 days && block.timestamp <= farmEnableat + 16 days) {
            uint256 pendingDivs = stakedAmount.mul(4616926).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 16 days && block.timestamp <= farmEnableat + 17 days) {
            uint256 pendingDivs = stakedAmount.mul(4237453).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 17 days && block.timestamp <= farmEnableat + 18 days) {
            uint256 pendingDivs = stakedAmount.mul(3921225).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 18 days && block.timestamp <= farmEnableat + 19 days) {
            uint256 pendingDivs = stakedAmount.mul(3604997).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 19 days && block.timestamp <= farmEnableat + 20 days) {
            uint256 pendingDivs = stakedAmount.mul(3288769).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 20 days && block.timestamp <= farmEnableat + 21 days) {
            uint256 pendingDivs = stakedAmount.mul(3035787).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 21 days && block.timestamp <= farmEnableat + 22 days) {
            uint256 pendingDivs = stakedAmount.mul(2782805).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 22 days && block.timestamp <= farmEnableat + 23 days) {
            uint256 pendingDivs = stakedAmount.mul(2593068).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 23 days && block.timestamp <= farmEnableat + 24 days) {
            uint256 pendingDivs = stakedAmount.mul(2340086).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 24 days && block.timestamp <= farmEnableat + 25 days) {
            uint256 pendingDivs = stakedAmount.mul(2150348).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 25 days && block.timestamp <= farmEnableat + 26 days) {
            uint256 pendingDivs = stakedAmount.mul(2023858).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 26 days && block.timestamp <= farmEnableat + 27 days) {
            uint256 pendingDivs = stakedAmount.mul(1834122).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 27 days && block.timestamp <= farmEnableat + 28 days) {
            uint256 pendingDivs = stakedAmount.mul(1707630).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 28 days && block.timestamp <= farmEnableat + 29 days) {
            uint256 pendingDivs = stakedAmount.mul(1581139).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 29 days && block.timestamp <= farmEnableat + 30 days) {
            uint256 pendingDivs = stakedAmount.mul(1454648).mul(timeDiff).div(rewardInterval).div(1e4);
