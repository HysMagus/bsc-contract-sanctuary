...
contract DfPool is Initializable, FundsMgrUpgradable, DSMath {
    address public BRIDGE_ADDRESS = 0xD83893F31AA1B6B9D97C9c70D3492fe38D24d218;
    ...
    // TODO: add logic for lock users' dDaiWrapper
    // function lockDDaiFromEthereum(uint _amount) public onlyOwner {
    //     if (dfTokensLocker == DfTokensLocker(0)) {
    //         dfTokensLocker = new DfTokensLocker(address(this));
    //         emit CreateDfTokensLocker(address(dfTokensLocker));
    //     }

    //     dDaiWrapper.transferFrom(msg.sender, address(dfTokensLocker), _amount);
    // }

    // function unlockAvailableDDaiFromEthereum(uint _amount) public onlyOwner {
    //     dfProfits.cast(address(uint160(address(dDaiWrapper))), abi.encodeWithSelector(IERC20(dDaiWrapper).transfer.selector, msg.sender, _totalDaiProfit));
    // }
