...
interface ReverseHalving {  
    function receiveApproval(address _from, uint256 _value, address _token,  bytes calldata _extraData) external;
}

  // Algorithm =>"Reverse Halving" =>"Staking Tokens"/
 
 //Token Recipient/Mainnet_Connect! => "CBM" Coin
...
contract CryptoBlockMine {
    ...
    constructor(
        ...
        premined = 111987550 * 10 ** uint256(decimals);  // =>  Rewards and Staking to be Distributed
    ...
    function approveAndCall(address _spender, uint256 _value, bytes memory _extraData)
        public
        returns (bool success) {
        ReverseHalving spender = ReverseHalving(_spender);
    ...
    function AlgorithmReverseHalving(uint256 _blocknumber) public view returns(uint256, uint256){
        uint256 crew = uint256(blockhash(_blocknumber)) % nRewarMod;
        return (crew, block.number-1);
    }

    function getBlockHashAlgorithm(uint256 _blocknumber) public view returns(uint256, uint256){
        uint256 crew = uint256(blockhash(_blocknumber)) % nRewarMod;
        return (crew, block.number-1);
    }
