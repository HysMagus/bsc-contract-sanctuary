...
contract Strategy is BaseStrategy {
    ...
    address token0;
    address token1;
    address reward                  = 0x849233FF1aea15D80EF658B2871664C9Ca994063;
    IERC20 public SOAKToken         = IERC20(reward);
    IPancakeFarm public farmer      = IPancakeFarm(0x303961805A22d76Bac6B2dE0c33FEB746d82544B);
    IERC20 public LPToken           = IERC20(farmer.poolInfo(pid).lpToken);
    IERC20 internal iWBNB           = IERC20(WBNB);
    IUniswapV2Router public router  = IUniswapV2Router(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    ...
    constructor(address _vault) public BaseStrategy(_vault) {
        ...
        SOAKToken.safeApprove(address(router),type(uint256).max);
    ...
    function pendingReward() public view returns (uint256) {
        return reward_to_want(farmer.pendingCake(pid,address(this)));
    }

    function estimatedTotalAssets() public override view returns (uint256) {
        //Add the vault tokens + rewards
   ...
   function reward_to_want(uint256 _earned) internal view returns (uint256) {
        ...
        )
    {
        ...
        if(pendingReward() > 0) {
            ...
            _earned = SOAKToken.balanceOf(address(this));
            if(_earned > 0)
    ...
    function prepareMigration(address _newStrategy) internal override {
        ...
        if(pendingReward() > 0) {
            farmer.deposit(pid,0);
            depositToFarm(SOAKToken.balanceOf(address(this)),false);
        }
    ...
    function protectedTokens() internal override view returns (address[] memory) {
        ...
        protected[0] = address(SOAKToken); // rewards
        return protected;
    }
}