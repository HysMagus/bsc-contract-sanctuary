...
contract MasterChef is Ownable {
    ...
    function pendingTree(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            if (now < afterPerBlockTime) {
                treeReward = multiplier.mul(treePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            } else {
                treeReward = multiplier.mul(treePerBlock.div(2)).mul(pool.allocPoint).div(totalAllocPoint);
            }
    ...
    function updatePool(uint256 _pid) public {
        ...
        tree.mint(address(this), treeReward);
