...
// File: contracts\LioEagleBase.sol


// Base class that implements: BEP20 interface, fees & swaps
abstract contract LioEagleBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "LioEagle Token";
	string private constant SYMBOL = "LioE";
	...
	uint256 private constant _totalTokens = 4000000000 * 10**DECIMALS;	//4 billion total supply
	...
	mapping (address => bool) private _addressesSignedForLargeAmount; // The list of addresses can trasfer amount that exceeds transactionLimit

	// TRANSACTION LIMIT
	uint256 private _transactionLimit = _totalTokens / 100; // The amount of tokens that can be sold at once(100 -> 1%)
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and LioE <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
	...
	// This function swaps a {tokenAmount} of LioE tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for LioE -> WBNB
	...
	function isTransferLimited(address sender, address recipient) private view returns(bool) {
		bool isSelling = isPancakeSwapPair(recipient);
		return isSelling && isMarketTransfer(sender, recipient) && !_addressesSignedForLargeAmount[sender];
	}
	...
	// Returns how many more $LioE tokens are needed in the contract before triggering a swap
    ...
    function setSignedForLargeAmount(address addr, bool value) public onlyOwner {
        _addressesSignedForLargeAmount[addr] =value;
    }
...
// File: contracts\LioEagle.sol

// Implements rewards & burns
contract LioEagle is LioEagleBase {
	...
	uint256 private _totalBNBAsLioEClaimed; // The total number of BNB that was converted to LioE and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsLioEClaimed; // The amount of BNB converted to LioE and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy LioE automatically
	...
	bool private _reimburseAfterLioEClaimFailure; // If true, and LioE reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress) LioEagleBase(routerAddress) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterLioEClaimFailure(true);
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough LioE balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim LioE tokens
		if (!claimLioE(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterLioEClaimFailure) {
	...
	function claimLioE(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsLioEClaimed[user] += bnbAmount;
		_totalBNBAsLioEClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $LioE on a zero balance address:
		...
		// Depending on the % of $LioE tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsLioE(address byAddress) public view returns (uint256) {
		return _bnbAsLioEClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsLioE() public view returns (uint256) {
		return _totalBNBAsLioEClaimed;
	}
	...
	function reimburseAfterLioEClaimFailure() public view returns(bool) {
		return _reimburseAfterLioEClaimFailure;
	}


	function setReimburseAfterLioEClaimFailure(bool value) public onlyOwner {
		_reimburseAfterLioEClaimFailure = value;
	}
