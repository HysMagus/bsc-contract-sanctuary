...
contract CheeseSwapPair is ICheeseSwapPair, CheeseSwapBEP20 {
    using SafeMath  for uint;
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (data.length > 0) ICheeseSwapCallee(to).kiwiswapCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IBEP20(_token0).balanceOf(address(this));
        balance1 = IBEP20(_token1).balanceOf(address(this));
        }
...
contract CheeseSwapFactory is ICheeseSwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(CheeseSwapPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'CheeseSwap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'CheeseSwap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'CheeseSwap: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(CheeseSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ICheeseSwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'CheeseSwap: FORBIDDEN');
        feeTo = _feeTo;
    }
