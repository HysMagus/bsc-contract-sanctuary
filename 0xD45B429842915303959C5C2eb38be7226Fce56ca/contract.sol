...
contract MarketOracle is Ownable {
    ...
    uint private galaxyBnbPrice0CumulativeLast;
    uint private galaxyBnbPrice1CumulativeLast;
    uint32 private galaxyBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _galaxy_bnb;
    ...
    constructor(
        address __galaxy_bnb,   // Address of the galaxy-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _galaxy_bnb = IUniswapV2Pair(__galaxy_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        galaxyBnbPrice0CumulativeLast = _galaxy_bnb.price0CumulativeLast();
        galaxyBnbPrice1CumulativeLast = _galaxy_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, galaxyBnbBlockTimestampLast) = _galaxy_bnb.getReserves();
    ...
    // Get the average price of 1 galaxy in the smallest BNB unit (18 decimals)
    function getgalaxyBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_galaxy_bnb));

        require(_blockTimestamp != galaxyBnbBlockTimestampLast, "GRV Last and current are equal");

        FixedPoint.uq112x112 memory galaxyBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - galaxyBnbPrice0CumulativeLast) / (_blockTimestamp - galaxyBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, galaxyBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint galaxyBnbAverage;
        uint busdBnbAverage;

        (galaxyBnbPrice0CumulativeLast, galaxyBnbPrice1CumulativeLast, galaxyBnbBlockTimestampLast, galaxyBnbAverage) = getgalaxyBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, galaxyBnbAverage) = getgalaxyBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = galaxyBnbAverage*(1e6) / busdBnbAverage;
