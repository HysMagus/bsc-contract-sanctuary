...
contract BEP20 is Context, IBEP20, Ownable {
     ...
     */
     function burntCFF() public view returns (uint256) {
       return _burntSupply;
     }
...
contract CFFToken is BEP20('CFF Token', 'CFF') {
    ...
    //@dev overrides transfer function to meet tokenomics of CFF - burn rate + CFF for Coffee
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), 'CFF::delegateBySig: invalid signature');
        require(nonce == nonces[signatory]++, 'CFF::delegateBySig: invalid nonce');
        require(now <= expiry, 'CFF::delegateBySig: signature expired');
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, ',CFF::getPriorVotes: not yet determined');
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CFF (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, 'CFF::_writeCheckpoint: block number exceeds 32 bits');
