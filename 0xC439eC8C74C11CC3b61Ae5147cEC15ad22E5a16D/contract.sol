...
contract TFCEPresale is Ownable, ReentrancyGuard, ContractGuard {
    ...
    uint256 public maxPurchasableTokenPerWallet = 5250 * 1e9; // 5000 TFCE considering 5% JETS bonus

    address public TFCEToken;
    ...
    modifier isNotOver() {
        require(!finishPresaleByAdmin, "TFCE_PRESALE:presale is over");
        require(block.timestamp >= startTime, "TFCE_PRESALE:presale is not started yet");
        require(block.timestamp <= endTime, "TFCE_PRESALE:presale is ended");
        require(IBEP20(BusdToken).balanceOf(address(this)) < presaleCapInBUSD, "TFCE_PRESALE: sold out already");
        _;
    }
    modifier isOver() {
        require(finishPresaleByAdmin || block.timestamp < startTime || block.timestamp > endTime || IBEP20(BusdToken).balanceOf(address(this)) >= presaleCapInBUSD, "TFCE_PRESALE:presale not over yet");
        _;
    }

    constructor(address _TFCEToken, uint256 _startTime, uint256 _endTime) public {
        require(
            _TFCEToken != address(0x0),
            "TFCEPresale: TFCE address is 0x0."
        );
        TFCEToken = _TFCEToken;
        ...
        returns (uint256)
    {
        ...
        uint256 purchaseTFCEAmount;
        if (puchaserJetsValueInUSD > 0) {
            if (puchaserJetsValueInUSD >= purchaseValueInUSD) {
                purchaseTFCEAmount = purchaseValueInUSD
                    .mul(uint256(100).add(bonusFee))
                    .div(100)
                    .div(1e19);
            } else {
                purchaseTFCEAmount = puchaserJetsValueInUSD
                    .mul(uint256(100).add(bonusFee))
                    .div(100)
                    .div(1e19);
                purchaseTFCEAmount = purchaseTFCEAmount.add(
                    purchaseValueInUSD.sub(puchaserJetsValueInUSD).div(1e19)
                );
            }
        } else {
            purchaseTFCEAmount = purchaseValueInUSD.div(1e19);
        }
        
        // calculate the token amount to return considering the salePrice
        return purchaseTFCEAmount.mul(1e18).div(salePrice);
    }
        ...
        isNotOver
    {
        ...
        // calculate the token amount to return considering the salePrice
        purchaseTFCEAmount = purchaseTFCEAmount.mul(1e18).div(salePrice);
        ...
        unclaimedPurchasedToken = unclaimedPurchasedToken.add(purchaseTFCEAmount);
        ...
        require(purchasedToken[_msgSender()] <= maxPurchasableTokenPerWallet, "TFCE_PRESALE: exceeded max purchasable TFCE amount per wallet.");
        
        //IBEP20(TFCEToken).transfer(_msgSender(), purchaseTFCEAmount);
    }
        ...
        isOver
    {
        for(uint i=0;i<purchasers.length;i++){
            ...
            IBEP20(TFCEToken).transfer(account, amount);
        }
    ...
    function withdrawTFCEAfterPresaleOver() public onlyOwner isOver {
        uint256 amount =
            IBEP20(TFCEToken).balanceOf(address(this)).sub(
                unclaimedPurchasedToken
            );
        require(amount > 0, "TFCE_PRESALE: zero amount");
        IBEP20(TFCEToken).transfer(_msgSender(), amount);
    }
}