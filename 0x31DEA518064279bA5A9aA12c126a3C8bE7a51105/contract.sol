...
contract GFI_staking is Ownable {
    ...
    uint private constant stakingAndGFITokens = 7250e18;
    
    function getStakingAndGFIAmount() public view returns (uint) {
        if (totalClaimedRewards >= stakingAndGFITokens) {
            return 0;
        }
        uint remaining = stakingAndGFITokens.sub(totalClaimedRewards);
        return remaining;
    }
    ...
    function transferAnyERC20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        if (_tokenAddr == tokenAddress) {
            if (_amount > getStakingAndGFIAmount()) {
                revert();
            }
