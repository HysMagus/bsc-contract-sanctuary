...
contract NFTReward is IPool,Governance {
    ...
    uint256 public _initReward = 300 * 1e18;
    ...
    constructor(address dego, address gego, address gegoFactory) public {
        _dego = IERC20(dego);
    ...
    */
    ...
    function getFixRate(uint256 grade) public pure returns (uint256){
    ...
    function getStakeInfo( uint256 gegoId ) public view returns ( uint256 stakeRate, uint256 degoAmount){
        ...
        (grade, degoAmount, createdTime,blockNum, author) = _gegoFactory.getGego(gegoId);

        require(degoAmount > 0,"the gego not dego");

        stakeRate = getFixRate(grade);
    }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            ...
            // }
                _dego.safeTransfer(msg.sender, reward );
    ...
    //for extra reward
    function changeDuration(uint256 du)
        external
        onlyGovernance
    {
        DURATION = du;
