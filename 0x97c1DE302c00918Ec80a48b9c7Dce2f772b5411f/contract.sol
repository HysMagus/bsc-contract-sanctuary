...
contract BvaultsStrategyStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public entranceFeeFactor = 10000; // 0% entrance fee (some target will charge entranceFee)
    ...
    function farm() external onlyOperator {
        _farm(0);
    }
    ...
    function convertDustToEarned() external whenNotPaused {
        revert("no dust for single asset staking");
    }
    ...
    function performanceFee() public view returns (uint256) {
        return buyBackRate1.add(buyBackRate2);
    }
