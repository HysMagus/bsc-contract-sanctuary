...
contract PizzaToken is BEP20('BORGv2', 'BORGv2') {
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PIZZAs (not scaled);
...
contract PastaBar is BEP20('Klingon Token', 'KLING') {
    ...
    function safePizzaTransfer(address _to, uint256 _amount) public onlyOwner {
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 pizzaReward = multiplier.mul(pizzaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pizza.mint(devaddr, pizzaReward.div(10));
        pizza.mint(address(pasta), pizzaReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit PIZZA by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw PIZZA by unstaking');
    ...
    function safePizzaTransfer(address _to, uint256 _amount) internal {
        pasta.safePizzaTransfer(_to, _amount);
    }
