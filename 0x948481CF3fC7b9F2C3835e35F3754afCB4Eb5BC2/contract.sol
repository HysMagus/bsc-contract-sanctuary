...
interface IKeep4rbHelper {
    function getQuoteLimit(uint gasUsed) external view returns (uint);
}

contract Keep4rb is ReentrancyGuard {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;

    /// @notice Keep4r Helper to set max prices for the ecosystem
    IKeep4rbHelper public KPRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "Keep4rb";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "KP4RB";
    ...
    function applyCreditToJob(address provider, address liquidity, address job) external {
        ...
        uint _liquidity = Keep4rbLibrary.getReserve(liquidity, address(this));
