...
// File: contracts/BIFI/interfaces/jetfuel/IMasterFuel.sol
...
// File: contracts/BIFI/strategies/Jetfuel/StrategyJetfuelLP.sol
...
contract StrategyJetfuelLP is Ownable, Pausable {
    ...
    function chargeFees() internal {
        ...
        IUniswapRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, fuelToWbnbRoute, address(this), now.add(600));
    ...
    function addLiquidity() internal { 
        ...
        if (lpToken0 != fuel) {
            IUniswapRouter(unirouter).swapExactTokensForTokens(fuelHalf, 0, fuelToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != fuel) {
            IUniswapRouter(unirouter).swapExactTokensForTokens(fuelHalf, 0, fuelToLp1Route, address(this), now.add(600));
        }
