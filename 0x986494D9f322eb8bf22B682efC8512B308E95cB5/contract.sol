...
// Marsdefi with Governance.
contract Marsdefi is BEP20("MarsDeFi Protocol token", "MARS"), Ownable {
...
// MasterChef was the master of mars. He now governs over MARS. He can make Mars and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    // The MARS TOKEN!
    Marsdefi public marsdefi;
    // Dev fund (10%, initially)
    uint256 public devFundDivRate = 10;
    // Dev address.
    address public devaddr;
    // Block number when bonus Mars period ends.
    ...
    // Update the given pool's MARS allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function pendingMars(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 marsReward = multiplier
                .mul(marsPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accMarsPerShare = accMarsPerShare.add(
                marsReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accMarsPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 marsReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        marsdefi.mint(devaddr, marsReward.div(devFundDivRate));
        marsdefi.mint(address(this), marsReward);
        pool.accMarsPerShare = pool.accMarsPerShare.add(
            marsReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accMarsPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safeMarsTransfer(msg.sender, pending);
    ...
    function safeMarsTransfer(address _to, uint256 _amount) internal {
        uint256 marsdefiBal = marsdefi.balanceOf(address(this));
        if (_amount > marsdefiBal) {
            marsdefi.transfer(_to, marsdefiBal);
        } else {
            marsdefi.transfer(_to, _amount);
        }
    }
    ...
    function setMarsPerBlock(uint256 _marsPerBlock) public onlyOwner {
        require(_marsPerBlock > 0, "!marsPerBlock-0");
