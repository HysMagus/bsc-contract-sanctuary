...
contract Liquidity is Ownable {
    ...
    function getSecondsLeftInLiquidityGenerationEvent() public view returns (uint256) {
        ...
        return contractStartTimestamp.add(5 minutes).sub(block.timestamp);
    }
    ...
    function withdrawAfterDone() public onlyOwner{
    ...
    function startReward(uint256 reward, address _rewardToken) public onlyOwner{
        require(LPGenerationCompleted == true, "Liquidity generation not finished");

        rewardStartTimestamp = block.timestamp;
        rewardCompleteTimestamp = block.timestamp.add(DURATION);
        //console.log("startReward:gap:",rewardCompleteTimestamp.sub(rewardStartTimestamp));
    ...
    function earned(address account) public view returns (uint256) {
        if(LPGenerationCompleted == false || block.timestamp < rewardStartTimestamp){
            return 0;
        }
        uint256 timePassed = Math.min(block.timestamp,rewardCompleteTimestamp.add(1)).sub(rewardStartTimestamp);
        //console.log("startReward:timePassed:",timePassed);
        uint256 totalEarned = busdContributed[account].mul(rewardRate).mul(timePassed).div(1e18);
        //console.log("startReward:totalEarned:",totalEarned);
