...
// File: contracts/libraries/LPTokenWrapper2.sol
...
contract LPTokenWrapper2 is IPool, Governance{
    using SafeMath for uint256;
    ...
    function setStake(uint256 amount,address account) public virtual {
...
// File: contracts/NarwhalRewardDividendV3.sol
...
contract NarwhalRewardDividendV3 is LPTokenWrapper2{
    using SafeERC20 for IERC20;
    ...
    uint256 public _initBuffrate = 1000;

    uint256 public _totalPower = 0;

    uint256 public feeRewardRate = 250;

    uint256 public feeBurnRate = 250;
    ...
    // types 3 for increase max buffrate
    ...
    address public contractCaller = address(0x0);
    ...
    function setFee(uint256 _feeReward,uint256 _feeBurn) public onlyGovernance{
        feeRewardRate = _feeReward;
        feeBurnRate = _feeBurn;
    }

    function setInitReward(uint256 initReward)  public onlyGovernance{
        _initReward = initReward;
    }
    ...
    function setDividendPool( address _dividenPool ) public onlyGovernance {
        dividenPool = _dividenPool;
    }

    function setCaller( address _contractCaller ) public onlyGovernance {
    ...
    function updateBuffRate(address account) updateReward(account)  public {
        ...
        uint256 maxBuffRate = getRuneRate(account,3);
    ...
    function setInitValue (uint256 lastUpdateTime,uint256 rewardPerTokenStored,uint256 startTime,uint256 initReward) external {
        require( msg.sender== contractCaller,"invalid address!");
        _lastUpdateTime = lastUpdateTime;
        _rewardPerTokenStored = rewardPerTokenStored;
        startReward(startTime,initReward);
        
    }

    function setUser(address addressUser,
                     uint256 userRewardPerTokenPaidOld,
                     uint256 rewardsOld,
                     uint256 currentBuffrateOld,
                     uint256 currentPowerOld,
                     uint256 timeStampOld,
                     uint256 amountOld
                    )
    external  {
        require( msg.sender== contractCaller,"invalid address!");
        userInfo[addressUser].userRewardPerTokenPaid = userRewardPerTokenPaidOld;
        userInfo[addressUser].rewards = rewardsOld;
        userInfo[addressUser].currentBuffrate = currentBuffrateOld;
        userInfo[addressUser].currentPower = currentPowerOld;
        userInfo[addressUser].timeStamp = timeStampOld;
        setStake(amountOld,addressUser);
        _totalPower = _totalPower.add(userInfo[addressUser].currentPower);
    }

     function setStake(uint256 amount,address account) public override {
        require( msg.sender== contractCaller,"invalid address!");
        super.setStake(amount,account);
    }
        ...
        checkStart
    {
        ...
        uint256 feeBurn = amount.mul(feeBurnRate).div(10000);

        uint256 feeReward = amount.mul(feeRewardRate).div(10000);

        uint256 amountAfterFee = (amount.sub(feeReward)).sub(feeBurn);

        super.stake(amountAfterFee);

        nar.safeTransferFrom(msg.sender, address(this), amount);

        nard.mint(msg.sender,amountAfterFee);
        
        uint256 initBuffRate = getRuneRate(msg.sender,1);

        if(oldBalance == 0){
            uint256 power =  amountAfterFee.mul(initBuffRate);
            ...
            uint256 averageBuffRate = (userInfo[msg.sender].currentBuffrate.mul(oldBalance)).add((amountAfterFee).mul(initBuffRate));
        ...
        checkStart
    {
        ...
        require(nard.balanceOf(msg.sender) >= amount);
        
        super.withdraw(amount);

        nar.safeTransfer( msg.sender, amount);
        
        nard.safeTransferFrom(msg.sender, burnPool, amount);
    ...
    modifier checkHalve() {
        if (block.timestamp >= _periodFinish && _emergencyStop != true) {
            
            uint256 amount  = nar.balanceOf(dividenPool).mul(50).div(100);
            ...
            IERC20(nar).safeTransferFrom(dividenPool , address(this), amount);

            _initReward = amountAfterFee;
    ...
    function startReward(uint256 startTime,uint256 initReward) internal {        
