...
/**
 *73652074726F76692071756573746F206D657373616767696F2073637269766920696E207072697661746F2073752074656C656772616D206120407836346C333374
*/

/**
 
 https://littlefeg.com/
 https://t.me/littlefegofficial
...
// File: contracts/BUSDPENCERDividendPayingToken.sol
...
contract LITTLEFEG is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LITTLEFEGDividendTracker public dividendTracker;
    ...
    address _marketingWalletAddress = 0x7B6fcfC97AF07C9f21631Fd99345A75ef72202A5; //change this
    ...
    constructor() ERC20("LITTLEFEG", "LITTLEFEG") {
    	...
    	dividendTracker = new LITTLEFEGDividendTracker();
    ...
    function changeSellFee(uint256 amount) external onlyOwner {
	    require(amount >= 100 && amount <= 150, "LITTLEFEG: SellFee must be updated to between 100 and 150.");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LITTLEFEG: The dividend tracker already has that address");

        LITTLEFEGDividendTracker newDividendTracker = LITTLEFEGDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LITTLEFEG: The new dividend tracker must be owned by the LITTLEFEG token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LITTLEFEG: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LITTLEFEG: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LITTLEFEG: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LITTLEFEG: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "LITTLEFEG: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LITTLEFEG: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LITTLEFEG: Cannot update gasForProcessing to same value");
    ...
    function updateRewardsFee(uint256 RewardFeePercent) external onlyOwner{
        require(RewardFeePercent <= 20, "LITTLEFEG: updateRewardsFee must be between 0 and 20.");
        TokenRewardsFee = RewardFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateLiquidityFee(uint256 liquidityFeePercent) external onlyOwner{
        require(liquidityFeePercent <= 5, "LITTLEFEG: updateRewardsFee must be between 0 and 5.");
        liquidityFee = liquidityFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateMarketingFee(uint256 marketingFeePercent) external onlyOwner{
        require(marketingFeePercent <= 10, "LITTLEFEG: updateRewardsFee must be between 0 and 10.");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "LITTLEFEG: This account cannot send tokens until trading is enabled");
        }
...
contract LITTLEFEGDividendTracker is Ownable, DividendPayingToken{
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("LITTLEFEG_Dividend_Tracker", "LITTLEFEG_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "LITTLEFEG: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LITTLEFEG_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LITTLEFEG_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }

    function updateMinimunForRewards(uint256 newValueMinimunForRewards) external onlyOwner {
        require(newValueMinimunForRewards >= 10000 && newValueMinimunForRewards <= 500000, "LITTLEFEG_Dividend_Tracker: newValueMinimunForRewards must be updated to between 10000 and 500000");
        require(newValueMinimunForRewards != minimunForRewards, "LITTLEFEG_Dividend_Tracker: Cannot update claimWait to same value");
