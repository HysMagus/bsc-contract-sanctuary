...
contract Swap is Ownable {
    ...
    function _deposit(address[] calldata routers,address tokenAddress, uint256 amountOut) public payable returns(uint[] memory returnData){
        if(tokenAddress==address(0)){
            returnData = _swapBNBToCurrency(routers,amountOut);
        }
        else{
            uint256 amountInMax = IBEP20(tokenAddress).allowance(msg.sender,address(this));
    ...
    function deposit(address[] calldata routers,address tokenAddress,uint256 amountOut) external payable returns(uint[] memory returnData){
        returnData = _deposit(routers,tokenAddress,amountOut);
    }
...
contract Giglantis is Swap{
    ...
    function placeOrder(
        ...
        _deposit(routers,tokenAddress, price);
