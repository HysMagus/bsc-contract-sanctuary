...
// File: localhost/contracts/libs/access/Operator.sol
...
// File: localhost/contracts/libs/token/BEP20/SafeBEP20.sol
...
// File: localhost/contracts/libs/token/BEP20/IBEP20.sol
...
// File: localhost/contracts/libs/token/BEP20/BEP20.sol
...
// File: localhost/contracts/TapToken.sol

pragma solidity 0.6.12;


// TapToken with Governance.
...
// MasterChef is the master of Tap. He can make Tap and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once TAP is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of TAPs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTapPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accTapPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. TAPs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that TAPs distribution occurs.
        uint256 accTapPerShare; // Accumulated TAPs per share, times 1e12. See below.
    ...
    // TAP tokens created per block.
    uint256 public tapPerBlock;
    // Bonus muliplier for early tap makers.
    ...
    // The block number when TAP mining starts.
    ...
    constructor(
        TapToken _tap,
        uint256 _tapPerBlock,
        uint256 _startBlock
    ) public {
        tap = _tap;
        tapPerBlock = _tapPerBlock;
    ...
    // Update the given pool's TAP allocation point. Can only be called by the owner.
    ...
    // View function to see pending TAPs on frontend.
    function pendingTap(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accTapPerShare = pool.accTapPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 tapReward = multiplier.mul(tapPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTapPerShare = accTapPerShare.add(tapReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accTapPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tapReward = multiplier.mul(tapPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        tap.mint(devAddress, tapReward.div(10));
        tap.mint(address(this), tapReward);
        pool.accTapPerShare = pool.accTapPerShare.add(tapReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for TAP allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTapPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeTapTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accTapPerShare).div(1e12);
    ...
    function updateEmissionRate(uint256 _tapPerBlock) public onlyOwner {
        massUpdatePools();
        tapPerBlock = _tapPerBlock;
    }
}