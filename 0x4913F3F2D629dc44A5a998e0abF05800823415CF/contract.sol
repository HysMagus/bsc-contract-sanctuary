...
contract ASCENSION is Context, IERC20, IAscension, Ownable {
  ...
  bool public ascensionActive = true;
  ...
  function _distributeFee(uint256 sFee, uint256 tFee) private {
    totalShares = totalShares.sub(sFee);
  ...
  function setPairToken(address _pair) external onlyOwner() {
    ...
    emit pairTokenSet(_pair, block.timestamp);
  }
  ...
  function ascensionEvent(
    ...
    uint256 pullAmount = IERC20(pairToken).balanceOf(address(this)).div(50);
    ...
    //require amountETH is greater than previous ascension event by at least ascensionAmount
    require(
      amountETH >= previousAscensionEth.add(previousAscensionEth.div(10)),
