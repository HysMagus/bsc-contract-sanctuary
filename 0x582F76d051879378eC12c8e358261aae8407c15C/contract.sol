...
contract LilFlokiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("LilFloki_Dividend_Tracker", "LilFloki_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "LilFloki_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "LilFloki_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LilFloki contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LilFloki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "LilFloki_Dividend_Tracker: Cannot update claimWait to same value");
...
contract LockToken is Ownable {
    ...
    function maintainenceComplete() public view returns(bool){
        return isOpen;
    }

    function Maintainence(bool _isCompleted) external onlyOwner {
        isOpen = _isCompleted;
    }
...
contract LilFloki is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    LilFlokiDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LilFloki", "LFLOKI") {
        BNBRewardsFee = 5;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0x795C25EE4A346217aEA8202D3342eCe027c7981C);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new LilFlokiDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LilFloki: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LilFloki: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LilFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LilFloki: Cannot update gasForProcessing to same value");
