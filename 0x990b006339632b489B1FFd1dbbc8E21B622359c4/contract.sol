 ...
 *  Twitter: @ETHbullz
 * 
 *  Bullish on ETH, Just Hate Gas Fees.
 */
...
contract ETHbullz is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ETHbullzDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ETHbullz", "ETHB") {
    	...
    	dividendTracker = new ETHbullzDividendTracker();

    	buyBackWallet = 0xf8434Dc7B128b593B406e2E0484100F6044085b5;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "ETHbullz: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "ETHbullz: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "ETHbullz: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ETHbullz: The dividend tracker already has that address");

        ETHbullzDividendTracker newDividendTracker = ETHbullzDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ETHbullz: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "ETHbullz: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ETHbullz: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ETHbullz: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ETHbullz: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ETHbullz: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "ETHbullz: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ETHbullz: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ETHbullz: Cannot update gasForProcessing to same value");
...
contract ETHbullzDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ETHbullz_Dividend_Tracker", "ETHbullz_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ETHbullz_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ETHbullz_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ETHbullz contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ETHbullz_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ETHbullz_Dividend_Tracker: Cannot update claimWait to same value");
