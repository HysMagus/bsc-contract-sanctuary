...
contract Stake is Owned {
    ...
    function STAKE(uint256 tokens) external {
        require(IERC20(BSC).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function ADDFUNDS(uint256 tokens) external {
        require(IERC20(BSC).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function CLAIMREWARD() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(BSC).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        require(IERC20(BSC).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
    ...
    function yourBSCBalance(address user) external view returns(uint256 BSCBalance){
        return IERC20(BSC).balanceOf(user);
    }
}