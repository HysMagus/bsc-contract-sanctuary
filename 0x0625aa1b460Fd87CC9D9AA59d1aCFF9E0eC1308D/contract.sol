...
*/
contract StrategyBEarnSingle is StrategyBase {
    ...
    function initialize(
        ...
        for (uint256 index = 0; index < rewardPoolLength; index++) {
            ...
            IERC20(_rewardToken).safeApprove(address(unirouter), type(uint256).max);
            IERC20(_rewardToken).safeApprove(address(vSwaprouter), type(uint256).max);
        }
        _initialized = true;
    }

    function getName() public pure override returns (string memory) {
        return "StrategyBEarnSingle";
    }
    ...
    function claimable_token() external view override returns (address farmToken, uint256 totalDistributedValue) {
        ...
        if (rewardPoolLength > 0) {
            (address _rewardToken, , ) = IBvaultsBank(bVaultBank).rewardPoolInfo(0);
            farmToken = _rewardToken;
            totalDistributedValue = IBvaultsBank(bVaultBank).pendingReward(poolId, 0, address(this));
        }
    }
