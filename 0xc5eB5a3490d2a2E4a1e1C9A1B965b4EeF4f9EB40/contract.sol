...
contract StratX_AUTO is Ownable, ReentrancyGuard, Pausable {
    ...
    function earn() public whenNotPaused {
        ...
        _wrapBNB(); // for cross chain profits
    ...
    // Mainly used to convert earned tokens to WBNB to allow earn() to be called.
    // This is in case profits coming from other chains / products is not in BNB/WBNB.
    function convertTokenToWBNB(address[] memory _path) public {
        require(
            msg.sender == govAddress || msg.sender == devAddress,
            "!authorised"
        );
        // Converts dust tokens into earned tokens, which will be reinvested on the next earn().

        address tokenStartAddress = _path[0];
        address tokenEndAddress = _path[_path.length - 1];

        require(_path.length <= 3, "_path length too long");
        require(tokenStartAddress != AUTOAddress, "Cannot sell AUTO");
        require(tokenStartAddress != wbnbAddress, "Cannot sell WBNB");
        require(tokenEndAddress == wbnbAddress, "Must end with WBNB");
        require(tokenEndAddress != AUTOAddress, "Cannot end with AUTO");
