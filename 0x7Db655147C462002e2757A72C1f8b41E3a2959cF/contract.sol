...
interface IBeltLP {
    function add_liquidity(uint256[] calldata amounts, uint256 minToMint) external;
}
...
contract StrategyBeltStableLp is StrategyBase {
    ...
    function initialize(
        ...
        IERC20(_targetCompound).safeApprove(beltLp, type(uint256).max);
        _initialized = true;
    }

    function getName() public pure override returns (string memory) {
        return "StrategyBeltStableLp";
    }

    function deposit() public override {
        uint256 _baseBal = IERC20(baseToken).balanceOf(address(this));
        if (_baseBal > 0) {
            IMasterBelt(autoFarm).deposit(poolId, _baseBal);
            emit Deposit(baseToken, _baseBal);
        }
    }

    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        uint256 _stakedAmount = IMasterBelt(autoFarm).stakedWantTokens(poolId, address(this));
        if (_amount > _stakedAmount) {
            _amount = _stakedAmount;
        }

        uint256 _before = IERC20(baseToken).balanceOf(address(this));
        IMasterBelt(autoFarm).withdraw(poolId, _amount);
    ...
    function _withdrawAll() internal override {
        IMasterBelt(autoFarm).withdraw(poolId, uint256(-1));
    }

    function claimReward() public override {
        IMasterBelt(autoFarm).deposit(poolId, 0);
    }
    ...
    function _addLiquidity() internal {
        ...
        if (_targetCompoundBal > 0) {
            uint256[] memory amounts = new uint256[](beltLpLength);
            amounts[targetCompoundIndex] = _targetCompoundBal;
            IBeltLP(beltLp).add_liquidity(amounts, 0);
        }
    }

    function balanceOfPool() public view override returns (uint256) {
        uint256 amount = IMasterBelt(autoFarm).stakedWantTokens(poolId, address(this));
        return amount;
    }
    ...
    function claimable_tokens() external view override returns (address[] memory farmToken, uint256[] memory totalDistributedValue) {
        ...
        totalDistributedValue[0] = IMasterBelt(autoFarm).pendingBELT(poolId, address(this));
    }
    ...
    function claimable_token() external view override returns (address farmToken, uint256 totalDistributedValue) {
        ...
        totalDistributedValue = IMasterBelt(autoFarm).pendingBELT(poolId, address(this));
    }
    ...
    function retireStrat() external onlyStrategist {
        IMasterBelt(autoFarm).emergencyWithdraw(poolId);
    ...
    function setBeltLp(address _beltLp) external onlyStrategist {
        beltLp = _beltLp;
        IERC20(targetCompoundToken).safeApprove(beltLp, type(uint256).max);
    }

    function setTargetCompoundIndex(uint256 _targetCompoundIndex) external onlyStrategist {
        targetCompoundIndex = _targetCompoundIndex;
    }

    function setBeltLpLength(uint256 _beltLpLength) external onlyStrategist {
        beltLpLength = _beltLpLength;
    }
}