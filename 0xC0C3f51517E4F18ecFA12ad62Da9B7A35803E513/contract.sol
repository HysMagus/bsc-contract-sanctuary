...
/*
 * CheeseCHSPool job is an experimental job from Keep3r BSC Network. Experimental implementation of secondary token as rewards.
...
contract CheeseCHSPool {
    ...
    uint public minKeep = 50e18;      //minKeep set to 50 kp3rb
  ...
  function acceptGovernance() external {
    ...
    IBEP20 public constant CHS = IBEP20(0xaDD8A06fd58761A5047426e160B2B88AD3B9D464);
    ...
    function pairForCHS(address tokenA) external pure returns (address) {
        return CheeseSwapLibrary.pairFor(factory, tokenA, address(CHS));
    }
    ...
    function add(address tokenA, address tokenB) external {
        ...
        (uint price0Cumulative, uint price1Cumulative,) = CheeseCHSPoolLibrary.currentCumulativePrices(pair);
    ...
    function _swap(uint _amount) internal returns (uint) {
        ...
        path[1] = address(CHS);
        uint[] memory amounts = UNI.swapExactTokensForTokens(_amount, uint256(0), path, address(this), block.timestamp.add(120));
        return amounts[1];
    }
}