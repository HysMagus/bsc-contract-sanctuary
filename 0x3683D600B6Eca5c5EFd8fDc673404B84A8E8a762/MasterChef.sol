...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accChtPerShare;   // Accumulated EGGs per share, times 1e12. See below.
    ...
    function pendingCht(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accChtPerShare = accChtPerShare.add(eggReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accChtPerShare = pool.accChtPerShare.add(eggReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
