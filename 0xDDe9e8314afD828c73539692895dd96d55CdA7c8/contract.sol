...
// File: contracts/NarTicketFarm.sol
...
// farmticket is the master of Nart. He can make Nart and he is a fair guy.
//
...
contract NarTicketFarm is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt; // Reward debt. See explanatio
    }

    // Info of each pool.
    struct PoolInfo {
        IERC20 nard;           // Address of LP token contract.
        uint256 lastRewardBlock;  // Last block number that Nart distribution occurs.
        uint256 accnartPerShare; // Accumulated Nart per share, times 1e12. See below.
    }

    // The Nart TOKEN!
    IERC20 public nart;

    // Nart tokens created per block.
    ...
    // View function to see pending Nart on frontend.
    function pendingNarTicket(address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        uint256 accnartPerShare = poolInfo.accnartPerShare;
        if (block.number > poolInfo.lastRewardBlock && totalSupply != 0) {
            uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
            uint256 nartReward = multiplier.mul(nartPerBlock);
            accnartPerShare = accnartPerShare.add(nartReward.mul(1e12).div(totalSupply));
        }
    ...
    function updatePool() public {
        ...
        poolInfo.accnartPerShare = poolInfo.accnartPerShare.add(nartReward.mul(1e12).div(totalSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to farmticket for Nart allocation.
    function deposit(uint256 _amount) public {
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        
        uint256 pending = user.amount.mul(poolInfo.accnartPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            safenartTransfer(msg.sender, pending);
        }
        
        if(_amount > 0) {
            poolInfo.nard.safeTransferFrom(address(msg.sender), address(this), _amount);
            user.amount = user.amount.add(_amount);
            totalSupply = totalSupply.add(_amount);
        }
        user.rewardDebt = user.amount.mul(poolInfo.accnartPerShare).div(1e12);
        emit Deposit(msg.sender, _amount);
    }

    // Withdraw LP tokens from farmticket.
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            poolInfo.nard.safeTransfer(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(poolInfo.accnartPerShare).div(1e12);
    ...
    function emergencyWithdraw() public {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        poolInfo.nard.safeTransfer(address(msg.sender), amount);
        emit EmergencyWithdraw(msg.sender, amount);
    }

    // Safe Nart transfer function, just in case if rounding error causes pool to not have enough Nart.
