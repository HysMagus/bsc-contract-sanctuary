...
// File: contracts/ido/IdoFactory.sol
...
contract IdoFactory is Ownable {
    ...
    event IdoCreated(address indexed ido, address indexed logicImplement);
    ...
    function createIdo(
        ...
        TransparentUpgradeableProxy proxyIdo = new TransparentUpgradeableProxy(logicImplement, _proxyAdminAddress, '');
        IIdo ido = IIdo(address(proxyIdo));
        ido.initialize(
            _startBlock,
            _endBlock,
            _maxTotalIdoUSDAmount,
            _maxIdoUSDAmountEachUser,
            _finAddr,
            _ownerAddress
        );
        emit IdoCreated(address(ido), logicImplement);
        return address(ido);
    }
