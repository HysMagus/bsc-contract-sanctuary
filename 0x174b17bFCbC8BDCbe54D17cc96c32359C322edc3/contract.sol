...
contract D5CoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("5DCoin_Dividend_Tracker", "5DCoin_Dividend_Tracker") {
        claimWait = 14400;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "5DCoin_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "5DCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main 5DCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "5DCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "5DCoin_Dividend_Tracker: Cannot update claimWait to same value");
...
contract D5Token is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    D5CoinDividendTracker public dividendTracker;
    ...
    constructor() ERC20("5D Token", "5DT") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        MarketingFee = 2;
        MarketingWallet = payable(0x8024B8E602f79fa6404b84455865721577F97f39);

        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee); // total fee transfer and buy

    	dividendTracker = new D5CoinDividendTracker();
	//IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // Test 
        ...
        */
        _mint(owner(), 2000000000000 * (10**9));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "5DCoin: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "5DCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "5DCoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "5DCoin: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "5DCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "5DCoin: Cannot update gasForProcessing to same value");
