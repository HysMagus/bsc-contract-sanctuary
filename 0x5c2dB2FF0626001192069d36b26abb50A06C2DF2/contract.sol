...
Telegaram : https://t.me/sailorflokitoken
...
contract SAILORFLOKI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private maxBuyTranscationAmount;
    uint256 private maxSellTransactionAmount;
    uint256 public swapTokensAtAmount;
    uint256 private maxWalletToken; 
    ...
    constructor() ERC20("SAILORFLOKI", "SAILORFLOKI") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xa68c45770BC3E48552d61a282B2e5D9Df31Ba0B7;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "SAILORFLOKI: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "SAILORFLOKI: The new dividend tracker must be owned by the SAILORFLOKI token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SAILORFLOKI_Dividend_Tracker", "SAILORFLOKI_Dividend_Tracker", 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SAILORFLOKI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SAILORFLOKI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "SAILORFLOKI_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "SAILORFLOKI_Dividend_Tracker: Cannot update claimWait to same value");
