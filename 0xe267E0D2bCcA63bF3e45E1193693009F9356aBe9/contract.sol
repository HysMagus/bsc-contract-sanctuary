...
pragma solidity 0.7.6;
contract Saturn is ERC20("Saturn", "STR"), Ownable {
...
//file:contract/iharvestTimerDynamic
pragma solidity 0.7.6;
interface IHarvestTimerDynamic{
    function getExactFarmHarvestTimerForSpnHolder(uint256 __harvestInterval, address _holder) external view returns (uint harvestIntervalRate);
}

// File: contracts/MasterChef.sol
pragma solidity 0.7.6;
// MasterChef is the master of Saturn. He can make STR and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once str is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of str
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accStrPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accStrPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. STR to distribute per block.
        uint256 lastRewardBlock;  // Last block number that str distribution occurs.
        uint256 accStrPerShare;   // Accumulated STR per share, times 1e12. See below.
    ...
    // The Saturn TOKEN!
    Saturn public saturn;
    
    // Addresses to link to price.
    IERC20 public busd;
    address public busdStrLP = address(0x0); 
    
    // Burn address.
    address public burnaddr;
    // STR tokens created per block.
    uint256 public strPerBlock = 100000000000000000;
    // Maximum emission rate
    uint256 public maxEmissionRate = 100000000000000000;
    // Bonus muliplier for early str makers.
    ...
    // The block number when str mining starts.
   ...
   // Dyna harvest Timer strat address
    ...
    // View function to see pending str on frontend.
    function pendingStr(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accStrPerShare = accStrPerShare.add(strReward.mul(1e12).div(lpSupply));
        }
        uint256 pending =  user.amount.mul(accStrPerShare).div(1e12).sub(user.rewardDebt);
    ...
    // View function to see if user can harvest str.	
    ...
    function updatePool(uint256 _pid) public {
        ...
        saturn.mint(burnaddr, strReward.div(10));
        saturn.mint (address(this),strReward);
        pool.accStrPerShare = pool.accStrPerShare.add(strReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    function getPoolHarvestIntervalAmount(uint256 _pid) internal view  returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        if (pool.harvestIntervalBP > 0 ){
            uint256 harvestIntervalRate = harvestTimerDynamic.getExactFarmHarvestTimerForSpnHolder(pool.harvestIntervalBP,msg.sender);
            uint256 timerCut = pool.harvestIntervalBP.mul(harvestIntervalRate).div(10000);
            return block.timestamp.add(timerCut);
        }  else {
            return block.timestamp.add(pool.harvestIntervalBP);
        }
    }
       ...
       payOrLockupPendingStr(_pid);
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
             ...
             user.nextHarvestUntil = getPoolHarvestIntervalAmount(_pid);//block.timestamp.add(rewardCut);
        }
        user.rewardDebt = user.amount.mul(pool.accStrPerShare).div(1e12);
    ...
    //    PoolInfo storage pool = poolInfo[_pid];
     //   if (pool.harvestIntervalBP > 0 ){
     //       uint256 harvestIntervalRate = harvestTimerDynamic.getExactFarmHarvestTimerForSpnHolder(pool.harvestIntervalBP,msg.sender);
     //       return block.timestamp.add(pool.harvestIntervalBP).mul(harvestIntervalRate).div(10000);
   //     }  else {
  //          return block.timestamp.add(pool.harvestIntervalBP);
  //      }
  //  }
   ...
   // }
    ...
    // Pay or lockup pending str.	
    function payOrLockupPendingStr(uint256 _pid) internal {	
        PoolInfo storage pool = poolInfo[_pid];	
        UserInfo storage user = userInfo[_pid][msg.sender];	
        if (user.nextHarvestUntil == 0) {	
            user.nextHarvestUntil = getPoolHarvestIntervalAmount(_pid) ;	
        }
         uint256 pending = user.amount.mul(pool.accStrPerShare).div(1e12).sub(user.rewardDebt);	
        if (canHarvest(_pid, msg.sender)) {	
            if (pending > 0 || user.rewardLockedUp > 0) {	
                ...
                user.nextHarvestUntil = getPoolHarvestIntervalAmount(_pid);	
                // send rewards	
                safeStrTransfer(msg.sender, rewardCut);	
            }	
        } else if (pending > 0) {	
    ...
    // Safe str transfer function, just in case if rounding error causes pool to not have enough str.
    function safeStrTransfer(address _to, uint256 _amount) internal {
        uint256 strBal = saturn.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > strBal) {
            transferSuccess = saturn.transfer(_to, strBal);
        } else {
            transferSuccess = saturn.transfer(_to, _amount);
        }
   ...
   function updateEmissionIfNeeded() public {
        ...
        if (block.number - lastBlockUpdate > emissionUpdateInterval) {
            ...
            uint strBalance = saturn.balanceOf(busdStrLP);
          
            if (strBalance > 0) {
           
                priceCents = busd.balanceOf(busdStrLP).mul(1e14).div(strBalance);
            }
            // Update pools before changing the emission rate
            massUpdatePools();
            uint256 emissionRatePercent = getEmissionRatePercent(priceCents);
            strPerBlock = maxEmissionRate.div(100).mul(emissionRatePercent);
        }
    }
    // for front end
    function getStrPriceCents() public view returns (uint256 spc){
         uint strBalance = saturn.balanceOf(busdStrLP);
          if (strBalance > 0) {
            uint256 priceCents = busd.balanceOf(busdStrLP).mul(1e14).div(strBalance);
            return priceCents;
          }
          return 0;
    }
    function getEmissionRatePercent(uint256 strPriceCents) public view returns (uint256 epr) {
        
       if (strPriceCents>=topPrice*100){return (1);}
       if (strPriceCents<=bottomPrice*100){return (100);}
       uint256 strPricePercentOfTop = strPriceCents.mul(100).div(topPrice*100);
       int256 curveCoefficient = FixidityLib.reciprocal(int256(curveRate));
       uint256 f1 = uint256(100).sub(strPricePercentOfTop);
       int256 p1 = FixidityLib.fromFixed(FixidityLib.multiply(curveCoefficient,int256(f1)));
       int256 p2 = FixidityLib.fromFixed(FixidityLib.divide(100,int256(strPricePercentOfTop)));
    ...
    function updateEmissionRate(uint256 _strPerBlock) public onlyOwner {
        massUpdatePools();
        strPerBlock = _strPerBlock;
        emit UpdateEmissionRate(msg.sender, _strPerBlock);
    }
    ...
    // For checking prices to link emission rate
    function setBusdStrLPAddress(address _address) public onlyOwner {
        busdStrLP = _address;
    }
}