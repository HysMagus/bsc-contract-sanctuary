...
contract BnbStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 limitation;     //maximum amount that be invested
        uint256 fees;           //deposit fees 
    }
    ...
    // The block number when MANY mining ends.
    ...
    constructor(
        ...
        poolInfo.push(PoolInfo({
            ...
            accManyPerShare: 0,
            limitation: 4000000000000000000000,
            fees: 4
        }));
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 manyReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function deposit() public payable {
        ...
        require (pool.lpToken.balanceOf(address(this)).add(msg.value) <= pool.limitation, 'exceed max limit');
        ...
        if(msg.value > 0) {
            uint256 depositFee = msg.value.mul(pool.fees).div(100);
            userAmount = msg.value.sub(depositFee);
            IWBNB(WBNB).deposit{value: userAmount}();
            assert(IWBNB(WBNB).transfer(address(this), userAmount));
            assert(IWBNB(WBNB).transfer(adminAddress, depositFee));
