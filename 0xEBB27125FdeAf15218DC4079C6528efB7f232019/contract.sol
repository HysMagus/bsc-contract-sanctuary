...
contract DittoStakingBeta is Ownable {
    ...
    uint256 USER_MAX_CAP = 5000000000000000; // 0.005 DITTO-BNB LP
    ...
    function batchAddToWhitelist(address[] calldata _addr) external onlyOwner { 
        for (uint i=0; i < _addr.length; i++) {
            whitelist[_addr[i]] = true;
        }
    }
    
    function maxStakeAmountForUser(address user) external view returns (uint256) {
        return(USER_MAX_CAP.sub(totalStakedFor(user)));
    }
     ...
     * @param data Not used.
     */
    function stake(uint256 amount, bytes calldata data) external onlyWhitelisted {
    ...
    function _stakeFor(address staker, address beneficiary, uint256 amount) private {
        ...
        require(totalStakedFor(beneficiary).add(amount) <= USER_MAX_CAP);
    ...
    function pendingCakeByUser(address _user) public view returns (uint256) {
        ...
        return totalCakeRewardsAvailable.mul(totalUserStakingShareSeconds).div(_totalStakingShareSeconds);
    }
