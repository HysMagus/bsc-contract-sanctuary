 ...
 */
contract SQRTToken is Context, IERC20, Ownable {
    ...
    modifier onlyCHAR() {
        require(_char == _msgSender(), "Ownable: caller is not the CHAR token contract address");
        _;
    }
    ...
    function setCHARAddress(address char) external onlyOwner {
        require(char != address(0), "Invalid CHAR token contract address");
        require(_char == address(0), "Arealdy set done");
        _char = char;
    }
    ...
    function burn(uint256 amount) external returns (bool) {
        ...
        IERC20(_char).mint(_msgSender(), amount);

        return true;
    }

    function mint(address account, uint256 amount) external override onlyCHAR {
