...
// File: contracts/JETFUEL/interfaces/jetfuel/IStrategy.sol
...
// File: contracts/JETFUEL/vaults/JetfuelVault.sol
 ...
 */
contract JetfuelAutoVault is ERC20, Ownable {
    ...
    function addLiquidity(uint256 _amount) internal {
        ...
        IUniswapRouter(unirouter)
            ...
            value: halfAmount
        }(0, _path(wbnb, autov2), address(this), now.add(600));

        uint256 autov2Amount = IERC20(autov2).balanceOf(address(this));
        IUniswapRouter(unirouter).addLiquidityETH{value: halfAmount}(
            autov2,
            autov2Amount,
            1,
            1,
    ...
    function withdraw(uint256 _shares) public {
        uint256 _withdraw = (balance().mul(_shares)).div(totalSupply());
    ...
    function withdrawBNB1(uint256 _shares) public {
        uint256 r = (balance().mul(_shares)).div(totalSupply());
        _burn(msg.sender, _shares);
    ...
    function withdrawBNB2(uint256 _shares) public {
        uint256 r = (balance().mul(_shares)).div(totalSupply());
        _burn(msg.sender, _shares);
