...
contract BNBRacing {
    ...
    uint256 public constant FUND_FEE = 400;
    uint256 public constant FUND_FEE2 = 100;
    ...
    uint256 public constant USERS_STEP = 100; // 100 Users
    ...
    constructor(address payable marketingAddr, address payable fundAddr, address payable fundAddr2, address payable projectAddr) {
        require(!isContract(marketingAddr) && !isContract(fundAddr) && !isContract(fundAddr2) && !isContract(projectAddr));
        
        marketingAddress = marketingAddr;
        fundAddress = fundAddr;
        fundAddress2 = fundAddr2;
        projectAddress = projectAddr;

        if (getChainID() == 97) {
            LAUNCH_TIME = block.timestamp; // Test Network
        } else {
            LAUNCH_TIME = 1615392000; // Wednesday, March 10, 2021 4:00:00 PM UTC Time
        }
    }
    ...
    function invest(address payable referrer) public payable beforeStarted() {
        ...
        fundAddress.transfer(msg.value.mul(FUND_FEE).div(PERCENTS_DIVIDER));
        fundAddress2.transfer(msg.value.mul(FUND_FEE2).div(PERCENTS_DIVIDER));
        marketingAddress.transfer(msg.value.mul(MARKETING_FEE).div(PERCENTS_DIVIDER));
        projectAddress.transfer(msg.value.mul(PROJECT_FEE).div(PERCENTS_DIVIDER));

        emit FeePayed(msg.sender,msg.value.mul(MARKETING_FEE.add(PROJECT_FEE).add(FUND_FEE).add(FUND_FEE2)).div(PERCENTS_DIVIDER));
        ...
        user.available = user.available.add(msg.value.mul(3).div(2));
    ...
    function withdraw() public beforeStarted() {
        require(getTimer(msg.sender) < block.timestamp,"withdrawal is available only once every 24 hours");
    ...
    function getContractUsersRate(address userAddress) public view returns (uint256)
    {
        ...
        uint256 contractUsersPercent = totalUsers.div(USERS_STEP).mul(10);
    ...
    function getUserPercentRate(address userAddress) public view returns (uint256)
    {
        ...
        if (isActive(userAddress)) {
            uint256 timeMultiplier = (block.timestamp.sub(user.checkpoint)).div(TIME_STEP).mul(5); // +0.05% per day
    ...
    function getTimer(address userAddress) public view returns (uint256) {
        return users[userAddress].checkpoint.add(TIME_STEP);
    }
