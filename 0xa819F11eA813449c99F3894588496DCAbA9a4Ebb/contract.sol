...
// File: contracts/BUTTER/interfaces/pancake/IPancakeRouter.sol
...
// File: contracts/BUTTER/interfaces/pancake/IPancakePair.sol
...
// File: contracts/BUTTER/interfaces/boltdollar/IShareRewardPool.sol
...
// File: contracts/BUTTER/strategies/boltdollar/StrategyCfiLP.sol
 ...
 * CFI rewards from providing liquidity are farmed every few minutes, sold and split 50/50. 
 ...
 */
contract StrategyCFIBNBLP is Ownable, Pausable {
     ...
     * {cfi} - Token generated by staking our funds. In this case it's the Cfis token.
     * {butter} - BeefyFinance token, used to send funds to the treasury.
    ...
    address constant public cfi = address(0x7269097A760961474dADBCAaBe2B33A205Ad5c2f); // CFI
    address constant public butter = address(0xfeD19a0e68ad5bBC4eA09751BFDFB0C273A2b0cb); // BUTTER
    ...
    address constant public shareRewardPool = address(0x2dB047ad4f2d52Cc3A76a83f72D7871CECCe45C5); // Shares BUTTER
    uint8 public poolId;
     ...
     *
     * {REWARDS_FEE} - 3% goes to CFI holders through the {rewards} pool.
     ...
     * {butterToWbnbRoute} - Route we take to get from {butter} into {wbnb}.
     * {wbnbToCfiRoute} - Route we take to get from {wbnb} into {butter}.
     * {cfiToLp0Route} - Route we take to get from {cfi} into {lpToken0}.
     * {cfiToLp1Route} - Route we take to get from {cfi} into {lpToken1}.
     */
    address[] public butterToWbnbRoute = [butter, cfi, wbnb];
    address[] public butterToCfiRoute = [butter,  cfi];
    address[] public wbnbToCfiRoute = [wbnb, cfi];
    address[] public cfiToLp0Route;
    address[] public cfiToLp1Route;
     ...
     */
    constructor() public {
        lpPair = address(0x5ad344c0Adf0fB8861687217610CBd59D1321F7e);
        lpToken0 = IPancakePair(lpPair).token0();
        lpToken1 = IPancakePair(lpPair).token1();
        poolId = 0;
        vault = address(0x404AEec2DA82e4Bed83cdBa1669144c0cfD224Cc);
        strategist = address(0xdE1864f3715605b5e55b2cd60492FB3dA017D3df);

        if (lpToken0 == wbnb) {
            cfiToLp0Route = [cfi, wbnb];
        } else if (lpToken0 != cfi) {
            cfiToLp0Route = [cfi, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            cfiToLp1Route = [cfi, wbnb];
        } else if (lpToken1 != cfi) {
            cfiToLp1Route = [cfi, wbnb, lpToken1];
        }

        IERC20(lpPair).safeApprove(shareRewardPool, uint(-1));
        IERC20(cfi).safeApprove(unirouter, uint(-1));
     ...
     * It deposits {lpPair} in the shareRewardPool to farm {cfi}
     */
     ...
     * 3. It swaps the {butter} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(butter).balanceOf(address(this)).mul(45).div(1000);
        IPancakeRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, butterToWbnbRoute, address(this), now.add(600));
        ...
        IPancakeRouter(unirouter).swapExactTokensForTokens(treasuryHalf, 0, wbnbToCfiRoute, treasury, now.add(600));
    ...
    /**
     * @dev Swaps {butter} to CFI and adds CFI + BNB to {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {   
        uint256 butterStack = IERC20(butter).balanceOf(address(this));

        
        if (butterStack > 0) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(butterStack, 0, butterToCfiRoute, address(this), now.add(600));
        }
    
        uint256 cfiHalf = IERC20(cfi).balanceOf(address(this)).div(2);
        
        if (lpToken0 != cfi) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(cfiHalf, 0, cfiToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != cfi) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(cfiHalf, 0, cfiToLp1Route, address(this), now.add(600));
        }
    ...
    function pause() public onlyOwner {
        ...
        IERC20(cfi).safeApprove(unirouter, 0);
