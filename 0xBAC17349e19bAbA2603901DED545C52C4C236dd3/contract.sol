/*
* DogenMustard © | GameFi - NFT Colletables - Play-To-Earn Token Contract
* https://dogenmustard.com © All Rights Reserved
* DogenMustardToken=Mustard=MSTD is the utility token for DogenMustard©. I just need some lub
*/
...
contract DogenMustardToken is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    address public _Incub8Wallet;

    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 8080080080 * 10**8;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string public constant name     = "DogenMustard";
    ...
    uint256 public _percentageOfLiquidityForIncub8 = 40;
    ...
    event setIncub8WalletTX(address TheFollowingAddressIsSetAsTheIncub8Wallet);
    ...
    event setPercentageOfLiquidityForIncub8TX(uint256 PercentageOfLiquidityForIncub8IsSetTo);
    ...
    constructor (address cOwner, address Incub8Wallet) Ownable(cOwner) {
        _Incub8Wallet = Incub8Wallet;
        ...
        _isExcludedFromFee[_Incub8Wallet] = true;
    ...
    function Lepus() public pure returns(string memory) {
        return "010001000110111101100111011001010110111001001101011101010111001101110100011000010111001001100100 ";
    }
    ...
    // STATE
    function setIncub8Wallet(address Incub8Wallet) external onlyOwner {
        _Incub8Wallet = Incub8Wallet;
        emit setIncub8WalletTX(Incub8Wallet);
    }
    ...
    function setPercentageOfLiquidityForIncub8(uint256 percentageOfLiquidityForIncub8) external onlyOwner {
        _percentageOfLiquidityForIncub8 = percentageOfLiquidityForIncub8;
        emit setPercentageOfLiquidityForIncub8TX(percentageOfLiquidityForIncub8);
    }
    ...
    // When DogenMustardToken becomes an utility token with 0% tax, withdrawToken function will allow us to remove the remaining token inside the contract address.
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        bool shouldAddLiquidity = (_percentageOfLiquidityForBnbReward + _percentageOfLiquidityForIncub8) < 100;
        ...
        uint256 bnbForIncub8  = newBalance * _percentageOfLiquidityForIncub8 / 100;
        uint256 bnbForLiquidity  = newBalance - bnbForRewardPool - bnbForIncub8;

        // send BNB to Incub8
        if (bnbForIncub8 > 0) {
            payable(_Incub8Wallet).transfer(bnbForIncub8);
        }
