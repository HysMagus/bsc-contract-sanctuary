...
contract SBVxBLZDCompound is IStakingRewardCompound, ReentrancyGuard, Initializable {
    ...
    function _harvest() private {
        ...
        // swap 51.25% of reward to BUSD
        uint256 earnAmtToBUSD = earnAmt.mul(5125).div(10000);
        if(earnAmt > 0){
          _safeSwap(
              uniRouterAddress,
              earnAmtToBUSD,
        ...
        // 47.25 in 51.25 busd to invest for gale
        uint256 usdAmountForGale = amountBusd.mul(4725).div(5125);
        ...
        // 4 in 51.25 busd from busd to blizzardPool
        uint256 usdBlizzardPool = amountBusd.sub(usdAmountForGale);
        IERC20(BUSD).safeTransfer(blizzardPool, usdBlizzardPool);

        // remaining go to caller 1.5%
        uint256 harvestCallerAmt = earnAmt.mul(15).div(1000);
        IERC20(XBLZD).safeTransfer(msg.sender, harvestCallerAmt);

         // 47.25% go to compound
        uint256 earnAmountForComp = earnAmt.sub(earnAmtToBUSD).sub(harvestCallerAmt);
