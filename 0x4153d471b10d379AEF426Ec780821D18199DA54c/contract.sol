...
contract TokenVault is Ownable {
    ...
    modifier lockNotActive() {
        require(teamUnlockTime == 0, "Unlock time already started");
        _;
    }
    ...
    // team lock length
    // reward tokens
    // reward tokens lock length
    constructor( address _tokenAddress, uint _teamTokens, uint _teamLockLength, uint _rewardTokens, uint _rewardLockLength) Ownable() public {
        // Variable 'token' is the address of token being sold
        token = BNFY(_tokenAddress);
    ...
    // Call function to start the lock time.. Once called timer will start
    function startLock() external onlyOwner() lockNotActive() {
