...
contract Kodi is ERC20, Ownable {
    ...
    uint256 public minimumTokensBeforeSwap = 200000 * (10**18); 
    ...
    event Launched(uint256 timestamp, uint256 blockNumber);
    ...
    constructor() public ERC20("GODZILLA", "GODZILLA") {
    ...
    function launch() public onlyOwner {
        ...
        _isLanched = true;
        emit Launched(_launchStartTimestamp, _launchBlockNumber);
    }
    ...
    function blacklistAccount(address account) public onlyOwner {
        ...
        require((!isTradingEnabled) || (_isLanched && currentTimestamp.sub(_launchStartTimestamp) < _blacklistTimeLimit) , "KODI: Time to blacklist accounts has expired");
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            ...
            if (_isInLaunch && currentTimestamp.sub(_launchStartTimestamp) <= 300) {
                uint256 lastBuy = _buyTimesInLaunch[to];
                require(isBuyFromLp && currentTimestamp.sub(lastBuy) > 60, "KODI: Cannot buy more than once per min in first 5min of launch");
            }
        ...
        if (
            ...
            _swapping = true;
            _swapAndLiquify();
            _transferToMarketingWallet();
            _transferToBuyBackWallet();
            _transferToSalaryWallet();
            _swapAndSendDividends(_holdersTokensToSwap);
            _swapping = false;
        }
    ...
    function _isInRoar3() internal view returns (bool) {
        uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _roarStartTimestamp  ? _tradingPausedTimestamp : _getNow();
        uint256 timeSinceRoar = currentTimestamp.sub(_roarStartTimestamp);
        if(timeSinceRoar > _roar1.timeInPeriod.add(_roar2.timeInPeriod) && timeSinceRoar < _roar1.timeInPeriod.add(_roar2.timeInPeriod).add(_roar3.timeInPeriod)) {
            return true;
        } else {
            return false;
        }
    ...
    function _transferToMarketingWallet() private {
        if(_marketingTokensToSwap < balanceOf(address(this))) {
            uint256 _marketingTokens = _marketingTokensToSwap;
            _marketingTokensToSwap = 0;
            super._transfer(address(this), marketingWallet, _marketingTokens);
        }
    }
    function _transferToSalaryWallet() private {
        if(_salaryTokensToSwap < balanceOf(address(this))){
            uint256 _salaryTokens = _salaryTokensToSwap;
            _salaryTokensToSwap = 0;
            super._transfer(address(this), salaryWallet, _salaryTokens);
        }
    }
    function _transferToBuyBackWallet() private {
        if(_buyBackTokensToSwap < balanceOf(address(this))){
            uint256 _buyBackTokens = _buyBackTokensToSwap;
            _buyBackTokensToSwap = 0;
            super._transfer(address(this), buyBackWallet, _buyBackTokens);
        }
    }
    function _swapAndLiquify() private {
        uint256 tokens = _liquidityTokensToSwap;
        _liquidityTokensToSwap = 0;
        
        uint256 tokensToBeConvertedToBNB = tokens.div(2);
        uint256 tokensForLiqudity = tokens.sub(tokensToBeConvertedToBNB);
        uint256 initialBNBBalance = address(this).balance;
        _swapTokensForBNB(tokensToBeConvertedToBNB); 
        uint256 bnbRecieved = address(this).balance.sub(initialBNBBalance);
        _addLiquidity(tokensForLiqudity, bnbRecieved);
        emit SwapAndLiquify(tokensToBeConvertedToBNB, bnbRecieved, tokensForLiqudity);
    }
    ...
    function _swapAndSendDividends(uint256 dividends) private {
