...
contract PoolWithMultipleWinnersBuilder {
    ...
    constructor(
        ...
        require(
            address(_compoundPrizePoolProxyFactory) != address(0),
            "GlobalBuilder/compoundPrizePoolProxyFactory-not-zero"
        );
        require(
            address(_venusPrizePoolProxyFactory) != address(0),
            "GlobalBuilder/venusPrizePoolProxyFactory-not-zero"
        );
        require(
    ...
    function createCompoundMultipleWinners(
        ...
        emit CompoundPrizePoolWithMultipleWinnersCreated(
            address(prizePool),
    ...
    function createVenusMultipleWinners(
        ...
        prizePool.transferOwnership(msg.sender);
        emit VenusPrizePoolWithMultipleWinnersCreated(
            address(prizePool),
            address(prizeStrategy)
        );
        return prizePool;
    ...
    function createStakeMultipleWinners(
        ...
        prizePool.transferOwnership(msg.sender);
        emit StakePrizePoolWithMultipleWinnersCreated(
            address(prizePool),
            address(prizeStrategy)
        );
        return prizePool;
