...
interface IMigratorToBSCXSwap {
    // Perform LP token migration from legacy UniswapV2 to BSCXSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // BSCXSwap must mint EXACTLY the same amount of BSCXSwap LP tokens or
...
// BSCXMasterFarmer is the master of BSCX. He can make BSCX and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once BSCX is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract BSCXMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BSCXs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBSCXPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBSCXPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BSCXs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BSCXs distribution occurs.
        uint256 accBSCXPerShare; // Accumulated BSCXs per share, times 1e12. See below.
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "BSCXMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (bscxForBurn > 0) {
            bscx.mint(address(this), bscxForBurn);
            bscx.burn(bscxForBurn);
        }

        if (bscxForDev > 0) {
            // Mint unlocked amount for Dev
            bscx.mint(devaddr, bscxForDev.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
            // Mint locked amount for Dev
            //For more simple, I lock reward for dev if mint reward in bonus time
            bscx.mint(address(this), bscxForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            farmLock(devaddr, bscxForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
        }
        bscx.mint(address(this), bscxForFarmer);
    ...
    // Deposit LP tokens to BSCXMasterFarmer for BSCX allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "BSCXMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from BSCXMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "BSCXMasterFarmer::withdraw: not good");
