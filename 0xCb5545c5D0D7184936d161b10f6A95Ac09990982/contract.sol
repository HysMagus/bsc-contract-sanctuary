...
contract Saving is Ownable, StakingInterface {
    using SafeMath for uint256;
    IERC20 public kimochi;
    ...
    constructor(address _kimochi) public {
        kimochi = IERC20(_kimochi);
    }
    ...
    function stake(uint256 _amount, uint256 _days) public {
        ...
        require(kimochi.balanceOf(address(this)) > totalOwedValue().add(_reward).add(_amount), "insufficient contract bal");

        require(kimochi.transferFrom(msg.sender, address(this), _amount), "transfer failed");
    ...
    function claim() public {
        ...
        require(kimochi.transfer(msg.sender, owed), "transfer failed");
    }
    ...
    function setEarlyExit(uint256 _earlyExit) public onlyOwner {
        require(_earlyExit < 1000000, "too big");
        close(true);
    ...
    function ownerReclaim(uint256 _amount) public onlyOwner {
        require(_amount < kimochi.balanceOf(address(this)).sub(totalOwedValue()), "cannot withdraw owed funds");
        kimochi.transfer(msg.sender, _amount);
    }
