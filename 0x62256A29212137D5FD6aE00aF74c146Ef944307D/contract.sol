...
interface IYetiMaster {
    ...
    function poolInfo(uint256 _pid) view external returns(address want, uint256 allocPoint, uint256 lastRewardBlock, uint256 accxBLZDPerShare, address strat, uint256 depositFeeBP);
...
// File: contracts/SBVxBLZDCompound.sol
...
contract SBVxBLZDCompound is IStakingRewardCompound, ReentrancyGuard, Initializable {
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        (address _stakingToken,,,,,) = IYetiMaster(_farmAddress).poolInfo(_pid);
        ...
        YETIMASTER = IYetiMaster(_farmAddress);
        uniRouterAddress = _uniRouterAddress;
        admin = _admin;

        earnedToBUSDPath = [_earnedAddress, BUSD];

        if(!_isCakeStaking){
          token0Address = IUniswapV2Pair(_stakingToken).token0();
          token1Address = IUniswapV2Pair(_stakingToken).token1();

          earnedToToken0Path = [_earnedAddress, BUSD, token0Address];
          if (BUSD == token0Address) {
              earnedToToken0Path = [_earnedAddress, BUSD];
          }
          earnedToToken1Path = [_earnedAddress, BUSD, token1Address];
          if (BUSD == token1Address) {
              earnedToToken1Path = [_earnedAddress, BUSD];
          }
        }
        ...
        IERC20(BUSD).safeApprove(address(_uniRouterAddress), uint256(-1));
    ...
    function withdraw(uint256 amount) public override nonReentrant updateReward(msg.sender) {
        ...
        if(emergencyStop != true){
          YETIMASTER.withdraw(pid, amountLP);
        }
    ...
    function _harvest() private {
        ...
        // swap 74.875% of reward to BUSD
        uint256 earnAmtToBUSD = earnAmt.mul(74875).div(100000);
        if(earnAmt > 0){
          _safeSwap(
              uniRouterAddress,
              earnAmtToBUSD,
        ...
        // 47.25 in 74.875 busd to invest for gale
        uint256 usdAmountForGale = amountBusd.mul(47250).div(74875);
        ...
        // 23.625 xblzd + 23.625 busd for compound
        uint256 busdForComp = amountBusd.mul(23625).div(74875);
        uint256 xblzdForComp = earnAmt.mul(23625).div(100000);

        // 4 in 74.875 busd from busd to blizzardPool
        uint256 usdBlizzardPool = amountBusd.sub(usdAmountForGale).sub(busdForComp);
        IERC20(BUSD).safeTransfer(blizzardPool, usdBlizzardPool);

        // remaining go to caller 1.5%
        uint256 harvestCallerAmt = earnAmt.sub(earnAmtToBUSD).sub(xblzdForComp);
        IERC20(XBLZD).safeTransfer(msg.sender, harvestCallerAmt);
