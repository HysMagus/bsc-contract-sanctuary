...
// KetchupToken with Governance capabilities.
contract KetchupBar is BEP20('fry.Ketchup', 'KHP') {
    ...
    function safeFriesTransfer(address _to, uint256 _amount) public onlyOwner {
    ...
    */
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "FRIES::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FRIES::_writeCheckpoint: block number exceeds 32 bits");
    ...
    // Enter the staking pool. Stake some FRIES. Earn some extra fries from strongfries, farming pools, and vaults
    function enter(uint256 _amount) public {
        uint256 totalFries = fries.balanceOf(address(this));
        uint256 totalShares = totalSupply();
        if (totalShares == 0 || totalFries == 0) {
            _mint(msg.sender, _amount);
        } else {
            uint256 what = _amount.mul(totalShares).div(totalFries);
            _mint(msg.sender, what);
        }
        fries.transferFrom(msg.sender, address(this), _amount);
    }
    
    // Leave the staking pool. Earn Fries back 
    function leave(uint256 _share) public {
        uint256 totalShares = totalSupply();
        uint256 what = _share.mul(fries.balanceOf(address(this))).div(totalShares);
        _burn(msg.sender, _share);
        uint256 fee = what.mul(25).div(1000); // 2.5% dev fee to sustain project 
        uint256 stakerAmount = what.sub(fee); // staker - dev fee
        fries.transfer(devAddr, fee); 
        fries.transfer(msg.sender, stakerAmount);
    }
