...
// You should have received a copy of the GNU General Public License
// along with this program.  If not, see <http://www.gnu.org/licenses/>.
contract DirectAssimilator is IAssimilator {
    ...
    uint public immutable denominator;

    uint public immutable truncator;

    constructor (address _asset, uint _decimals) public {
        asset = IERC20(_asset);
        denominator = 10 ** _decimals;
        truncator = 10 ** (_decimals / 6);
    }

    function intakeRawAndGetBalance (uint256 _amount) public override returns (int128 amount_, int128 balance_) {

        bool _success = asset.transferFrom(msg.sender, address(this), _amount);

        require(_success, "Component/asset-transfer-from-failed");

        uint256 _balance = asset.balanceOf(address(this));

        balance_ = _balance.divu(denominator);

        amount_ = _amount.divu(denominator);
    ...
    function intakeNumeraire (int128 _amount) public override returns (uint256 amount_) {
        ...
        amount_ = _amount.mulu(denominator) / truncator * truncator;

        bool _success = asset.transferFrom(msg.sender, address(this), amount_);

        require(_success, "Component/asset-transfer-from-failed");

    }

    function outputRawAndGetBalance (address _dst, uint256 _amount) public override returns (int128 amount_, int128 balance_) {

        bool _success = asset.transfer(_dst, _amount);

        require(_success, "Component/asset-transfer-failed");
    ...
    function outputNumeraire (address _dst, int128 _amount) public override returns (uint256 amount_) {
        ...
        amount_ = _amount.mulu(denominator);

        bool _success = asset.transfer(_dst, amount_);
    ...
    function viewRawAmount (int128 _amount) public override view returns (uint256 amount_) {
    ...
    function viewNumeraireAmount (uint256 _amount) public override view returns (int128 amount_) {
    ...
    function viewNumeraireBalance (address _addr) public override view returns (int128 balance_) {

        uint256 _balance = asset.balanceOf(_addr);
    ...
    // takes raw amount, returns numeraire amount
    function viewNumeraireAmountAndBalance (address _addr, uint256 _amount) public override view returns (int128 amount_, int128 balance_) {
