...
contract Master is Ownable {
    ...
    uint256 public percentRate = 200; //2%
    ...
    constructor(address _token) public {
        ...
        targetRate = 200000000; //20000000000000 for (1 Quadrillion) - Initial Price

        ceiling = 2000000 * 10 ** DECIMALS;
    ...
    function rebase() public {
        ...
        uint256 supplyAfterRebase = token.rebase(epoch, supplyDelta);
    ...
    function adjustTargetRate() external
    {
        ...
        if (exchangeRate > targetRate){
            targetRate = exchangeRate.mul(percentRate).div(100);
        }
    }

    // increment by 2% to achieve 50% every 24hrs
    function incrementTargetRate() internal {
      uint256 newRate = targetRate.mul(percentRate).div(100);
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && token.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(token.totalSupply())).toInt256Safe();
        }
    ...
    function computeSupplyDelta(uint256 rate) internal view returns (int256) {
        ...
        return token.totalSupply().toInt256Safe()
     ...
     */
    function withinDeviationThreshold(uint256 rate) internal view returns (bool) {
    ...
    //Default 102 for 2%
    function setRatePercentage(uint256 _percent, bool _update) public onlyOwner {
        percentRate = _percent;
