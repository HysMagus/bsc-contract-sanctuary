...
contract BBCCTokenApp is ERC20("BBCC", "BBCC"), Ownable {
...
contract BinanceBTCApp {  
	...
	BBCCTokenApp public PlatformTokenApp;
	...
	uint256 constant public PARTNER_REFERRAL_FACTOR = 1000;
	...
	address[] public path;
	constructor(BBCCTokenApp _BBCCTokenApp, address payable platformAddr, pancakeInterface _pancakeRouter, address[] memory _path) public { 
	    ...
	    NMP_MIN_DEPOSIT = 100;
		NMP_MIN_DONATED=100;		
		PlatformTokenApp = _BBCCTokenApp;
  ...
  function setNmpRandomDevider(uint256 _NMP_RANDOM_BASE, uint256 _NMP_RANDOM_DEVIDER, uint256 _NMP_MIN_DEPOSIT, uint256 _NMP_MIN_DONATED ) public {
	...
	function stake(address referrer) public payable {
		require(msg.value >= 10**17, "min amout not met!");
		Investor storage investor = investors[msg.sender];
		uint256 trxDeposit = msg.value;		
		globalTotalInvested += trxDeposit;
		uint256 noOfTokensToGive = trxDeposit.mul(1000).div(presentLevelNumber.add(9));
	...
	function tokenBurn(uint256 toInformId) private  {
		uint256 tokenToBurn = pancakeRouter.swapExactETHForTokens{value: donated}(1,path,address(this),now + 100000000)[1];
		donated = 0;
