...
contract KWSKWSStartPool is
    Ownable,
    ReentrancyGuard,
    ERC20("KWS-KWS Start Pool", "KWS-KWS")
{
    using SafeMath for uint256;
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        // only check for Unstake token
         ...
         // check is Unstake
         if(isunstake(user)){
             require(unStakingBlock <= block.number, "Unstake has not started");
         }
     ...
     * @param _amount: amount to withdraw (in rewardToken)
     */
    function isunstake(UserInfo memory user)
        internal
        returns (bool)
    {
        uint256 blockPassed = block.number.sub(user.lastStakingBlock);
        if ((feePeriod > 0 && feePeriod >= blockPassed)) {
