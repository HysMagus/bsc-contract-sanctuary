...
contract BlockPresale is Ownable {
    using SafeMath for uint256;
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "BlockPresale: sale is not open");
        require(!hasEnded(), "BlockPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "BlockPresale: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "BlockPresale: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "BlockPresale: presale is not over");
        require(softCapReached(), "BlockPresale: soft cap not reached, refund is available");
        require(
            contributions[msg.sender] > 0,
            "BlockPresale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "BlockPresale: presale is not over");
        require(!softCapReached(), "BlockPresale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "BlockPresale: nothing to claim"
        );
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "BlockPresale: presale is not over");
