...
contract WhalesLinkToken is BEP20('WhalesLinkToken', 'WHALE') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (WhalesLinkSwapRouter).
...
contract WhalesLinkMaster is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (userInfo.amount * pool.accWhalePerShare) - userInfo.rewardDebt
        //
    ...
    // WHALE tokens created first block.
    uint256 public whaleStartBlock;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when WHALE mining starts.
    uint256 public startBlock;
    // Block number when bonus WHALE period ends.
    ...
    // WHALE tokens created at bonus end block.
    uint256 public whaleBonusEndBlock;
    // max reward block
    ...
    // Accumulated WHALEs per share, times 1e12.
    uint256 public accWhalePerShareMultiple = 1E12;
    // The WHALE TOKEN!
    WhalesLinkToken public whale;
    // Dev address.
    ...
    constructor(
        WhalesLinkToken _whale,
        address _devAddr,
        uint256 _whaleStartBlock,
        uint256 _startBlock,
        ...
        whale = _whale;
        devAddr = _devAddr;
        whaleStartBlock = _whaleStartBlock;
        ...
        // whale created when bonus end first block.
        // (whaleStartBlock - bonusBeforeCommonDifference * ((bonusEndBlock-startBlock)/bonusBeforeBulkBlockSize - 1)) * bonusBeforeBulkBlockSize*(bonusEndBulkBlockSize/bonusBeforeBulkBlockSize) * bonusEndBulkBlockSize
        whaleBonusEndBlock = whaleStartBlock
        ...
        // max mint block number, _whaleInitBlock - (MAX-1)*_commonDifference = 0
        // MAX = startBlock + bonusEndBulkBlockSize * (_whaleInitBlock/_commonDifference + 1)
        maxRewardBlockNumber = startBlock.add(
            bonusEndBulkBlockSize.mul(whaleBonusEndBlock.div(bonusEndCommonDifference).add(1))
        );
    }
    ...
    function getTotalRewardInfoInSameCommonDifference(
        ...
        if (fromBulkNumber == currentBulkNumber) {
            return _to.sub(_from).mul(_whaleInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
        }
        ...
        uint256 currentPreviousBulkLastBlock = startBlock.add(_bulkBlockSize.mul(currentBulkNumber.sub(1)));
        {
            ...
            .mul(_whaleInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
            if (lastRewardBulkLastBlock > tempFrom && lastRewardBulkLastBlock <= tempTo) {
                totalReward = totalReward.add(
                    lastRewardBulkLastBlock.sub(tempFrom).mul(
                        _whaleInitBlock.sub(fromBulkNumber > 0 ? fromBulkNumber.sub(1).mul(_commonDifference) : 0)
                    )
                );
            }
        }
        {
            ...
            uint256 tempWhaleInitBlock = _whaleInitBlock;
            ...
            if (currentPreviousBulkLastBlock > lastRewardBulkLastBlock) {
                ...
                if (N > 1) {
                    ...
                    totalReward = totalReward.add(
                        tempBulkBlockSize.mul(tempWhaleInitBlock.sub(currentBulkNumber.sub(2).mul(tempCommonDifference)))
                    );
                }
            }
        }
    ...
    function pendingWhale(address _pair, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && pool.lastRewardBlock < maxRewardBlockNumber) {
            ...
            uint256 whaleReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
            accWhalePerShare = accWhalePerShare.add(whaleReward.mul(accWhalePerShareMultiple).div(lpSupply));
        }
        return userInfo.amount.mul(accWhalePerShare).div(accWhalePerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updatePool(address _pair) public {
        ...
        uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
        uint256 whaleReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
        whale.mintTo(devAddr, whaleReward.div(100));
        whale.mintTo(address(this), whaleReward);
        pool.accWhalePerShare = pool.accWhalePerShare.add(whaleReward.mul(accWhalePerShareMultiple).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to WhalesLinkMaster for WHALE allocation.
    function deposit(address _pair, uint256 _amount) public {
        PoolInfo storage pool = poolInfoMap[_pair];
        UserInfo storage userInfo = poolUserInfoMap[_pair][msg.sender];
        updatePool(_pair);
        if (userInfo.amount > 0) {
            uint256 pending = userInfo.amount.mul(pool.accWhalePerShare).div(accWhalePerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending > 0) {
        ...
        userInfo.rewardDebt = userInfo.amount.mul(pool.accWhalePerShare).div(accWhalePerShareMultiple);
        emit Deposit(msg.sender, _pair, _amount);
    }

    // Withdraw LP tokens from WhalesLinkMaster.
