...
contract GFCEPresale is Ownable, ReentrancyGuard, ContractGuard {
    ...
    uint256 public totalPurchasedGfce;
    uint256 public unclaimedPurchasedGfce;
    bool public isPreSaleOver;

    mapping(address => uint256) public purchasedGfceToken;
    modifier isNotOver() {
        require(!isPreSaleOver, "GFCE_PRESALE:presale is over");
        _;
    }
    modifier isOver() {
        require(isPreSaleOver, "GFCE_PRESALE:presale not over yet");
        _;
    }
    ...
    {
        require(
            purchaseBusdAmount <= maxPurchasableInOneTx,
            "GFCE_PRESALE: max purchasable busd amount per tx is 25000."
        );
        require(
            IBEP20(BusdToken).balanceOf(address(this)) <
                maxTotalPurchasableBusd,
            "GFCE_PRESALE: we already sold all."
        );
        uint256 BnbPriceInUSD = getBnbPriceInUSD();
        uint256 FuelPriceInUSD = getFuelPriceInUSD(BnbPriceInUSD);
        uint256 JetsPriceInUSD = getJetsPriceInUSD(FuelPriceInUSD);
        ...
        unclaimedPurchasedGfce = unclaimedPurchasedGfce.add(purchaseGfceAmount);
        purchasedGfceToken[_msgSender()] = purchasedGfceToken[_msgSender()].add(
            purchaseGfceAmount
        );
        ...
        isOver
    {
        require(
            IBEP20(BusdToken).balanceOf(address(this)) >=
                maxTotalPurchasableBusd ||
                isPreSaleOver,
            "GFCE_PRESALE: not sold or not over yet."
        );
        uint256 amount = purchasedGfceToken[_msgSender()];
        require(amount > 0, "GFCE_PRESALE: zero amount");

        purchasedGfceToken[_msgSender()] = 0;
        unclaimedPurchasedGfce = unclaimedPurchasedGfce.sub(amount);
