...
Thunder Storm is a double rewards token.   Created this token to give rewards of multiple tokens every month.  Thunderstorm promote other tokens by giving them as rewards.
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    address public adminAddress = 0x0b98E83e4B80eb07585DFa3a3e541E9129B9425B;
...
contract ThunderStorm is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public toBurnThunderStormBnbAddress = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;  //Sample here. BurningToken Address. Can be changed...  
    uint256 public toBurnThunderStormBnbFee = 2;
    ...
    constructor() ERC20("ThunderStorm", "STORM") {
        ...
        teamWallet = 0x0b98E83e4B80eb07585DFa3a3e541E9129B9425B; 
        a80DividendToken = 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47;
        b20DividendToken =  0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    function updateBurnThunderStormBnbAddress(address _newAddress) external onlyOwner {   
        toBurnThunderStormBnbAddress = _newAddress;
    }
    ...
    function setToBurnThunderStormBnbFee(uint256 newFee) external onlyOwner {    
        require(newFee <= MAX_FEE_RATE, "wrong");
        toBurnThunderStormBnbFee = newFee;
        _updateTotalFee();
    }
    ...
    function _updateTotalFee() internal {
        totalFees = buyBackAndLiquidityFee.add(marketingFee).add(a80DividendRewardsFee).add(b20DividendRewardsFee).add(toBurnThunderStormBnbFee);
    }
    ...
    function updateA80DividendTracker(address newAddress) external onlyOwner {
        ...
        require(newA80DividendTracker.owner() == address(this), "must be owned by STORM");
    ...
    function updateB20DividendTracker(address newAddress) external onlyOwner {
        ...
        require(newB20DividendTracker.owner() == address(this), "must be owned by STORM");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if(marketingEnabled && buyBackAndLiquifyEnabled && toBurnThunderStormBnbFee > 0 && buyBackMode){

                    uint256 total3Fees = marketingFee.add(buyBackAndLiquidityFee).add(toBurnThunderStormBnbFee);
                    ...
                    if(increaseAmount > 0){
                        ...
                        uint256 bnbToBurnThunderStormBnb = increaseAmount.mul(toBurnThunderStormBnbFee).div(total3Fees);
                        useBnbToBurnThunderStormBnb(bnbToBurnThunderStormBnb);
                    }
                }else{
                    if (marketingEnabled) {
                    ...
                    if(toBurnThunderStormBnbFee > 0){
                        uint256 swapTokensToBurnThunderStormBnb = contractTokenBalance.mul(toBurnThunderStormBnbFee).div(totalFees);
                        buyBackThunderStormBnbAndBurn(swapTokensToBurnThunderStormBnb);
                    }
                }
    ...
    function useBnbToBurnThunderStormBnb(uint256 amount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = toBurnThunderStormBnbAddress;
    ...
    function buyBackThunderStormBnbAndBurn(uint256 amount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = toBurnThunderStormBnbAddress;
...
contract B20DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ADA20_Dividend_Tracker", "ADA20_Dividend_Tracker",  0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 20000 * (10**18); //must hold 20000+ tokens
    }
