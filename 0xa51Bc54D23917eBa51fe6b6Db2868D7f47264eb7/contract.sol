...
contract BvaultsStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    address public bcashAddress;
    address public mdgLockerAddress;
    ...
    constructor(
        ...
        address _mdgLockerAddress,
        address[] memory _bonusRewards,
        ...
        if (isAutoComp) {
            ...
            mdoAddress = _bonusRewards[0];
            bcashAddress = _bonusRewards[1];
            mdgLockerAddress = _mdgLockerAddress;
        }
    ...
    function earn() public whenNotPaused {
        ...
        // convert bcash to mdg
        uint256 bcashEarnedAmt = IERC20(bcashAddress).balanceOf(address (this));
        if (bcashEarnedAmt > 0) {
            _vswapSwapToken(bcashAddress, earnedAddress, bcashEarnedAmt);
        }
        // convert mdo to mdg
        uint256 mdoEarnedAmount = IERC20(mdoAddress).balanceOf(address (this));
        if (mdoEarnedAmount > 0) {
            _vswapSwapToken(mdoAddress, earnedAddress, mdoEarnedAmount);
        }
        // unlock mdg if possible
        if (checkForUnlockReward) {
            if (IMdgLocker(mdgLockerAddress).canUnlockAmount(address (this)) > 0) {
                IMdgLocker(mdgLockerAddress).unlock();
            }
        }
    ...
    function convertDustToEarned() public whenNotPaused {
        ...
        // Converts want dust (if any) to earned tokens
        uint256 _wantAmount = IERC20(wantAddress).balanceOf(address(this));
        if (wantAddress != earnedAddress && _wantAmount > 0) {
            _vswapSwapToken(wantAddress, earnedAddress, _wantAmount);
        }
    }
    ...
    function setMainPaths(
        ...
        address[] memory _mdoToEarnedPath,
        address[] memory _bcashToEarnedPath
        ...
        vswapPaths[earnedAddress][wantAddress] = _earnedToWantPath;
        vswapPaths[wantAddress][earnedAddress] = _wantToEarnedPath;
        vswapPaths[earnedAddress][busdAddress] = _earnedToBusdPath;
        vswapPaths[mdoAddress][earnedAddress] = _mdoToEarnedPath;
        vswapPaths[bcashAddress][earnedAddress] = _bcashToEarnedPath;
    }
