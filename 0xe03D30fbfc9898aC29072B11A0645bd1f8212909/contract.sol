...
library SafeMath {
    ...
    /**
     * @dev Returns the subtraction of two unsigned integers, reverting with cethom message on
     * overflow (when the result is negative).
     *
     * Counterpart to Solidity's `-` operator.
     *
    ...
    /**
     * @dev Returns the integer division of two unsigned integers. Reverts with cethom message on
     * division by zero. The result is rounded towards zero.
     *
     * Counterpart to Solidity's `/` operator. Note: this function uses a
     * `revert` opcode (which leaves remaining gas untouched) while Solidity
    ...
    /**
     * @dev Returns the remainder of dividing two unsigned integers. (unsigned integer modulo),
     * Reverts with cethom message when dividing by zero.
     *
     * Counterpart to Solidity's `%` operator. This function uses a `revert`
     * opcode (which leaves remaining gas untouched) while Solidity uses an
     * invalid opcode to revert (consuming all remaining gas).
...
library Address {
     ...
     *
     * IMPORTANT: because control is transferred to `recipient`, care meth be
     ...
     * Requirements:
     *
     * - `target` meth be a contract.
     * - calling `target` with `data` meth not revert.
     *
     ...
     * Requirements:
     *
     * - the calling contract meth have an ETH balance of at least `value`.
     * - the called Solidity function meth be `payable`.
     *
     * _Available since v3.1._
     */
...
library SafeERC20 {
     ...
     * on the return value: the return value is optional (but if data is returned, it meth not be false).
...
contract Pausable is Context {
     ...
     * Requirements:
     *
     * - The contract meth not be paused.
     */
 ...
 */
contract StrategyCakeBethLP is Ownable, Pausable {
     ...
     * {eth} - Required for liquidity routing into beth asset.
    ...
    address constant public eleven = address(0xb8005Dc9a8d48793Ba60b72E10380465f9412c73);
     ...
     */
    address constant public rewards  = address(0x6Da85bBc6a0dc20AFAA236Afe2e917B10D6E88F7);
    ...
    uint constant public BURN_FEE       = 20;
     ...
     */
    constructor() public {
        lpPair = 0x99d865Ed50D2C32c1493896810FA386c1Ce81D91;
        lpToken0 = IPancakePair(lpPair).token0();
        lpToken1 = IPancakePair(lpPair).token1();
        poolId = 70;
        vault = 0x299120f86fcd654bB8Aedf0A1Cc48290160BF0a7;

        if (lpToken0 == eth) {
            cakeToLp0Route = [cake, wbnb, eth];
        } else if (lpToken0 != cake) {
            cakeToLp0Route = [cake, wbnb, eth, lpToken0];
        }

        if (lpToken1 == eth) {
            cakeToLp1Route = [cake, wbnb, eth];
        } else if (lpToken1 != cake) {
            cakeToLp1Route = [cake, wbnb, eth, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(cake).balanceOf(address(this)).mul(15).div(1000);
