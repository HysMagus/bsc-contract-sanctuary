...
// File: contracts/interfaces/uniswapV2/IPancakePair.sol
...
// File: contracts/libs/LimitOrderUtils.sol

pragma solidity ^0.6.8;

library LimitOrderUtils {
    address internal constant BNB_ADDRESS = address(0x00eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee);

    /**
     * @notice Get the account's balance of token or BNB
     * @param _token - Address of the token
     * @param _addr - Address of the account
     * @return uint256 - Account's balance of token or BNB
     */
    function balanceOf(IERC20 _token, address _addr) internal view returns (uint256) {
        if (BNB_ADDRESS == address(_token)) {
     ...
     /**
     * @notice Transfer token or BNB to a destinatary
     * @param _token - Address of the token
...
// File: contracts/handlers/PancakeSwapHandler.sol

pragma solidity ^0.6.8;

/// @notice UniswapV2 Handler used to execute an order
contract PancakeSwapHandler is IHandler {
    using SafeMath for uint256;
     ...
     * @param _wbnb - Address of WBNB contract
     * @param _codeHash - Bytes32 of the uniswap v2 pair contract unit code hash
     */
    constructor(address _factory, address _wbnb, bytes32 _codeHash) public {
        FACTORY = _factory;
        WBNB = IWBNB(_wbnb);
    ...
    receive() external override payable {
        require(msg.sender != tx.origin, "PancakeSwapHandler#receive: NO_SEND_ETH_PLEASE");
    }
    ...
    function handle(
        ...
        uint256 amount = LimitOrderUtils.balanceOf(_inputToken, address(this));
        address inputToken = address(_inputToken);
        address outputToken = address(_outputToken);
        address wbnb = address(WBNB);

        // Decode extra data
        (,address relayer, uint256 fee) = abi.decode(_data, (address, address, uint256));

        if (inputToken == wbnb || inputToken == LimitOrderUtils.BNB_ADDRESS) {
            // Swap WBNB -> outputToken
            amount = amount.sub(fee);

            // Convert from ETH to WBNB if necessary
            if (inputToken == LimitOrderUtils.BNB_ADDRESS) {
        ...
        } else if (outputToken == wbnb || outputToken == LimitOrderUtils.BNB_ADDRESS) {
            // Swap inputToken -> WBNB
            bought = _swap(inputToken, wbnb, amount, address(this));

            // Convert from WBNB to ETH if necessary
            if (outputToken == LimitOrderUtils.BNB_ADDRESS) {
                WBNB.withdraw(bought);
            } else {
                WBNB.withdraw(fee);
            }
            ...
            LimitOrderUtils.transfer(IERC20(outputToken), msg.sender, bought);
        } else {
            // Swap inputToken -> WBNB -> outputToken
            //  - inputToken -> WBNB
            bought = _swap(inputToken, wbnb, amount, address(this));
            ...
            // - WBNB -> outputToken
            bought = _swap(wbnb, outputToken, bought.sub(fee), msg.sender);
        }

        // Send fee to relayer
        (bool successRelayer,) = relayer.call{value: fee}("");
        require(successRelayer, "PancakeSwapHandler#handle: TRANSFER_ETH_TO_RELAYER_FAILED");
    }
    ...
    function canHandle(
        ...
        if (inputToken == wbnb || inputToken == LimitOrderUtils.BNB_ADDRESS) {
            ...
            return _estimate(wbnb, outputToken, _inputAmount.sub(fee)) >= _minReturn;
        } else if (outputToken == wbnb || outputToken == LimitOrderUtils.BNB_ADDRESS) {
            uint256 bought = _estimate(inputToken, wbnb, _inputAmount);
            ...
            return _estimate(wbnb, outputToken, bought.sub(fee)) >= _minReturn;
        }
    }
    ...
    function simulate(
        ...
        if (inputToken == wbnb || inputToken == LimitOrderUtils.BNB_ADDRESS) {
            ...
            bought = _estimate(wbnb, outputToken, _inputAmount.sub(fee));
        } else if (outputToken == wbnb || outputToken == LimitOrderUtils.BNB_ADDRESS) {
            bought = _estimate(inputToken, wbnb, _inputAmount);
            if (bought <= fee) {
                 return (false, 0);
            }
            ...
            bought = _estimate(wbnb, outputToken, bought.sub(fee));
        }
    ...
    function _estimate(address _inputToken, address _outputToken, uint256 _inputAmount) internal view returns (uint256 bought) {
        ...
        IPancakePair pair = IPancakePair(UniswapUtils.pairForSorted(FACTORY, token0, token1, FACTORY_CODE_HASH));
    ...
    function _swap(address _inputToken, address _outputToken, uint256 _inputAmount, address _recipient) internal returns (uint256 bought) {
        ...
        if (_inputToken != address(WBNB)) {
            prevPairBalance = LimitOrderUtils.balanceOf(IERC20(_inputToken), address(pair));
        }

        // Send tokens to uniswap pair
        require(SafeERC20.transfer(IERC20(_inputToken), address(pair), inputAmount), "PancakeSwapHandler#_swap: ERROR_SENDING_TOKENS");

        if (_inputToken != address(WBNB)) {
            inputAmount = LimitOrderUtils.balanceOf(IERC20(_inputToken), address(pair)).sub(prevPairBalance);
        }
