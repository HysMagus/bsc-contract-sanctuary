...
// AppleShake with Governance.
contract AppleShake is BEP20('AppleShake', 'Shake') {
    ...
    constructor(AppleToken _apple) public {
        apple = _apple;
    ...
    event AppleShakeTransfer(address _to, uint256 _total, uint256 _amount, uint256 _tax, uint256 appleBal);
    // Safe apple transfer function, just in case if rounding error causes pool to not have enough CREAMs.
    function safeAppleTransfer(address _to, uint256 _total) external onlyOwner {

        (uint256 _amount, uint256 _tax) = taxUser(_total);
        if( _tax > 0 ){ // send the taxed amount to be burned
            apple.transfer(taxTo, _tax);
        }
        ...
        emit AppleShakeTransfer(_to, _total, _amount, _tax, appleBal);
    }
