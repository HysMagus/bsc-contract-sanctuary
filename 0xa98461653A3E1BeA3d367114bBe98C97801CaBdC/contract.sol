...
contract YumCoin is Context, IBEP20, Ownable {
  ...
  */
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    function buyYUM() public payable {     // Used for presale
    ...
    function endSale() external onlyOwner {         // When the sale is over we add half the ETH and sold tokens to Uniswap
        ...
        saleEnabled = false;
        freeze = false;
        uint liquidityYUM = mulDiv((1962000E18 - numLeftSale), 50, 100);      // However many were sold, take 50% and add them to Uniswap
        uint devShare = mulDiv(address(this).balance, 50, 100);         // 50% goes to the administrative wallet for bounty and dev distribution
        uint liquidityBNB = address(this).balance.sub(devShare);        // 50% goes to Pancakeswap
        payable(owner()).transfer(devShare);                                       // 50% goes to devs for development and marketing
        _create(address(this), liquidityYUM);
        (uint amountToken, uint amountBNB, uint liquidity) = pancakeRouter.addLiquidityETH{value: liquidityBNB}(address(this), liquidityYUM, 0, 0, address(this), block.timestamp.add(86400));
