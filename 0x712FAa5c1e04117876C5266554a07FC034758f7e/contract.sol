...
// File: contracts/cream/interfaces/pancake/IPancakeRouter.sol
...
// File: contracts/cream/interfaces/pancake/IPancakePair.sol
...
// File: contracts/cream/interfaces/boltdollar/IShareRewardPool.sol
...
// File: contracts/cream/strategies/boltdollar/StrategyButterLP.sol
 ...
 * BUTTER rewards from providing liquidity are farmed every few minutes, sold and split 50/50. 
 ...
 */
contract StrategyCFIBUTTERLP is Ownable, Pausable {
     ...
     * {butter} - Token generated by staking our funds. In this case it's the Butters token.
     * {cream} - BeefyFinance token, used to send funds to the treasury.
    ...
    address constant public butter = address(0x7269097A760961474dADBCAaBe2B33A205Ad5c2f); // BUTTER
    address constant public cream = address(0xdF6585D955fD55687c0b8A3000D0573faB341bee); // cream
    ...
    address constant public shareRewardPool = address(0xB5F0Dbb48aC2DaB7da7D139de7aB35f03B1EE86B); // Shares cream
    uint8 public poolId;

    /**
     * @dev Creamery Contracts:
     * {rewards} - Reward pool where the strategy fee earnings will go.
     * {treasury} - Address of the Creamery treasury
     * {vault} - Address of the vault that controls the strategy's funds.
     * {strategist} - Address of the strategy author/deployer where strategist fee will go.
     */
    address constant public rewards  = address(0x0f1C9d1dDfe699c9E13DB6F56BE95d387D655225);
    address constant public treasury = address(0x07c2Bc843a8AC58d05ca26223D7d4f6b74f42477);
     ...
     *
     * {REWARDS_FEE} - 3% goes to BUTTER holders through the {rewards} pool.
     ...
     * {creamToWbnbRoute} - Route we take to get from {cream} into {wbnb}.
     * {wbnbToButterRoute} - Route we take to get from {wbnb} into {cream}.
     * {butterToLp0Route} - Route we take to get from {butter} into {lpToken0}.
     * {butterToLp1Route} - Route we take to get from {butter} into {lpToken1}.
     */
    address[] public creamToWbnbRoute = [cream, butter, wbnb];
    address[] public creamToButterRoute = [cream,  butter];
    address[] public wbnbToButterRoute = [wbnb, butter];
    address[] public butterToLp0Route;
    address[] public butterToLp1Route;
     ...
     */
    constructor() public {
        lpPair = address(0x2CD679e2F93b2a9d15738183E9AdaE967F595C5C);
        lpToken0 = IPancakePair(lpPair).token0();
        lpToken1 = IPancakePair(lpPair).token1();
        poolId = 0;
        vault = address(0xD6B6d298aDC00361D1dEcbcbcf05a95f6E8C530a);
        strategist = address(0xdE1864f3715605b5e55b2cd60492FB3dA017D3df);

        if (lpToken0 == wbnb) {
            butterToLp0Route = [butter, wbnb];
        } else if (lpToken0 != butter) {
            butterToLp0Route = [butter, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            butterToLp1Route = [butter, wbnb];
        } else if (lpToken1 != butter) {
            butterToLp1Route = [butter, wbnb, lpToken1];
        }

        IERC20(lpPair).safeApprove(shareRewardPool, uint(-1));
        IERC20(butter).safeApprove(unirouter, uint(-1));
        IERC20(cream).safeApprove(unirouter, uint(-1));
     ...
     * It deposits {lpPair} in the shareRewardPool to farm {butter}
     */
     ...
     * 3. It swaps the {cream} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(cream).balanceOf(address(this)).mul(45).div(1000);
        IPancakeRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, creamToWbnbRoute, address(this), now.add(600));
        ...
        IPancakeRouter(unirouter).swapExactTokensForTokens(treasuryHalf, 0, wbnbToButterRoute, treasury, now.add(600));
    ...
    /**
     * @dev Swaps {cream} to BUTTER and adds BUTTER + BNB to {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {   
        uint256 creamStack = IERC20(cream).balanceOf(address(this));

        
        if (creamStack > 0) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(creamStack, 0, creamToButterRoute, address(this), now.add(600));
        }
    
        uint256 butterHalf = IERC20(butter).balanceOf(address(this)).div(2);
        
        if (lpToken0 != butter) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(butterHalf, 0, butterToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != butter) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(butterHalf, 0, butterToLp1Route, address(this), now.add(600));
        }
    ...
    function pause() public onlyOwner {
        ...
        IERC20(butter).safeApprove(unirouter, 0);
        IERC20(wbnb).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
        IERC20(cream).safeApprove(unirouter, 0);
    }
