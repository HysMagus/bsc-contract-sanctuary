...
contract Brokoli is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0x3Ecd0cb9EDdb71642F4C4DFd1D6555238C670613); // Marketing Address
    ...
    string private _name = "Brokoli";
    string private _symbol = "BRKL";
    uint8 private _decimals = 9;


    uint256 public _liquidityFee = 0;
    uint256 public _marketingFee = 1;
    uint256 public _ANMbuyBackFee = 0;
    uint256 public _totalTaxPercent = 0;
    uint256 private _prevTotalTaxPercent = 0;

    uint256 public _maxBuyTransactioAmount = 1000000000 * 10**9;
    uint256 public _maxSellTransactionAmount = 1000000000 * 10**9;
    uint256 private minimumTokensBeforeSwap = 800000000 * 10**9; 
    uint256 public _swapAmount = 1000000000 * 10**9;
    ...
    constructor () {
        ...
        _totalTaxPercent = _liquidityFee.add(_marketingFee).add(_ANMbuyBackFee);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(to == uniswapV2Pair && !_isExcludedFromFee[from]) {
            require(amount <= _swapAmount, "Transfer amount exceeds the maxBuyTxAmount.");
        }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 forANMbuyBack = tAmount.div(_totalTaxPercent).mul(_ANMbuyBackFee);
        uint256 forMarketing = tAmount.sub(forLiquidity);
        ...
        if(forMarketing > 0 )
        {
         ...
         if(forANMbuyBack > 0) {
                swapTokensForBUSD(forANMbuyBack);
        }
    ...
    function setSwapAmount(uint256 swapAmount) external onlyOwner() {
