...
contract LFIVault is ERC20 {
    ...
    constructor (address _token) public ERC20(
        string(abi.encodePacked("Tested ", ERC20(_token).name())),
        string(abi.encodePacked("T", ERC20(_token).symbol()))
    ) {
        token = IERC20(_token);
     ...
     * ltoken to keep some of the system funds at hand in the vault, instead
    ...
    function setNewGovernance(address _gov) public {
        require(msg.sender == governance, "governance: wut?");
        governance = _gov;
    }
}