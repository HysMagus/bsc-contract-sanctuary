...
contract BrikFarmingBRIKBUSD is Ownable {
    ...
    address public trustedDepositTokenAddress = 0x3f357C5ccB2095FeDc64f486012387da3160FD1F;
    ...
    uint public next_turbo = 60;
    
    uint public duree_turbo = 12;
    
    uint public startDate ;
    

    
    event Info(bool Mode, uint epok , uint duree, uint start);
    ...
    function randomTurboStart() internal view returns (uint8) {
       //return uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, _player, _tickets))) % 289;
        uint8 time8 = uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, now))) % 300);
        uint256 time256 = uint256(time8);
        time256 = time256.max(50);
   ...
   function randomTurboduration() internal view returns (uint8) {
       ...
       uint8 time8 = uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, now))) % 240);
       uint256 time256 = uint256(time8);
       time256 = time256.max(60);
