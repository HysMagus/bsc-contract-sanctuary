...
Telegaram : https://t.me/ForeverMessiah
Website : https://t.me/ForeverMessiah
...
contract ForeverMessiah is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("ForeverMessiah", "ForeverMessiah") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0x7Ab8Efe29d78068fb9C267Fd3687b43181072968;
    ...
    function tradeStart() external onlyOwner {
        ...
        maxBuyTranscationAmount = 100000000000000000 * (10**18);
        maxSellTransactionAmount = 10000000000000000 * (10**18);
        maxWalletToken = 100000000000000000 * (10**18);
    }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "ForeverMessiah: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "ForeverMessiah: The new dividend tracker must be owned by the ForeverMessiah token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ForeverMessiah_Dividend_Tracker", "ForeverMessiah_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ForeverMessiah_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ForeverMessiah_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "ForeverMessiah_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "ForeverMessiah_Dividend_Tracker: Cannot update claimWait to same value");
