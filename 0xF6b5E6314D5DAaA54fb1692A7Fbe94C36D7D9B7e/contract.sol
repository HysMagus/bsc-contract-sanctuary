...
contract Getters is State {
    ...
    function name() public view returns (string memory) {
        return "Binance New Stable Coin Stake";
    }
...
contract Liquidity is PoolGetters {
    ...
    function addLiquidity(uint256 dollarAmount) internal returns (uint256, uint256) {
        ...
        uint256 busdAmount = (reserveA == 0 && reserveB == 0) ?
             dollarAmount :
             PancakeLibrary.quote(dollarAmount, reserveA, reserveB);

        address pair = address(cakeLp());
        IERC20(dollar).transfer(pair, dollarAmount);
        IERC20(busd).transferFrom(msg.sender, pair, busdAmount);
        return (busdAmount, IPancakePair(pair).mint(address(this)));
    }

    // overridable for testing
    function getReserves(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = PancakeLibrary.sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IPancakePair(PancakeLibrary.pairFor(PANCAKE_FACTORY, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }
}
...
contract Implementation is State, Bonding, Market, Regulator, Govern {
    ...
    function initialize() initializer public {
        // Liquidity 
        incentivize(address(0xF98095beAbDec0B7D5C7b3D0Fcd7B56fc490eF5d), 1000e18);
    }
    ...
    function calculatePrice() public view returns(uint256) {
        IPancakePair pair = IPancakePair(address(Pool(pool()).cakeLp()));
