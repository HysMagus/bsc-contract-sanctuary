...
// File: contracts/BIFI/strategies/soak/Strategysoak.sol
...
/**
 * @dev Strategy to farm soak through a Pancake based MasterChef contract.
 */
contract Strategysoak is Ownable, Pausable {
     ...
     * {soak} - Token that the strategy maximizes. The same token that users deposit in the vault.
     * {bifi} - BeefyFinance token, used to send funds to the treasury.
     */
    address constant public wbnb = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address constant public soak = address(0x849233FF1aea15D80EF658B2871664C9Ca994063);
    address constant public bifi = address(0xCa3F508B8e4Dd382eE878A314789373D80A5190A);

    /**
     * @dev Third Party Contracts:
     * {unirouter} - PancakeSwap unirouter
     * {masterchef} - MasterChef contract. Stake soak, get rewards.
     */
    address constant public unirouter  = address(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    address constant public masterchef = address(0x303961805A22d76Bac6B2dE0c33FEB746d82544B);
     ...
     * {soakToWbnbRoute} - Route we take to go from {soak} into {wbnb}.
    ...
    constructor(address _vault, address _strategist) public {
        ...
        IERC20(soak).safeApprove(masterchef, uint(-1));
        IERC20(soak).safeApprove(unirouter, uint(-1));
     ...
     * It deposits soak in the MasterChef to earn rewards in soak.
     */
    ...
    /**
     * @dev It withdraws soak from the MasterChef and sends it to the vault.
     */
    function withdraw(uint256 _amount) external {
        ...
        if (soakBal < _amount) {
            IMasterChef(masterchef).leaveStaking(_amount.sub(soakBal));
            soakBal = IERC20(soak).balanceOf(address(this));
        }
        ...
        if (tx.origin == owner()) {
            IERC20(soak).safeTransfer(vault, soakBal);
        } else {
            uint256 withdrawalFee = soakBal.mul(WITHDRAWAL_FEE).div(WITHDRAWAL_MAX);
            IERC20(soak).safeTransfer(vault, soakBal.sub(withdrawalFee));
        }
    }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(soak).balanceOf(address(this)).mul(6).div(100);
        IUniswapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWbnb, 0, soakToWbnbRoute, address(this), now.add(600));
        ...
        IUniswapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(treasuryHalf, 0, wbnbToBifiRoute, treasury, now.add(600));
    ...
    /**
     * @dev Function to calculate the total underlaying {soak} held by the strat.
     * It takes into account both the funds in hand, as the funds allocated in the MasterChef.
     */
    function balanceOf() public view returns (uint256) {
        return balanceOfsoak().add(balanceOfPool());
    }

    /**
     * @dev It calculates how much {soak} the contract holds.
     */
    function balanceOfsoak() public view returns (uint256) {
        return IERC20(soak).balanceOf(address(this));
    }

    /**
     * @dev It calculates how much {soak} the strategy has allocated in the MasterChef
     */
    ...
    function retireStrat() external {
        ...
        uint256 soakBal = IERC20(soak).balanceOf(address(this));
        IERC20(soak).transfer(vault, soakBal);
    }
    ...
    function pause() public onlyOwner {
        ...
        IERC20(soak).safeApprove(masterchef, 0);
