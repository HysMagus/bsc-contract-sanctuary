...
contract Strategy is BaseStrategy {
    ...
    IERC20 public EGGToken          = IERC20(0xF952Fc3ca7325Cc27D15885d37117676d25BfdA6);
    IPancakeFarm public farmer      = IPancakeFarm(0xe70E9185F5ea7Ba3C5d63705784D8563017f2E57);

    constructor(address _vault) public BaseStrategy(_vault) {
        require(address(want) == address(EGGToken),"wrong want token");
        EGGToken.safeApprove(address(farmer),type(uint256).max);
    }

    function name() external override view returns (string memory) {
        return "StrategyEGGStaking";
    }

    // returns balance of egg
    ...
    function pendingReward() public view returns (uint256) {
        return farmer.pendingEgg(pid,address(this));
    }

    function estimatedTotalAssets() public override view returns (uint256) {
        //Add the vault tokens + staked tokens from egg  contract
    ...
    function depositToFarm() internal {
        ...
        if (_wantAvailable > 0) {
            farmer.deposit(pid,_wantAvailable);
        }
    }
    ...
    function protectedTokens() internal override view returns (address[] memory) {
        ...
        protected[0] = address(EGGToken); // Staked egg tokens from governance contract
        return protected;
    }
}