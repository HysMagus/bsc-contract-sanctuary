...
// TestSwap with Governance.
contract TestSwap is BEP20('TestSwap', 'TEST') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BETA::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BETA::delegateBySig: invalid nonce");
        require(now <= expiry, "BETA::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BETA::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BETAs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BETA::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of Beta. He can make Beta and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once BETA is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BETAs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBetaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBetaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accBetaPerShare;   // Accumulated BETAs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint16 withdrawFeeBP;     // Withdraw fee in basis point
    }

    // The TEST TOKEN!
    TestSwap public beta;
    // Dev address.
    ...
    function pendingBeta(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accBetaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function  massUpdateFee(uint16 _depositFeeBP, uint16 _withdrawFeeBP) public onlyOwner {
        require(_depositFeeBP <= 10000, "set: invalid deposit fee basis points");
        require(_withdrawFeeBP <= 10000, "add: invalid withdraw fee basis points");

        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            poolInfo[pid].depositFeeBP = _depositFeeBP;
            poolInfo[pid].withdrawFeeBP = _withdrawFeeBP;
        }
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            if(pool.withdrawFeeBP > 0){
                uint256 wihdrawFee = _amount.mul(pool.withdrawFeeBP).div(10000);
                pool.lpToken.safeTransfer(feeAddress, wihdrawFee);
                pool.lpToken.safeTransfer(address(msg.sender), _amount.sub(wihdrawFee));
            }else{
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        if(pool.withdrawFeeBP > 0){
            uint256 wihdrawFee = amount.mul(pool.withdrawFeeBP).div(10000);
            pool.lpToken.safeTransfer(feeAddress, wihdrawFee);
            pool.lpToken.safeTransfer(address(msg.sender), amount.sub(wihdrawFee));
        }else{
    ...
    function emergencyWithdraw(uint256 _pid, address _user) public {
        ...
        if(pool.withdrawFeeBP > 0){
            uint256 wihdrawFee = amount.mul(pool.withdrawFeeBP).div(10000);
            pool.lpToken.safeTransfer(feeAddress, wihdrawFee);
            pool.lpToken.safeTransfer(address(_user), amount.sub(wihdrawFee));
        }else{
