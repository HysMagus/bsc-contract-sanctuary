...
// File: contracts\BroccoliToken.sol
pragma solidity ^0.6.12;

contract BroccoliToken is ERC20("Broccoli", "BROC"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (BroccoliMaster).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }

    // Copied and modified from Softdrink code:
    // https://github.com/softdrinkswap/softdrinkswap-contract
    // Which is copied and modified from YAM:
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BROT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BROT::delegateBySig: invalid nonce");
        require(now <= expiry, "BROT::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BROT::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BROT::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts\BroccoliMaster.sol
pragma solidity ^0.6.12;
...
contract BroccoliMaster is Ownable {
    ...
    //apy dolar x share = accBrocPerShare.div(1e12) * year block * dolarPriceBroco
    //apy % = apy dolar x share * 100 / total staked dolar
