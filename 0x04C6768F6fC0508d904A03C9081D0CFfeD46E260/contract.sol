...
contract YSLOptApe is YSLOpt {
    using SafeERC20 for IERC20;

    ///@notice ApeSwap router
    address public apeSwap;
    address public apeMaster;
    ...
    function initialize(
        ...
        apeMaster = 0x5c8D727b265DBAfaba67E050f2f739cAeEB4A6F9;
    }
    ...
    function compound() external override {
        ...
        if (lpBalance > DUST) {
            ...
            IPancakeMaster(apeMaster).deposit(id, lpToken.balanceOf(address(this)));
        }
    }
    ...
    function earn(address _user, bool _isAmplified) external override onlyRole(STRAT_ROLE) {
        ...
        uint256 usersCompUSD = IStrategySwap(stratSwap).swapLPToBusd(userCompounds, apeSwap, path0, path1, lpToken);
    ...
    function transferOut(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        IStrategySwap(stratSwap).swapLPToBusd(fee, apeSwap, path0, path1, lpToken);
        ...
        IStrategySwap(stratSwap).swapLPToBusd(_amount, apeSwap, path0, path1, lpToken);
    ...
    function transferIn(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        IStrategySwap(stratSwap).swapBusdToLP(_amount, apeSwap, path0, path1, lpToken);
    ...
    function getUserDepositedUSD(address _user) public view override returns (uint256 amount) {
        ...
        if (path0.length == 1) {
            return 2 * IStrategySwap(stratSwap).getBusdAmount(userDeposited[_user], path1, apeSwap);
        }
        if (path1.length == 1) {
            return 2 * IStrategySwap(stratSwap).getBusdAmount(userDeposited[_user], path0, apeSwap);
        }
        return IStrategySwap(stratSwap).getBusdAmount(userDeposited[_user], lpToken, apeSwap, path0, path1);
    }
    ...
    function getTotalDepositedUSD() public view returns (uint256 amount) {
        ...
        if (path0.length == 1) {
            return 2 * IStrategySwap(stratSwap).getBusdAmount(totalDeposited, path1, apeSwap);
        }
        if (path1.length == 1) {
            return 2 * IStrategySwap(stratSwap).getBusdAmount(totalDeposited, path0, apeSwap);
        }
        return IStrategySwap(stratSwap).getBusdAmount(totalDeposited, lpToken, apeSwap, path0, path1);
    }
    ...
    function withdrawAcc(uint256 _amount) internal override {
        uint256 totalCompounds = IPancakeMaster(apeMaster).userInfo(id, address(this)).amount;
        if (totalCompounds > 0) {
            IPancakeMaster(apeMaster).withdraw(id, _amount);
        }
    }

    function restorePool() public onlyRole(STRAT_ROLE) {
        uint256 totalCompounds = IPancakeMaster(apeMaster).userInfo(id, address(this)).amount;
        IPancakeMaster(apeMaster).withdraw(id, totalCompounds);
    }
