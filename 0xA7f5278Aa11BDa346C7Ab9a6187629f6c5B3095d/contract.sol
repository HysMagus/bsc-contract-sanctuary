...
// MintRich is a DeFi 2.0 token that pays you just for holding it.
// You dont need to interact with confusing interfaces or pay expensive gas fees.
// As soon as you hold MIRI in any non-custodial Binance Smart Chain wallet you will instantly start earning.
// Each MIRI TX has a 1% fee 0.5% is instantly split between MIRI holders and 0.5% is burned.
// Sit back and let your tokens earn you more.
// If you are looking for a cryptocurrency with a centralized team or CEO that will make partnerships, MIRI is not the token for you.
...
contract MintRich is Context, IERC20, Ownable {
    ...
    uint256 private constant _tInitial = 1 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tInitial));
    uint256 private _tFeeTotal;
    uint256 private _tBurned;
    uint256 private _startTime;
    address private multisig = address(0x139A6EFE3066B5dcb163a4a7Cf640B87244774bA);

    string private _name = 'Mint Rich';
    string private _symbol = 'MIRI';
    ...
    function _getInflatedSupply(uint256 initialSupply, uint256 startTime, uint256 currentTime) private pure returns(uint256) {
        return initialSupply.add(_getElapsedSeconds(startTime, currentTime).mul(1615413600));
    }
