// Leveraged BNB
// 3% of Each TX Taken for Adding to Venus Vault (BNB) which will buy back LevBNB
...
contract LevBNB is IERC20, Ownable, ReentrancyGuard {

    using SafeMath for uint256;

    mapping (address => mapping (address => uint256)) private _allowances;
    string private _name = "Leveraged BNB";
    string private _symbol = "LevBNB";
    uint8 private _decimals = 18;

    IPancakeRouter02 public constant cakeswapRouter = IPancakeRouter02(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    ...
    uint256 private constant _tTotal = 1 * 10**5 * 10**18;
    uint256 private _aTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    uint8 public feeDecimals = 0;
    uint32 public feePercentage = 3; // 3%
    uint256 taxToHolders = 100; // 1%
    ...
    constructor() public {
       ...
       _aOwned[_msgSender()] = _aTotal;
        ...
        cakeswapPair = IPancakeFactory(cakeswapRouter.factory())
            .createPair(address(this), cakeswapRouter.WETH());
    ...
    function sesVault(address _Treasury) public onlyOwner {
        VaultAddr = _Treasury;
    }
...
function _transfer(
    ...
    if (VaultTX > 0){
        if (_isExcluded[from] && !_isExcluded[to]) {
             _transferFromExcluded(from, VaultAddr, VaultTX);
        } else if (!_isExcluded[from] && _isExcluded[to]) {
             _transferToExcluded(from, VaultAddr, VaultTX);
        } else if (!_isExcluded[from] && !_isExcluded[to]) {
            _transferStandard(from, VaultAddr, VaultTX);
        } else if (_isExcluded[from] && _isExcluded[to]) {
            _transferBothExcluded(from, VaultAddr, VaultTX);
        } else {
            _transferStandard(from, VaultAddr, VaultTX);
        }
    }

        uint256 tokensToTransfer = amount.sub(VaultTX);
        if (_isExcluded[from] && !_isExcluded[to]) {
   ...
   receive () external payable {
    ...
    function updateFee(uint8 _feeDecimals, uint32 _feePercentage)
        ...
        nonReentrant
    {
        require(_feePercentage <= 5, "The fee can't be more than 5%");
    ...
    function changeTaxReturnedToHolders(uint256 n) external onlyOwner {
        require(n >= 50, "The Tax Returned to Holders not more than 2%");
