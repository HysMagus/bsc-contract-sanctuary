...
interface Swap {
    function swapExactTokensForETH(uint, uint, address[] calldata, address, uint) external;
}
...
contract FrogPool is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        path[0] = address(frog);
        path[1] = wbnb;
        Swap(router).swapExactTokensForETH(FrogFunds, uint256(0), path, address(this), now.add(1800));
        safeBNBTransfer(funds);
    ...
    // Safe bnb transfer function, transfer swapped BNB to the fund address.
    function safeBNBTransfer(address payable _to) internal {
