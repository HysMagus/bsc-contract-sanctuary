...
contract BURGERDistributor is IBURGERDistributor {
    ...
    // once the minimum distribution limit is hit after "minPeriod" it awards the reward
    uint256 public minPeriod = 30 minutes;
    uint256 public minDistribution = 0.2 * 10**18; // 0.2 BURGER minimum auto send
...
contract CAKEDistributor is ICAKEDistributor {
    ...
    // once the minimum distribution limit is hit after "minPeriod" it awards the reward
    uint256 public minPeriod = 30 minutes;
    uint256 public minDistribution = 0.3 * 10**18; // 0.3 CAKE minimum auto send
...
contract BAKEaBURGERCAKE is IERC20, Context, Ownable {
    ...
    uint256 minimumToDistribute = 0.1 * 10**18; // 0.1 BNB
    ...
    function swapBack() private swapping {
        ...
        uint256 liquifyAmount = swapThreshold.mul(dynamicLiquidityFee).div(totalFee).div(2);
        ...
        uint256 amountBNBLiquidity = address(this).balance.mul(dynamicLiquidityFee).div(totalFee.sub((dynamicLiquidityFee).div(2))).div(2);
    ...
    // Sets Various Fees
    function setFees(uint256 _burnFee, uint256 _liquidityFee, uint256 _BAKEreflectionFee, uint256 _BURGERreflectionFee, uint256 _CAKEreflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external onlyOwner {
        ...
        totalFee = _liquidityFee.add(_burnFee).add(_BAKEreflectionFee).add(_BURGERreflectionFee).add(_CAKEreflectionFee).add(_marketingFee);
