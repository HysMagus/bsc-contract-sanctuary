...
// File: contracts/BIFI/interfaces/monster/IMssRewardPool.sol
...
// File: contracts/BIFI/strategies/Monster/StrategyMssLP.sol
...
contract StrategyMssLP is Ownable, Pausable {
    ...
    function chargeFees() internal {
        ...
        IPancakeRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, mssToWbnbRoute, address(this), now.add(600));
    ...
    function addLiquidity() internal {   
        ...
        if (lpToken0 != mss) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(mssHalf, 0, mssToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != mss) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(mssHalf, 0, mssToLp1Route, address(this), now.add(600));
        }
