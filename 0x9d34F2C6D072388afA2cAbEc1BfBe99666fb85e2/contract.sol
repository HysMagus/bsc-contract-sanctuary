...
contract StratHomora is Ownable, ReentrancyGuard, Pausable {
        ...
        returns (uint256)
    {
        ...
        IAlpacaFarm.PoolInfo memory test = IAlpacaFarm(farmContractAddress).poolInfo(pid);
        uint256 accAlpacaPerShareTilBonusEnd = test.accAlpacaPerShareTilBonusEnd;
        ...
        returns (uint256)
    {
        ...
        if (isAutoComp) {
            IAlpacaFarm(farmContractAddress).withdraw(msg.sender, pid, _wantAmt);
        }
    ...
    function pendingBonusAlpaca(address _user) public view returns (uint256) {
        ...
        uint256 accAlpacaPerShareTilBonusEnd = IAlpacaFarm(farmContractAddress).poolInfo(pid).accAlpacaPerShareTilBonusEnd;
        uint bonusLockUpBps = IAlpacaFarm(farmContractAddress).bonusLockUpBps();
        
        return user.amount.mul(accAlpacaPerShareTilBonusEnd).div(1e12).sub(user.bonusDebt).mul(bonusLockUpBps);
    }
