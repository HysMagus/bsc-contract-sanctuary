...
contract MasterChef is Ownable {
    ...
    // The LBC TOKEN!
    LuckyBestCoin public lbc;
    // Dev address.
    address public devaddr;
    // LBC tokens created per block.
    uint256 public lbcPerBlock;
    // Bonus muliplier for early luckybestcoin makers.
    ...
    // The block number when LBC mining starts.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _lbcPerBlock,
        uint256 _startBlock
    ) public {
        lbc = _lbc;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        lbcPerBlock = _lbcPerBlock;
    ...
    // Update the given pool's LBC allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 lbcReward = multiplier.mul(lbcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(lbcReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 lbcReward = multiplier.mul(lbcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        lbc.mint(devaddr, lbcReward.div(10));
        lbc.mint(address(this), lbcReward);
        pool.accEggPerShare = pool.accEggPerShare.add(lbcReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe LBC transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 lbcBal = lbc.balanceOf(address(this));
        if (_amount > lbcBal) {
            lbc.transfer(_to, lbcBal);
        } else {
    ...
    function updateEmissionRate(uint256 _lbcPerBlock) public onlyOwner {
        massUpdatePools();
        lbcPerBlock = _lbcPerBlock;
    }
}