...
// File: contracts/BIFI/interfaces/bdollar/IShareRewardPool.sol
...
// File: contracts/BIFI/strategies/BDollar/StrategySbdoLP.sol
...
contract StrategySbdoLP is Ownable, Pausable {
    ...
    constructor(address _lpPair, uint8 _poolId, address _vault, address _strategist) public {
        ...
        IERC20(sbdo).safeApprove(unirouter, uint(-1));
    ...
    function chargeFees() internal {
        ...
        IPancakeRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, sbdoToWbnbRoute, address(this), now.add(600));
    ...
    function addLiquidity() internal {   
        ...
        if (lpToken0 != sbdo) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(sbdoHalf, 0, sbdoToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != sbdo) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(sbdoHalf, 0, sbdoToLp1Route, address(this), now.add(600));
        }
