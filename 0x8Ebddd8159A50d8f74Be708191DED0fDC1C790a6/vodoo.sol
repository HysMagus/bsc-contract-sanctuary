...
contract Voodoo is Ownable {
    ...
    function deposit(
        ...
        if (_amount > 0) {
            ...
            if (pool.fee > 0) {
                // if pool.fee = 10 ==>  10 * 10/10000 = 2% fee
                // if pool.fee = 5 ==>  10 * 5/10000 = 1% fee
                uint256 treasuryfee =
                    _amount.mul(pool.fee).mul(divPoolFee).div(10000);
                uint256 devfee =
                    _amount.mul(pool.fee).mul(divdevfee).div(10000);
    ...
    function updateFees(uint256 _devFee, uint256 _divPoolFee) public onlyOwner {
        require(_devFee <= MAX_FEE_ALLOWED && _divPoolFee <= MAX_FEE_ALLOWED);
        divdevfee = _devFee;
