...
// File: contracts\BambooToken.sol

pragma solidity 0.6.12;


// BambooToken with Governance.
contract BambooToken is BEP20("Bamboo Token", "BBOO") {
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber =
            safe32(
                block.number,
                "BAMBOO::_writeCheckpoint: block number exceeds 32 bits"
            );
...
// File: contracts\LeafBar.sol

pragma solidity 0.6.12;



// LeafBar with Governance.
contract LeafBar is BEP20('Leaf Token', 'LEAF') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    */
        ...
        external
    {
        ...
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "BBOO::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BBOO::delegateBySig: invalid nonce");
        require(now <= expiry, "BBOO::delegateBySig: signature expired");
        return _delegate(signatory, delegatee);
    }
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BBOO::getPriorVotes: not yet determined");
    ...
    {
        address currentDelegate = _delegates[delegator];
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BBOOs (not scaled);
        _delegates[delegator] = delegatee;
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BBOO::_writeCheckpoint: block number exceeds 32 bits");
