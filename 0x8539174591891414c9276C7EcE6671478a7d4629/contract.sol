...
// wasabiToken with Governance.
contract wasabiToken is BEP20('wasabi', 'wasabi') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "wasabi::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "wasabi::delegateBySig: invalid nonce");
        require(now <= expiry, "wasabi::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "wasabi::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying wasabis (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "wasabi::_writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChef is Ownable {
    ...
    // wasabi tokens created per block.
    ...
    // Update the given pool's wasabi allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 wasabiReward = multiplier.mul(wasabiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        wasabi.mint(devaddr, wasabiReward.div(10));
        wasabi.mint(address(this), wasabiReward);
        pool.accwasabiPerShare = pool.accwasabiPerShare.add(wasabiReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function updateEmissionRate(uint256 _wasabiPerBlock) public onlyOwner {
        massUpdatePools();
        wasabiPerBlock = _wasabiPerBlock;
    }
}