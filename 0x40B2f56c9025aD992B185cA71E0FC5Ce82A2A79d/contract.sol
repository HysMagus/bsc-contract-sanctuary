...
contract FishToken is ERC20("CryptoFish.Finance", "FISH"), Ownable {

    /// @notice Creates `_amount` token to `_to`. Must only be called by the FishPool.
...
contract FishPool is Ownable {
    ...
    // Bonus muliplier for early fish stakers.
    ...
    constructor(
        ...
        fish.approve(router, 0);

        fish.approve(router, uint(-1));
    ...
    function getRewardPerBlock(uint blockNumber) public view returns (uint256) {
        ...
        if (block.number > startBlock){
            ...
            return fishPerBlock/( 2**( ( blockNumber.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
    ...
    function getCurrentRewardPerBlock() public view returns (uint256) {
        ...
        if (block.number > startBlock){
            ...
            return fishPerBlock/( 2**( ( block.number.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
    ...
    function pendingFish(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 FishReward = multiplier.mul(fishPerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(halvFactor);

            accFishPerShare = accFishPerShare.add(FishReward.mul(1e12).div(lpSupply));
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 FishFunds = FishReward.div(20); // 5%

        fish.mint(address(this), FishReward);

        fish.mint(address(this), FishFunds);
        ...
        Swap(router).swapExactTokensForETH(FishFunds, uint256(0), path, funds, now.add(1800));

        pool.accFishPerShare = pool.accFishPerShare.add(FishReward.mul(1e12).div(lpSupply));

        pool.lastRewardBlock = block.number;

    }

    // Deposit LP tokens to StakingContract for FISH allocation.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            uint256 pending = user.amount.mul(pool.accFishPerShare).div(1e12).sub(user.rewardDebt);
