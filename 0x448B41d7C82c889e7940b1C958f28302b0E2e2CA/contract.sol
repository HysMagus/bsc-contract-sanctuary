...
// File: contracts/BIFI/interfaces/belt/IMasterBelt.sol
...
// File: contracts/BIFI/interfaces/belt/IBeltLP.sol
...
// File: contracts/BIFI/strategies/belt/StrategyBeltCakeLP.sol
...
/**
 * @dev Implementation of a strategy to get yields from farming LP Pools on Belt.
 */
contract StrategyBeltCakeLP is Ownable, Pausable {
     ...
     * {beltToLp0Route} - Route we take to get from {belt} into {lpToken0}.
     * {beltToLp1Route} - Route we take to get from {belt} into {lpToken1}.
     */
    ...
    constructor(address _lpPair, uint8 _poolId, address _vault, address _strategist) public {
        ...
        IERC20(lpPair).safeApprove(masterbelt, uint(-1));
     ...
     * It deposits {lpPair} in the MasterBelt to farm {belt}
     */
    function deposit() public whenNotPaused {
        uint256 pairBal = IERC20(lpPair).balanceOf(address(this));

        if (pairBal > 0) {
            IMasterBelt(masterbelt).deposit(poolId, pairBal);
        }
    }

    /**
     * @dev Withdraws funds and sends them back to the vault.
     * It withdraws {lpPair} from the MasterBelt.
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {
            IMasterBelt(masterbelt).withdraw(poolId, _amount.sub(pairBal));
     ...
     * 3. It swaps the {belt} token for {lpToken0} & {lpToken1}
    ...
    /**
     * @dev Swaps {belt} for {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {
        uint256 beltHalf = IERC20(belt).balanceOf(address(this)).div(2);

        if (lpToken0 != belt) {
            IUniswapRouter(unirouter).swapExactTokensForTokens(beltHalf, 0, beltToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != belt) {
            IUniswapRouter(unirouter).swapExactTokensForTokens(beltHalf, 0, beltToLp1Route, address(this), now.add(600));
        }
    ...
    /**
     * @dev It calculates how much {lpPair} the strategy has allocated in the MasterBelt
     */
    ...
    function pause() public onlyOwner {
        ...
        IERC20(lpPair).safeApprove(masterbelt, 0);
