...
contract OPSYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("OPSY_Dividend_Tracker", "OPSY_Dividend_Tracker") {
        claimWait = 14400;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "OPSY_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "OPSY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OPSY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "OPSY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "OPSY_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Opsya is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    OPSYDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Opsya", "OPSY") {
        BNBRewardsFee = 0;
        liquidityFee = 0;
        extraFeeOnSell = 0; // extra fee on sell
        MarketingFee = 2;
        MarketingWallet = payable(0x05ECc360b761E15aC4f98131F9Ca053Dad555489);  

        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee); // total fee transfer and buy

    	dividendTracker = new OPSYDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "OPSY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "OPSY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "OPSY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "OPSY: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "OPSY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "OPSY: Cannot update gasForProcessing to same value");
