...
contract NewMiniCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NewMiniCakeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NewMiniCake", "NewMiniCake") {
        ...
        dividendTracker = new NewMiniCakeDividendTracker();

        buyBackWallet = 0xE215a857327aE0E10B51b79bEd4aecB45104c4de;
    ...
    function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
        require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "NewMiniCake: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
        sellFeeIncreaseFactor = multiplier;
    }
    
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(marketingDivisor >= 0 && marketingDivisor <= 100, "NewMiniCake: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "NewMiniCake: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "NewMiniCake: The dividend tracker already has that address");

        NewMiniCakeDividendTracker newDividendTracker = NewMiniCakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "NewMiniCake: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 20, "NewMiniCake: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "NewMiniCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "NewMiniCake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NewMiniCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NewMiniCake: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "NewMiniCake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NewMiniCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NewMiniCake: Cannot update gasForProcessing to same value");
...
contract NewMiniCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("NewMiniCake_Dividend_Tracker", "NewMiniCake_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "NewMiniCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "NewMiniCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NewMiniCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "NewMiniCake_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours");
        require(newClaimWait != claimWait, "NewMiniCake_Dividend_Tracker: Cannot update claimWait to same value");
