...
// File: contracts/MusicalBeats.sol
...
// MusicalBeats are generative visual & audio art pieces. The recipe and instructions to re-create the visualization and music reside on Ethereum blockchain.
//
...
// - Run the getScriptAtIndex method in the MusicalBeats smart contract starting with parameter 0, this is will return a transaction hash
...
contract MusicalBeats is Ownable, ERC1155 {
    using SafeMath for uint256;
    ...
    address payable public mbOwner;
    ...
    uint256 constant MAX_SEEDS_SUPPLY = 30;
    uint256[30] public seedAtIndex;

    function getSeedIds() public view returns (uint256[] memory) {
        uint256[] memory ids = new uint256[](originalsMinted);
        for (uint256 i = 0; i < originalsMinted; ++i) {
            ids[i] = seedAtIndex[i];
        }
        return ids;
    }

    function getTokensSupply() public view returns (uint256[] memory) {
        uint256[] memory supplyArray = new uint256[](originalsMinted);
        for (uint256 i = 0; i < originalsMinted; ++i) {
            supplyArray[i] = seedToPrintsSupply(seedAtIndex[i]);
        }
        return supplyArray;
    }
    ...
    constructor(string memory _uri)
        public
        ERC1155("MusicalBeats", "mBEATS", _uri)
    {
        mbOwner = _msgSender();
    }
    ...
    function mint() public payable onlyWhenEnabled returns (uint256) {
        ...
        require(newOriginalsSupply <= MAX_SEEDS_SUPPLY, "Max supply reached");
        ...
        returns (uint256)
    {
        ...
        // Remaining 20% kept for contract
     ...
     */
    function getPrintPrice(uint256 printNumber)
        public
        pure
        returns (uint256 price)
    {
        ...
        if (printNumber < B) {
            price = (10**(B.sub(printNumber))).mul(decimals).div(
                11**(B.sub(printNumber))
            );
        } else if (printNumber == B) {
            price = decimals; // price = decimals * (A ^ 0)
        } else {
            price = (11**(printNumber.sub(B))).mul(decimals).div(
                10**(printNumber.sub(B))
            );
        }
        price = price.add(C.mul(printNumber));

        price = price.sub(D);
        price = price.mul(20 ether).div(decimals);
    }
    ...
    function getBurnPrice(uint256 supply) public pure returns (uint256 price) {
        ...
        price = (printPrice * 80) / 100; // 80 % of print price
    }
     ...
     * @param seed The seed/original NFT token id
     */
    function getPrintTokenIdFromSeed(uint256 seed)
        public
        pure
        returns (uint256)
    {
    ...
    |__________________________________*/
    function getScriptAtIndex(uint256 index)
        public
        view
        returns (string memory)
    {
    ...
    |__________________________________*/
    function _generateSeed(uint256 uniqueValue)
        internal
        view
        returns (uint256)
    {
        ...
        return
            uint256(
                (uint40(gridLength) << 32) |
                    (uint40(horizontalLever) << 24) |
                    (uint40(diagonalLever) << 16) |
                    (uint40(palette) << 8) |
                    uint40(innerShape)
            );
    }
    ...
    function updateScript(string memory _script, uint256 index)
        public
        onlyOwner
        onlyUnlocked
    {
