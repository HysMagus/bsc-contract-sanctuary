...
contract SousChef {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many FOAM tokens the user has provided.
        ...
        //
        // Whenever a user deposits or withdraws FOAM tokens to a pool. Here's what happens:
    ...
    // Info of each user that stakes Foam tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = foam.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 foamSupply = foam.balanceOf(address(this));
        if (foamSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(foamSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit Foam tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        foam.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Foam tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        foam.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        foam.safeTransfer(address(msg.sender), user.amount);
