...
// MeerkatToken with Governance.
contract MeerkatToken is ERC20("Meerkat Finance", "MKAT"), Ownable {
  ...
  constructor(uint256 _startReleaseBlock, uint256 _endReleaseBlock) public {
    ...
    manualMint(msg.sender, 300000e18);
  }
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no MKATs can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked MKATs can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some MKATs can be released
    else
    {
  ...
  function unlock() public {
    require(_locks[msg.sender] > 0, "no locked MKATs");
  ...
  // @dev move MKATs with its locked funds to another account
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "MKAT::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "MKAT::delegateBySig: invalid nonce");
    require(now <= expiry, "MKAT::delegateBySig: signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
    require(blockNumber < block.number, "MKAT::getPriorVotes: not yet determined");
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying MKATs (not scaled);
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "MKAT::_writeCheckpoint: block number exceeds 32 bits");
...
interface IFairLaunch {
  ...
  function pendingMkat(uint256 _pid, address _user) external view returns (uint256);
...
// FairLaunch is a smart contract for distributing MKAT by asking user to stake the ERC20-based token.
contract FairLaunch is IFairLaunch, Ownable {
  ...
  struct UserInfo {
    ...
    //
    // We do some fancy math here. Basically, any point in time, the amount of MKATs
    // entitled to a user but is pending to be distributed is:
    //
    //   pending reward = (user.amount * pool.accMkatPerShare) - user.rewardDebt
    //
    // Whenever a user deposits or withdraws Staking tokens to a pool. Here's what happens:
    //   1. The pool's `accMkatPerShare` (and `lastRewardBlock`) gets updated.
  ...
  struct PoolInfo {
    ...
    uint256 allocPoint; // How many allocation points assigned to this pool. MKATs to distribute per block.
    uint256 lastRewardBlock; // Last block number that MKATs distribution occurs.
    uint256 accMkatPerShare; // Accumulated MKATs per share, times 1e12. See below.
    uint256 accMkatPerShareTilBonusEnd; // Accumated MKATs per share until Bonus End.
  }

  // The Mkat TOKEN!
  MeerkatToken public mkat;
  // Dev address.
  address public devaddr;
  // MKAT tokens created per block.
  uint256 public mkatPerBlock;
  // Bonus muliplier for early mkat makers.
  uint256 public bonusMultiplier;
  // Block number when bonus MKAT period ends.
  ...
  // The block number when MKAT mining starts.
  ...
  */
  ...
  function setMkatPerBlock(uint256 _mkatPerBlock) public onlyOwner {
    mkatPerBlock = _mkatPerBlock;
  }
  ...
  function addPool(
    ...
    poolInfo.push(
      PoolInfo({
        ...
        accMkatPerShare: 0,
        accMkatPerShareTilBonusEnd: 0
      })
    );
  }

  // Update the given pool's MKAT allocation point. Can only be called by the owner.
  function setPool(
    uint256 _pid,
    uint256 _allocPoint,
    bool _withUpdate
  ...
  */
  ...
  // View function to see pending MKATs on frontend.
  function pendingMkat(uint256 _pid, address _user) external override view returns (uint256) {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 accMkatPerShare = pool.accMkatPerShare;
    uint256 lpSupply = IERC20(pool.stakeToken).balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 mkatReward = multiplier.mul(mkatPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accMkatPerShare = accMkatPerShare.add(mkatReward.mul(1e12).div(lpSupply));
    }
    return user.amount.mul(accMkatPerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function updatePool(uint256 _pid) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 mkatReward = multiplier.mul(mkatPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    mkat.mint(devaddr, mkatReward.div(10));
    mkat.mint(address(this), mkatReward);
    pool.accMkatPerShare = pool.accMkatPerShare.add(mkatReward.mul(1e12).div(lpSupply));
    // update accMkatPerShareTilBonusEnd
    if (block.number <= bonusEndBlock) {
      mkat.lock(devaddr, mkatReward.div(10).mul(bonusLockUpBps).div(10000));
      pool.accMkatPerShareTilBonusEnd = pool.accMkatPerShare;
    }
    if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 mkatBonusPortion = bonusEndBlock.sub(pool.lastRewardBlock).mul(bonusMultiplier).mul(mkatPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      mkat.lock(devaddr, mkatBonusPortion.div(10).mul(bonusLockUpBps).div(10000));
      pool.accMkatPerShareTilBonusEnd = pool.accMkatPerShareTilBonusEnd.add(mkatBonusPortion.mul(1e12).div(lpSupply));
    }
    pool.lastRewardBlock = block.number;
  }
  ...
  function deposit(address _for, uint256 _pid, uint256 _amount) public override {
    ...
    user.rewardDebt = user.amount.mul(pool.accMkatPerShare).div(1e12);
    user.bonusDebt = user.amount.mul(pool.accMkatPerShareTilBonusEnd).div(1e12);
  ...
  // Harvest MKATs earn from the pool.
  ...
  function _harvest(address _to, uint256 _pid) internal {
    ...
    uint256 pending = user.amount.mul(pool.accMkatPerShare).div(1e12).sub(user.rewardDebt);
    require(pending <= mkat.balanceOf(address(this)), "wtf not enough mkat");
    uint256 bonus = user.amount.mul(pool.accMkatPerShareTilBonusEnd).div(1e12).sub(user.bonusDebt);
    safeMkatTransfer(_to, pending);
    mkat.lock(_to, bonus.mul(bonusLockUpBps).div(10000));
  }
  ...
  function emergencyWithdraw(uint256 _pid) public {
    ...
    // Safe mkat transfer function, just in case if rounding error causes pool to not have enough MKATs.
  function safeMkatTransfer(address _to, uint256 _amount) internal {
    uint256 mkatBal = mkat.balanceOf(address(this));
    if (_amount > mkatBal) {
      mkat.transfer(_to, mkatBal);
    } else {
      mkat.transfer(_to, _amount);
    }
  }
