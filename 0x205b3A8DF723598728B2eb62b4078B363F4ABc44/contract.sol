...
contract MarketOracle is Ownable {
    ...
    uint private sbetBnbPrice0CumulativeLast;
    uint private sbetBnbPrice1CumulativeLast;
    uint32 private sbetBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _sbet_bnb;
    ...
    constructor(
        address __sbet_bnb,   // Address of the sbet-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _sbet_bnb = IUniswapV2Pair(__sbet_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        sbetBnbPrice0CumulativeLast = _sbet_bnb.price0CumulativeLast();
        sbetBnbPrice1CumulativeLast = _sbet_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, sbetBnbBlockTimestampLast) = _sbet_bnb.getReserves();
    ...
    // Get the average price of 1 sbet in the smallest BNB unit (18 decimals)
    function getsbetBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_sbet_bnb));

        require(_blockTimestamp != sbetBnbBlockTimestampLast, "GRV Last and current are equal");

        FixedPoint.uq112x112 memory sbetBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - sbetBnbPrice0CumulativeLast) / (_blockTimestamp - sbetBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, sbetBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        (sbetBnbPrice0CumulativeLast, sbetBnbPrice1CumulativeLast, sbetBnbBlockTimestampLast, sbetBnbAverage) = getsbetBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, sbetBnbAverage) = getsbetBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = sbetBnbAverage*(1e6) / busdBnbAverage;
