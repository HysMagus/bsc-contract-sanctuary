...
contract ABCSwap is IABCSwap, Ownable{
    ...
    function setPancakeRouterAddress(address newAddress) external onlyOwner returns(bool){
        require(newAddress != address(0),"ABCSwap.setPancakeRouterAddress: pancakeRouterAddress is zero address");
    ...
    function setWbnbAddress(address newAddress) external onlyOwner returns(bool){
        require(newAddress != address(0),"ABCSwap.setWbnbAddress: wbnbAddress is zero address");
    ...
    */ 
    ...
    function _swapNativeToOtherTokens(address outAddress1, address outAddress2, uint out1Percent) internal returns (bool){
        require(outAddress1 != address(0) || outAddress2 != address(0),"ABCSwap.swapTokenToOtherTokens: At least 1 to address is not zero address");
        
        uint fromAmount = msg.value;
        require(fromAmount > uint(0), "ABCSwap.swapTokenToOtherTokens: fromAmount should be greater than zero");
    ...
    function _swapTokenToNativeAndOtherToken(address fromAddress, uint fromAmount, address otherTokenAddress, uint nativePercent) internal returns (bool){
        require(fromAddress != address(0),"ABCSwap.swapTokenToOtherTokens: fromAddress is zero address");
        require(fromAddress != otherTokenAddress, "ABCSwap.swapTokenToOtherTokens: Can not swap to the same token1");
        require(fromAmount > uint(0), "ABCSwap.swapTokenToOtherTokens: fromAmount should be greater than zero");
        
        //Transfer token balance from user's address to ABCSwap address
        require(IBEP20(fromAddress).transferFrom(_msgSender(), address(this), fromAmount),"ABCSwap.swapTokenToOtherTokens: Can not transfer token to ABCSwap");
        ...
        if(nativePercent == 100){
            ...
            fromOtherAmount = fromAmount - fromNativeAmount; 
        }
        
        //Approve PancakeRouter to transfer fromAmount token of ABCSwap
        require(IBEP20(fromAddress).approve(_pancakeRouterAddress, fromAmount),
            "ABCSwap.swapTokenToOtherTokens: Can not approve PancakeRouter to transfer token from ABCSwap");
    ...
    function _swapTokenToOtherTokens(address fromAddress, uint fromAmount, address outAddress1, address outAddress2, uint out1Percent) internal returns (bool){
        require(fromAddress != address(0),"ABCSwap.swapTokenToOtherTokens: fromAddress is zero address");
        require(outAddress1 != address(0) || outAddress2 != address(0),"ABCSwap.swapTokenToOtherTokens: At least 1 to address is not zero address");
        require(fromAddress != outAddress1, "ABCSwap.swapTokenToOtherTokens: Can not swap to the same token1");
        require(fromAddress != outAddress2, "ABCSwap.swapTokenToOtherTokens: Can not swap to the same token2");
        require(fromAmount > uint(0), "ABCSwap.swapTokenToOtherTokens: fromAmount should be greater than zero");
        
        //Transfer token balance from user's address to ABCSwap address
        require(IBEP20(fromAddress).transferFrom(msg.sender, address(this), fromAmount),"ABCSwap.swapTokenToOtherTokens: Can not transfer token to ABCSwap");
