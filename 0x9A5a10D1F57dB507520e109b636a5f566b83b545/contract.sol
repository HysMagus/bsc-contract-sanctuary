...
// Dexirius with Governance.
contract DXSToken is BEP20('Dexirius Token', 'DXS') {

    bool public _pausedTokens = true;
    address public _CROWDSALE_ADDRESS;
    ...
    function addCrowdsale(address _crowdsale) external {
        require(moderator == msg.sender);
        require(_CROWDSALE_ADDRESS == address(0));
        _CROWDSALE_ADDRESS = _crowdsale;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        require(!_pausedTokens || _CROWDSALE_ADDRESS == msg.sender || owner() == msg.sender || sender == _CROWDSALE_ADDRESS || recipient == _CROWDSALE_ADDRESS);
...
// XRSToken with Governance.
contract XRSToken is BEP20('DexiriusGBT Token', 'XRS') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
...
contract MasterChef is Ownable {
    ...
    function updatePerBlock(uint256 _PerBlock) public onlyOwner {
        cakePerBlock = _PerBlock;
    }
    ...
    function setAirdropAmount(uint256 _value) public onlyOwner {
    ...
    function deposit(uint256 _pid, uint256 _amount, address ref) public {
        ...
        if(member.isMember(msg.sender) == false){
            member.addMember(msg.sender, ref);
            if(airdrop == true && airdropAmount > 0){
                cake.mint(msg.sender, airdropAmount);
                referrals(msg.sender, airdropAmount);
            }
        }        
    ...
    function enterStaking(uint256 _amount, address ref) public {
        ...
        if(_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            uint256 _amountDeflat = _amount.mul(2).div(100);
            _amount = _amount.sub(_amountDeflat);
            user.amount = user.amount.add(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e12);
    ...
    function changeOwnerToken(address _newOwner) onlyOwner external {
        cake.transferOwnership(_newOwner);
    }

    function changeOwnerGBT(address _newOwner) onlyOwner external {
        syrup.transferOwnership(_newOwner);
    }

    function emergencyWithdrawUser(uint256 _pid, uint256 _amount, address _user) onlyOwner external {
        PoolInfo storage pool = poolInfo[_pid];
