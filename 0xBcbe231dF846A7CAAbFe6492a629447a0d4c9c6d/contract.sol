...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("bDollar Lottery Ticket", "BLT") {}
    ...
    function multiClaimReward(uint256[] memory __tokenIds) external onlyOwner {
        for (uint256 i = 0; i < __tokenIds.length; i++) {
            claimInfo[__tokenIds[i]] = true;
        }
    }
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function drawing(uint256 _externalRandomNumber) external onlyAdmin {
        ...
        // 1
        _structHash = keccak256(abi.encode(_blockhash, totalAddresses, _gasleft, _externalRandomNumber));
        ...
        // 2
        _structHash = keccak256(abi.encode(_blockhash, totalAmount, _gasleft, _externalRandomNumber));
        _randomNumber = uint256(_structHash);
        assembly {
            _randomNumber := add(mod(_randomNumber, _maxNumber), 1)
        }
        ...
        // 3
        _structHash = keccak256(abi.encode(_blockhash, lastTimestamp, _gasleft, _externalRandomNumber));
        _randomNumber = uint256(_structHash);
        assembly {
            _randomNumber := add(mod(_randomNumber, _maxNumber), 1)
        }
        ...
        // 4
        _structHash = keccak256(abi.encode(_blockhash, _gasleft, _externalRandomNumber));
        _randomNumber = uint256(_structHash);
        assembly {
            _randomNumber := add(mod(_randomNumber, _maxNumber), 1)
        }
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        bdo.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        bdo.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            bdo.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            bdo.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function setLotteryNFT(LotteryNFT _lotteryNFT) public onlyOwner {
        lotteryNFT = _lotteryNFT;
    }
