...
contract LFIStrategy {
    ...
    /**
     * @dev Tokens Used:
     * {fortoken} - Token generated by staking our funds. In this case it's the FOR governance token.
     * {token} - Token that the strategy maximizes. The same token that users deposit in the vault.
    */
    ...
    /**
     * @dev LFI Contracts:
     * {governance} - Reward pool where the for token earnings will go.
     ...
     */
    constructor(address _token, address _vault, address _gov) public {
        token = _token;
        vault = _vault;
        governance = _gov;

        getName = string(
            abi.encodePacked("LFI:Strategy:",
     ...
     * in the strategy's vault contract. It provides whatever {token} it has available to be
     * lent out on Fortube.
     */
    ...
    /**
     * @dev Core function of the strat, in charge of collecting and re-investing governance.
     * 1. It claims governance from Fortube reward pool
     */
    ...
    /**
     * @dev It calculates how much {token} the strategy has allocated in Fortube.
     */
