...
contract DrawStake is Owned {
    ...
    address public MFI = 0x6C3d3F5734B305240E722bC82Ea874138b701585;
    ...
    function stake(uint256 tokens) external {
        require(IBEP20(MFI).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function drawToken(uint256 tokens) external {
        require(IBEP20(MFI).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function getReward() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IBEP20(MFI).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
    ...
    function withdraw(uint256 tokens) external {
        ...
        require(IBEP20(MFI).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedMFI(address staker) external view returns(uint256 stakedMFI){
        return stakers[staker].stakedTokens;
    }
    
    // ------------------------------------------------------------------------
    // Get the MFI balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourMFIBalance(address user) external view returns(uint256 MFIBalance){
        return IBEP20(MFI).balanceOf(user);
    }
}