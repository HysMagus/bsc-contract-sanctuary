...
contract BvaultsStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    uint8 public faasRewardLength = 0;
    ...
    constructor(
        ...
        if (isAutoComp) {
            ...
            faasRewards = _faasRewards;
            faasRewardLength = uint8(_faasRewards.length);
        }
    ...
    function _farm() internal {
        ...
        IStakePool(farmContractAddress).stake(wantAmt);
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        if (isAutoComp) {
            IStakePool(farmContractAddress).withdraw(_wantAmt);
        }
    ...
    function earn() public whenNotPaused {
        ...
        // Harvest farm tokens
        IStakePool(farmContractAddress).getAllRewards(address(this));

        // convert reward to earned
        for (uint8 i = 0; i < faasRewardLength; i++) {
            if (faasRewards[i] != earnedAddress) {
                uint256 amt = IERC20(faasRewards[i]).balanceOf(address(this));
                if (amt > 0) {
                    _vswapSwapToken(faasRewards[i], earnedAddress, amt);
                }
            }
        }
    ...
    function pendingHarvest() public view returns (uint256) {
        ...
        return IStakePool(farmContractAddress).pendingReward(0, address(this)).add(_earnedBal);
    }

    function pendingHarvestDollarValue() public view returns (uint256) {
        uint256 pendingHarvestDollar = 0;
        for (uint8 pid = 0; pid < faasRewardLength; pid++) {
            uint256 _rewardBal = IERC20(faasRewards[pid]).balanceOf(address(this));
            uint256 pendingReward = IStakePool(farmContractAddress).pendingReward(pid, address(this)).add(_rewardBal);
            pendingHarvestDollar = pendingHarvestDollar.add((pendingReward == 0) ? 0 : exchangeRate(faasRewards[pid], busdAddress, pendingReward));

        }
        return pendingHarvestDollar;
    }
    ...
    function setFaasRewards(address[] memory _faasRewards) external onlyOperator {
        faasRewardLength = uint8(_faasRewards.length);
        faasRewards = _faasRewards;
    }
