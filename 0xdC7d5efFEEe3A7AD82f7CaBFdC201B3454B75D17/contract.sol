...
// PsychicToken with Governance.
contract PSYCHICToken is ERC20("Psychic", "PSY"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "PSY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PSY::delegateBySig: invalid nonce");
        require(now <= expiry, "PSY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PSY::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PSY::_writeCheckpoint: block number exceeds 32 bits");
    ...
    // In some circumstance, we should not burn PSY on transfer, eg: Transfer from owner to distribute bounty, from depositing to swap for liquidity
...
interface IMigratorToPSYSwap {
    // Perform LP token migration from legacy UniswapV2 to PSYSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // PSYSwap must mint EXACTLY the same amount of PSYSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// PSYMasterFarmer is the master of PSY. He can make PSY and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PSY is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract PSYMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PSYs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPSYPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPSYPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. PSYs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that PSYs distribution occurs.
        uint256 accPSYPerShare; // Accumulated PSYs per share, times 1e12. See below.
    }
    ...
    // The PSY TOKEN!
    PSYCHICToken public psy;
    // Dev address.
    address public devaddr;
    // PSY tokens created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early PSY makers.
    ...
    // The block number when PSY mining starts.
    uint256 public START_BLOCK;

    uint256 public constant PERCENT_LOCK_BONUS_REWARD = 75; // lock 75% of bounus reward in 1 year
    uint256 public constant PERCENT_FOR_DEV = 15;

    uint256 public burnPercent = 0; // init 0% burn psy

    uint256 public lockFromBlock;
    uint256 public lockToBlock;

    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorToPSYSwap public migrator;
    ...
    event SendPSYReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    event Lock(address indexed to, uint256 value);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "PSYMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's PSY allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToPSYSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 psyForBurn;
        uint256 psyForDev;
        uint256 psyForFarmer;
        (psyForBurn, psyForDev, psyForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);

        if (psyForBurn > 0) {
            psy.mint(address(this), psyForBurn);
            psy.burn(psyForBurn);
        }

        if (psyForDev > 0) {
            // Mint unlocked amount for Dev
            psy.mint(devaddr, psyForDev.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
            // Mint locked amount for Dev
            //For more simple, I lock reward for dev if mint reward in bonus time
            psy.mint(address(this), psyForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            farmLock(devaddr, psyForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
        }
        psy.mint(address(this), psyForFarmer);
        pool.accPSYPerShare = pool.accPSYPerShare.add(psyForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forBurn, uint256 forDev, uint256 forFarmer) {
        ...
        uint256 psyCanMint = psy.cap().sub(psy.totalSupply());

        if (psyCanMint < amount) {
            forBurn = 0;
            forDev = 0;
            forFarmer = psyCanMint;
        }
        else {
    ...
    // View function to see pending PSYs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPSYPerShare = pool.accPSYPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 psyForFarmer;
            (, , psyForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accPSYPerShare = accPSYPerShare.add(psyForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accPSYPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPSYPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = psy.balanceOf(address(this));
            ...
            if(pending > 0) {
                psy.transfer(msg.sender, pending.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
                ...
                emit SendPSYReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accPSYPerShare).div(1e12);
        }
    }

    // Deposit LP tokens to PSYMasterFarmer for PSY allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "PSYMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from PSYMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "PSYMasterFarmer::withdraw: not good");
    ...
    // Safe psy transfer function, just in case if rounding error causes pool to not have enough PSYs.
    function safePSYTransfer(address _to, uint256 _amount) internal {
        uint256 psyBal = psy.balanceOf(address(this));
        if (_amount > psyBal) {
            psy.transfer(_to, psyBal);
        } else {
            psy.transfer(_to, _amount);
        }
    }
    ...
    function farmLock(address _holder, uint256 _amount) internal {
        require(_holder != address(0), "ERC20: lock to the zero address");
        require(_amount <= psy.balanceOf(address(this)), "ERC20: lock amount over blance");
        _locks[_holder] = _locks[_holder].add(_amount);
        _totalLock = _totalLock.add(_amount);
        if (_lastUnlockBlock[_holder] < lockFromBlock) {
            _lastUnlockBlock[_holder] = lockFromBlock;
    ...
    function unlock() public {
        ...
        uint256 amount = canUnlockAmount(msg.sender);
        // just for sure
        if (amount > psy.balanceOf(address(this))) {
            amount = psy.balanceOf(address(this));
        }
        psy.transfer(msg.sender, amount);
        _locks[msg.sender] = _locks[msg.sender].sub(amount);
        _lastUnlockBlock[msg.sender] = block.number;
        _totalLock = _totalLock.sub(amount);
    }
    ...
    function setTransferBurnRate(uint256 _tranferBurnRate) public onlyOwner {
        psy.setTransferBurnRate(_tranferBurnRate);
    }

    // In some circumstance, we should not burn PSY on transfer, eg: Transfer from owner to distribute bounty, from depositing to swap for liquidity
    function addTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        psy.addTransferBurnExceptAddress(_transferBurnExceptAddress);
    }

    function removeTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        psy.removeTransferBurnExceptAddress(_transferBurnExceptAddress);
    }
}