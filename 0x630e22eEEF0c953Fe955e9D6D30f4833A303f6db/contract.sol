...
contract VotePool is StakingPool {
    bytes32 internal constant _proposal_ = 'proposal';
    
    function stake(uint256) virtual override public {
        revert('obsolete, replace by stakeAndVote');
    }

    function stakeAndVote(uint256 amount, uint256 proposalID) virtual public {
        super.stake(amount);
        _setConfig(_proposal_, proposalID, getConfigI(_proposal_, proposalID).add(amount));
        config[bytes32(uint(_proposal_) ^ proposalID ^ uint(msg.sender))] += amount;
        emit StakeAndVote(amount, proposalID);
    }
    event StakeAndVote(uint256 amount, uint256 proposalID);

    function withdraw(uint256 amount) virtual override public {
        require(now >= periodFinish, "it's not time yet");
        super.withdraw(amount);
    }
    
    function getVotes(address voter, uint256 proposalID) virtual public view returns (uint256) {
        if(voter == address(0) && proposalID == 0)
            return _totalSupply;
        else
            return config[bytes32(uint(_proposal_) ^ proposalID ^ uint(msg.sender))];
    }
}
