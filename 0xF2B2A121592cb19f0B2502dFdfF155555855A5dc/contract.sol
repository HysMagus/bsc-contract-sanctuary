...
// import "./PMPToken.sol";
abstract contract PMPToken is ERC20 {
...
contract PMPFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        // We do some fancy math here. Basically, any point in time, the amount of PMP
        // entitled to a user but is pending to be distributed is:
        //
        //   amount = user.shares / sharesTotal * wantLockedTotal
        //   pending reward = (amount * pool.accPMPPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws want tokens to a pool. Here's what happens:
        //   1. The pool's `accPMPPerShare` (and `lastRewardBlock`) gets updated.
    ...
    uint256 public ownerPMPReward = 111; // ~ 10 owner reward %
    ...
    // Update the given pool's PMP allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending PMP on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        PMPToken(PMPTokenAddress).mint(
            owner(),
            PMPReward.mul(ownerPMPReward).div(1000)
        );
        PMPToken(PMPTokenAddress).mint(address(this), PMPReward);

        pool.accPMPPerShare = pool.accPMPPerShare.add(
            PMPReward.mul(1e12).div(sharesTotal)
        );
        pool.lastRewardBlock = block.number;
    }

    // Want tokens moved from user -> PMPFarm (PMP allocation) -> Strat (compounding)
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (user.shares > 0) {
            ...
            if (pending > 0) {
                safePMPTransfer(msg.sender, pending);
            }
        }
        if (_wantAmt > 0) {
    ...
    // Safe PMP transfer function, just in case if rounding error causes pool to not have enough
    ...
    function inCaseTokensGetStuck(address _token, uint256 _amount)
        ...
        onlyOwner
    {
        require(_token != PMPTokenAddress, "!safe");
