...
contract randompow is Owned {
	...
	constructor(ERC20Interface _raptor, uint256 _ticketPrice, address feeOwner) Owned(feeOwner) {
		chainId = _chainId();
	...
	function deductTicketPrice(address guy) internal {
		drawPrize();
		RAPTOR.transferFrom(guy,address(this), (ticketPrice*80)/100);
		RAPTOR.transferFrom(guy,address(this), (ticketPrice/20)/100);
	}
	...
	function drawPrize() public {
		if ((block.timestamp - lastDraw) >= drawTimeInSeconds) {
			round[currentDraw].jackpot = ((RAPTOR.balanceOf(address(this)))/100);
