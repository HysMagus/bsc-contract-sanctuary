...
contract Strategy is BaseStrategy {
    ...
    address internal VAI        = 0x4BD17003473389A42DAF6a0a729f6Fdb328BbBd7;
    address internal XVS        = 0xcF6BB5389c92Bdda8a3747Ddb454cB7a64626C63;
    ...
    IERC20 public VAIToken      = IERC20(VAI);
    IERC20 public XVSToken      = IERC20(XVS);

    IVAIVault public vaiVault   = IVAIVault(0x0667Eed0a0aAb930af74a3dfeDD263A73994f216);
    IUniswapV2Router public router  = IUniswapV2Router(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);

    address[] public xvsToVaiPath;

    constructor(address _vault) public BaseStrategy(_vault) {
        require(address(want) == address(VAIToken),"wrong want token");
        //Approve staking contract to spend VAI tokens
        VAIToken.safeApprove(address(vaiVault),type(uint256).max);
        XVSToken.safeApprove(address(router),type(uint256).max);
        //Initiate with default xvs to vai path
        xvsToVaiPath = new address[](4);
        xvsToVaiPath[0] = XVS;
        xvsToVaiPath[1] = WBNB;
        xvsToVaiPath[2] = BUSD;
        xvsToVaiPath[3] = VAI;
    ...
    function name() external override view returns (string memory) {
        return "StrategyXVSVAI";
    }

    // returns balance of VAI
    ...
    function balanceOfStake() public view returns (uint256) {
        return vaiVault.userInfo(address(this)).amount;
    }

    function pendingReward() public view returns (uint256) {
        if(vaiVault.pendingXVS(address(this)) == 0) return 0;
        uint256[] memory amounts = router.getAmountsOut(vaiVault.pendingXVS(address(this)), xvsToVaiPath);
        return amounts[amounts.length - 1];
    }

    function estimatedTotalAssets() public override view returns (uint256) {
        //Add the vault tokens + staked tokens from VAI Vault
        return balanceOfWant().add(balanceOfStake()).add(pendingReward());
    }

    function harvestRewards() internal {
        vaiVault.claim();
        if(XVSToken.balanceOf(address(this)) > 0) {
            //Swap To VAI
            router.swapExactTokensForTokens(
                XVSToken.balanceOf(address(this)),
                0,
                xvsToVaiPath,
                address(this),
                block.timestamp
    ...
    function setRouter(address routerNew) external onlyStrategist {
        //Remove old router approve
        XVSToken.safeApprove(address(router),0);
        XVSToken.safeApprove(address(routerNew),type(uint256).max);
        router = IUniswapV2Router(routerNew);
    }

    function setSwapPath(address[] memory swapPathNew) external onlyStrategist {
        xvsToVaiPath = swapPathNew;
    }
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (_wantAvailable > 0) {
            vaiVault.deposit(_wantAvailable);
        }
    }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > balanceWant) {
            ...
            vaiVault.withdraw((Math.min(balanceStaked, _amountNeeded - balanceWant)));
        }
    ...
    function prepareMigration(address _newStrategy) internal override {
        ...
        vaiVault.withdraw(balanceOfStake());
    }
    ...
    function protectedTokens() internal override view returns (address[] memory) {
        ...
        protected[0] = address(XVSToken); // XVS Rewards
        return protected;
    }
}