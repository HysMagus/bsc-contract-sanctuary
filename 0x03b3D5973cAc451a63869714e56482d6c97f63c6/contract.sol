...
// File: contracts/ICommonNftMaster.sol

pragma solidity =0.6.6;

interface ICommonNftMaster {
    event AddPool(
        address indexed user,
        address indexed _pair,
        uint256 _rewardStartBlock,
        uint256 _rewardEndBlock,
        uint256 _accSharePerNFT
    );
    event SetPool(
        address indexed user,
        address indexed _pair,
    ...
    event RemovePool(address indexed user, address indexed _pair);
    ...
    function removePool(address _pair) external;

    function poolAddresses() external view returns (address[] memory pools);

    function pendingShare(address _pair, address _user) external view returns (uint256);
...
// File: contracts/CommonNftMasterFactory.sol
...
contract CommonNftMasterFactory is Ownable {
    address public logicImplement;

    event CommonNftMasterCreated(address indexed commonNftMaster, address indexed logicImplement);
    ...
    function createCommonNftMaster(
        address _nftToken,
        bool _oneByOne,
        address owner,
        address proxyAdmin
    ) external onlyOwner returns (address) {
        TransparentUpgradeableProxy proxyCommonNftMaster =
            new TransparentUpgradeableProxy(logicImplement, proxyAdmin, '');
        ICommonNftMaster commonNftMaster = ICommonNftMaster(address(proxyCommonNftMaster));
        commonNftMaster.initialize(_nftToken, _oneByOne, owner);
        emit CommonNftMasterCreated(address(commonNftMaster), logicImplement);
        return address(commonNftMaster);
    }
