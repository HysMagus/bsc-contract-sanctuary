...
contract D100Presale is Ownable, ReentrancyGuard, ContractGuard {
    ...
    uint256 public presaleCapInBUSD = 13 * 1e23; // 1300k BUSD
    uint256 public maxPurchasableTokenPerWallet = 2020 * 1e9; // 5000 D100 considering 5% JETS bonus

    address public D100Token;
    uint256 public salePrice = 26E17; // 2.6 BUSD
    ...
    modifier isNotOver() {
        require(!finishPresaleByAdmin, "D100_PRESALE:presale is over");
        ...
        require(IBEP20(BusdToken).balanceOf(address(this)) < presaleCapInBUSD, "D100_PRESALE: sold out already");
        _;
    }
    modifier isOver() {
        require(finishPresaleByAdmin || block.timestamp < startTime || block.timestamp > endTime || IBEP20(BusdToken).balanceOf(address(this)) >= presaleCapInBUSD, "D100_PRESALE:presale not over yet");
        _;
    }
        ...
        isOver
    {
        for(uint i=0;i<purchasers.length;i++){
            ...
            IBEP20(D100Token).transfer(account, amount);
        }
