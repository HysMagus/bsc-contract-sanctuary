...
How Do the Romulans Operate?

---------------------------------------------------------------------------------------
#1. Romulans are a race based on rebases.
[Expands Supply with Volume]
---------------------------------------------------------------------------------------
When more Romulans are called into battle (increasing volume), the rebase is positive.
When Romulas stop responding to battles (decreasing volume), the rebase is negative.
---------------------------------------------------------------------------------------

---------------------------------------------------------------------------------------
#2. Romulans die in battle, and the more that go into battle, the more die in battle.
[Decreases supply on each TX and increases burn with more volume]
---------------------------------------------------------------------------------------
When more Romulans are called into battle (increasing volume), the death rate is higher.
When Romulas stop responding to battles (decreasing volume), the death rate is lower.
---------------------------------------------------------------------------------------

---------------------------------------------------------------------------------------
#3. Romulans rebase their military ranks often, once per hour, and in a Public() fashion.
---------------------------------------------------------------------------------------
The Rebase trigger can be called by anyone on the BSC network, once per hour.
-----------------
[Rebase Operation:]
-----------------
1. User Calls Rebase(); -> Romulans Token Contract.address
2. Rebase Trigger Adjusts Supply to Match Current Volume Rates (Sampled Hourly)
3. Rebase Distributes 50% of Dead Romulans Back into Service via Partner Pairs.
3. Rebase Trigger Syncs PancakeSwap Pairs.
4. Rebase Trigger Pauses Transfers for 1 Minute to ensure proper balance updates.
5. Rebase Timer Reset to 1 Hour Countdown Before Allowing Public Call
---------------------------------------------------------------------------------------
...
contract ERC20 is Context, IERC20 {
    ...
    uint256 private _totalSupply = 10 * 10**4 * 10**18;
...
contract ERC20ElasticTransferBurn is ERC20TransferBurn {
    ...
    uint256 private _elasticMultiplier = 1;
...
// File: contracts/Romulans.sol

pragma solidity ^0.6.2;


contract TheRomulans is ERC20ElasticTransferBurn("The Romulans", "ROMS"), Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _iSupply = 10 * 10**4 * 10**18;

    constructor() public {
        // set to initial values
        _setBurnDivisor(100);
        _setElasticMultiplier(1);

        // freeze transfers for 1 minutes after rebase
        // to mitigate users transferring wrong amounts
        transferAfterRebaseFreezeTime = 1 minutes;

        transferVolumeNowBucketTimestamp = getTransferVolumeNowBucketTimestamp();
        _mint(_msgSender(), _iSupply);
    }
    ...
    function rebase() public {
        ...
        // set elastic modifier to 10x the burn rate (10 * 100 / burnDivisor)
    ...
    uint256 public constant maxBurnPercent = 5;
    ...
    // mint half the burn to the pair to incentivize liquidity
    // swapping or providing liquidity on any other ROMS pairs will cause
    // loss of funds after every rebase
    address public ROMBNB_Pair;
    address public ROMBORG_Pair;
    address public ROMAutoLP_Pair;
    address public ROMS_Treasury = 0x3Af9D8B2BbE76B25464b08d2630dd93a9C632429;
    ...
    function setUniswapPairs(address _ROMBNB_Pair, address _ROMBORG_Pair, address _ROMAutoLP_Pair) public virtual onlyOwner {
        ROMBNB_Pair = _ROMBNB_Pair;
        ROMBORG_Pair = _ROMBORG_Pair;
        ROMAutoLP_Pair = _ROMAutoLP_Pair;
    }
    ...
    function mintToUniswapPairs(uint256 uniswapPairRewardAmount) internal {
        ...
        // reward is half of the burn split 4 ways
        uint256 amountPerPair = uniswapPairRewardAmount.div(4);
        if (uniswapPairRewardAmount == 0) {
            return;
        }
        if (ROMBNB_Pair != address(0)) {
            _mint(ROMBNB_Pair, amountPerPair);
        }
        if (ROMBORG_Pair != address(0)) {
            _mint(ROMBORG_Pair, amountPerPair);
        }
        if (ROMAutoLP_Pair != address(0)) {
            _mint(ROMAutoLP_Pair, amountPerPair);
        }
        _mint(ROMS_Treasury, amountPerPair);
    }
    ...
    function syncUniswapPairs() internal {
        if (ROMBNB_Pair != address(0)) {
            IPancakePair(ROMBNB_Pair).sync();
        }
        if (ROMBORG_Pair != address(0)) {
            IPancakePair(ROMBORG_Pair).sync();
        }
        if (ROMAutoLP_Pair != address(0)) {
            IPancakePair(ROMAutoLP_Pair).sync();
        }
    }

    // called by owner to airdrop for BORG-ETH and KLING Holders.
    function AirDropforBORGETH_MultiSender(address[] memory recipients, uint256[] memory amounts) public onlyOwner {
        for (uint i = 0; i < recipients.length; i++) {
            _mint(recipients[i], amounts[i]);
        }
    }

    // called by owner to airdrop for BORG-ETH and KLING Holders. 111 ROMS
    function AirDropforBORGETH_Subscribers(address[] memory recipients) public onlyOwner {
        for (uint i = 0; i < recipients.length; i++) {
            _mint(recipients[i], 111000000000000000000);
        }
    }

    // called by owner to airdrop for BORG-ETH and KLING Holders.
    function AirDropforBORGETH_toOneUser(address recipient, uint256 amount) public onlyOwner {
          _mint(recipient, amount);
    }

    // called by owner to airdrop for BORG-ETH and KLING Holders. 111 ROMS.
    function AirDropforBORGETH_Subscriber111(address recipient) public onlyOwner {
          _mint(recipient, 111000000000000000000);
    }
    ...
    function balanceInUniswapPair(address user, address uniswapPair) public view returns (uint256) {
        ...
        uint256 userLpBalance = IPancakePair(uniswapPair).balanceOf(user);
        if (userLpBalance == 0) {
            return 0;
        }
        uint256 lpTotalSupply = IPancakePair(uniswapPair).totalSupply();
    ...
    function balanceInUniswapPairs(address user) public view returns (uint256) {
        return balanceInUniswapPair(user, ROMBNB_Pair)
            .add(balanceInUniswapPair(user, ROMBORG_Pair))
            .add(balanceInUniswapPair(user, ROMAutoLP_Pair));
    }
