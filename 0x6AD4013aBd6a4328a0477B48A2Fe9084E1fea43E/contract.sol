...
contract TENFI_WBNB_Pool is Ownable {
    ...
    address public LPtokenAddress = 0x5817911b1647baF3b30B947f544954F001622856;
    ...
    function getPendingDivs(address _holder) public view returns (uint256 _pendingDivs) {
        ...
        if (block.timestamp <= farmEnableat + 1 days) {
            uint256 pendingDivs = stakedAmount.mul(333300000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 1 days && block.timestamp <= farmEnableat + 2 days) {
            uint256 pendingDivs = stakedAmount.mul(306600000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 2 days && block.timestamp <= farmEnableat + 3 days) {
            uint256 pendingDivs = stakedAmount.mul(282100000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 3 days && block.timestamp <= farmEnableat + 4 days) {
            uint256 pendingDivs = stakedAmount.mul(259500000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 4 days && block.timestamp <= farmEnableat + 5 days) {
            uint256 pendingDivs = stakedAmount.mul(238800000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 5 days && block.timestamp <= farmEnableat + 6 days) {
            uint256 pendingDivs = stakedAmount.mul(219700000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 6 days && block.timestamp <= farmEnableat + 7 days) {
            uint256 pendingDivs = stakedAmount.mul(202100000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 7 days && block.timestamp <= farmEnableat + 8 days) {
            uint256 pendingDivs = stakedAmount.mul(185900000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 8 days && block.timestamp <= farmEnableat + 9 days) {
            uint256 pendingDivs = stakedAmount.mul(171100000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 9 days && block.timestamp <= farmEnableat + 10 days) {
            uint256 pendingDivs = stakedAmount.mul(157400000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 10 days && block.timestamp <= farmEnableat + 11 days) {
            uint256 pendingDivs = stakedAmount.mul(144800000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 11 days && block.timestamp <= farmEnableat + 12 days) {
            uint256 pendingDivs = stakedAmount.mul(133200000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 12 days && block.timestamp <= farmEnableat + 13 days) {
            uint256 pendingDivs = stakedAmount.mul(122500000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 13 days && block.timestamp <= farmEnableat + 14 days) {
            uint256 pendingDivs = stakedAmount.mul(112700000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 14 days && block.timestamp <= farmEnableat + 15 days) {
            uint256 pendingDivs = stakedAmount.mul(1037).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 15 days && block.timestamp <= farmEnableat + 16 days) {
            uint256 pendingDivs = stakedAmount.mul(95400000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 16 days && block.timestamp <= farmEnableat + 17 days) {
            uint256 pendingDivs = stakedAmount.mul(88700000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 17 days && block.timestamp <= farmEnableat + 18 days) {
            uint256 pendingDivs = stakedAmount.mul(80800000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 18 days && block.timestamp <= farmEnableat + 19 days) {
            uint256 pendingDivs = stakedAmount.mul(74300000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 19 days && block.timestamp <= farmEnableat + 20 days) {
            uint256 pendingDivs = stakedAmount.mul(68400000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 20 days && block.timestamp <= farmEnableat + 21 days) {
            uint256 pendingDivs = stakedAmount.mul(62900000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 21 days && block.timestamp <= farmEnableat + 22 days) {
            uint256 pendingDivs = stakedAmount.mul(57900000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 22 days && block.timestamp <= farmEnableat + 23 days) {
            uint256 pendingDivs = stakedAmount.mul(53200000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 23 days && block.timestamp <= farmEnableat + 24 days) {
            uint256 pendingDivs = stakedAmount.mul(49000000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 24 days && block.timestamp <= farmEnableat + 25 days) {
            uint256 pendingDivs = stakedAmount.mul(45100000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 25 days && block.timestamp <= farmEnableat + 26 days) {
            uint256 pendingDivs = stakedAmount.mul(41500000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 26 days && block.timestamp <= farmEnableat + 27 days) {
            uint256 pendingDivs = stakedAmount.mul(38100000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 27 days && block.timestamp <= farmEnableat + 28 days) {
            uint256 pendingDivs = stakedAmount.mul(35100000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 28 days && block.timestamp <= farmEnableat + 29 days) {
            uint256 pendingDivs = stakedAmount.mul(32300000).mul(timeDiff).div(rewardInterval).div(1e4);
            
            return pendingDivs;
            
        } else if (block.timestamp > farmEnableat + 29 days && block.timestamp <= farmEnableat + 30 days) {
            uint256 pendingDivs = stakedAmount.mul(29700000).mul(timeDiff).div(rewardInterval).div(1e4);
