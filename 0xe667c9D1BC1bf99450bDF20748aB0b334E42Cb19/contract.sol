// SPDX-License-Identifier: lgpl-v3-or-later
...
contract CheeseStake is Owned {
    
    //initializing safe computations
    using SafeMath for uint;

    //CHS contract address
    address public chs;
    //total amount of staked chs
     ...
     * transfers CHS from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IBEP20(chs).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough CHS to pay registration fee.");
        //makes sure smart contract transfers CHS from user
        require(IBEP20(chs).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked CHS amount in the pool
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IBEP20(chs).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IBEP20(chs).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient CHS balance in pool');
        ...
        IBEP20(chs).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IBEP20(chs).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IBEP20(chs).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient CHS balance in pool');
        //transfers _amount to _address
        IBEP20(chs).transfer(msg.sender, _amount);
        //emit event
