...
contract JubyToken is BEP20('JubySwap Token', 'JUBY') {
...
contract MasterChef is Ownable {
    ...
    // The JUBY TOKEN!
    JubyToken public juby;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public jubyPerBlock;
    // Bonus muliplier for early juby makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _jubyPerBlock,
        uint256 _startBlock
    ) public {
        juby = _juby;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        jubyPerBlock = _jubyPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 jubyReward = multiplier.mul(jubyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(jubyReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 jubyReward = multiplier.mul(jubyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        juby.mint(devaddr, jubyReward.div(10));
        juby.mint(address(this), jubyReward);
        pool.accEggPerShare = pool.accEggPerShare.add(jubyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe juby transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 jubyBal = juby.balanceOf(address(this));
        if (_amount > jubyBal) {
            juby.transfer(_to, jubyBal);
        } else {
            juby.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _jubyPerBlock) public onlyOwner {
        massUpdatePools();
        jubyPerBlock = _jubyPerBlock;
    }
}