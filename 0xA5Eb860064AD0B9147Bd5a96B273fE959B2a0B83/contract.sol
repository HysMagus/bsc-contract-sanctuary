...
// File: contracts/adapters/IVErc20.sol
...
// File: contracts/adapters/BUSDVenusAdapter.sol
...
contract BUSD_Venus_Adapter is ISaffronAdapter {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  address public governance;                    // Governance address
  ISaffronPool public saffron_pool;             // SaffronPool that owns this adapter
  IERC20 public BUSD;                            // Base asset (BUSD)
  IVErc20 public vBUSD;                          // vBUSD redeemable for BUSD
  string public constant platform = "Venus"; // Platform name
  string public constant name = "BUSD/Venus"; // Adapter name

  constructor(address VErc20_contract_address, address base_asset_address) {
    require(VErc20_contract_address != address(0x0) && base_asset_address != address(0x0), "can't construct with 0x0 address");
    governance   = msg.sender;
    vBUSD         = IVErc20(VErc20_contract_address);
    BUSD          = IERC20(base_asset_address);
  }

  function set_pool(address pool) external override {
    require(msg.sender == governance, "must be governance");
    require(pool != address(0x0), "can't set pool to 0 address");
    saffron_pool = ISaffronPool(pool);
  }

  // Called by anyone
  event DeployCapital(uint256 vBUSD_after, uint256 vBUSD_added, uint256 exchange_rate_current);
  function deploy_capital(uint256 amount) external override {
    BUSD.safeApprove(address(vBUSD), amount); // Approve the transfer
    uint256 vBUSD_before = vBUSD.balanceOf(address(this));
    uint mint_result = vBUSD.mint(amount);   // Mint the cTokens and assert there is no error
    require(mint_result == 0, "compound returned nonzero response");
    uint256 vBUSD_after = vBUSD.balanceOf(address(this));
    require(vBUSD_after > vBUSD_before, "compound error"); // Big problem
    emit DeployCapital(vBUSD_after, vBUSD_after.sub(vBUSD_before), vBUSD.exchangeRateCurrent());
  }

  // Called from remove_liquidity
  event ReturnCapital(uint256 cdai_balance, uint256 amount, uint256 ctokens_redeemed, uint256 exchange_rate);
  function return_capital(uint256 amount, address to) external override {
    require(msg.sender == address(saffron_pool), "must be pool");
    uint256 redeem_result = vBUSD.redeemUnderlying(amount);
    require(redeem_result == 0, "compound returned nonzero response");
    BUSD.safeTransfer(to, amount);
  }

  function get_underlying_exchange_rate() external override returns(uint256) {
    return vBUSD.exchangeRateCurrent();
  }

  // WARNING: holdings expressed in 18 decimals (vBUSD only has 8 decimals)
  function get_holdings() external override returns(uint256) {
    uint256 holdings = vBUSD.balanceOf(address(this)).mul(vBUSD.exchangeRateCurrent()).div(10**18);
    return holdings;
  }
  ...
  function get_ctoken_balance() public view returns(uint256) {
    return vBUSD.balanceOf(address(this));
  }
