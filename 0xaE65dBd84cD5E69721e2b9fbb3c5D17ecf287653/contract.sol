...
contract Master is Ownable {
    ...
    // Market oracle provides the GRM/USD exchange rate as an 18 decimal fixed point number.
    ...
    constructor(address _grm) public {
    ...
    function rebase() public {
        ...
        uint256 supplyAfterRebase = grm.rebase(epoch, supplyDelta);
    ...
    function adjustTargetRate() external
    {
      ...
      if (exchangeRate > targetRate){
        targetRate = exchangeRate.mul(8125000).div(8000000);
      }

    }

    // increment by 1.015625 to achieve 12.5% every 8hrs
    function incrementTargetRate() internal {

      uint256 newRate = targetRate.mul(8125000).div(8000000);
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && grm.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(grm.totalSupply())).toInt256Safe();
        }
        ...
        returns (int256)
    {
        ...
        return grm.totalSupply().toInt256Safe()
