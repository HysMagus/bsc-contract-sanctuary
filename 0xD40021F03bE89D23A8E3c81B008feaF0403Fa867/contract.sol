...
// File: contracts/BIFI/strategies/CafeSwap/StrategyCafeLP.sol
...
contract StrategyCafeLP is Ownable, Pausable {
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != brew) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(brewHalf, 0, brewToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != brew) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(brewHalf, 0, brewToLp1Route, address(this), now.add(600));
        }
