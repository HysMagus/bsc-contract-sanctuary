...
Telegaram : https://t.me/KishuAda
Website : https://t.me/KishuAda
...
contract KishuAda is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("KishuAda", "KishuAda") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet =0x310b4f5e176a60776f813da9D8fEc53ac1eDeeEe;
    ...
    function tradeStart() external onlyOwner {
        ...
        maxBuyTranscationAmount = 1000000000000000000000 * (10**18);
        maxSellTransactionAmount = 100000000000000000000 * (10**18);
        maxWalletToken = 1000000000000000000000 * (10**18);
    }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "KishuAda: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "KishuAda: The new dividend tracker must be owned by the KishuAda token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("KishuAda_Dividend_Tracker", "KishuAda_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "KishuAda_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "KishuAda_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "KishuAda_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "KishuAda_Dividend_Tracker: Cannot update claimWait to same value");
