...
contract RewardPool {
    ...
    uint256 public reservePercent = 600; // 100 = 1%
    ...
    uint256[30] public epochRewardPerBlock;
    ...
    function initialize(address _vbswapToken, uint256 _startBlock) public notInitialized {
        require(block.number < _startBlock, "late");

        vbswapToken = vBSWAP(_vbswapToken);
        startBlock = _startBlock; // supposed to be 5,055,000 (Sat Feb 20 2021 23:00:00 UTC)

        // teaser epoch 0: 1 day with 10% reward only
        epochEndBlocks[0] = startBlock + BLOCKS_PER_DAY;
        epochRewardPerBlock[0] = firstEpochRewardPerBlock / 10;

        epochEndBlocks[1] = epochEndBlocks[0] + BLOCKS_PER_WEEK * 4;
        epochRewardPerBlock[1] = firstEpochRewardPerBlock;
        uint256 i;
        for (i = 2; i < 29; i++) {
            epochEndBlocks[i] = epochEndBlocks[i - 1] + BLOCKS_PER_WEEK * 4;
            epochRewardPerBlock[i] = (epochRewardPerBlock[i - 1] * 9) / 10;
        }
        epochRewardPerBlock[29] = 0;
    ...
    function getGeneratedReward(uint256 _from, uint256 _to) public view returns (uint256) {
        for (uint8 epochId = 29; epochId >= 1; --epochId) {
    ...
    function safeVbswapMint(address _to, uint256 _amount) internal {
        if (vbswapToken.minters(address(this)) && _to != address(0)) {
            ...
            if (_mintAmount > 0) {
                vbswapToken.mint(_to, _mintAmount);
                if (reservePercent > 0 && reserveFund != address(0)) {
                    ...
                    if (_reserveAmount > 0) {
                        vbswapToken.mint(reserveFund, _reserveAmount);
                    }
                }
            }
        }
    ...
    function setReservePercent(uint256 _reservePercent) external onlyOperator {
        require(_reservePercent <= 2000, "_reservePercent > 20%");
        reservePercent = _reservePercent;
    }
    
    function setEpochEndBlock(uint256 _index, uint256 _value) external onlyOperator {
        require(block.number < epochEndBlocks[_index], "late");
        require(_index == 28 || _value < epochEndBlocks[_index + 1], "out of range");
