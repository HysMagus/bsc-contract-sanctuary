...
// People lock their `BNB` and get rewarded `rewardToken`
// SETUP: First add the rewardToken into the contract, say 400 ONEK and then set the rewardPerBlock to decide how many are given in a day
/// @notice This contract allows you to lock BNB tokens and receive earnings
/// It also allows you to extract those earnings
contract BNBVault is Initializable, OwnableUpgradeSafe {
    using SafeMath for uint256;

    // How many BNB tokens each user has
    ...
    // When the user started locking his BNB tokens
    ...
    // How many BNB tokens are locked
    ...
    function initialize(address _rewardToken, address payable _devTreasury) public initializer {
        __Ownable_init();
        pricePadding = 1e18;
    ...
    function updateFeeIn() internal {
        ...
        if (lastBlockFee != 0) {
            ...
            // We don't need to divide by the padding since we want the result padded since the ONEK token has 18 decimals
    ...
    function lockLiquidity(uint256 _amount, uint256 _timeLock) public payable {
        updateFeeIn();
        ...
        // Transfer BNB tokens inside here while earning fees from every transfer
        require(msg.value >= _amount, 'Vault: You must send the desired amount of BNB');
        totalLiquidityLocked = totalLiquidityLocked.add(_amount);
        // Extract earnings in case the user is not a new Locked BNB
        ...
        // The price doesn't change when locking BNB. It changes when fees are generated from transfers
        amountLocked[msg.sender] = amountLocked[msg.sender].add(_amount);
        // Notice that the locking time is reset when new BNB is added
    ...
    function extractEarnings() public {
      updateFeeIn();
      require(amountLocked[msg.sender] > 0, 'Vault: You must have locked BNB to extract your earnings');
    ...
    // The user must lock the BNB for 1 year and only then can extract his Locked BNB tokens
    ...
    function extractLiquidity() public {
      ...
      require(now.sub(depositStarts[msg.sender]) >= lockingTime[msg.sender], 'Vault: You must wait the locking time to extract your BNB');
