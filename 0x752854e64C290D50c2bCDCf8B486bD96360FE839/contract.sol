...
interface IPancakeFarm {
    ...
    function checkPoolDuplicate ( address _lpToken ) external;
    function deposit ( uint256 _pid, uint256 _amount ) external;
    function dev ( address _governance ) external;
    function devFee (  ) external view returns ( address );
    ...
    function milkshake (  ) external view returns ( address );
    ...
    function updateBonus ( uint256 _bonusEndBlock ) external;
...
contract Strategy is BaseStrategy {
    ...
    address token0;
    address token1;
    address reward                  = 0x58f651DDE51CAa87c4111B16ee0A6Fab061Ee564;
    IERC20 public iCREAMToken       = IERC20(reward);
    IPancakeFarm public farmer      = IPancakeFarm(0x78Bd56CA4D781d1Be3808a7AF0A8b5446048c1AC);
    IERC20 public LPToken           = IERC20(farmer.poolInfo(pid).lpToken);
    IERC20 internal iWBNB           = IERC20(WBNB);
    IUniswapV2Router public router  = IUniswapV2Router(0x6728f3c8241C44Cc741C9553Ff7824ba9E932A4A);

    constructor(address _vault) public BaseStrategy(_vault) {
        require(address(want) == address(LPToken),"wrong want token");
        token0 = IPancakePair(address(want)).token0();
        token1 = IPancakePair(address(want)).token1();
        iCREAMToken.safeApprove(address(router),type(uint256).max);
        LPToken.safeApprove(address(farmer),type(uint256).max);
        LPToken.safeApprove(address(router),type(uint256).max);
        iWBNB.safeApprove(address(router),type(uint256).max);
    }

    function name() external override view returns (string memory) {
        return "StrategyICREAMLP";
    }
    ...
    function pendingReward() public view returns (uint256) {
        return cream_to_want(farmer.pendingCream(pid,address(this)));
    }

    function estimatedTotalAssets() public override view returns (uint256) {
        //Add the vault tokens + pending rewards
    ...
    function quote(address token_in, address token_out, uint256 amount_in) internal view returns (uint256) {
        bool is_weth = token_in == WBNB || token_out == WBNB;
        ...
        uint256[] memory amounts = router.getAmountsOut(amount_in, path);
        return amounts[amounts.length - 1];
    }

   function cream_to_want(uint256 _earned) internal view returns (uint256) {
        if (_earned / 2 == 0) return 0;
        uint256 _amount0 = quote(reward, token0, _earned / 2);
        uint256 _amount1 = quote(reward, token1, _earned / 2);
        (uint112 _reserve0, uint112 _reserve1, ) = IPancakePair(address(want)).getReserves();
    ...
    function depositToFarm(uint256 amount,bool fDeposit) internal {
        //Sell half to bnb
        _sell(amount / 2);
        //Add liq to pair
        _addLiq();
        if(fDeposit) farmer.deposit(pid,LPToken.balanceOf(address(this)));
    }
        ...
        )
    {
        ...
        if(pendingReward() > 0) {
            farmer.deposit(pid,0);
            _earned = iCREAMToken.balanceOf(address(this));
            if(_earned > 0)
                depositToFarm(_earned, false);
        }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > balanceWant) {
            ...
            farmer.withdraw(pid,(Math.min(balanceStaked, _amountNeeded - balanceWant)));
        }
    ...
    function prepareMigration(address _newStrategy) internal override {
        ...
        if(pendingReward() > 0) {
            farmer.deposit(pid,0);
            depositToFarm(iCREAMToken.balanceOf(address(this)),false);
        }
        //Unstake from governance
        farmer.withdraw(pid,balanceOfStake());
    }
    ...
    function protectedTokens() internal override view returns (address[] memory) {
        ...
        protected[0] = address(iCREAMToken); // rewards
        return protected;
    }
}