...
// File: contracts/BIFI/interfaces/bakery/IBakerySwapRouter.sol
...
// File: contracts/BIFI/interfaces/bakery/IBakerySwapPair.sol
pragma solidity ^0.6.0;
...
// File: contracts/JETFUEL/interfaces/bakery/IBakeryMaster.sol
...
interface IBakeryMaster {
    ...
    function pendingBake(address _pair, address _user)
        external
        view
        returns (uint256);

    function poolUserInfoMap(address _pair, address _user)
        external
        view
...
// File: contracts/JETFUEL/strategies/Bakery/StrategyBakeryLP.sol
...
contract StrategyBakeryLP is Ownable, Pausable {
     ...
     * {pcUniRouter} - PancakeSwap unirouter
     */
    address public constant unirouter =
        address(0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F);
    address public constant bakeryMaster =
        address(0x20eC291bB8459b6145317E7126532CE7EcE5056f);
    address public constant pcUniRouter =
    ...
    constructor(address _lpPair, address _vault) public {
        ...
        IERC20(bake).safeApprove(unirouter, uint256(-1));
        IERC20(wbnb).safeApprove(unirouter, uint256(-1));

        IERC20(wbnb).safeApprove(pcUniRouter, uint256(-1));
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(bake).balanceOf(address(this)).mul(6).div(100);
        ...
        // swap wbnb for FUEL using pancake
        IPancakeRouter(pcUniRouter).swapExactTokensForTokens(
            rewardsFee,
            0,
            wbnbToFuelRoute,
            rewards,
    ...
    function addLiquidity() internal {
        ...
        IBakerySwapRouter(unirouter).addLiquidity(
            lpToken0,
            lpToken1,
            lp0Bal,
            lp1Bal,
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) =
            IBakeryMaster(bakeryMaster).poolUserInfoMap(lpPair, address(this));
        return _amount;
    }
