...
contract KiwiToken is BEP20("Hot Sausage", "KIWI"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (KiwiFarmer).
...
contract KiwiFarmer is Ownable {
    using SafeMath for uint256;
    ...
    // Block number when bonus KIWI period ends.
    ...
    // Update the given pool's KIWI allocation point. Can only be called by the owner.
    ...
    // Deposit LP tokens to KiwiFarmer for KIWI allocation.
    ...
    // Withdraw LP tokens from KiwiFarmer.
    ...
    function safeKiwiTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > kiwiBal) {
            kiwi.transfer(_to, kiwiBal);
        } else {
            kiwi.transfer(_to, _amount);
        }
    }
