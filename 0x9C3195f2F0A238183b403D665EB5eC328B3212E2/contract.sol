...
contract SelfAirdrop is Owned{
  ...
  uint public minBuy = 0 ether;
  uint public maxBuy = 0 ether;
 ...
 function () external payable {
    ...
    require(BEP20Interface(token).balanceOf(address(this)) >= _qtty*decimal, "Insufficient tokens in the contract");
    
    saleswallet.transfer(msg.value);
    buyamount[msg.sender] += msg.value;
    require(BEP20Interface(token).transfer(msg.sender, _qtty*decimal), "Transaction failed");
  ...
  function RetrieveToken() public onlyAdmin returns(bool){
      ...
      uint bal = BEP20Interface(token).balanceOf(address(this));
      require(BEP20Interface(token).transfer(saleswallet, bal), "Transaction failed");
