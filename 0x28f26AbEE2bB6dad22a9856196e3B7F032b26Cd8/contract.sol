...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    MojitoToken public mojito;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public mojitoPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 mojitoReward = multiplier.mul(mojitoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(mojitoReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 mojitoReward = multiplier.mul(mojitoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        mojito.mint(devaddr, mojitoReward.div(10));
        mojito.mint(address(this), mojitoReward);
        pool.accEggPerShare = pool.accEggPerShare.add(mojitoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe mojito transfer function, just in case if rounding error causes pool to not have enough EGGs.
    ...
    function updateEmissionRate(uint256 _mojitoPerBlock) public onlyOwner {
        massUpdatePools();
        mojitoPerBlock = _mojitoPerBlock;
    }
}