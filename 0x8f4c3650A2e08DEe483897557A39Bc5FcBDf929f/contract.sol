...
contract PAWZToken is Context, IERC20, Ownable {
    ...
    uint256 public _liquidityFee = charityDivisor + marketingDivisor + buyBackDivisor;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != PancakeswapV2Pair && to != PancakeswapV2Pair) {
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressBNB(charityAddress, transferredBalance.mul(charityDivisor).div(_liquidityFee));
        //Send to Marketing address
        transferToAddressBNB(marketingAddress, transferredBalance.mul(marketingDivisor).div(_liquidityFee));
        //Send to BuyBack address
        transferToAddressBNB(buybackAddress, transferredBalance.mul(buyBackDivisor).div(_liquidityFee));
    }
    ...
    function setCharityDivisor(uint256 divisor) external onlyOwner() {
        ...
        _liquidityFee = charityDivisor + marketingDivisor + buyBackDivisor;
    }
