...
contract JulProtocolV2_BUSD_JULb is Ownable {
    ...
    function addBUSD(uint256 busd_amount)
        public
        payable
        returns (
            uint256 amountToken,
    ...
    {
        require(busd_amount >= MINIMUM_DEPOSIT_AMOUNT, "Insufficient BUSD");  

        TransferHelper.safeTransferFrom(BUSD_TOKEN, msg.sender, address(this), busd_amount);
        ...
        uint julb_amount = BSCswapLibrary.quote(busd_amount, reserveA, reserveB); 
        ...
        JulToken.approve(router02Address, julb_amount);
        BusdToken.approve(router02Address, busd_amount);

        (amountToken, amountBUSD, liquidity) = bscswapRouter02.addLiquidity(
            BUSD_TOKEN,
            JUL_TOKEN, 
            busd_amount, 
            julb_amount, 
    ...
    function removeBUSD(uint256 _amountBUSD, uint256 amountOutMin)
    public payable returns(uint256 amountToken, uint256 amountBUSD)
    {
