...
contract BTRILPStaker is Ownable {
    ...
    uint256 public _rewardTokenDecimal = 9;                // decimal of reward token
    ...
    constructor (IPancakeERC20 BTriBnbPairContract, address devWallet, uint256 devRewardPct, uint256 duration, uint256 lockTime) public {
    ...
    function stake(uint256 amount) public {
        ...
        require(
            _startTime > 0 &&
            now >= _startTime,
            'BTriLPStaker: staking not started yet.'
        );
        
        require(
            _BTriBnbPairContract.transferFrom(
                _msgSender(),
                address(this),
                amount
            ),
    ...
    function calcReward(address staker, uint256 currentBlockNumber) private view returns (uint256) {
        ...
        for(uint i = _stakeMap[staker].lastSnapShotIndex; i < snapShotHistory.length; i++) {
            ...
            rewardAmount = rewardAmount.add(_rewardAmountPerBlock.mul(passedBlockCount).mul(10**(_rewardTokenDecimal - 2)).mul(rewardPctForStakers).div(100).mul(stakedAmount).div(prevTotalStakedAmount));
        }
        return rewardAmount;
    }
    ...
    /**
     * Get reward pool balance (bTrinity)
     */
    ...
    function setStartTime() external onlyOwner {
        require (
            ...
            'BTriLPStaker: should set start time later.'
        );
    ...
    function claimDevReward() external {
        require(
            ...
            "BTriLPStaker: can't claim dev reward yet."
        );
