...
interface IDelfyLocker {
    ...
    function addAuctionDetails(address dexFactory, address dexWETH,address token, address _owner, address _auctionAddr, uint _lockPeriod,bool _burnLP) external;
...
interface IAuctionFactory{
    ...
    function createAuction( address[] calldata _dexRouter, uint256 _salesPeriod, address _token, uint256 _rate, uint256 _roi, uint256 _lockPeriod, bool burnLiquidityToken) external returns(address payable newAuction);
    function getAllAuctions() external view returns(address[] memory);
    // function setDelfyLocker (address _lockerAddr) external;
...
contract AuctionFactory is IAuctionFactory {
    ...
    // IDelfyLocker public DelfyLocker;
    IDelfyLocker public constant DelfyLocker =
    IDelfyLocker(0x449E96F7EBA744846452aFEd6d75d371c2F052Cf);
    ...
    function createAuction(
        address[] calldata _dexData,
        uint256 _salesPeriod,
        address _token,
        uint256 _rate,
        uint256 _roi,
        uint256 _lockPeriod,
        bool _burnLiquidityToken
    ) public override returns (address payable newAuction) {
        ...
        Auction(newAuction).initialaize(
            _dexData,
            msg.sender,
            _salesPeriod,
            _token,
        ...
        IDelfyLocker(DelfyLocker).addAuctionDetails(
            _dexData[0],
            _dexData[2],
            _token,
            msg.sender,
...
interface IAuction {
    ...
    function initialaize( address[] calldata _dexInfo, 
...
contract Auction is IAuction, Mutex {
    ...
    event Bought(address indexed _buyer, uint256 amount);
    event SentToDex(
    ...
    modifier onlyCommunity() {
        require(
            msg.sender == owner || isContributor[msg.sender],
    ...
    function initialaize(
        ...
        address _lockerAddr,
        bool _burnLiquidityToken,
        address payable fees_to
    ) external override {
        assert(msg.sender == factory);
        require(
            _owner != address(0) &&
                _token != address(0) &&
                _dexData[0] != address(0) &&
                _dexData[1] != address(0) &&
                _dexData[2] != address(0) &&
                _rate > 0 &&
                _roi <= 60,
            "AUCTION: invalid_parameters"
        );
        dexFactory = IUniswapV2Factory(_dexData[0]);
        dexRouter = IUniswapV2Router02(_dexData[1]);
        dexWETH = _dexData[2];
        owner = _owner;
        token = IERC20(_token);
    ...
    function getEthBal() external view override returns (uint256) {
        return address(this).balance;
    }

    function getTokenBal() external view override returns (uint256) {
    ...
    function whitelistAddresses(address[] calldata _allowdAddresses)
        external
        override
        onlyOwner
    {
        assert(_allowdAddresses.length > 0);
        for (uint256 index = 0; index < _allowdAddresses.length; index++) {
    ...
    function deposit(
        ...
        uint256 _reserve,
        uint256 _deflationFactor
    ) external override {
        require(
            _initialTokenLiquidity >= _calculatePecentage(_amountForSale, 30),
            "initialLiquidity must >= 30% of availableTokenForSale"
        ); // initial liquidity must be greater than 30% of amount to be sold
        require(!salesStarted, "AUCTION: sales_is_on");

        initialLiquidity = _initialTokenLiquidity.sub(
            _calculatePecentage(_initialTokenLiquidity, _deflationFactor)
        );
        availableTokenForSale = _amountForSale.sub(
            _calculatePecentage(_amountForSale, _deflationFactor)
        );
        amountForSale = _amountForSale.sub(
            _calculatePecentage(_amountForSale, _deflationFactor)
        );
        reserve = _reserve.sub(_calculatePecentage(_reserve, _deflationFactor));
        deflationFactor = _deflationFactor;
        uint256 _total =
            _initialTokenLiquidity.add(_amountForSale).add(_reserve);
    ...
    function getFeesValueFromUniswap()internal{
      ...
      address cakeRouter = 0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F;
      address cakeFactory = 0xBCfCcbde45cE874adCB698cC183deBcF17952812;
      address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

        uint reserveWBNB;
        uint reserveBUSD;
        address usdPair = IUniswapV2Factory(cakeFactory).getPair(BUSD, WBNB);

      (uint reserve0, uint reserve1,) = IUniswapV2Pair(usdPair).getReserves();
        ...
        uint one_dollar_ether = IUniswapV2Router02(cakeRouter).quote(1e18, reserveBUSD, reserveWBNB);
         createCaseFee = one_dollar_ether*(5);
        supportCaseFee = one_dollar_ether*(3);
        createReFundCaseFees =one_dollar_ether*(8);
        supportRefundCaseFees =one_dollar_ether*(4);
    }

    function buyTokenWithEth() external payable override noReentrance {
        closeSales();
        ...
        if (minimumPurchaseEth > 0 && maximumPurchaseEth > 0) {
            require(
                msg.value >= minimumPurchaseEth &&
                    msg.value <= maximumPurchaseEth,
                "AUCTION: value_out_of_range"
            );
            require(
                amountContributed[msg.sender].add(msg.value) <=
                    maximumPurchaseEth,
        ...
        require(
            tokenAmount <= availableTokenForSale,
            "AUCTION: more_than_available_token_for_sale"
        );
    ...
    function _calculatePecentage(uint256 value, uint256 percentage)
        internal
        pure
        returns (uint256)
    {
    ...
    function _calcDexEthAllocationWithInitialROI()
        internal
        view
        returns (uint256)
    {
        uint256 _factor =
            _calculatePecentage(initialLiquidity.div(rate.div(1e18)), roi);
        return initialLiquidity.div(rate.div(1e18)).add(_factor);
    }

    function launchEthTokenExchange(uint256 deadline)
        external
        override
        onlyCommunity
    {
        ...
        uint256 fees = balanz.div(100);
        TransferHelper.safeTransferETH(feesTo, fees);
        emit Transfer(address(this), feesTo, fees);
        uint256 newBalanz = balanz.sub(fees);
        if (newBalanz < _amountInEth) {
            uint256 _amntToken =
                _getTokenAmount(address(this).balance) >
                    token.balanceOf(address(this))
                    ? _getTokenAmount(address(this).balance).div(2)
                    : _getTokenAmount(address(this).balance); /*modified*/

            if (burnLiquidityToken == true) {
                dexRouter.addLiquidityETH{value: newBalanz}(
                    address(token),
                    _amntToken,
                    0,
                    0,
            ...
            emit SentToDex(msg.sender, newBalanz, _amntToken);
        } else {
            if (burnLiquidityToken == true) {
                dexRouter.addLiquidityETH{value: _amountInEth}(
                    address(token),
                    initialLiquidity,
                    0,
                    0,
            ...
            emit SentToDex(msg.sender, _amountInEth, initialLiquidity);
        }
    ...
    function changeCaseState() internal {
        // reset case state
        if (cases.length > 0) cases[cases.length - 1].active = false;
    ...
    function _startCase() internal {
        require(
            block.timestamp >= exchangeLaunchedTime.add(24 hours),
            "AUCTION: can_not_create_case"
        );
        if (
            block.timestamp >= exchangeLaunchedTime.add(24 hours) &&
            cases.length == 0
    ...
    function createTypedCase(string calldata _issue, uint8 _caseType)
        external
        payable
        override
        noReentrance
        onlyCommunity
    {
        ...
        require(
            token.balanceOf(msg.sender) > _calculatePecentage(amountForSale, 1),
            "AUCTION: Only_real_investors"
        );
        require(
            exchangeLaunched || block.timestamp <= thirdRelease,
        ...
        if (CaseType(_caseType) == CaseType.refundable) {
            require(
                msg.value >= createReFundCaseFees,
        ...
        amountContributed[msg.sender] = amountContributed[msg.sender].add(
            msg.value
        );
        userBalance[msg.sender] = userBalance[msg.sender].add(msg.value);
    ...
    function viewCases(uint8 _caseType)
        external
        view
        override
        returns (Case[] memory)
    {
        if (
            CaseType(_caseType) == CaseType.refundable &&
            refundableCases.length > 0
        ) return refundableCases;
        if (
            CaseType(_caseType) == CaseType.releaseLiquidity && cases.length > 0
        ) return cases;
    }

    // @dev about $2 (0.004 ether) is required to upvote a Case for sanity sake
    function upvoteCase(uint8 _caseType)
        external
        payable
        override
        noReentrance
    {
        require(
            token.balanceOf(msg.sender) > amountForSale.mul(1).div(1000),
            "AUCTION: Only_real_investors"
        );
        if (
            CaseType(_caseType) == CaseType.releaseLiquidity && cases.length > 0
            ...
            require(
                cases[cases.length - 1].active == true,
                "AUCTION: case_is_not_active"
            );
            require(
                !voted[msg.sender][cases[cases.length - 1].id],
                "AUCTION: Voted"
            );
            ...
            require(
                msg.value >= supportRefundCaseFees,
                "AUCTION: You_have_to_donate"
            );
            require(
                refundableCases[refundableCases.length - 1].active == true,
                "AUCTION: case_is_not_active"
            );
            require(
                !voted[msg.sender][
                    refundableCases[refundableCases.length - 1].id
                ],
                "AUCTION: Voted"
            );
    ...
    function checkStopRelease() private view returns (bool) {
        uint256 acceptableRUpvotes = _calculatePecentage(contributorsCount, 55);
        if (refundableCases.length > 0)
            return
                refundableCases[refundableCases.length - 1].upvotes >
                acceptableRUpvotes;
    }

    function calcWithdrawableEth() private {
        if (
            block.timestamp > firstRelease &&
            !isFirstReleased &&
            block.timestamp < secondRelease
        ) {
            availableEthToBeWithdrawn = availableEthToBeWithdrawn.add(
                address(this).balance.div(5)
            );
            isFirstReleased = true;
        } else if (
            block.timestamp > secondRelease &&
            !isSecondReleased &&
            block.timestamp < thirdRelease
        ) {
            availableEthToBeWithdrawn = availableEthToBeWithdrawn.add(
    ...
    function releaseLiquidity(uint256 deadline)
        external
        override
        noReentrance
        onlyCommunity
    {
        ...
        uint256 acceptableUpvotes = _calculatePecentage(contributorsCount, 40);

        uint256 reserveWeth;
        uint256 reserveToken;
        (uint256 reserve0, uint256 reserve1, ) = tokenPair.getReserves();

        if (dexWETH > address(token)) {
        ...
        if (
            cases.length > 0 &&
            cases[cases.length - 1].upvotes > acceptableUpvotes &&
            cases[cases.length - 1].completed == false
        ) {
            ...
            if (token.balanceOf(address(this)) < _tokenAmount) {
                if (deflationFactor > 0) {
                    dexRouter
                        ...
                        value: availableEthToBeWithdrawn
                    }(0, path, deadAddress, deadline);
                } else {
                    dexRouter.swapExactETHForTokens{
                        value: availableEthToBeWithdrawn
                    }(0, path, deadAddress, deadline);
                }
            } else {
                dexRouter.addLiquidityETH{value: availableEthToBeWithdrawn}(
                    address(token),
                    _tokenAmount,
                    0,
            ...
            emit SentToDex(msg.sender, availableEthToBeWithdrawn, _tokenAmount);
        } else {
            if (cases.length > 0) cases[cases.length - 1].completed = true;
    ...
    function burnInitialLiquidity() external override {
        require(
            passedCases.length >= 2 || checkIfCaseRelease3Passed(),
            "AUCTION: require_2_passedCases_or_3rd_case"
        );
        require(
            token.balanceOf(msg.sender) > _calculatePecentage(amountForSale, 1),
            "AUCTION: Only_real_investors"
        );
        uint256 amountToBurn = tokenPair.balanceOf(locker);
    ...
    function checkIfCaseRelease3Passed() private view returns (bool) {
        if (cases[cases.length - 1].time > secondRelease)
            return
                cases[cases.length - 1].upvotes >
                _calculatePecentage(contributorsCount, 40);
    }

    function getClaimable(address _addr) public view returns (uint256) {
        uint256 _amnt = amountRaised.add(caseFunds);
        return availableEthBalance.mul(userBalance[_addr]).div(_amnt);
    }

    function bailout() private view returns (bool) {
        return
            block.timestamp > salesCompletedTime.add(9 days) &&
    ...
    function refundBuyers() external override noReentrance {
        require(
            block.timestamp > salesCompletedTime.add(8 days) &&
                !isThirdReleased,
            "AUCTION: not_released"
        );
        require(
            checkStopRelease() || bailout(),
            "AUCTION: only_refundable_case"
        );
        require(userBalance[msg.sender] > 0, "Only_real_investors");

        uint256 claimable = getClaimable(msg.sender);

        // require(claimable>0,"zero_value");
        TransferHelper.safeTransferETH(msg.sender, claimable);
        refundClaimed[msg.sender] = claimable;
