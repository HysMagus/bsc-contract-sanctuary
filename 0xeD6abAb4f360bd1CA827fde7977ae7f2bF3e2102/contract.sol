...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function betdice(uint amount, uint choose, bytes32 dealerhash, bytes memory dealerSign) public payable nonReentrant returns (bool) {
        EIP20Interface diceToken = EIP20Interface(0x9468dec2e19240d6E287f27e1d757Ae7b9f15F7d);
        require(dealerSign.length == 65, "Require correct length");
        require(getMaxBet(msg.value, choose), "MAX_BET_ERROR");
        bytes32 r;
        bytes32 s;
        uint8 v;
        ...
        require(recoverSigner2(dealerhash, v, r, s) == 0x7b75d33D50a4d734c520051b955F3E57E9031604);
        
        require(betContent[uint(dealerhash)].betAmount == 0);
        
        betContent[uint(dealerhash)].betAmount = msg.value;
        betContent[uint(dealerhash)].chooseNum = choose;
        betContent[uint(dealerhash)].playerAddress = msg.sender;
        emit Bet(msg.sender, msg.value, choose, dealerhash, now);

        
        uint actualBetAmount = doTransferIn(msg.sender, amount);
        (MathError err1, uint scaledNumerator1) = divUInt(actualBetAmount, 500);
        require(err1 == MathError.NO_ERROR, "err1 error");
        
        (MathError err3, uint scaledNumerator3) = mulUInt(actualBetAmount, 3e18);
        require(err3 == MathError.NO_ERROR, "err3 error");
        
         (MathError err6, uint scaledNumerator4) = divUInt(scaledNumerator3, 1000);
        require(err6 == MathError.NO_ERROR, "err6 error");

         (MathError err4, uint rational) = divUInt(scaledNumerator4, totalSupply);
        require(err4 == MathError.NO_ERROR, "err4 error");

         (MathError err5, uint exchangeWaterpriceNew) = addUInt(exchangeWaterprice, rational);
        require(err5 == MathError.NO_ERROR, "err5 error");

        exchangeWaterprice = exchangeWaterpriceNew;
        if(diceToken.balanceOf(address(this)) >= scaledNumerator1){
            diceToken.transfer(msg.sender, scaledNumerator1);
        }
        address(0x9dD195880694A695344979404Aa2cFde239EE080).call.value(scaledNumerator4/1e18)("");
        return true;
    }
    ...
    function revealdice(string memory dealerseed, bytes32 dealerhash) public nonReentrant returns (bool){
        ...
        require(betContent[uint(dealerhash)].betAmount > 0);
        // bytes memory tempEmptyStringTest = bytes(betContent[uint(dealerhash)].seed); // Uses memory
    ...
    function getMaxBet(uint amount, uint choose) internal view returns (bool) {
        require(amount > 0 && amount <= 1e21, "Require correct amount");
