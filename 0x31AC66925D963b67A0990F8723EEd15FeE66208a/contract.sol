 ...
 */
 ...
 //_wbnb_busd: 0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16
 ...
 *      This Oracle calculates the average USD price of TOKEN based on the BNB-TOKEN and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private tokenBnbPrice0CumulativeLast;
    uint private tokenBnbPrice1CumulativeLast;
    uint32 private tokenBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _token_bnb;
    ...
    constructor(
        address __token_bnb,   // Address of the token-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswap
        ) public {

        controller = msg.sender;

        _token_bnb = IUniswapV2Pair(__token_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        tokenBnbPrice0CumulativeLast = _token_bnb.price0CumulativeLast();
        tokenBnbPrice1CumulativeLast = _token_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, tokenBnbBlockTimestampLast) = _token_bnb.getReserves();
    ...
    // Get the average price of 1 token in the smallest BNB unit (18 decimals)
    function gettokenBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_token_bnb));

        require(_blockTimestamp != tokenBnbBlockTimestampLast, "token Last and current are equal");

        FixedPoint.uq112x112 memory tokenBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - tokenBnbPrice0CumulativeLast) / (_blockTimestamp - tokenBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, tokenBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint tokenBnbAverage;
        uint busdBnbAverage;

        (tokenBnbPrice0CumulativeLast, tokenBnbPrice1CumulativeLast, tokenBnbBlockTimestampLast, tokenBnbAverage) = gettokenBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, tokenBnbAverage) = gettokenBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = tokenBnbAverage*(1e6) / busdBnbAverage;
