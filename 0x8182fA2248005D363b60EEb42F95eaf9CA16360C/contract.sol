...
// File: @opengsn/gsn/contracts/RelayHub.sol
...
// File: @opengsn/gsn/contracts/StakeManager.sol
pragma solidity ^0.6.2;
...
// File: @opengsn/gsn/contracts/utils/RLPReader.sol
/*
...
// File: @opengsn/gsn/contracts/interfaces/IPenalizer.sol
pragma solidity ^0.6.2;
...
// File: @opengsn/gsn/contracts/Penalizer.sol
pragma solidity ^0.6.2;
...
// File: @opengsn/gsn/contracts/interfaces/IVersionRegistry.sol
pragma solidity ^0.6.2;
...
// File: @opengsn/gsn/contracts/utils/VersionRegistry.sol
pragma solidity ^0.6.2;
// solhint-disable not-rely-on-time
...
// File: templates/EtcGsnDeployer.sol
pragma solidity ^0.6.10;


/**
 * helper contract to deploy all GSN contracts in one transaction.
 * NOTE: future releases should use the same forwarder and stakemanager to allow seamless upgrade.
 */
contract EtcGsnDeployer {
    constructor() public {
        StakeManager sm = new StakeManager();
        Penalizer p = new Penalizer();
        Forwarder f = new Forwarder();

        f.registerRequestType(
            "RelayRequest",
            "RelayData relayData)RelayData(uint256 gasPrice,uint256 pctRelayFee,uint256 baseRelayFee,address relayWorker,address paymaster,address forwarder,bytes paymasterData,uint256 clientId)"
        );
        f.registerDomainSeparator("GSN Relayed Transaction", "2");

        uint maxWorkerCount = 10;
        uint gasReserve = 100000;
        uint postOverhead = 13950;          // MUST match the value from defaultRelayHubConfiguration
        uint gasOverhead = 35965;           // MUST match the value from defaultRelayHubConfiguration
        uint maximumRecipientDeposit = 10 ether; // ETC
        uint minimumUnstakeDelay = 43200;  // one week in 14-second blocks
        uint minimumStake = 50 ether;    // ETC 

        RelayHub hub = new RelayHub(
            sm, address(p),
            maxWorkerCount,
            gasReserve,
            postOverhead,
    	...
    	//copy of constructor params. emitted, to simplify source verify&publish
    	bytes memory hubparams = abi.encode(
            sm, address(p),
            maxWorkerCount,
            gasReserve,
            postOverhead,
        ...
        emit Deployed(f, hub, hubparams);
    }
    event Deployed( Forwarder f, RelayHub hub, bytes hubparams);
}