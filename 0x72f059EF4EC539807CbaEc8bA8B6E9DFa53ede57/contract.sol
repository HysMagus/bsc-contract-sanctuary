...
contract ExtensionExchangePool is Ownable {
    ...
    uint256 private releaseDuration = 86400 * 30;
    ...
    function buy(address invitor) public payable {
        ...
        uint256 invitorLockAmoumt1 = 0;
        uint256 invitorLockAmoumt2 = 0;
        if (invitor != address(0) && invitor != msg.sender && whiteListContract != address(0) && IWhiteList(whiteListContract).isInWhiteList(invitor)) {
               ...
               if(invitorAmount > 0){
                   invitorLockAmoumt1 = invitorAmount * 20 /100;
                   invitorLockAmoumt2 = invitorAmount - invitorLockAmoumt1;
               }
        ...
        if (invitorLockAmoumt1 > 0) {
            Record memory record = Record({
            amount : invitorLockAmoumt1,
            timestamp : block.timestamp,
            withdrawAmount:0
            });
            invitorRecords[invitor].push(record);
            totalLockAmount = totalLockAmount + invitorLockAmoumt1;
        }

        if (invitorLockAmoumt2 > 0) {
            Record memory record = Record({
            amount : invitorLockAmoumt2,
            timestamp : block.timestamp,
            withdrawAmount:0
            });
            invitorLockRecords[invitor].push(record);
            totalLockAmount = totalLockAmount + invitorLockAmoumt2;
        }
    ...
    function releaseBuyToken(uint256 index) public {
        ...
        buyRecords[msg.sender][index].withdrawAmount = record.amount;
        totalLockAmount = totalLockAmount - record.amount;
    }
    
    function releaseBuyLockToken(uint256 index) public {
        require(buyLockRecords[msg.sender].length > 0, "not any release token");
        Record memory record = buyLockRecords[msg.sender][index];
        require(record.amount > 0, "not any release token");
        uint256 time = block.timestamp - releaseDuration - record.timestamp;
        uint256 durations = time / releaseDuration;
        uint256 releaseAmont =  record.amount * durations / 6 - record.withdrawAmount;
        if (releaseAmont + record.withdrawAmount > record.amount) {
            releaseAmont = record.amount - record.withdrawAmount;
        }
        require(releaseAmont > 0, "has no release token");
        ERC20(withdrawToken).transfer(msg.sender, releaseAmont);
        buyLockRecords[msg.sender][index].withdrawAmount = buyLockRecords[msg.sender][index].withdrawAmount + releaseAmont;
        totalLockAmount = totalLockAmount - releaseAmont;
    }
     ...
     function releaseInvitorToken(uint256 index) public {
        ...
        invitorRecords[msg.sender][index].withdrawAmount = record.amount;
        totalLockAmount = totalLockAmount - record.amount;
    }
     ...
     function releaseInvitorLockToken(uint256 index) public {
        ...
        require(releaseAmont > 0, "has no release token");
        ERC20(withdrawToken).transfer(msg.sender, releaseAmont);
        invitorLockRecords[msg.sender][index].withdrawAmount = invitorLockRecords[msg.sender][index].withdrawAmount + releaseAmont;
        totalLockAmount = totalLockAmount - releaseAmont;
    }
    ...
    function setReleaseDruation(uint256 _releaseDuration) public onlyOwner {
