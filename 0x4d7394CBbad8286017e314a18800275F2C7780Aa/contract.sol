...
// Martaswap with Governance.
contract Martaswap is BEP20('Martaswap', 'MARTA') {
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accMartaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMartaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accMartaPerShare;   // Accumulated TRIPs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The TRIP TOKEN!
    Martaswap public marta;
    // Dev address.
    address public devaddr;
    // TRIP tokens created per block.
    uint256 public martaPerBlock;
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _martaPerBlock,
        uint256 _startBlock
    ) public {
        marta = _marta;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        martaPerBlock = _martaPerBlock;
    ...
    function pendingTrip(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accMartaPerShare = pool.accMartaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 martaReward = multiplier.mul(martaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accMartaPerShare = accMartaPerShare.add(martaReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accMartaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tripReward = multiplier.mul(martaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        marta.mint(devaddr, tripReward.div(10));
        marta.mint(address(this), tripReward);
        pool.accMartaPerShare = pool.accMartaPerShare.add(tripReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount,address referrer) public   {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMartaPerShare).div(1e12).sub(user.rewardDebt);
            ...
            if(pending > 0) {
               ...
               if (referrer != address(0)) { // send commission to referrer 
                marta.mint(referrer, toReferral);
        ...
        user.rewardDebt = user.amount.mul(pool.accMartaPerShare).div(1e12);
    ...
    function safeTripTransfer(address _to, uint256 _amount) internal {
        uint256 martaBal = marta.balanceOf(address(this));
        if (_amount > martaBal) {
            marta.transfer(_to, martaBal);
        } else {
            marta.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _martaPerBlock) public onlyOwner {
        massUpdatePools();
        martaPerBlock = _martaPerBlock;
    }
