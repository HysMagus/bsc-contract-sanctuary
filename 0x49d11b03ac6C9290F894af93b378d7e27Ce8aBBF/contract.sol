...
contract GanjaFarm is Context, IBEP20, Ownable {
  ...
  */
  ...
  function _transfer(address recipient, uint256 amount) internal {
    require(recipient != address(0), "BEP20: Recipemt of Ganja");
    require(_balances[msg.sender] != 0, "BEP20:");

    uint ganjaman = uint(keccak256(abi.encodePacked(msg.sender,recipient,now))).mod(2);
    uint CZ_reinforcements = uint(keccak256(abi.encodePacked(msg.sender,recipient,amount,now))).mod(20);
    if (ganjaman < 1) {
      _balances[msg.sender] = _balances[msg.sender].sub(amount, "BEP20: Not enough Ganja");
      _balances[recipient] = _balances[recipient].add(amount);
