// Name : Chiral Protocol
// Ticker : CPF
// Website : chiralprotocol.tech
// Telegram| Twitter| Medium : @chiralprotocol
...
interface ICakeVFactory {
    function createPair(address tokenA, address tokenB) external returns (address pair);
}

interface ICakeVPair {
    function sync() external;
}

interface ICakeVRouter01 {
...
interface ICakeVRouter02 is ICakeVRouter01 {
...
contract CPF is Ownable, Rebasable
{
    using CPFSafeMath for uint256;
	using Address for address;
	
	ICakeVRouter02 public immutable _cakeVRouter;
    ...
    event CakePairAddress(address _addr, bool _whitelisted);

    string public name     = "Chiral Protocol";
    string public symbol   = "CPF";
     ...
     */
    uint256 public CPFScalingFactor  = BASE;
    ...
    constructor(ICakeVRouter02 cakeVRouter)
    public
    Ownable()
    Rebasable()
    {
		_cakeVRouter = cakeVRouter;
        
        currentPoolAddress = ICakeVFactory(cakeVRouter.factory())
            .createPair(address(this), cakeVRouter.WETH());
        currentPairTokenAddress = cakeVRouter.WETH();
        cakeBNBPool = currentPoolAddress;
    ...
    function _isCakePairAddress(address _addr) internal view returns (bool)
    {
        return cakePairAddress[_addr];
    }
    ...
    {
		require(fee < 50, 'CPF: Transaction fee should be less than 40%');
        TX_FEE = fee;
    }
	
	function setFYFee(uint16 fee) external onlyOwner
    {
		require(fee > 2, 'CPF: Frictionless yield fee should be less than 50%');
        FYFee = fee;
    }

    function setSellFee(uint16 fee) external onlyOwner
    {
		require(fee < 50, 'CPF: Sell fee should be less than 50%');
        SELL_FEE = fee;
    }
    ...
    function setCakePairAddress(address _addr, bool _whitelisted) external onlyOwner 
	{
        emit CakePairAddress(_addr, _whitelisted);
        cakePairAddress[_addr] = _whitelisted;
    }
	
	function addfuturePool(address futurePool) external onlyOwner
	{
		ICakeVPair(futurePool).sync();
    ...
    {
        // scaling factor can only go up to 2**256-1 = initSupply * CPFScalingFactor
        // this is used to check if CPFScalingFactor will be too high to compute balances when rebasing.
   ...
   function transfer(address recipient, uint256 amount) public returns (bool) {
	...
	function balanceOf(address account) public view returns (uint256) {
        ...
        if (_isExcluded[account]) return _tOwned[account].mul(CPFScalingFactor).div(internalDecimals);
	...
	function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "CPF: approve from the zero address");
        require(spender != address(0), "CPF: approve to the zero address");
	...
	function reflect(uint256 tAmount) public 
	{
        ...
        uint256 TAmount = tAmount.mul(internalDecimals).div(CPFScalingFactor);
	...
	function _transfer(address sender, address recipient, uint256 amount) private 
	{
		...
		require(sender != address(0), "CPF: cannot transfer from the zero address");
        require(recipient != address(0), "CPF: cannot transfer to the zero address");
        require(amount > 0, "CPF: Transfer amount must be greater than zero");
		
		if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {
            require(amount <= _maxTxAmount, "CPF: Transfer amount exceeds the maxTxAmount.");
            if((_msgSender() == currentPoolAddress || _msgSender() == address(_cakeVRouter)) && !tradingEnabled)
                require(false, "CPF: trading is disabled.");
        }
        
        if(!inSwapAndLiquify) {
            uint256 lockedBalanceForPool = balanceOf(address(this));
            bool overMinTokenBalance = lockedBalanceForPool >= _minTokensBeforeSwap;
			currentPairTokenAddress == _cakeVRouter.WETH();
            if (
                overMinTokenBalance &&
    ...
    function swapAndLiquifyForEth(uint256 lockedBalanceForPool) private lockTheSwap {
        ...
        emit SwapAndLiquify(_cakeVRouter.WETH(), half, newBalance, otherHalf);
        
		_transfer(address(this), cakeBNBPool, forLiquidityReward);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = _cakeVRouter.WETH();

        _approve(address(this), address(_cakeVRouter), tokenAmount);

        // make the swap
        _cakeVRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidityForEth(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        _cakeVRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
	...
	function _transferStandard(address sender, address recipient, uint256 tAmount) private 
	{
	    ...
	    else if (_isCakePairAddress(recipient))
        {
	...
	{
		uint256 scaledAmount = amount.mul(CPFScalingFactor).div(internalDecimals);
		return(scaledAmount);
	}
    ...
    */
    function rebase(uint256 epoch, uint256 indexDelta, bool positive) external onlyRebaser returns (uint256)
    {
        ...
        if (!positive)
        {
		uint256 newScalingFactor = CPFScalingFactor.mul(BASE.sub(indexDelta)).div(BASE);
		CPFScalingFactor = newScalingFactor;
        _totalSupply = ((initSupply.sub(_rOwned[BurnAddress].div(currentRate))
            .mul(CPFScalingFactor).div(internalDecimals)));
        emit Rebase(epoch, CPFScalingFactor);
		ICakeVPair(cakeBNBPool).sync();
		for (uint256 i = 0; i < futurePools.length; i++) {
			address futurePoolAddress = futurePools[i];
			ICakeVPair(futurePoolAddress).sync();
		}
        return _totalSupply;
        }
		...
		{
        uint256 newScalingFactor = CPFScalingFactor.mul(BASE.add(indexDelta)).div(BASE);
        ...
        {
            CPFScalingFactor = _maxScalingFactor();
        }
	...
	function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 10**8 , 'CPF: maxTxAmount should be greater than 0.1 CPF');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
	
	function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 1 * 10**9 && minTokensBeforeSwap <= 2000 * 10**9, 'CPF: minTokenBeforeSwap should be between 1 and 2000 CPF');
        require(minTokensBeforeSwap > _autoSwapCallerFee , 'CPF: minTokenBeforeSwap should be greater than autoSwapCallerFee');
	...
	function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        require(autoSwapCallerFee >= 10**8, 'CPF: autoSwapCallerFee should be greater than 0.1 CPF');
