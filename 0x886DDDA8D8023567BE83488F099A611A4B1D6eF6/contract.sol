...
contract StratX_Value is Ownable, ReentrancyGuard, Pausable {
    ...
    function earn(uint256 testStopAt) public whenNotPaused {
        ...
        if (testStopAt == 0){ return; }

        // Converts farm tokens into want tokens
        uint256 earnedAmt = IERC20(earnedAddress).balanceOf(address(this));

        earnedAmt = distributeFees(earnedAmt);
        if (testStopAt == 1){ return; }
        earnedAmt = buyBack(earnedAmt);
        if (testStopAt == 2){ return; }
        ...
        if (earnedAddress != token0Address) {
            ...
            IPancakeRouter02(uniRouterAddress)
                ...
                earnedToToken0Path[0],
                earnedToToken0Path[earnedToToken0Path.length.sub(1)],
                earnedAmt.div(2),
                0,
                earnedToToken0Path,
                address(this),
        ...
        if (earnedAddress != token1Address) {
            ...
            IPancakeRouter02(uniRouterAddress)
                .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                earnedToToken1Path[0],
                earnedToToken1Path[earnedToToken1Path.length.sub(1)],
                earnedAmt.div(2),
                0,
                earnedToToken1Path,
                address(this),
    ...
    function buyBack(uint256 _earnedAmt) internal returns (uint256) {
        ...
        IPancakeRouter02(uniRouterAddress)
            .swapExactTokensForTokensSupportingFeeOnTransferTokens(
            earnedToAUTOPath[0],
            earnedToAUTOPath[earnedToAUTOPath.length.sub(1)],
            buyBackAmt,
            0,
            earnedToAUTOPath,
            buyBackAddress,
    ...
    function convertDustToEarned() public whenNotPaused {
        ...
        if (token0Address != earnedAddress && token0Amt > 0) {
            ...
            IPancakeRouter02(uniRouterAddress)
                .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                token0ToEarnedPath[0],
                token0ToEarnedPath[token0ToEarnedPath.length.sub(1)],
                token0Amt,
                0,
                token0ToEarnedPath,
                address(this),
        ...
        if (token1Address != earnedAddress && token1Amt > 0) {
            ...
            IPancakeRouter02(uniRouterAddress)
                .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                token1ToEarnedPath[0],
                token1ToEarnedPath[token1ToEarnedPath.length.sub(1)],
                token1Amt,
                0,
                token1ToEarnedPath,
                address(this),
