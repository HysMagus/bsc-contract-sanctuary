...
contract SousChef {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many console tokens the user has provided.
        uint256 rewardDebt;  // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of consoles
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws console tokens to a pool. Here's what happens:
    ...
    // The console TOKEN!
    IBEP20 public console;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes console tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = console.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 consoleSupply = console.balanceOf(address(this));
        if (consoleSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(consoleSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit console tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        console.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw console tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        console.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        console.safeTransfer(address(msg.sender), user.amount);
