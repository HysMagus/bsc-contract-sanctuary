...
contract BullToken is BEP20('BullFarm Token', 'BULL') {
    /// @dev Creates `_amount` token to `_to`. Must only be called by the owner (BullFarm).
...
contract ToDaMoon is BEP20('Moon Token', 'MOON') {
    /// @dev Creates `_amount` token to `_to`. Must only be called by the owner (BullFarm).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
...
// BullFarm is the master of Bull. He can make Bull and he is a fair guy.
//
...
contract BullFarm is Ownable {
    ...
    BullToken public bull;
    // The MOON TOKEN!
    ToDaMoon public moon;
    // Dev address.
    address public devaddr;
    // BULL tokens created per block.
    uint256 public bullPerBlock = 5000000000000000;
    ...
    /////////////////////////////////
    enum Functions { ADD, ADDLIST, SETTOKEN, SET, SETLIST, UPDATEMULTIPLIER, UPGRADE }    
    // 48 hours timelock
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 bullReward = multiplier.mul(bullPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bull.mint(devaddr, bullReward.mul(10).div(100));
        bull.mint(address(moon), bullReward.mul(90).div(100));
        pool.accBullPerShare = pool.accBullPerShare.add(bullReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to BullFarm for BULL allocation.
    ...
    // Withdraw LP tokens from BullFarm.
    ...
    function safeBullTransfer(address _to, uint256 _amount) internal {
        moon.safeBullTransfer(_to, _amount);
    }
...
//See Above for Timelock configuration
/////////////////////////////////
//// Timelock Configuration /////
/////////////////////////////////
// enum Functions { ADD, ADDLIST, SETTOKEN, SET, UPDATEMULTIPLIER, UPGRADE }    
...
// All functions below are timelocked and the ownership will be migrated to A "Timelock" Contract with governance before it is possible to adjust parameters. These functions are ownership-free for at least 100 days.

// 1.Remove the possibility of changing _allocation point into a huge number
// 2.Remove the possibility of changing parameters too early that makes farmers risky
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public onlyOwner notLocked(Functions.ADD) {
        /////////////////////////
        // RESTRICT ALLOCPOINT //
        /////////////////////////
        require (_allocPoint <= 200, 'Forbid Infinite AllocPoint');
    ...
    function addList(address[] memory _lpToken, uint256 _allocPoint) public onlyOwner notLocked(Functions.ADDLIST) {
        /////////////////////////
        // RESTRICT ALLOCPOINT //
        /////////////////////////
        require (_allocPoint <= 200, 'Forbid Infinite AllocPoint');
    ...
    // setToken, can only be called by the owner.
    function setToken(uint _pid, IBEP20 _newToken) public onlyOwner notLocked(Functions.SETTOKEN) {
        ...
        //////////////////////////////////////////////
        require ( IBEP20(pool.lpToken).balanceOf(address(this)) != 0 && pool.lpToken != moon);
    ...
    // Update the given pools' BULL allocation point. Can only be called by the owner.
    function setList(uint256[] memory _poolAlloc) public onlyOwner notLocked(Functions.SETLIST) {
        uint256 i;
        for (i = 0; i < _poolAlloc.length; ++i) {
            /////////////////////////
            // RESTRICT ALLOCPOINT //
            /////////////////////////
            require (_poolAlloc[i] <= 200, 'Forbid Infinite AllocPoint');
    ...
    function updateMultiplier(uint256 multiplierNumber) public onlyOwner notLocked(Functions.UPDATEMULTIPLIER) {
        // Additional 100 days' Timelock
        //////////////////////////////////////////////////////////////////////
        require (block.number > 8200000, 'Can only upgrade after 100 days.');
        //////////////////////////////////////////////////////////////////////
    ...
    function upgrade(address _address) public onlyOwner notLocked(Functions.UPGRADE) {
        ...
        bull.transferOwnership(_address);
        moon.transferOwnership(_address);
