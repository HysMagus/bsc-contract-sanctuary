...
contract DelfyLocker is Mutex, IDelfyLocker {
    ...
    function getLPAmount(address _auctionAddr)
        external
        view
        override
        returns (uint256)
    {
        return
            IERC20(auctionDetails[_auctionAddr].LPTokenAddress).balanceOf(
                address(this)
            );
    }
    ...
    function addAuctionDetails(
        ...
        require(isContract(token), "Locker: only_contract_address");
        ...
        auctionDetails[_auctionAddr].auctionLockPeriod = block.timestamp.add(
            _lockPeriod
        );
        auctionDetails[_auctionAddr].LPTokenburned = _burnLP;
        tokenAddress[_auctionAddr] = token;
        address dexFactory = _dexFactory;
        address weth = _dexWETH;
        ...
        auctionDetails[_auctionAddr].LPTokenAddress = IUniswapV2Factory(
            dexFactory
        )
            .getPair(weth, token);
    }
    ...
    function withdrawLpToken(address _token, uint256 _amount)
        external
        override
        noReentrance
    {
        require(
            auctionAddress[msg.sender][_token] != address(0),
            "LOCKER: no_auction"
        );
        address _auctionAddress = auctionAddress[msg.sender][_token];
        require(
            now > auctionDetails[_auctionAddress].auctionLockPeriod,
            "LOCKER: in_lock_period"
        );
        require(
            IERC20(auctionDetails[_auctionAddress].LPTokenAddress).balanceOf(
                address(this)
            ) >= _amount,
            "LOCKER:: Balance_Zero"
        );
        TransferHelper.safeTransfer(
            auctionDetails[_auctionAddress].LPTokenAddress,
            auctionDetails[_auctionAddress].auctionOwner,
            _amount
        );
    ...
    function burnDexLPToken(address _token, uint256 _amount)
        external
        override
        noReentrance
    {
        require(
            auctionAddress[msg.sender][_token] != address(0) ||
                tokenAddress[msg.sender] == _token,
            "LOCKER: no_auction_no_balance"
        );
        address _auctionAddress;
        auctionAddress[msg.sender][_token] != address(0)
            ? _auctionAddress = auctionAddress[msg.sender][_token]
            : _auctionAddress = msg.sender;
        require(
            IERC20(auctionDetails[_auctionAddress].LPTokenAddress).balanceOf(
                address(this)
            ) >= _amount,
            "LOCKER:: insufficient_bal"
        );
        TransferHelper.safeTransfer(
            auctionDetails[_auctionAddress].LPTokenAddress,
            deadAddress,
