...
contract ATDS is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 6800 * 10**4 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    string private _name = "TDSToken"; 
    ...
    uint256 public _maxTxAmount = 6800 * 10**4 * 10**18; 
    ...
    uint256 public fomoMin = 200 * 10**4 * 10**18; 
    // IERC20 public usdt = IERC20(0x9b1f07dF714b5AAC0DC96Fc3253D37f2eEFc6C58);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFomo, uint256 tDev, uint256 tBurn, uint256 tFee) {
        if (!feeIt) {
            return (tAmount, 0, 0, 0, 0);
        }
        tFomo = tAmount.mul(2).div(100);
        tDev = tAmount.div(50);
        tBurn = tAmount.mul(3).div(100);
        tFee = tAmount.mul(3).div(100);
        tTransferAmount = tAmount.sub(tFomo).sub(tDev).sub(tBurn).sub(tFee);
    }
    ...
    function _takeTax(uint256 tFomo, uint256 tDev, uint256 tBurn) private {
        uint256 currentRate =  _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rFomo = tFomo.mul(currentRate);
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[blackHole] = _rOwned[blackHole].add(rBurn);
        if(_isExcluded[blackHole]) {
            _tOwned[blackHole] = _tOwned[blackHole].add(tBurn);
        }
        _tTotal -= tBurn;
    ...
    function _transfer(
        ...
        if(from == address(this) || from == owner() || from == fomoReceiver ||_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            takeFee = false;
        }

        if (enterCount == 1) {
            if (from == uniswapV2Pair && amount >= fomoMin) {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFomo, uint256 tDev, uint256 tBurn, uint256 tFee) = _getTValues(tAmount);
