...
interface IMigratorChef {
    // Perform LP token migration from legacy PancakeSwap to ZULA.
...
// Secured by a Timelock Contract, also the contract itself is now protected with ReentrancyGuard
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The HONEY TOKEN!
    ZulaToken public honey;
    // HONEY tokens created per block.
    uint256 public honeyPerBlock;
    // Bonus muliplier for early honey makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // The migrator contract. It has a lot of power. Can only be set through timelock contract.
    ...
    // discount deposit fees until this block
    uint256 public discountBlock;
    ...
    constructor(
        ZulaToken _zula,
        uint256 _startBlock,
        uint256 _discountBlock
    ) public {
        honey = _zula;
        feeAddress = msg.sender;
        honeyPerBlock = 2480000000000000;
    ...
    // Set the migrator contract. Can only be called by the timelock contract.
    function setMigrator(IMigratorChef _migrator) public onlyOwner {
        migrator = _migrator;
    }

    // Migrate PancakeSwap LP token to ZulaSwap LP contract. Secured by the 72 Hours timelock and the masterchef contract itself.
    // XXX THIS IS NOT A RUGPULL FUNCTION AS IT IS NOT MODIFIED FOR RUGPULL!!! RUGPULLS OFTEN IMPLEMENT OTHER MALCIOUS FUNCTION OR HAVE NO TIMELOCK, ETC!
    function migrate(uint256 _pid) public {
        require(address(migrator) != address(0), "migrate: no migrator");
        PoolInfo storage pool = poolInfo[_pid];
        IBEP20 lpToken = pool.lpToken;
        uint256 bal = lpToken.balanceOf(address(this));
        lpToken.safeApprove(address(migrator), uint(-1));
    ...
    function getRewardPerBlock(uint blockNumber) public view returns (uint256) {
        if (block.number > startBlock){
            return honeyPerBlock/( 2**( ( blockNumber.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
        } else {
            return 0;
        }
    }
    ...
    function getCurrentRewardPerBlock() public view returns (uint256) {
        if (block.number > startBlock){
            return honeyPerBlock/( 2**( ( block.number.sub(startBlock) ).div(halvingPeriod) ) )/10**18;
        } else {
            return 0;
        }
    }
    ...
    function pendingHoney(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 honeyReward = multiplier.mul(honeyPerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(halvFactor);
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(block.number > startBlock,"Farming not started, please wait");
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if(_amount > 0) {
            ...
            if(block.number > discountBlock && pool.depositFeeBP > 0){
