...
contract SousChef {
    ...
    // The TADPOLE TOKEN!
    IBEP20 public tadpole;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes Tadpole tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = tadpole.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 tadpoleSupply = tadpole.balanceOf(address(this));
        if (tadpoleSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(tadpoleSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit Tadpole tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];

        updatePool();
        tadpole.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Tadpole tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        tadpole.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        tadpole.safeTransfer(address(msg.sender), user.amount);
