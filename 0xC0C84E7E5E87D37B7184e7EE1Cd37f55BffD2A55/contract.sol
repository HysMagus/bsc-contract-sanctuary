...
// File: contracts/BIFI/interfaces/thugs/IOriginalGangsterV2.sol
...
// File: contracts/BIFI/strategies/Thugs/StrategyDrugsV3.sol
 ...
 */
contract StrategyDrugsV3 is Ownable, Pausable {
    ...
    /**
     * @dev The strat can set the {smartGangster} once. This will be done as soon as the 
     * address for the new gangster is known.
     */ 
    ...
    /**
     * @dev Initializes the strategy with the {output} and Vault that it will use.
     */
    constructor(address _vault, address _output) public {
        vault = _vault;
        output = _output;
    ...
    /**
     * @dev Required for a migration to the new 'thugs' pool once 'guns' emission 
     * stops. Our upgrade are delayed by two days, but we will only have the {smartgangster} address 
     * a few hours before the switch has to happen. The strat will be able to set the {smartgangster}
     * only once. We will set the correct gangster as soon as we have the address.
     */ 
    function initialize(address _smartGangster) external onlyOwner {
