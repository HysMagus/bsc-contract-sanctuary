...
contract JuicyToken is BEP20('JuicySwap Token', 'JUICY') {
...
contract MasterChef is Ownable {
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 juicyReward = multiplier.mul(juicyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(juicyReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accEggPerShare = pool.accEggPerShare.add(juicyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe juicy transfer function, just in case if rounding error causes pool to not have enough EGGs.
    ...
    function updateEmissionRate(uint256 _juicyPerBlock) public onlyOwner {
        massUpdatePools();
        juicyPerBlock = _juicyPerBlock;
    }
}