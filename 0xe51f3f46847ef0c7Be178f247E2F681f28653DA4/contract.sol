...
contract StratVLEV is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        IERC20(venusAddress).safeIncreaseAllowance(uniRouterAddress, uint256(-1));
        IERC20(wantAddress).safeIncreaseAllowance(uniRouterAddress, uint256(-1));
        if (!wantIsWBNB) {
            IERC20(wantAddress).safeIncreaseAllowance(vTokenAddress, uint256(-1));
        }
