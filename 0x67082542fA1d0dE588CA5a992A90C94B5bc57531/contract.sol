...
contract StratX is Ownable, ReentrancyGuard, Pausable {
    ...
    bool public isAutoComp; // this vault is purely for staking. eg. WBNB-MDO staking vault.
    ...
    constructor(
        ...
        if (isAutoComp) {
            ...
            earnedToMDOPath = [earnedAddress, wbnbAddress, MDOAddress];
            if (wbnbAddress == earnedAddress) {
                earnedToMDOPath = [wbnbAddress, MDOAddress];
            }
    ...
    function buyBack(uint256 _earnedAmt) internal returns (uint256) {
        ...
        uint256 pathLength = earnedToMDOPath.length;
        if (earnedToMDOPath[0] != earnedToMDOPath[pathLength - 1]) {
            IERC20(earnedAddress).safeIncreaseAllowance(uniRouterAddress, buyBackAmt);
            IPancakeRouter02(uniRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(buyBackAmt, 0, earnedToMDOPath, buyBackAddress, now + 60);
        } else {
    ...
    function setEarnedToMDOPath(address[] memory _path) external {
        require(msg.sender == govAddress, "!gov");
        earnedToMDOPath = _path;
    }
