...
/**
 * @dev Contract for access control where the controller contract specified
 * in the Storage contract can be granted access to specific functions
 * on a contract that inherits this contract.
 * 
 * This contract also allows you to make a function exclusive to either the governance address
 * specified in the storage contract or the controller contract.
 */
...
contract Controller is IController, Governable {
    ...
    function salvageStrategy(address _strategy, address _token, uint256 _amount) external onlyGovernance {
        // Te strategy is responsible for maintaining the list of
