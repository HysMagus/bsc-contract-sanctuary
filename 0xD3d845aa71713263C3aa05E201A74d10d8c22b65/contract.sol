...
interface Main {
    ...
    function check(address addr_) external view returns (uint, uint, uint, uint, uint, uint, uint, uint);
}

interface IDO {
    function userInfo(address addr_) external view returns (uint, uint, uint);
}
...
contract SPE_Air_Drop is Ownable {
    ...
    function openAirDrop(uint round_) public onlyAdmin {
        ...
        regEnd = block.timestamp + 2 days;
        claimTime = block.timestamp + 2 days;
        claimEnd = block.timestamp + 4 days;
        round = round_;
    ...
    function regAirDrop() public {
        ...
        require(isQualified(msg.sender), 'no qualified');
        require(status, '1');
    ...
    function isQualified(address addr_) public view returns (bool){
        (,uint a,) = ido.userInfo(addr_);
        (uint spe_,uint nbtc,uint stake_spe,uint stake_nbtc,,,,uint quota) = main.check(addr_);
        if (a > 0) {
            return true;
        } else if (spe_ > 5 ether || nbtc > 0 || stake_nbtc > 0 || stake_spe > 0 || quota > 0) {
            return true;
        }
        return false;
    }
    ...
    function setMain(address addr_, address IDO_) public onlyOwner {
        main = Main(addr_);
        setAdamin(addr_);
        Admin[addr_] = true;
        ido = IDO(IDO_);
    ...
    function safePull(address token_, address wallet_) public onlyOwner {
        IERC20(token_).transfer(wallet_, IERC20(token_).balanceOf(address(this)));
    }
