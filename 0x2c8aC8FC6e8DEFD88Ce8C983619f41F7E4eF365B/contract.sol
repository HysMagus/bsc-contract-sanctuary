...
contract XemmBank is Ownable, StakingInterface {
    ...
    constructor(address _xemm) public {
        xemm = IERC20(_xemm);
    }
    ...
    function stake(uint256 _amount, uint256 _days) public {
        ...
        require(xemm.balanceOf(address(this)) > totalOwedValue().add(_reward).add(_amount), "insufficient contract bal");

        require(xemm.transferFrom(msg.sender, address(this), _amount), "transfer failed");
    ...
    function claim() public {
        ...
        require(xemm.transfer(msg.sender, owed), "transfer failed");
    }
    ...
    function ownerReclaim(uint256 _amount) public onlyOwner {
        require(_amount < xemm.balanceOf(address(this)).sub(totalOwedValue()), "cannot withdraw owed funds");
        xemm.transfer(msg.sender, _amount);
    }
