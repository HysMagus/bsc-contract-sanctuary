...
contract BABYDOXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BABYDOX_Dividend_Tracker", "BABYDOX_Dividend_Tracker", 9) {
    	claimWait = 14400;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BABYDOX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BABYDOX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BABYDOX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BABYDOX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BABYDOX_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BABYDOX is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0xc3cE2f4D4805C43EE5EE05017946edE7DF83dEa5;

    bool private swapping;

    BABYDOXDividendTracker public dividendTracker;

    address public liquidityWallet;
    uint256 public maxBuyTransactionAmount = 50000000000 * (10**9);
    ...
    constructor() public BEP20("BABYDOX", "BDOX", 9) {
    	...
    	dividendTracker = new BABYDOXDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BABYDOX: The dividend tracker already has that address");

        BABYDOXDividendTracker newDividendTracker = BABYDOXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BABYDOX: The new dividend tracker must be owned by the BABYDOX token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "BABYDOX: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BABYDOX: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "BABYDOX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BABYDOX: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BABYDOX: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BABYDOX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BABYDOX: Cannot update gasForProcessing to same value");
    ...
    function setMaxselltxn(uint256 txnAmount) external onlyOwner {
        maxSellTransactionAmount= txnAmount;
    }
    function setMaxbuytxn(uint256 txnAmount) external onlyOwner {
        maxBuyTransactionAmount= txnAmount;
    }    
    ...
    function _transfer(
        ...
        if (
            ...
            !_isExcludedFromFees[from]  //no max for those excluded from fees
                ...
                require(amount <= maxBuyTransactionAmount, "Transfer amount exceeds the maxBuyTransactionAmount.");
            } 
