...
contract LootboxPBOX is Ownable {
    ...
    constructor(IERC20 _genx, GenNFT _nft) {
        ...
        chances = [8800, 9500, 9800, 9900, 9950, 9990];
        genxChances = [45,66,80,88,93,97];
        winMultiplier = [2,4,9,15,30,100];
    }
    ...
    function buy(uint numPacks) public {
        ...
        genx.transferFrom(msg.sender, address(this), price);
    ...
    function redeem() public {
        ...
        uint genxNumber = nftSeedIterator % 100;
        nftSeedIterator = nftSeedIterator / 100;

        uint multiplier = 1;

        for (uint j = 0; j < genxChances.length; ++j) {
            if (genxNumber >= genxChances[j]) {
                multiplier = winMultiplier[j];
            }
        }

        uint reward = purchase.numPacks * config.packPrice * multiplier * 1e18 / 10;
        safeGenTransfer(msg.sender, reward);

        for(uint i = 0; i < purchase.numPacks; ++i) {
            uint luckyNumber = nftSeedIterator % 10000;
            uint nftTypeId = 0;
            for (uint j = 0; j < chances.length; ++j)
            if (luckyNumber >= chances[j]) {
                nftTypeId = j + 1;
            }
