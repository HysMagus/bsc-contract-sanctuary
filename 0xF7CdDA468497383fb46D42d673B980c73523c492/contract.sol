...
contract StratVLEV is Ownable, Pausable {
    ...
    function farm(bool _withLev) public returns (uint256) {
        if (wantIsWBNB) {
            _unwrapBNB(); // Venus accepts BNB, not WBNB.
        }
    ...
    function earn() external whenNotPaused {
        ...
        farm(false); // Supply wantToken without leverage, to cater for net -ve interest rates.
    }
    ...
    function _wrapBNB() internal {
        ...
        uint256 bnbBal = address(this).balance;
        if (bnbBal > 0) {
            IWBNB(wantAddress).deposit{value: bnbBal}(); // Wrap. BNB -> WBNB
        }
    }
