...
// File: contracts/token/AkitaToken.sol
...
// AkitaToken with Governance.
contract AkitaToken is ERC20("AkitaToken", "AKITA"), Ownable {
  uint256 private _cap = 21000000e18;
  ...
  constructor(uint256 _startReleaseBlock, uint256 _endReleaseBlock) public {
    ...
    // maunalMint 20k for seeding liquidity
    manualMint(msg.sender, 20000e18);
  }
...
// File: contracts/token/interfaces/IFairLaunch.sol
...
interface IFairLaunch {
  ...
  function pendingAkita(uint256 _pid, address _user) external view returns (uint256);
...
// File: contracts/token/interfaces/ILocker.sol
...
// File: contracts/token/FairLaunch.sol
...
// FairLaunch is a smart contract for distributing AKITA by asking user to stake the ERC20-based token.
contract FairLaunch is IFairLaunch, Ownable {
  ...
  struct UserInfo {
    ...
    //
    // We do some fancy math here. Basically, any point in time, the amount of AKITAs
    // entitled to a user but is pending to be distributed is:
    //
    //   pending reward = (user.amount * pool.accAkitaPerShare) - user.rewardDebt
    //
    // Whenever a user deposits or withdraws Staking tokens to a pool. Here's what happens:
    //   1. The pool's `accAkitaPerShare` (and `lastRewardBlock`) gets updated.
  ...
  struct PoolInfo {
    ...
    uint256 allocPoint; // How many allocation points assigned to this pool. AKITAs to distribute per block.
    uint256 lastRewardBlock; // Last block number that AKITAs distribution occurs.
    uint256 accAkitaPerShare; // Accumulated AKITAs per share, times 1e12. See below.
    uint256 accAkitaPerShareTilBonusEnd; // Accumated AKITAs per share until Bonus End.
    address locker; // locker
  }

  // The Akita TOKEN!
  AkitaToken public akita;
  // Dev address.
  address public devaddr;
  // Dev reward balance
  uint256 public devBal;
  // Op Fund address
  address public opFundAddr;
  // AKITA tokens created per block.
  uint256 public akitaPerBlock;
  // Bonus muliplier for early akita makers.
  uint256 public bonusMultiplier;
  // Block number when bonus AKITA period ends.
  ...
  // The block number when AKITA mining starts.
  ...
  */
  ...
  function devClaim(address _to, uint256 _amount) public {
    ...
    safeAkitaTransfer(_to, _amount);
  }

  // Update op fund address
  function setOpFund(address _addr) public {
    require(msg.sender == opFundAddr, "op: wut?");
    opFundAddr = _addr;
  }

  function setAkitaPerBlock(uint256 _akitaPerBlock) public onlyOwner {
    akitaPerBlock = _akitaPerBlock;
  }
  ...
  function addPool(
    ...
    poolInfo.push(
      PoolInfo({
        ...
        accAkitaPerShare: 0,
        accAkitaPerShareTilBonusEnd: 0,
        locker: _locker
      })
    );
  }

  // Update the given pool's AKITA allocation point. Can only be called by the owner.
  function setPool(
    uint256 _pid,
    uint256 _allocPoint,
    address _locker,
  ...
  */
  ...
  // View function to see pending AKITAs on frontend.
  function pendingAkita(uint256 _pid, address _user) external override view returns (uint256) {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 accAkitaPerShare = pool.accAkitaPerShare;
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      accAkitaPerShare = accAkitaPerShare.add(akitaReward.mul(1e12).div(lpSupply));
    }
    return user.amount.mul(accAkitaPerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function updatePool(uint256 _pid) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 akitaReward = multiplier.mul(akitaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    akita.mint(opFundAddr, akitaReward.div(20));
    akita.mint(address(this), akitaReward.add(akitaReward.div(20)));
    devBal = devBal.add(akitaReward.div(20));
    pool.accAkitaPerShare = pool.accAkitaPerShare.add(akitaReward.mul(1e12).div(lpSupply));
    // update accAkitaPerShareTilBonusEnd
    if (block.number <= bonusEndBlock) {
      akita.lock(opFundAddr, akitaReward.div(20).mul(bonusLockUpBps).div(10000));
      pool.accAkitaPerShareTilBonusEnd = pool.accAkitaPerShare;
    }
    if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 akitaBonusPortion = bonusEndBlock.sub(pool.lastRewardBlock).mul(bonusMultiplier).mul(akitaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      akita.lock(opFundAddr, akitaBonusPortion.div(20).mul(bonusLockUpBps).div(10000));
      pool.accAkitaPerShareTilBonusEnd = pool.accAkitaPerShareTilBonusEnd.add(akitaBonusPortion.mul(1e12).div(lpSupply));
    }
    pool.lastRewardBlock = block.number;
  }

  // Deposit Staking tokens to FairLaunchToken for AKITA allocation.
  function deposit(address _for, uint256 _pid, uint256 _amount) public override {
    ...
    user.rewardDebt = user.amount.mul(pool.accAkitaPerShare).div(1e12);
    user.bonusDebt = user.amount.mul(pool.accAkitaPerShareTilBonusEnd).div(1e12);
  ...
  // Harvest AKITAs earn from the pool.
  ...
  function _harvest(address _to, uint256 _pid) internal {
    ...
    uint256 pending = user.amount.mul(pool.accAkitaPerShare).div(1e12).sub(user.rewardDebt);
    require(pending <= akita.balanceOf(address(this)), "wtf not enough akita");
    uint256 bonus = user.amount.mul(pool.accAkitaPerShareTilBonusEnd).div(1e12).sub(user.bonusDebt);
    if (pool.locker == address(0)) {
      safeAkitaTransfer(_to, pending);
      akita.lock(_to, bonus.mul(bonusLockUpBps).div(10000));
    } else {
      uint256 lockAmount = ILocker(pool.locker).calLockAmount(pending);
      akita.approve(pool.locker, lockAmount);
      ILocker(pool.locker).lock(_to, lockAmount);
      akita.approve(pool.locker, 0);
      safeAkitaTransfer(_to, pending.sub(lockAmount));
    }
  }
  ...
  function emergencyWithdraw(uint256 _pid) public {
    ...
    // Safe akita transfer function, just in case if rounding error causes pool to not have enough AKITAs.
  function safeAkitaTransfer(address _to, uint256 _amount) internal {
