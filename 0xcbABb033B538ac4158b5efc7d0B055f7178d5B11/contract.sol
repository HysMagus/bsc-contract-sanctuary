...
contract WaterMineFarm is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WaterMineFarmDividendTracker public dividendTracker;
    address public marketingWallet = 0x0ec5ea8f3AB769F4AEB8d693C41063232FB6e977; //marketingWallet
    address public autoBoostWallet = 0x83C9457bB2855b7Dd95CC86d5942b127eBcb89C1; // autoBoostWallet

    uint256 public maxTransactionAmount;
    uint256 public maxHoldingAmount;
    uint256 public swapTokensAtAmount = 10000 * (10**decimals());

    uint256 public autoBoost = 2;
    uint256 public marketingFee = 4;
    uint256 public dividendFee = 12;

    uint256 public sellautoBoostFee = 2;
    uint256 public sellMarketingFee = 4;
    uint256 public sellDividendFee = 12;


    uint256 public accautoBoostFee = 1;
    ...
    event UpdateWallets(address oldMarketing, address newMarketing, address oldautoBoost, address newautoBoost);
    ...
    event SetBuyFees(uint256 autoBoostFee, uint256 MarketingFee, uint256 DividendFee);

    event SetSellFees(uint256 SellautoBoostFee, uint256 SellMarketingFee, uint256 SellDividendFee);
    ...
    event CalculatedBNBForEachRecipient(uint256 forMarketing, uint256 forautoBoost, uint256 forDividends);
    ...
    constructor() ERC20("WaterMineFarm", "WMF") {
        dividendTracker = new WaterMineFarmDividendTracker();

        //updatePancakeRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //testnet
         updatePancakeRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); // mainnet 
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "WaterMineFarm: The router already has that address");
    ...
    function setBuyFees(uint256 _autoBoostFee, uint256 _MarketingFee, uint256 _DividendFee)  external onlyOwner {
        autoBoost = _autoBoostFee;
        marketingFee = _MarketingFee;
        dividendFee = _DividendFee;
        uint256 totalFees = autoBoost + marketingFee + dividendFee;
        require(totalFees < 51, "Too much");
        emit SetBuyFees(_autoBoostFee, _MarketingFee, _DividendFee);
    }

    function setSellFees(uint256 _SellautoBoostFee, uint256 _SellMarketingFee, uint256 _SellDividendFee)  external onlyOwner {
        sellautoBoostFee = _SellautoBoostFee;
        sellMarketingFee = _SellMarketingFee;
        sellDividendFee = _SellDividendFee;
        uint256 totalFees = sellautoBoostFee + sellMarketingFee + sellDividendFee;
        require(totalFees < 51, "Too much");
        emit SetSellFees(_SellautoBoostFee, _SellMarketingFee, _SellDividendFee);
    }

    function setWallets(address newMarketingWallet, address newautoBoostWallet) external onlyOwner {
        require(newautoBoostWallet != address(0), 'autoBoost Zero address');
        require(newMarketingWallet != address(0), 'Marketing Zero address');
        emit UpdateWallets(marketingWallet, newMarketingWallet, autoBoostWallet, newautoBoostWallet);
        marketingWallet = newMarketingWallet;
        autoBoostWallet = newautoBoostWallet;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "WaterMineFarm: The dividend tracker already has that address");

        WaterMineFarmDividendTracker newDividendTracker = WaterMineFarmDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "WaterMineFarm: The new dividend tracker must be owned by the WaterMineFarm token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WaterMineFarm: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "WaterMineFarm: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "WaterMineFarm: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WaterMineFarm: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WaterMineFarm: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "WaterMineFarm: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndDistributeBNB(uint256 tokens) private {
        ...
        uint256 accTotal = accautoBoostFee.add(accDividendFee).add(accMarketingFee);
        uint256 forMarketing = balance.mul(accMarketingFee).div(accTotal);
        uint256 forautoBoost = balance.mul(accautoBoostFee).div(accTotal);
        uint256 forDividends = balance.sub(forautoBoost).sub(forMarketing);

        emit CalculatedBNBForEachRecipient(forMarketing, forautoBoost, forDividends);
        ...
        (success,) = address(autoBoostWallet).call{value: forautoBoost}("");

        if(success) {
            emit SwapAndSendTo(accautoBoostFee, forautoBoost, "ANTI DUMP");
            accautoBoostFee = 1;
        }
...
contract WaterMineFarmDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("WaterMineFarmDividendTracker", "Honey_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 500000000 * (10**decimals()); //must hold 500000000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "WaterMineFarmDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "WaterMineFarmDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Honey contract.");
    }
