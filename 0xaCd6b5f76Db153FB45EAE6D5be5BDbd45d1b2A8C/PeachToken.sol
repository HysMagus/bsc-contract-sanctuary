...
contract PeachToken is ERC20("PeachToken", "PEACH"), Ownable {

    uint256 public restPeachSupply = 60000*10**18;
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (PickPeach).
    function mint(address _to, uint256 _amount) public {
        require(minters[msg.sender], "!minter");
        require (restPeachSupply >= _amount);
        _mint(_to, _amount);
        restPeachSupply -= _amount;
