...
contract Strategy is BaseStrategy {
    ...
    address internal MilkShake      = 0x8Cf93F2b41bA17F9189Aa7a86576f2764A442eca;
    IERC20 public iCREAMToken       = IERC20(0x58f651DDE51CAa87c4111B16ee0A6Fab061Ee564);
    ...
    constructor(address _vault) public BaseStrategy(_vault) {
        require(address(want) == address(iCREAMToken),"wrong want token");
        iCREAMToken.safeApprove(address(router),type(uint256).max);
        iCREAMToken.safeApprove(address(farmer),type(uint256).max);
    }

    function name() external override view returns (string memory) {
        return "StrategyICREAMStaking";
    }

    // returns balance of cream
    ...
    function balanceOfStake() public view returns (uint256) {
        return farmer.userInfo(0, address(this)).amount;
    }

    function pendingReward() public view returns (uint256) {
        return farmer.pendingCream(0,address(this));
    }

    function estimatedTotalAssets() public override view returns (uint256) {
        //Add the vault tokens + staked tokens from cream  contract
    ...
    function depositToFarm() internal {
        ...
        if (_wantAvailable > 0) {
            farmer.enterStaking(_wantAvailable);
        }
    }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > balanceWant) {
            ...
            farmer.leaveStaking((Math.min(balanceStaked, _amountNeeded - balanceWant)));
        }
    ...
    function prepareMigration(address _newStrategy) internal override {
        ...
        farmer.leaveStaking(balanceOfStake());
    }
    ...
    function protectedTokens() internal override view returns (address[] memory) {
        ...
        protected[0] = address(iCREAMToken); // Staked cream tokens from governance contract
        return protected;
    }
}