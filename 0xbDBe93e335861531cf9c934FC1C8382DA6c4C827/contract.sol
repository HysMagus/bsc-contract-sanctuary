...
contract BEP20Token is Context, IBEP20, AccessControl {
  ...
  function addOperator(address account, uint256 quota) public virtual onlyAdmin {
  ...
  */
   ...
   */
  function checkCTKTx(string calldata ctkTxHash) external view returns (uint256) {
    require(bytes(ctkTxHash).length != 0, "BEP20: transaction hash should not be empty");
   ...
   */
  function mint(string memory ctkTxHash, address account, uint256 amount) public onlyOperator returns (bool) {
    require(bytes(ctkTxHash).length != 0, "BEP20: transaction hash should not be empty");
    string memory uppercaseCtkTxHash = _toUpper(ctkTxHash);
    require(_ctkTxMap[uppercaseCtkTxHash] == 0, "BEP20: transaction hash exists");
    require(_operatorQuota[msg.sender] > amount, "operator amount is insufficient, please rotate key");
    _operatorQuota[msg.sender] -= amount;
    _mint(uppercaseCtkTxHash, account, amount);
    return true;
  }
   ...
   */
  function _mint(string memory ctkTxHash, address account, uint256 amount ) internal {
    require(account != address(0), "BEP20: mint to the zero address");

    _totalSupply = _totalSupply.add(amount);
    _balances[account] = _balances[account].add(amount);
    _ctkTxMap[ctkTxHash] = amount;
  ...
  /**
   * @dev Destroys `amount` tokens and remove them from `msg.sender`, decrease
   * the total supply.
   *
   * Requirements
   *
