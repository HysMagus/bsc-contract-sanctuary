...
contract BscMdexLiquidateLends {
    ...
    constructor(address _router, address _defaultAgainstToken) {
        owner = msg.sender;
    ...
    function liquidate(address borrower, address collateralMarket, address borrowMarket, uint repayAmount) external {
        ...
        if (repayAmount == 0) {
            repayAmount = _calculateAmount(borrower, collateralMarket, borrowMarket);
        }
        address borrowToken = _isEthMarket(borrowMarket) ? WETH : IErcToken(borrowMarket).underlying();
        address collateralToken = _isEthMarket(collateralMarket) ? WETH : IErcToken(collateralMarket).underlying();

        (IUniswapV2Pair borrowPair, uint amount) = _getBorrowPair(borrowToken, collateralToken, repayAmount);

        uint amount0 = borrowPair.token0() == borrowToken ? amount : 0;
        uint amount1 = borrowPair.token1() == borrowToken ? amount : 0;
        bytes memory data = abi.encode(borrower, collateralMarket, borrowMarket);
    ...
    function swapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external {
        ...
        (address borrower, address collateralMarket, address borrowMarket) = abi.decode(data, (address, address, address));
        address borrowToken = _isEthMarket(borrowMarket) ? WETH : IErcToken(borrowMarket).underlying();
        address collateralToken = _isEthMarket(collateralMarket) ? WETH : IErcToken(collateralMarket).underlying();
        if (borrowToken == WETH) {
        ...
        _doLiquidateBorrowAndRedeem(borrower, collateralMarket, borrowMarket, amount);
    ...
    // 执行清算 且 赎回
    function _doLiquidateBorrowAndRedeem(address borrower, address _marketCollateral, address _marketBorrow, uint repayAmount) internal {

        _enterMarket(_marketBorrow, _marketCollateral);

        // 执行清算
        if (_isEthMarket(_marketBorrow)) {
            IEthToken(_marketBorrow).liquidateBorrow{value : repayAmount}(borrower, _marketCollateral);
        } else {
            _approveInternal(IErcToken(_marketBorrow).underlying(), _marketBorrow, repayAmount);
    ...
    function _getUnderlyingPrice(address _market) internal view returns (uint){
        IComptroller comptroller = IComptroller(IToken(_market).comptroller());
    ...
    function _calculateAmount(address _borrower, address _collateralMarket, address _borrowMarket) internal returns (uint256){
        IComptroller comptroller = IComptroller(IToken(_borrowMarket).comptroller());
        (, uint liquidity, uint shortfall) = comptroller.getAccountLiquidity(_borrower);
        ...
        uint repayMax = IToken(_borrowMarket).borrowBalanceCurrent(_borrower) * closeFact / uint(10 ** 18);
        uint seizeMax = IErcToken(_collateralMarket).balanceOfUnderlying(_borrower) * uint(10 ** 18) / liqIncent;
