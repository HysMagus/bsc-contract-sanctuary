...
contract OtakuXFloki is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxSellTransactionAmount = totalSupply().mul(1).div(1000);
    uint256 public maxBuyTranscationAmount = totalSupply().mul(1).div(100);
    uint256 public maxWalletBalance = totalSupply().mul(5).div(100);
    uint256 public swapTokensAtAmount = 20 * 10**9 * 10**18;
 
    uint256 public liquidityFee = 4;
    uint256 public previousLiquidityFee;
    uint256 public firstDividendRewardsFee = 4;
    ...
    constructor() ERC20("OtakuXFloki", "OXF") {
    	firstDividendTracker = new FIRSTDividendTracker();
 
    	marketingWallet = 0x7a7445A732c3fb9284b747987c653c68C0C7A4f8;
    	firstDividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external {
        require(marketingWallet == _msgSender(), "Caller is not marketing wallet");
  	    require(_newWallet != marketingWallet, "OtakuXFloki: The marketing wallet is already this address");
        emit MarketingWalletUpdated(marketingWallet, _newWallet);
  	    marketingWallet = _newWallet;
  	    
        excludeMarketinWalletFromFees(_newWallet, true);
    ...
    function updatefirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "OtakuXFloki: The dividend tracker already has that address");
 
        FIRSTDividendTracker newfirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newfirstDividendTracker.owner() == address(this), "OtakuXFloki: The new dividend tracker must be owned by the OtakuXFloki token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "OtakuXFloki: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "OtakuXFloki: Account is already exluded from fees");
        isExcludedFromFees[account] = excluded;
 
        emit ExcludeFromFees(account, excluded);
    }
    
    function excludeMarketinWalletFromFees(address account, bool excluded) internal {
        require(marketingWallet == _msgSender(), "Caller is not marketing wallet");
        require(isExcludedFromFees[account] != excluded, "OtakuXFloki: Account is already exluded from fees");
        isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "OtakuXFloki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "OtakuXFloki: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "OtakuXFloki: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "OtakuXFloki: Trading has not started yet");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("OtakuXFloki_Dividend_Tracker", "OtakuXFloki_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 100M+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "OtakuXFloki_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "OtakuXFloki_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OtakuXFloki contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "OtakuXFloki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "OtakuXFloki_Dividend_Tracker: Cannot update claimWait to same value");
