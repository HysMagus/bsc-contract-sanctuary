 ...
 */
contract LegionNetwork is Ownable {
    using SafeERC20 for IERC20;

    // timestamp 11/11/2030 token release is enabled
    uint256 public releaseTime = 1632945941;
    
    address public BUFFPARAToken = 0x9AfD9575BcE8bff0E5a1A4B34c62C61EB28a0153;
    address public lpToken = 0xee6558287c43F64a0dd0a61e4B9b5E81d70282e6;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseBUFFPARAToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(BUFFPARAToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(BUFFPARAToken).safeTransfer(owner(), amount);
    }
    ...
    function getBUFFPARATokenBalance() external view returns(uint256) {
        return IERC20(BUFFPARAToken).balanceOf(address(this));
    }
    ...
    function setKodiToken(address _BUFFPARAToken) external onlyOwner {
        BUFFPARAToken = _BUFFPARAToken;
    }
