...
/**
 * @dev Implementation of a strategy to get yields from farming LP Pools in PanalpacaSwap.
 * PanalpacaSwap is an automated market maker (“AMM”) that allows two tokens to be exchanged on the Binance Smart Chain.
 * It is fast, cheap, and allows anyone to participate. PanalpacaSwap is aiming to be the #1 liquidity provider on BSC.
 *
...
contract StrategyCakeLP is Ownable, Pausable {
     ...
     * {alpaca} - Token generated by staking our funds. In this case it's the CAKEs token.
     ...
     * {lpToken0, lpToken1} - Tokens that the strategy maximizes. IPanalpacaPair tokens
     */
     ...
     * {unirouter} - PanalpacaSwap unirouter
    ...
    address constant public masterchef = address(0xA625AB01B08ce023B2a342Dbb12a16f2C8489A8F);
    uint8 public poolId; 
     ...
     */
    address constant public rewards  = address(0x3Ed531BfB3FAD41111f6dab567b33C4db897f991);
     ...
     */
constructor() public {
        lpPair = 0xF3CE6Aac24980E6B657926dfC79502Ae414d3083;
        lpToken0 = IPanalpacaPair(lpPair).token0();
        lpToken1 = IPanalpacaPair(lpPair).token1();
        poolId = 4;
        vault = msg.sender;
     ...
     * It deposits {lpPair} in the MasterChef to farm {alpaca}
     */
    function deposit() public whenNotPaused {
        uint256 pairBal = IERC20(lpPair).balanceOf(address(this));

        if (pairBal > 0) {
            IMasterChef(masterchef).deposit(address(this), poolId, pairBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            IMasterChef(masterchef).withdraw(address(this), poolId, _amount.sub(pairBal));
    ...
    function harvest() external whenNotPaused {
        ...
        IMasterChef(masterchef).deposit(address(this), poolId, 0);
        chargeFees();
        addLiquidity();
        deposit();
     ...
     * 0.375% -> Burn Fee
     */
    function chargeFees() internal {//no fees for the first version.
        uint256 toWbnb = IERC20(alpaca).balanceOf(address(this)).mul(15).div(1000);
        IPancakeRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, alpacaToWbnbRoute, address(this), now.add(600));
        ...
        uint256 treausuryFee = wbnbBal.mul(TREASURY_FEE).div(MAX_FEE);
        IPancakeRouter(unirouter).swapExactTokensForTokens(treausuryFee, 0, wbnbToElevenRoute, treasury, now.add(600));

        uint256 rewardsFee = wbnbBal.mul(REWARDS_FEE).div(MAX_FEE);
        IPancakeRouter(unirouter).swapExactTokensForTokens(rewardsFee, 0, wbnbToElevenRoute, rewards, now.add(600));

        uint256 burnFee = wbnbBal.mul(BURN_FEE).div(MAX_FEE);
        IPancakeRouter(unirouter).swapExactTokensForTokens(burnFee, 0, wbnbToElevenRoute, address(this), now.add(600));
        IERC20(eleven).burn(IERC20(eleven).balanceOf(address(this)));
    }

    /**
     * @dev Swaps {alpaca} for {lpToken0}, {lpToken1} & {wbnb} using PanalpacaSwap.
     */
    function addLiquidity() internal {   
        uint256 alpacaHalf = IERC20(alpaca).balanceOf(address(this)).div(2);
        
        if (lpToken0 != alpaca) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(alpacaHalf, 0, alpacaToWbnbRoute, address(this), now.add(600));
        }
