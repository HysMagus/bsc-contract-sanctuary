...
abstract contract ICT {
    function createToken(address _owner, string memory _name, string memory _class, string memory _syndicate, bool _isBoss,
                            string memory _genAttr, string memory _genElem) public virtual returns (uint256);
    function levelUp(uint256 _fighter, uint8 _h, uint8 _a, uint8 _d, uint8 _s, uint16 _potentialMax) public virtual;
}
...
contract Buy {
    ...
    event NewAvatar(string name, bool isBoss, string class);
    ...
    function addAvatar(string memory _name, bool _isBoss, string memory _class, string memory _syndicate) public onlyContractOwner ifNotPaused {
        isBoss[_name] = _isBoss;
        classFor[_name] = _class;
        syndicateFor[_name] = _syndicate;
        
        emit NewAvatar(_name, _isBoss, _class);
    }
    ...
    function _firstLevelUp(uint256 _newFighterId, string memory _name) internal {
        ...
        uint8 _hp = _random(initialRanges[rangeId]["hp"], 1);
        uint8 _attack = _random(initialRanges[rangeId]["attack"], 2);
        uint8 _defense = _random(initialRanges[rangeId]["defense"], 3);
        uint8 _agility = _random(initialRanges[rangeId]["agility"], 4);
        
        uint16 attrs_sum = uint16(_hp + _attack + _defense + _agility);
        uint16 potentialMax = isBoss[_name] ? uint16(263 + attrs_sum) : uint16(118 + attrs_sum);
        nft.levelUp(_newFighterId, _hp, _attack, _defense, _agility, potentialMax);
    }
    ...
    function buyFighter(string memory _name) external ifNotPaused { 
        ...
        uint256 newFighterId = nft.createToken(msg.sender, _name, classFor[_name], syndicateFor[_name], isBoss[_name], 
                                                genAttr[_random(4, 1)], 
                                                isBoss[_name] ? elements[_random(4, 2)] : "");
