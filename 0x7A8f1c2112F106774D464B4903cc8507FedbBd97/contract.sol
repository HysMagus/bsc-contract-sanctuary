...
contract WingsToken is BEP20('chickenWings', 'WINGS') {
...
// HotSauce with Governance.
contract HotSauce is BEP20('HotSauce', 'HSA') {
    ...
    // Safe cake transfer function, just in case if rounding error causes pool to not have enough WINGS
    function safeWingsTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 wingsBal = wings.balanceOf(address(this));
        if (_amount > wingsBal) {
            wings.transfer(_to, wingsBal);
        } else {
            wings.transfer(_to, _amount);
        }
    }
    ...
    */
    ...
    // Enter the staking pool. Pay some WINGS. Earn some hot sizzle sauce and more wings 
    function enter(uint256 _amount) public {
        
        uint256 totalWings = wings.balanceOf(address(this));
        uint256 totalShares = totalSupply();
        
        //uint256 totalToSend = _amount.mul(98).div(100); // amount minus 2% burn. accounts wings burn. + makes 1:1 for hotsauce:wings
        
        if (totalShares == 0 || totalWings == 0) {
            _mint(msg.sender, _amount);
        } else {
            uint256 what = _amount.mul(totalShares).div(totalWings);
            _mint(msg.sender, what);
        }
        wings.transferFrom(msg.sender, address(this), _amount);
    }
    
    // Leave the staking pool. Earn wings back 
    function leave(uint256 _share) public {
        uint256 totalShares = totalSupply();
        uint256 what = _share.mul(wings.balanceOf(address(this))).div(totalShares);
        _burn(msg.sender, _share);
        uint256 fee = what.mul(25).div(1000); // 2.5% dev fee
        uint256 stakerAmount = what.sub(fee); // staker
        wings.transfer(devAddr, fee); 
        wings.transfer(msg.sender, stakerAmount);
    }
