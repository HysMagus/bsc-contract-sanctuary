...
// ICEToken
contract ICEToken is ERC20("ICE.finance", "ICE"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (ICEChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
    }
}

contract ICEChef is Ownable {
    using SafeMath for uint256;
    ...
    // Block number when bonus ICE period ends.
    ...
    uint256 public totalStakedIceAmount;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 iceReward = multiplier.mul(icePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ice.mint(devaddr, iceReward.div(20)); // 5%
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            user.amount = user.amount.add(currentBalance.sub(initialBalance));
            
            if(pool.lpToken == ice)
                totalStakedIceAmount = totalStakedIceAmount.add(currentBalance.sub(initialBalance));
        }
        user.rewardDebt = user.amount.mul(pool.accICEPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from ICEChef.
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.lpToken == ice)
                totalStakedIceAmount = totalStakedIceAmount.sub(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accICEPerShare).div(1e12);
