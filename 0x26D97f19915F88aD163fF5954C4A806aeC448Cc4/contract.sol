...
//
contract TalonsPool is Ownable {
    using SafeMath for uint256;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = talons.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && stakedSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 tokenReward = multiplier.mul(rewardTokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accRewardTokenPerShare = accRewardTokenPerShare.add(tokenReward.mul(1e12).div(stakedSupply));
        }
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            talons.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Talons from TalonsPool
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            talons.safeTransfer(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accRewardTokenPerShare).div(1e12);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        talons.safeTransfer(address(msg.sender), user.amount);
        user.amount = 0;
        user.rewardDebt = 0;
